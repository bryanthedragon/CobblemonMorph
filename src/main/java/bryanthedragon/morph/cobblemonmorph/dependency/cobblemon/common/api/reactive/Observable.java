...
public interface Observable<T> {
    ObservableSubscription<T> subscribe(Priority priority = Priority.NORMAL, handler: (T) -> Unit)
    ObservableSubscription<T> subscribe(Consumer<T> handler) = subscribe(Priority.NORMAL) { handler.accept(it) }
    ObservableSubscription<T> subscribe(Priority priority, Consumer<T> handler) = subscribe(priority) { handler.accept(it) }
    ObservableSubscription<T> unsubscribe(ObservableSubscription<T> subscription)
     ...
     */
    Observable<O> pipe(Transform<T, O> transform) TransformObservable(this, transform)
    fun <O1, O2> pipe(Transform<T, O1> t1, Transform<O1, O2> t2) = TransformObservable(this, map { t2(t1(it)) })
    fun <O1, O2, O3> pipe(Transform<T, O1> t1, Transform<O1, O2> t2, Transform<O2, O3> t3) = TransformObservable(this, map { t3(t2(t1(it))) })
    fun <O1, O2, O3, O4> pipe(Transform<T, O1> t1, Transform<O1, O2> t2, Transform<O2, O3> t3, Transform<O3, O4> t4) = TransformObservable(this, map { t4(t3(t2(t1(it)))) })
    fun <O1, O2, O3, O4, O5> pipe(Transform<T, O1> t1, Transform<O1, O2> t2, Transform<O2, O3> t3, Transform<O3, O4> t4, Transform<O4, O5> t5) = TransformObservable(this, map { t5(t4(t3(t2(t1(it))))) })
    fun <O1, O2, O3, O4, O5, O6> pipe(Transform<T, O1> t1, Transform<O1, O2> t2, Transform<O2, O3> t3, Transform<O3, O4> t4, Transform<O4, O5> t5, Transform<O5, O6>) t6 = TransformObservable(this, map { t6(t5(t4(t3(t2(t1(it)))))) })
    ...
    final class Companion {
        ...
        Observable<T> just(vararg values: T) {
            return SingularObservable<T>().also { it.emit(*values) }
        }

        Observable<T> of(future: CompletableFuture<T>): Observable<T> {
        ...
        Observable<T> takeFirst(amount: Int = 1) = TakeFirstTransform<T>(amount)
        /** Gets a [IgnoreFirstTransform] */
        Observable<T> ignoreFirst(amount: Int = 1) = IgnoreFirstTransform<T>(amount)
        /** Gets a [FilterTransform] */
        Observable<T> filter(predicate: (T) -> Boolean) = FilterTransform(predicate)
        /** Gets a [MapTransform] */
        fun <T, O> map(mapping: (T) -> O) = MapTransform(mapping)
        /** Gets a [EmitWhileTransform] */
        Observable<T> emitWhile(predicate: (T) -> Boolean) = EmitWhileTransform(predicate)
        /** Gets a [StopAfterTransform] */
        Observable<T> stopAfter(predicate: (T) -> Boolean) = StopAfterTransform(predicate)
        /** Gets a transform which will take values until the given predicate is met. */
        Observable<T> emitUntil(predicate: (T) -> Boolean) = EmitWhileTransform<T> { !predicate(it) }
        /** Gets a transform which does not change the value but lets you access the value at this stage of the pipe. */
        Observable<T> tap(handler: (T) -> Unit) = map<T, T> { handler(it); return@map it }
    }
}