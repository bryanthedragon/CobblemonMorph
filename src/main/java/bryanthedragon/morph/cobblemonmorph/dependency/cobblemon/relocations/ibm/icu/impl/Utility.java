...
public final class Utility {
    ...
    private static final <T extends Appendable> void encodeRun(T buffer, int value2, int length) {
        if (length < 4) {
            for (int j = 0; j < length; ++j) {
                if (value2 == 42405) {
                    Utility.appendInt(buffer, value2);
                }
                Utility.appendInt(buffer, value2);
            }
        } else {
    ...
    private static final <T extends Appendable> void appendInt(T buffer, int value2) {
        try {
            buffer.append((char)(value2 >>> 16));
            buffer.append((char)(value2 & 0xFFFF));
        }
        catch (IOException e) {
    ...
    private static final <T extends Appendable> void encodeRun(T buffer, short value2, int length) {
        try {
            char valueChar = (char)value2;
            if (length < 4) {
                for (int j = 0; j < length; ++j) {
                    if (valueChar == '\ua5a5') {
                        buffer.append('\ua5a5');
    ...
    private static final <T extends Appendable> void encodeRun(T buffer, byte value2, int length, byte[] state) {
        if (length < 4) {
            for (int j = 0; j < length; ++j) {
                if (value2 == -91) {
                    Utility.appendEncodedByte(buffer, (byte)-91, state);
                }
                Utility.appendEncodedByte(buffer, value2, state);
            }
        } else {
            if ((byte)length == -91) {
                if (value2 == -91) {
    ...
    private static final <T extends Appendable> void appendEncodedByte(T buffer, byte value2, byte[] state) {
        try {
            if (state[0] != 0) {
                char c = (char)(state[1] << 8 | value2 & 0xFF);
                buffer.append(c);
                state[0] = 0;
            } else {
                state[0] = 1;
    ...
    private static int unescapeAndLengthAt(CharSequence s, int offset, int length) {
        ...
        if (minDig != 0) {
            ...
            return Utility.codePointAndLength(result, start2, offset);
        }
        for (int i = 0; i < UNESCAPE_MAP.length; i += 2) {
            if (c == UNESCAPE_MAP[i]) {
                return Utility.codePointAndLength(UNESCAPE_MAP[i + 1], start2, offset);
            }
            if (c < UNESCAPE_MAP[i]) break;
        }
        if (c == 99 && offset < length) {
            c = Character.codePointAt(s, offset);
            return Utility.codePointAndLength(c & 0x1F, start2, offset + Character.charCount(c));
        }
        if (UTF16.isLeadSurrogate(c) && offset < length && UTF16.isTrailSurrogate(c2 = s.charAt(offset))) {
            ++offset;
            c = UCharacter.toCodePoint(c, c2);
        }
        return Utility.codePointAndLength(c, start2, offset);
    }
    ...
    private static int codePointAndLength(int c, int start2, int limit) {
        return Utility.codePointAndLength(c, limit - start2);
    }
    ...
    public static String hex(byte[] o, int start2, int end2, String separator) {
    ...
    public static int quotedIndexOf(String text, int start2, int limit, String setOfChars) {
    ...
    public static int charSequenceHashCode(CharSequence value2) {
        int hash = 0;
        for (int i = 0; i < value2.length(); ++i) {
            hash = hash * 31 + value2.charAt(i);
        }
        return hash;
    }
    ...
    public static String joinStrings(CharSequence delimiter, Iterable<? extends CharSequence> elements2) {
        if (delimiter == null || elements2 == null) {
            throw new NullPointerException("Delimiter or elements is null");
        }
        StringBuilder buf = new StringBuilder();
        Iterator<? extends CharSequence> itr = elements2.iterator();
