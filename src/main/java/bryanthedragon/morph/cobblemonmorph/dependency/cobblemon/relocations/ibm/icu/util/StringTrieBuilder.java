...
public abstract class StringTrieBuilder {
    ...
    @Deprecated
    protected void addImpl(CharSequence s, int value2) {
        ...
        this.root = this.root == null ? this.createSuffixNode(s, 0, value2) : this.root.add(this, s, 0, value2);
    }
    ...
    private final ValueNode registerFinalValue(int value2) {
        this.lookupFinalValueNode.setFinalValue(value2);
        Node oldNode = this.nodes.get(this.lookupFinalValueNode);
        if (oldNode != null) {
            return (ValueNode)oldNode;
        }
        ValueNode newNode = new ValueNode(value2);
        oldNode = this.nodes.put(newNode, newNode);
        assert (oldNode == null);
        return newNode;
    }

    private ValueNode createSuffixNode(CharSequence s, int start2, int sValue) {
        ValueNode node = this.registerFinalValue(sValue);
        if (start2 < s.length()) {
            int offset = this.strings.length();
            this.strings.append(s, start2, s.length());
            node = new LinearMatchNode(this.strings, offset, s.length() - start2, node);
        }
        return node;
    }
        ...
        public void write(StringTrieBuilder builder) {
            ...
            while (--unitNumber >= 0) {
                ...
                if (this.equal[unitNumber] == null) {
                    value2 = this.values[unitNumber];
                    isFinal = true;
                } else {
                    assert (this.equal[unitNumber].getOffset() > 0);
                    value2 = this.offset - this.equal[unitNumber].getOffset();
                    isFinal = false;
                }
                builder.writeValueAndFinal(value2, isFinal);
                this.offset = builder.write(this.units[unitNumber]);
            }
        }

        public void add(int c, int value2) {
            this.units[this.length] = (char)c;
            this.equal[this.length] = null;
            this.values[this.length] = value2;
            ++this.length;
            this.hash = (this.hash * 37 + c) * 37 + value2;
        }
    ...
    extends ValueNode {
        ...
        @Override
        public Node add(StringTrieBuilder builder, CharSequence s, int start2, int sValue) {
            char c;
            int i;
            if (start2 == s.length()) {
                if (this.hasValue) {
            ...
            if ((i = this.find(c = s.charAt(start2++))) < this.chars.length() && c == this.chars.charAt(i)) {
        ...
        public Node register(StringTrieBuilder builder) {
            BranchHeadNode head5;
            Node subNode = this.register(builder, 0, this.chars.length());
            ValueNode result = head5 = new BranchHeadNode(this.chars.length(), subNode);
            if (this.hasValue) {
                if (builder.matchNodesCanHaveValues()) {
                    head5.setValue(this.value);
                } else {
                    result = new IntermediateValueNode(this.value, builder.registerNode(head5));
                }
            }
            return builder.registerNode(result);
        }

        private Node register(StringTrieBuilder builder, int start2, int limit) {
            int length = limit - start2;
            if (length > builder.getMaxBranchLinearSubNodeLength()) {
                int middle = start2 + length / 2;
                return builder.registerNode(new SplitBranchNode(this.chars.charAt(middle), this.register(builder, start2, middle), this.register(builder, middle, limit)));
            }
            ListBranchNode listNode = new ListBranchNode(length);
            do {
                char c = this.chars.charAt(start2);
                Node node = this.equal.get(start2);
    ...
    extends ValueNode {
        ...
        public Node add(StringTrieBuilder builder, CharSequence s, int start2, int sValue) {
            ...
            while (i < limit) {
                ...
                if (thisChar != (newChar = s.charAt(start2))) {
                    ValueNode result;
                    Node thisSuffixNode;
                    ...
                    ValueNode newSuffixNode = builder.createSuffixNode(s, start2 + 1, sValue);
            ...
            this.next = this.next.add(builder, s, start2, sValue);
            return this;
        }
    ...
    extends Node {
        ...
        public Node add(StringTrieBuilder builder, CharSequence s, int start2, int sValue) {
            ...
            ValueNode node = builder.createSuffixNode(s, start2, sValue);
