...
import com.cobblemon.mod.relocations.ibm.icu.impl.DateNumberFormat;
import com.cobblemon.mod.relocations.ibm.icu.impl.DayPeriodRules;
...
import com.cobblemon.mod.relocations.ibm.icu.text.MessageFormat;
...
import com.cobblemon.mod.relocations.ibm.icu.util.HebrewCalendar;
...
extends DateFormat {
    ...
    private static synchronized String getDefaultPattern() {
        ...
        if (!defaultLocale.equals(cachedDefaultLocale)) {
            ...
            try {
                ICUResourceBundle rb = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", cachedDefaultLocale);
    ...
    private StringBuffer format(Calendar cal, DisplayContext capitalizationContext, StringBuffer toAppendTo, FieldPosition pos, List<FieldPosition> attributes) {
        ...
        for (int i = 0; i < items.length; ++i) {
            ...
            if (attributes == null || (end2 = toAppendTo.length()) - start2 <= 0) continue;
    ...
    protected void subFormat(StringBuffer buf, char ch, int count, int beginOffset, int fieldNum, DisplayContext capitalizationContext, FieldPosition pos, char patternCharToOutput, Calendar cal) {
        ...
        int value2 = 0;
        if (field >= 0) {
            value2 = patternCharIndex != 34 ? cal.get(field) : cal.getRelatedYear();
        }
        ...
        switch (patternCharIndex) {
            case 0: {
                if (cal.getType().equals("chinese") || cal.getType().equals("dangi")) {
                    this.zeroPaddingNumber(currentNumberFormat, buf, value2, 1, 9);
                    break;
                }
                if (count == 5) {
                    SimpleDateFormat.safeAppend(this.formatData.narrowEras, value2, buf);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.ERA_NARROW;
                    break;
                }
                if (count == 4) {
                    SimpleDateFormat.safeAppend(this.formatData.eraNames, value2, buf);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.ERA_WIDE;
                    break;
                }
                SimpleDateFormat.safeAppend(this.formatData.eras, value2, buf);
                capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.ERA_ABBREV;
                break;
            }
            case 30: {
                if (this.formatData.shortYearNames != null && value2 <= this.formatData.shortYearNames.length) {
                    SimpleDateFormat.safeAppend(this.formatData.shortYearNames, value2 - 1, buf);
                    break;
                }
            }
            case 1: 
            case 18: {
                if (this.override != null && (this.override.compareTo("hebr") == 0 || this.override.indexOf("y=hebr") >= 0) && value2 > 5000 && value2 < 6000) {
                    value2 -= 5000;
                }
                if (count == 2) {
                    this.zeroPaddingNumber(currentNumberFormat, buf, value2, 2, 2);
                    break;
                }
                this.zeroPaddingNumber(currentNumberFormat, buf, value2, count, Integer.MAX_VALUE);
                break;
            }
            case 2: 
            case 26: {
                ...
                if (count == 5) {
                    if (patternCharIndex == 2) {
                        SimpleDateFormat.safeAppendWithMonthPattern(this.formatData.narrowMonths, value2, buf, isLeapMonth != 0 ? this.formatData.leapMonthPatterns[2] : null);
                    } else {
                        SimpleDateFormat.safeAppendWithMonthPattern(this.formatData.standaloneNarrowMonths, value2, buf, isLeapMonth != 0 ? this.formatData.leapMonthPatterns[5] : null);
                    }
                ...
                if (count == 4) {
                    if (patternCharIndex == 2) {
                        SimpleDateFormat.safeAppendWithMonthPattern(this.formatData.months, value2, buf, isLeapMonth != 0 ? this.formatData.leapMonthPatterns[0] : null);
                        capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.MONTH_FORMAT;
                        break;
                    }
                    SimpleDateFormat.safeAppendWithMonthPattern(this.formatData.standaloneMonths, value2, buf, isLeapMonth != 0 ? this.formatData.leapMonthPatterns[3] : null);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.MONTH_STANDALONE;
                    break;
                }
                if (count == 3) {
                    if (patternCharIndex == 2) {
                        SimpleDateFormat.safeAppendWithMonthPattern(this.formatData.shortMonths, value2, buf, isLeapMonth != 0 ? this.formatData.leapMonthPatterns[1] : null);
                        capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.MONTH_FORMAT;
                        break;
                    }
                    SimpleDateFormat.safeAppendWithMonthPattern(this.formatData.standaloneShortMonths, value2, buf, isLeapMonth != 0 ? this.formatData.leapMonthPatterns[4] : null);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.MONTH_STANDALONE;
                    break;
                }
                StringBuffer monthNumber = new StringBuffer();
                this.zeroPaddingNumber(currentNumberFormat, monthNumber, value2 + 1, count, Integer.MAX_VALUE);
            ...
            case 8: {
                ...
                this.numberFormat.format(value2, buf, p);
                if (count <= 3) break;
            ...
            case 9: {
                if (count == 5) {
                    SimpleDateFormat.safeAppend(this.formatData.narrowWeekdays, value2, buf);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.DAY_NARROW;
                    break;
                }
                if (count == 4) {
                    SimpleDateFormat.safeAppend(this.formatData.weekdays, value2, buf);
                ...
                if (count == 6 && this.formatData.shorterWeekdays != null) {
                    SimpleDateFormat.safeAppend(this.formatData.shorterWeekdays, value2, buf);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.DAY_FORMAT;
                    break;
                }
                SimpleDateFormat.safeAppend(this.formatData.shortWeekdays, value2, buf);
                capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.DAY_FORMAT;
                break;
            }
            case 14: {
                if (count < 5 || this.formatData.ampmsNarrow == null) {
                    SimpleDateFormat.safeAppend(this.formatData.ampms, value2, buf);
                    break;
                }
                SimpleDateFormat.safeAppend(this.formatData.ampmsNarrow, value2, buf);
                break;
            }
            case 15: {
                if (value2 == 0) {
            ...
            case 25: {
                if (count < 3) {
                    this.zeroPaddingNumber(currentNumberFormat, buf, value2, 1, Integer.MAX_VALUE);
                    break;
                }
                value2 = cal.get(7);
                if (count == 5) {
                    SimpleDateFormat.safeAppend(this.formatData.standaloneNarrowWeekdays, value2, buf);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.DAY_NARROW;
                    break;
                }
                if (count == 4) {
                    SimpleDateFormat.safeAppend(this.formatData.standaloneWeekdays, value2, buf);
                ...
                if (count == 6 && this.formatData.standaloneShorterWeekdays != null) {
                    SimpleDateFormat.safeAppend(this.formatData.standaloneShorterWeekdays, value2, buf);
                    capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.DAY_STANDALONE;
                    break;
                }
                SimpleDateFormat.safeAppend(this.formatData.standaloneShortWeekdays, value2, buf);
                capContextUsageType = DateFormatSymbols.CapitalizationContextUsage.DAY_STANDALONE;
                break;
            }
            case 27: {
                if (count >= 5) {
                    SimpleDateFormat.safeAppend(this.formatData.narrowQuarters, value2 / 3, buf);
                    break;
                }
                if (count == 4) {
                    SimpleDateFormat.safeAppend(this.formatData.quarters, value2 / 3, buf);
                    break;
                }
                if (count == 3) {
                    SimpleDateFormat.safeAppend(this.formatData.shortQuarters, value2 / 3, buf);
                    break;
                }
                this.zeroPaddingNumber(currentNumberFormat, buf, value2 / 3 + 1, count, Integer.MAX_VALUE);
                break;
            }
            case 28: {
                if (count >= 5) {
                    SimpleDateFormat.safeAppend(this.formatData.standaloneNarrowQuarters, value2 / 3, buf);
                    break;
                }
                if (count == 4) {
                    SimpleDateFormat.safeAppend(this.formatData.standaloneQuarters, value2 / 3, buf);
                    break;
                }
                if (count == 3) {
                    SimpleDateFormat.safeAppend(this.formatData.standaloneShortQuarters, value2 / 3, buf);
                    break;
                }
                this.zeroPaddingNumber(currentNumberFormat, buf, value2 / 3 + 1, count, Integer.MAX_VALUE);
                break;
            ...
            case 35: {
                ...
                if (!(hour != 12 || this.hasMinute && cal.get(12) != 0 || this.hasSecond && cal.get(13) != 0)) {
                    value2 = cal.get(9);
                    toAppend = count <= 3 ? this.formatData.abbreviatedDayPeriods[value2] : (count == 4 || count > 5 ? this.formatData.wideDayPeriods[value2] : this.formatData.narrowDayPeriods[value2]);
                }
                if (toAppend == null) {
    ...
    private static void safeAppend(String[] array, int value2, StringBuffer appendTo) {
        if (array != null && value2 >= 0 && value2 < array.length) {
            appendTo.append(array[value2]);
        }
    }

    private static void safeAppendWithMonthPattern(String[] array, int value2, StringBuffer appendTo, String monthPattern) {
        if (array != null && value2 >= 0 && value2 < array.length) {
            if (monthPattern == null) {
                appendTo.append(array[value2]);
            } else {
                String s = SimpleFormatterImpl.formatRawPattern(monthPattern, 1, 1, array[value2]);
                appendTo.append(s);
            }
        }
    }
    ...
    @Deprecated
    protected void zeroPaddingNumber(NumberFormat nf, StringBuffer buf, int value2, int minDigits, int maxDigits) {
        if (this.useLocalZeroPaddingNumberFormat && value2 >= 0) {
            this.fastZeroPaddingNumber(buf, value2, minDigits, maxDigits);
        } else {
            nf.setMinimumIntegerDigits(minDigits);
            nf.setMaximumIntegerDigits(maxDigits);
            nf.format(value2, buf, new FieldPosition(-1));
        }
    }
    ...
    private void fastZeroPaddingNumber(StringBuffer buf, int value2, int minDigits, int maxDigits) {
        int padding;
        int limit = this.decimalBuf.length < maxDigits ? this.decimalBuf.length : maxDigits;
        int index = limit - 1;
        while (true) {
            this.decimalBuf[index] = this.decDigits[value2 % 10];
            if (index == 0 || (value2 /= 10) == 0) break;
            --index;
        }
    ...
    protected String zeroPaddingNumber(long value2, int minDigits, int maxDigits) {
    ...
    private int matchLiteral(String text, int pos, Object[] items, int itemIndex, boolean[] complete) {
        ...
        if (!complete[0] && this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_ALLOW_WHITESPACE) && 0 < itemIndex && itemIndex < items.length - 1 && originalPos < tlen) {
            Object before = items[itemIndex - 1];
            Object after2 = items[itemIndex + 1];
            if (before instanceof PatternItem && after2 instanceof PatternItem) {
                char beforeType = ((PatternItem)before).type;
                char afterType = ((PatternItem)after2).type;
    ...
    protected int matchString(String text, int start2, int field, String[] data, Calendar cal) {
        return this.matchString(text, start2, field, data, null, cal);
    }

    @Deprecated
    private int matchString(String text, int start2, int field, String[] data, String monthPattern, Calendar cal) {
        int i = 0;
        ...
        while (i < count) {
            ...
            if (length > bestMatchLength && (matchLength = this.regionMatchesWithOptionalDot(text, start2, data[i], length)) >= 0) {
                bestMatch = i;
                bestMatchLength = matchLength;
                isLeapMonth = 0;
            }
            if (monthPattern != null && (length = (leapMonthName = SimpleFormatterImpl.formatRawPattern(monthPattern, 1, 1, data[i])).length()) > bestMatchLength && (matchLength = this.regionMatchesWithOptionalDot(text, start2, leapMonthName, length)) >= 0) {
                bestMatch = i;
                bestMatchLength = matchLength;
                isLeapMonth = 1;
            }
        ...
        if (bestMatch >= 0) {
            ...
            return start2 + bestMatchLength;
        }
        return ~start2;
    }

    private int regionMatchesWithOptionalDot(String text, int start2, String data, int length) {
        boolean matches2 = text.regionMatches(true, start2, data, 0, length);
        if (matches2) {
            return length;
        }
        if (data.length() > 0 && data.charAt(data.length() - 1) == '.' && text.regionMatches(true, start2, data, 0, length - 1)) {
            return length - 1;
        }
        return -1;
    }

    protected int matchQuarterString(String text, int start2, int field, String[] data, Calendar cal) {
        int count = data.length;
        int bestMatchLength = 0;
        int bestMatch = -1;
        int matchLength = 0;
        for (int i = 0; i < count; ++i) {
            int length = data[i].length();
            if (length <= bestMatchLength || (matchLength = this.regionMatchesWithOptionalDot(text, start2, data[i], length)) < 0) continue;
            bestMatch = i;
            bestMatchLength = matchLength;
        }
        if (bestMatch >= 0) {
    ...
    private int matchDayPeriodString(String text, int start2, String[] data, int dataLength, Output<DayPeriodRules.DayPeriod> dayPeriod) {
        int bestMatchLength = 0;
        int bestMatch = -1;
        int matchLength = 0;
        for (int i = 0; i < dataLength; ++i) {
            int length;
            if (data[i] == null || (length = data[i].length()) <= bestMatchLength || (matchLength = this.regionMatchesWithOptionalDot(text, start2, data[i], length)) < 0) continue;
            bestMatch = i;
            bestMatchLength = matchLength;
        }
        if (bestMatch >= 0) {
    ...
    protected int subParse(String text, int start2, char ch, int count, boolean obeyCount, boolean allowNegative, boolean[] ambiguousYear, Calendar cal) {
        return this.subParse(text, start2, ch, count, obeyCount, allowNegative, ambiguousYear, cal, null, null);
    }

    private int subParse(String text, int start2, char ch, int count, boolean obeyCount, boolean allowNegative, boolean[] ambiguousYear, Calendar cal, MessageFormat numericLeapMonthFormatter, Output<TimeZoneFormat.TimeType> tzTimeType) {
        return this.subParse(text, start2, ch, count, obeyCount, allowNegative, ambiguousYear, cal, null, null, null);
    }

    @Deprecated
    private int subParse(String text, int start2, char ch, int count, boolean obeyCount, boolean allowNegative, boolean[] ambiguousYear, Calendar cal, MessageFormat numericLeapMonthFormatter, Output<TimeZoneFormat.TimeType> tzTimeType, Output<DayPeriodRules.DayPeriod> dayPeriod) {
        ...
        if (patternCharIndex == 4 || patternCharIndex == 15 || patternCharIndex == 2 && count <= 2 || patternCharIndex == 26 || patternCharIndex == 19 || patternCharIndex == 25 || patternCharIndex == 1 || patternCharIndex == 18 || patternCharIndex == 30 || patternCharIndex == 0 && isChineseCalendar || patternCharIndex == 27 || patternCharIndex == 28 || patternCharIndex == 8) {
            ...
            if (numericLeapMonthFormatter != null && (patternCharIndex == 2 || patternCharIndex == 26)) {
                ...
                if (args != null && pos.getIndex() > start2 && args[0] instanceof Number) {
        ...
        switch (patternCharIndex) {
            case 0: {
                ...
                int ps = 0;
                ps = count == 5 ? this.matchString(text, start2, 0, this.formatData.narrowEras, null, cal) : (count == 4 ? this.matchString(text, start2, 0, this.formatData.eraNames, null, cal) : this.matchString(text, start2, 0, this.formatData.eras, null, cal));
                if (ps == ~start2) {
                    ps = -32000;
                }
                return ps;
            ...
            case 1: 
            case 18: {
                if (this.override != null && (this.override.compareTo("hebr") == 0 || this.override.indexOf("y=hebr") >= 0) && value2 < 1000) {
                    value2 += 5000;
                } else if (count == 2 && SimpleDateFormat.countDigits(text, start2, pos.getIndex()) == 2 && cal.haveDefaultCentury()) {
                    int ambiguousTwoDigitYear = this.getDefaultCenturyStartYear() % 100;
                    ambiguousYear[0] = value2 == ambiguousTwoDigitYear;
                    value2 += this.getDefaultCenturyStartYear() / 100 * 100 + (value2 < ambiguousTwoDigitYear ? 100 : 0);
                }
                cal.set(field, value2);
                if (DelayedHebrewMonthCheck) {
                    if (!HebrewCalendar.isLeapYear(value2)) {
                        cal.add(2, 1);
                    }
            ...
            case 30: {
                int newStart;
                if (this.formatData.shortYearNames != null && (newStart = this.matchString(text, start2, 1, this.formatData.shortYearNames, null, cal)) > 0) {
                    return newStart;
                }
                if (number != null && (this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_ALLOW_NUMERIC) || this.formatData.shortYearNames == null || value2 > this.formatData.shortYearNames.length)) {
                    cal.set(1, value2);
                    return pos.getIndex();
                }
                return ~start2;
            ...
            case 26: {
                if (count <= 2 || number != null && this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_ALLOW_NUMERIC)) {
                    cal.set(2, value2 - 1);
                    if (cal.getType().equals("hebrew") && value2 >= 6) {
                        if (cal.isSet(1)) {
                ...
                if (this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) {
                    int n = patternCharIndex == 2 ? this.matchString(text, start2, 2, this.formatData.months, haveMonthPat ? this.formatData.leapMonthPatterns[0] : null, cal) : (newStart = this.matchString(text, start2, 2, this.formatData.standaloneMonths, haveMonthPat ? this.formatData.leapMonthPatterns[3] : null, cal));
                    if (newStart > 0) {
                        return newStart;
                    }
                }
                if (this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 3) {
                    return patternCharIndex == 2 ? this.matchString(text, start2, 2, this.formatData.shortMonths, haveMonthPat ? this.formatData.leapMonthPatterns[1] : null, cal) : this.matchString(text, start2, 2, this.formatData.standaloneShortMonths, haveMonthPat ? this.formatData.leapMonthPatterns[4] : null, cal);
                }
                return newStart;
            }
            case 4: {
                if (value2 == cal.getMaximum(11) + 1) {
                    value2 = 0;
                }
                cal.set(11, value2);
                return pos.getIndex();
            ...
            case 8: {
                int i;
                if (i < 3) {
                    for (i = SimpleDateFormat.countDigits(text, start2, pos.getIndex()); i < 3; ++i) {
                        value2 *= 10;
                    }
                } else {
                    int a = 1;
            ...
            case 9: {
                int newStart = 0;
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchString(text, start2, 7, this.formatData.weekdays, null, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 3) && (newStart = this.matchString(text, start2, 7, this.formatData.shortWeekdays, null, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 6) && this.formatData.shorterWeekdays != null && (newStart = this.matchString(text, start2, 7, this.formatData.shorterWeekdays, null, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 5) && this.formatData.narrowWeekdays != null && (newStart = this.matchString(text, start2, 7, this.formatData.narrowWeekdays, null, cal)) > 0) {
                    return newStart;
                }
                return newStart;
            }
            case 25: {
                ...
                int newStart = 0;
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchString(text, start2, 7, this.formatData.standaloneWeekdays, null, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 3) && (newStart = this.matchString(text, start2, 7, this.formatData.standaloneShortWeekdays, null, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 6) && this.formatData.standaloneShorterWeekdays != null) {
                    return this.matchString(text, start2, 7, this.formatData.standaloneShorterWeekdays, null, cal);
                }
                return newStart;
            }
            case 14: {
                int newStart = 0;
                if ((this.formatData.ampmsNarrow == null || count < 5 || this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH)) && (newStart = this.matchString(text, start2, 9, this.formatData.ampms, null, cal)) > 0) {
                    return newStart;
                }
                if (this.formatData.ampmsNarrow != null && (count >= 5 || this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH)) && (newStart = this.matchString(text, start2, 9, this.formatData.ampmsNarrow, null, cal)) > 0) {
                    return newStart;
                }
                return ~start2;
            }
            case 15: {
                if (value2 == cal.getLeastMaximum(10) + 1) {
                    value2 = 0;
                }
                cal.set(10, value2);
                return pos.getIndex();
            ...
            case 27: {
                if (count <= 2 && number != null) {
                    cal.set(2, (value2 - 1) * 3);
                    return pos.getIndex();
                }
                int newStart = 0;
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchQuarterString(text, start2, 2, this.formatData.quarters, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 3) && (newStart = this.matchQuarterString(text, start2, 2, this.formatData.shortQuarters, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 5) && (newStart = this.matchQuarterString(text, start2, 2, this.formatData.narrowQuarters, cal)) > 0) {
                    return newStart;
                }
            ...
            case 28: {
                ...
                int newStart = 0;
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchQuarterString(text, start2, 2, this.formatData.standaloneQuarters, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 3) && (newStart = this.matchQuarterString(text, start2, 2, this.formatData.standaloneShortQuarters, cal)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 5) && (newStart = this.matchQuarterString(text, start2, 2, this.formatData.standaloneNarrowQuarters, cal)) > 0) {
                    return newStart;
                }
                if (this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_ALLOW_NUMERIC) && number != null) {
                    cal.set(2, (value2 - 1) * 3);
            ...
            case 37: {
                ...
                return this.matchString(text, start2, -1, data.toArray(new String[0]), cal);
            }
            case 35: {
                int ampmStart = this.subParse(text, start2, 'a', count, obeyCount, allowNegative, ambiguousYear, cal, numericLeapMonthFormatter, tzTimeType, dayPeriod);
                if (ampmStart > 0) {
                    return ampmStart;
                }
                int newStart = 0;
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 3) && (newStart = this.matchDayPeriodString(text, start2, this.formatData.abbreviatedDayPeriods, 2, dayPeriod)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchDayPeriodString(text, start2, this.formatData.wideDayPeriods, 2, dayPeriod)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchDayPeriodString(text, start2, this.formatData.narrowDayPeriods, 2, dayPeriod)) > 0) {
                    return newStart;
                }
                return newStart;
            }
            case 36: {
                int newStart = 0;
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 3) && (newStart = this.matchDayPeriodString(text, start2, this.formatData.abbreviatedDayPeriods, this.formatData.abbreviatedDayPeriods.length, dayPeriod)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchDayPeriodString(text, start2, this.formatData.wideDayPeriods, this.formatData.wideDayPeriods.length, dayPeriod)) > 0) {
                    return newStart;
                }
                if ((this.getBooleanAttribute(DateFormat.BooleanAttribute.PARSE_MULTIPLE_PATTERNS_FOR_MATCH) || count == 4) && (newStart = this.matchDayPeriodString(text, start2, this.formatData.narrowDayPeriods, this.formatData.narrowDayPeriods.length, dayPeriod)) > 0) {
                    return newStart;
                }
                return newStart;
            }
        ...
        if (!this.isLenient() && pos.getIndex() < start2 + count) {
            return -start2;
        }
        if (number != null) {
    ...
    private static int countDigits(String text, int start2, int end2) {
        int cp;
        int numDigits = 0;
        for (int idx = start2; idx < end2; idx += UCharacter.charCount(cp)) {
    ...
    private boolean diffCalFieldValue(Calendar fromCalendar, Calendar toCalendar, Object[] items, int i) throws IllegalArgumentException {
        ...
        int field = PATTERN_INDEX_TO_CALENDAR_FIELD[patternCharIndex];
        return field >= 0 && (value2 = fromCalendar.get(field)) != (value_2 = toCalendar.get(field));
    }
    ...
    private void processOverrideString(ULocale loc, String str) {
        ...
        while (moreToProcess) {
            ...
            String nsName;
            int end2;
            int delimiterPosition = str.indexOf(";", start2);
            if (delimiterPosition == -1) {
                moreToProcess = false;
                end2 = str.length();
            } else {
                end2 = delimiterPosition;
            }
            String currentString = str.substring(start2, end2);
