...
public final class UBiDiProps {
    ...
    public final int getMaxValue(int which) {
        int max2 = this.indexes[15];
        switch (which) {
            case 4096: {
                return max2 & 0x1F;
            }
            case 4102: {
                return (max2 & 0xFF0000) >> 16;
            }
            case 4103: {
                return (max2 & 0xE0) >> 5;
            }
    ...
    public final int getJoiningGroup(int c) {
        ...
        int limit = this.indexes[5];
        if (start2 <= c && c < limit) {
            return this.jgArray[c - start2] & 0xFF;
        }
        start2 = this.indexes[6];
        limit = this.indexes[7];
        if (start2 <= c && c < limit) {
            return this.jgArray2[c - start2] & 0xFF;
        }
        return 0;
    }
    ...
    private static final boolean getFlagFromProps(int props, int shift2) {
        return (props >> shift2 & 1) != 0;
    }
