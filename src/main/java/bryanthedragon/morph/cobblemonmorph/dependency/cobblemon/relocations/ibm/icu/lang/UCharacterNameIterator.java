...
implements ValueIterator {
    ...
    public boolean next(ValueIterator.Element element) {
        ...
        if ((this.m_choice_ == 0 || this.m_choice_ == 2) && this.m_algorithmIndex_ < (length = this.m_name_.getAlgorithmLength())) {
            ...
            if (this.m_algorithmIndex_ < length) {
                int start2 = this.m_name_.getAlgorithmStart(this.m_algorithmIndex_);
                if (this.m_current_ < start2) {
                    int end2 = start2;
                    if (this.m_limit_ <= start2) {
                        end2 = this.m_limit_;
                    }
                    if (!this.iterateGroup(element, end2)) {
                        ++this.m_current_;
                        return true;
                    }
                }
    ...
    public void setRange(int start2, int limit) {
        ...
        this.m_start_ = start2 < 0 ? 0 : start2;
