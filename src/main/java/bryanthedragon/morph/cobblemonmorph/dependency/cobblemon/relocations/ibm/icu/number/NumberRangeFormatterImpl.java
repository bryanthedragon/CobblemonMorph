...
class NumberRangeFormatterImpl {
   ...
   private static void getNumberRangeData(ULocale locale, String nsName, NumberRangeFormatterImpl out) {
      ...
      NumberRangeFormatterImpl.NumberRangeDataSink sink = new NumberRangeFormatterImpl.NumberRangeDataSink(sb);
   ...
   public NumberRangeFormatterImpl(RangeMacroProps macros) {
      ...
      if (nsName != null && nsName.equals(this.formatterImpl2.getRawMicroProps().nsName)) {
         ...
         if (!this.fSameFormatters
            || this.fIdentityFallback != NumberRangeFormatter.RangeIdentityFallback.APPROXIMATELY
               && this.fIdentityFallback != NumberRangeFormatter.RangeIdentityFallback.APPROXIMATELY_OR_SINGLE_VALUE) {
            this.fApproximatelyFormatter = null;
         } else {
            MacroProps approximatelyMacros = new MacroProps();
            approximatelyMacros.approximately = true;
            this.fApproximatelyFormatter = new NumberFormatterImpl(formatter1.macros(approximatelyMacros).resolve());
         }
   ...
   private void formatRange(DecimalQuantity quantity1, DecimalQuantity quantity2, FormattedStringBuilder string, MicroProps micros1, MicroProps micros2) {
      ...
      if (repeatInner || repeatMiddle || repeatOuter) {
         if (!PatternProps.isWhiteSpace(string.charAt(h.index1()))) {
            h.lengthInfix = h.lengthInfix + string.insertCodePoint(h.index1(), 32, null);
         }

         if (!PatternProps.isWhiteSpace(string.charAt(h.index2() - 1))) {
            h.lengthInfix = h.lengthInfix + string.insertCodePoint(h.index2(), 32, null);
         }
      }

      h.length1 = h.length1 + NumberFormatterImpl.writeNumber(micros1, quantity1, string, h.index0());
      ...
      h.length2 = h.length2 + string.insert(h.index2(), tempString);
      if (collapseInner) {
         Modifier mod = this.resolveModifierPlurals(micros1.modInner, micros2.modInner);
         h.lengthSuffix = h.lengthSuffix + mod.apply(string, h.index0(), h.index4());
         h.lengthPrefix = h.lengthPrefix + mod.getPrefixLength();
         h.lengthSuffix = h.lengthSuffix - mod.getPrefixLength();
      } else {
         h.length1 = h.length1 + micros1.modInner.apply(string, h.index0(), h.index1());
         h.length2 = h.length2 + micros2.modInner.apply(string, h.index2(), h.index4());
      }
      ...
      if (collapseMiddle) {
         ...
         h.length1 = h.length1 + micros1.modMiddle.apply(string, h.index0(), h.index1());
         h.length2 = h.length2 + micros2.modMiddle.apply(string, h.index2(), h.index4());
      }
      ...
      if (collapseOuter) {
         ...
         h.length1 = h.length1 + micros1.modOuter.apply(string, h.index0(), h.index1());
         h.length2 = h.length2 + micros2.modOuter.apply(string, h.index2(), h.index4());
      }
