...
extends MeasureUnit {
    ...
    public int getNumericCode() {
        int result = 0;
        try {
            UResourceBundle bundle = UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", "currencyNumericCodes", ICUResourceBundle.ICU_DATA_CLASS_LOADER);
    ...
    private static void setupCurrencyTrieVec(ULocale locale, List<TextTrieMap<CurrencyStringInfo>> trieVec) {
        ...
        for (Map.Entry<String, String> e : names.symbolMap().entrySet()) {
            ...
            CurrencyStringInfo value2 = new CurrencyStringInfo(isoCode, symbol);
            if (key != null) {
                UnicodeSet equivalents = StaticUnicodeSets.get(key);
                for (String equivalentSymbol : equivalents) {
                    symTrie.put(equivalentSymbol, value2);
                }
                continue;
            }
            symTrie.put(symbol, value2);
        }
    ...
    private static synchronized Set<String> getAllCurrenciesAsSet() {
        Set<String> all;
        Set<String> set2 = all = ALL_CODES_AS_SET == null ? null : ALL_CODES_AS_SET.get();
        if (all == null) {
            CurrencyMetaInfo info = CurrencyMetaInfo.getInstance();
