...
extends DictionaryBreakEngine {
    ...
    private static float[][] make2DArray(int[] data, int start2, int d1, int d2) {
    ...
    private static float[] make1DArray(int[] data, int start2, int d1) {
        byte[] bytes = new byte[4];
    ...
    public LSTMBreakEngine(int script, UnicodeSet set2, LSTMData data) {
        this.setCharacters(set2);
        this.fScript = script;
        this.fData = data;
    ...
    private static void sigmoid(float[] result, int start2, int length) {
        assert (start2 < result.length);
        assert (start2 + length <= result.length);
        for (int i = start2; i < start2 + length; ++i) {
            result[i] = (float)(1.0 / (1.0 + Math.exp(-result[i])));
        }
    }

    private static void tanh(float[] result, int start2, int length) {
        assert (start2 < result.length);
        assert (start2 + length <= result.length);
        for (int i = start2; i < start2 + length; ++i) {
    ...
    public static LSTMData createData(int script) {
        ...
        UResourceBundle rb = UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b/brkitr", name, ICUResourceBundle.ICU_DATA_CLASS_LOADER);
    ...
    public static LSTMBreakEngine create(int script, LSTMData data) {
        ...
        set2.applyPattern(setExpr);
        set2.compact();
        return new LSTMBreakEngine(script, set2, data);
    }
    ...
    public static class LSTMData {
        ...
        public LSTMData(UResourceBundle rb) {
            ...
            int start2 = 0;
            this.fEmbedding = LSTMBreakEngine.make2DArray(data, start2, numIndex + 1, embeddings);
            this.fForwardW = LSTMBreakEngine.make2DArray(data, start2 += mat1Size, embeddings, 4 * hunits);
            this.fForwardU = LSTMBreakEngine.make2DArray(data, start2 += mat2Size, hunits, 4 * hunits);
            this.fForwardB = LSTMBreakEngine.make1DArray(data, start2 += mat3Size, 4 * hunits);
            this.fBackwardW = LSTMBreakEngine.make2DArray(data, start2 += mat4Size, embeddings, 4 * hunits);
            this.fBackwardU = LSTMBreakEngine.make2DArray(data, start2 += mat5Size, hunits, 4 * hunits);
            this.fBackwardB = LSTMBreakEngine.make1DArray(data, start2 += mat6Size, 4 * hunits);
            this.fOutputW = LSTMBreakEngine.make2DArray(data, start2 += mat7Size, 2 * hunits, 4);
            this.fOutputB = LSTMBreakEngine.make1DArray(data, start2 += mat8Size, 4);
        }
    }
