...
public final class RubyFlags extends AbstractConstantKeysObject {
   ...
   private final RubyFlags.Mode mode;
   ...
   public RubyFlags(String source) {
      ...
      this.value = bits;
      this.mode = RubyFlags.Mode.Default;
   }

   private RubyFlags(int value, RubyFlags.Mode mode) {
      this.value = value;
      this.mode = mode;
   }

   private static int maskForFlag(int flagChar) {
      return 1 << indexOfUnrolled("mixy", flagChar);
   }

   public boolean hasFlag(int flagChar) {
      return isTypeFlag(flagChar) ? this.mode.equals(RubyFlags.Mode.fromFlagChar(flagChar)) : (this.value & maskForFlag(flagChar)) != 0;
   }
   ...
   public RubyFlags addFlag(int flagChar) {
      return isTypeFlag(flagChar)
         ? new RubyFlags(this.value, RubyFlags.Mode.fromFlagChar(flagChar))
         : new RubyFlags(this.value | maskForFlag(flagChar), this.mode);
   }
   ...
   public static boolean isValidFlagChar(int candidateChar) {
      return indexOfUnrolled("mixadu", candidateChar) >= 0;
   }

   public static boolean isBitFlag(int candidateChar) {
      return indexOfUnrolled("mixy", candidateChar) >= 0;
   }

   public static boolean isTypeFlag(int candidateChar) {
      return indexOfUnrolled("adu", candidateChar) >= 0;
   }
   ...
   public String toString() {
      StringBuilder out = new StringBuilder("mix".length());

      for (int i = 0; i < "mix".length(); i++) {
         char flag = "mix".charAt(i);
   ...
   private static enum Mode {
      ...
      public static final RubyFlags.Mode[] VALUES = values();

      public static RubyFlags.Mode fromFlagChar(int ch) {
         return VALUES[RubyFlags.indexOfUnrolled("adu", ch)];
      }
   }
}
