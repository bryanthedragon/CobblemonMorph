...
import com.cobblemon.mod.relocations.ibm.icu.impl.coll.CollationFCD;
...
public abstract class CollationIterator {
    ...
    private final void appendNumericSegmentCEs(CharSequence digits) {
        ...
        if (length <= 7) {
            int value2 = digits.charAt(0);
            for (int i = 1; i < length; ++i) {
                value2 = value2 * 10 + digits.charAt(i);
            }
            int firstByte = 2;
            int numBytes = 74;
            if (value2 < numBytes) {
                long primary = numericPrimary | (long)(firstByte + value2 << 16);
            ...
            firstByte += numBytes;
            numBytes = 40;
            if (value2 < numBytes * 254) {
                long primary = numericPrimary | (long)(firstByte + value2 / 254 << 16) | (long)(2 + value2 % 254 << 8);
                this.ceBuffer.append(Collation.makeCE(primary));
                return;
            }
            value2 -= numBytes * 254;
            firstByte += numBytes;
            numBytes = 16;
            if (value2 < numBytes * 254 * 254) {
                long primary = numericPrimary | (long)(2 + value2 % 254);
                primary |= (long)(2 + (value2 /= 254) % 254 << 8);
                this.ceBuffer.append(Collation.makeCE(primary |= (long)(firstByte + (value2 /= 254) % 254 << 16)));
                return;
            }
        }
        assert (length >= 7);
        ...
        while (pos < length) {
            if (shift2 == 0) {
                ...
                primary |= (long)(pair << shift2);
                shift2 -= 8;
            }
            pair = 11 + 2 * (digits.charAt(pos) * 10 + digits.charAt(pos + 1));
            pos += 2;
        }
        this.ceBuffer.append(Collation.makeCE(primary |= (long)(pair - 1 << shift2)));
    }
