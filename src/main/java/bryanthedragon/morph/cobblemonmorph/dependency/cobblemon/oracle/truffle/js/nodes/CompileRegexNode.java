...
public abstract class CompileRegexNode extends JavaScriptBaseNode {
   ...
   @Specialization(
      guards = {"stringEquals(equalsNode, pattern, cachedPattern)", "stringEquals(equalsNode2, flags, cachedFlags)"},
      limit = "MaxCompiledRegexCacheLength"
   )
   ...
   protected Object doCompile(TruffleString pattern, TruffleString flags) {
      return RegexCompilerInterface.compile(
         Strings.toJavaString(pattern), Strings.toJavaString(flags), this.context, this.getRealm(), this.getIsCompiledRegexNullNode()
      );
   }
