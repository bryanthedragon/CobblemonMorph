...
public abstract class RootNode extends ExecutableNode {
   private static final AtomicReferenceFieldUpdater<RootNode, ReentrantLock> LOCK_UPDATER = AtomicReferenceFieldUpdater.newUpdater(
      RootNode.class, ReentrantLock.class, "lock"
   );
   ...
   public boolean isInternal() {
      if (this.getLanguageInfo() == null) {
         ...
         return sc != null ? sc.getSource().isInternal() : false;
      }
   }
   ...
   protected Object translateStackTraceElement(TruffleStackTraceElement element) {
      ...
      return NodeAccessor.EXCEPTION
         .createDefaultStackTraceElementObject(element.getTarget().getRootNode(), location != null ? location.getEncapsulatingSourceSection() : null);
   }
   ...
   public static RootNode createConstantNode(Object constant) {
      return new RootNode.Constant(constant);
   }
