...
package bryanthedragon.morph.cobblemonmorph.dependency.cobblemon.mod.relocations.ibm.icu.math;

import com.cobblemon.mod.relocations.ibm.icu.lang.UCharacter;
import com.cobblemon.mod.relocations.ibm.icu.math.MathContext;
...
Comparable<BigDecimal> {
    ...
    public BigDecimal abs(MathContext set2) {
        if (this.ind == -1) {
            return this.negate(set2);
        }
        return this.plus(set2);
    }
    ...
    public BigDecimal add(BigDecimal rhs, MathContext set2) {
        int newlen = 0;
        int tlen = 0;
        int mult = 0;
        byte[] t = null;
        ...
        byte cb = 0;
        if (set2.lostDigits) {
            this.checkdigits(rhs, set2.digits);
        }
        BigDecimal lhs = this;
        if (lhs.ind == 0 && set2.form != 0) {
            return rhs.plus(set2);
        }
        if (rhs.ind == 0 && set2.form != 0) {
            return lhs.plus(set2);
        }
        int reqdig = set2.digits;
        if (reqdig > 0) {
            if (lhs.mant.length > reqdig) {
                lhs = BigDecimal.clone(lhs).round(set2);
            }
            if (rhs.mant.length > reqdig) {
                rhs = BigDecimal.clone(rhs).round(set2);
            }
        }
    ...
    public int compareTo(BigDecimal rhs, MathContext set2) {
        int thislength = 0;
        int i = 0;
        if (set2.lostDigits) {
            this.checkdigits(rhs, set2.digits);
        ...
        if (this.ind == rhs.ind & this.exp == rhs.exp) {
            ...
            if (thislength <= set2.digits | set2.digits == 0) {
                int $6 = thislength;
                i = 0;
                while ($6 > 0) {
        ...
        return this.add((BigDecimal)newrhs, (MathContext)set2).ind;
    }
    ...
    public BigDecimal divide(BigDecimal rhs, int round) {
        MathContext set2 = new MathContext(0, 0, false, round);
        return this.dodivide('D', rhs, set2, -1);
    }
    ...
    public BigDecimal divide(BigDecimal rhs, int scale, int round) {
        ...
        MathContext set2 = new MathContext(0, 0, false, round);
        return this.dodivide('D', rhs, set2, scale);
    }

    public BigDecimal divide(BigDecimal rhs, MathContext set2) {
        return this.dodivide('D', rhs, set2, -1);
    }
    ...
    public BigDecimal divideInteger(BigDecimal rhs, MathContext set2) {
        return this.dodivide('I', rhs, set2, 0);
    }
    ...
    public BigDecimal max(BigDecimal rhs, MathContext set2) {
        if (this.compareTo(rhs, set2) >= 0) {
            return this.plus(set2);
        }
        return rhs.plus(set2);
    }
    ...
    public BigDecimal min(BigDecimal rhs, MathContext set2) {
        if (this.compareTo(rhs, set2) <= 0) {
            return this.plus(set2);
        }
        return rhs.plus(set2);
    ...
    public BigDecimal multiply(BigDecimal rhs, MathContext set2) {
        byte[] multer = null;
        byte[] multand = null;
        int acclen = 0;
        int n = 0;
    ...
    public BigDecimal negate(MathContext set2) {
        if (set2.lostDigits) {
            this.checkdigits(null, set2.digits);
        }
    ...
    public BigDecimal plus(MathContext set2) {
        if (set2.lostDigits) {
            this.checkdigits(null, set2.digits);
        }
        if (set2.form == 0 && this.form == 0) {
            if (this.mant.length <= set2.digits) {
                return this;
            }
            if (set2.digits == 0) {
                return this;
        ...
        return BigDecimal.clone(this).finish(set2, false);
    }
    ...
    public BigDecimal pow(BigDecimal rhs, MathContext set2) {
        int workdigits = 0;
        int L = 0;
        int i = 0;
        if (set2.lostDigits) {
        ...
        MathContext workset = new MathContext(workdigits, set2.form, false, set2.roundingMode);
        BigDecimal res = ONE;
        if (n == 0) {
            return res;
        }
    ...
    public BigDecimal remainder(BigDecimal rhs, MathContext set2) {
        return this.dodivide('R', rhs, set2, -1);
    }
    ...
    public BigDecimal subtract(BigDecimal rhs, MathContext set2) {
        if (set2.lostDigits) {
            this.checkdigits(rhs, set2.digits);
        }
        BigDecimal newrhs = BigDecimal.clone(rhs);
        newrhs.ind = -newrhs.ind;
        return this.add(newrhs, set2);
    }
    ...
    public String format(int before, int after2) {
        return this.format(before, after2, -1, -1, 1, 4);
    }

    public String format(int before, int after2, int explaces, int exdigits, int exformint, int exround) {
        int mag = 0;
        int thisafter = 0;
        int lead = 0;
        byte[] newmant = null;
        ...
        if (after2 < -1) {
            this.badarg("format", 2, String.valueOf(after2));
        }
        ...
        if (after2 >= 0) {
            while (true) {
                ...
                if (thisafter == after2) break;
                if (thisafter < after2) {
                    newmant = BigDecimal.extend(num.mant, num.mant.length + after2 - thisafter);
                    num.mant = newmant;
                    num.exp -= after2 - thisafter;
                    if (num.exp >= -999999999) break;
                    throw new ArithmeticException("Exponent Overflow: " + num.exp);
                }
                chop = thisafter - after2;
    ...
    private int intcheck(int min2, int max2) {
    ...
    private BigDecimal dodivide(char code, BigDecimal rhs, MathContext set2, int scale) {
        int newexp;
        int thisdigit = 0;
        int i = 0;
        byte v2 = 0;
        ...
        if ((newexp = lhs.exp - rhs.exp + lhs.mant.length - rhs.mant.length) < 0 && code != 'D') {
            ...
            return BigDecimal.clone(lhs).finish(set2, false);
        }
        BigDecimal res = new BigDecimal();
        res.ind = (byte)(lhs.ind * rhs.ind);
        res.exp = newexp;
        ...
        block0: while (true) {
            ...
            block1: while (var1len >= var2len) {
                ...
                int $23 = var1len - 2;
                for (start2 = 0; start2 <= $23 && var1[start2] == 0; ++start2) {
                    --var1len;
                }
                if (start2 == 0) continue;
                System.arraycopy(var1, start2, var1, 0, var1len);
            }
        ...
        if (scale >= 0) {
            ...
            res.round(actdig, set2.roundingMode);
            if (res.exp != -scale) {
                res.mant = BigDecimal.extend(res.mant, res.mant.length + 1);
                --res.exp;
            }
    ...
    private void badarg(String name, int pos, String value2) {
        throw new IllegalArgumentException("Bad argument " + pos + " to " + name + ": " + value2);
    }
    ...
    private BigDecimal round(MathContext set2) {
        return this.round(set2.digits, set2.roundingMode);
    }
    ...
    private static final boolean allzero(byte[] array, int start2) {
        int i = 0;
        if (start2 < 0) {
            start2 = 0;
        }
    ...
    private BigDecimal finish(MathContext set2, boolean strip) {
        int d = 0;
        int i = 0;
        byte[] newmant = null;
        int mag = 0;
        int sig = 0;
        if (set2.digits != 0 && this.mant.length > set2.digits) {
            this.round(set2);
        }
        if (strip && set2.form != 0) {
            d = this.mant.length;
                ...
                block22: {
                    block23: {
                        ...
                        if ((mag = this.exp + this.mant.length) > 0) {
                            if (mag > set2.digits && set2.digits != 0) {
                                this.form = (byte)set2.form;
                            }
                            if (mag - 1 <= 999999999) {
                                return this;
                            }
