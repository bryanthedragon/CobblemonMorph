...
public abstract class JSMultiplyNode extends JSBinaryNode {
   ...
   public static JavaScriptNode create(JavaScriptNode left, JavaScriptNode right) {
      if (left instanceof JSConstantNode.JSConstantIntegerNode && right instanceof JSConstantNode.JSConstantIntegerNode) {
         ...
         if (result != 0L || leftValue >= 0 && rightValue >= 0) {
            return JSRuntime.longIsRepresentableAsInt(result) ? JSConstantNode.createInt((int)result) : JSConstantNode.createDouble(result);
         } else {
   ...
   protected int doInt(int a, int b, @Cached("create()") BranchProfile resultZeroBranch) {
   ...
   protected JavaScriptNode copyUninitialized(Set<Class<? extends Tag>> materializedTags) {
      return JSMultiplyNodeGen.create(cloneUninitialized(this.getLeft(), materializedTags), cloneUninitialized(this.getRight(), materializedTags));
   }
}
