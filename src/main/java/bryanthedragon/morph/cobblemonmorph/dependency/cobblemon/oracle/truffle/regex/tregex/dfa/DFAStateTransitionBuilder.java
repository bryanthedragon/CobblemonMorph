...
JsonConvertible {
    ...
    public DFAStateTransitionBuilder(NFAStateTransition[] transitions, StateSet<NFA, NFAState> targetStateSet, CodePointSet matcherBuilder) {
        super((AbstractTransition[])transitions, targetStateSet, matcherBuilder);
    }
    ...
    public JsonValue toJson() {
        ...
        return Json.obj(Json.prop("id", this.id), Json.prop("source", this.source.getId()), Json.prop("target", this.target.getId()), Json.prop("matcherBuilder", this.getCodePointSet().toString()), Json.prop("nfaTransitions", nfaTransitions));
    }
}
