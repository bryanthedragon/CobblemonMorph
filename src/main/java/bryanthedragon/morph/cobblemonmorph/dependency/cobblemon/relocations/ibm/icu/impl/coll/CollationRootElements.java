...
public final class CollationRootElements {
   ...
   long getPrimaryBefore(long p, boolean isCompressible) {
      ...
      return (p & 65535L) == 0L
         ? Collation.decTwoBytePrimaryByOneStep(p, isCompressible, step)
         : Collation.decThreeBytePrimaryByOneStep(p, isCompressible, step);
   }
   ...
   int getTertiaryBefore(long p, int s, int t) {
      ...
      long st;
      for (st = (long)s << 16 | t; st > secTer; secTer = this.elements[index++] & -129L) {
         if ((int)(secTer >> 16) == s) {
   ...
   long getPrimaryAfter(long p, int index, boolean isCompressible) {
      ...
      if ((q & 128L) != 0L || (step = (int)q & 127) == 0) {
         ...
         return (p & 65535L) == 0L
            ? Collation.incTwoBytePrimaryByOffset(p, isCompressible, step)
            : Collation.incThreeBytePrimaryByOffset(p, isCompressible, step);
      }
   }
   ...
   int getTertiaryAfter(int index, int s, int t) {
      ...
      long st = (s & 4294967295L) << 16 | t;

      while (secTer <= st) {
         secTer = this.elements[++index];
         if ((secTer & 128L) == 0L || secTer >> 16 > s) {
            return terLimit;
         }
