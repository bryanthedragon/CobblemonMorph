...
public abstract class JSStringToNumberNode extends JavaScriptBaseNode {
   ...
   protected final boolean startsWithI(TruffleString input) {
      return Strings.length(input) >= Strings.length(Strings.INFINITY)
         && Strings.length(input) <= Strings.length(Strings.INFINITY) + 1
         && (this.charAt(input, 0) == 'I' || this.charAt(input, 1) == 'I');
   }
   ...
   protected final boolean startsWithValidDouble(TruffleString input) {
      ...
      if (!JSRuntime.isAsciiDigit(firstChar) && firstChar != '-' && firstChar != '.' && firstChar != '+') {
         return false;
      } else if (Strings.length(input) < 2) {
         return true;
      } else {
   ...
   protected final boolean startsWithValidInt(TruffleString input) {
      char firstChar = this.charAt(input, 0);
      return (JSRuntime.isAsciiDigit(firstChar) || firstChar == '-' || firstChar == '+')
         && (Strings.length(input) < 2 || JSRuntime.isAsciiDigit(this.charAt(input, 1)));
   }
   ...
   protected final boolean allDigits(TruffleString input, int maxLength) {
      ...
      for (int i = 0; i < maxLength; i++) {
         ...
         if (!JSRuntime.isAsciiDigit(this.charAt(input, i))) {
            return false;
         }
      }
   ...
   protected double doInfinity(TruffleString input, @Cached ConditionProfile endsWithInfinity, @Cached TruffleString.RegionEqualByteIndexNode regionEqualsNode) {
      return endsWithInfinity.profile(Strings.endsWith(regionEqualsNode, input, Strings.INFINITY))
         ? JSRuntime.identifyInfinity(this.charAt(input, 0), Strings.length(input))
         : Double.NaN;
   }

   @Specialization(
      guards = {"stringLength(input) > 0", "!startsWithI(input)", "!startsWithValidDouble(input)", "!isHex(input)", "!isOctal(input)", "!isBinary(input)"}
   )
   ...
   protected double doSmallPosInt(TruffleString input) {
      ...
      for (int len = Strings.length(input); pos < len; pos++) {
      ...
      assert result >= 0 && checkLongResult((long)result, input);

      return result;
   }

   @Specialization(
      guards = {"stringLength(input) > 0", "stringLength(input) <= MAX_SAFE_INTEGER_LENGTH", "startsWithValidInt(input)"},
      rewriteOn = SlowPathException.class,
      replaces = "doSmallPosInt"
   )
