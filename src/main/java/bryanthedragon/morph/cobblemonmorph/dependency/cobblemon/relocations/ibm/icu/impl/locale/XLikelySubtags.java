...
public final class XLikelySubtags {
    ...
    private LSR maximize(String language, String script, String region) {
        ...
        int value2 = language.length() >= 2 && 0 <= (c0 = language.charAt(0) - 97) && c0 <= 25 && (state = this.trieFirstLetterStates[c0]) != 0L ? XLikelySubtags.trieNext(iter.resetToState64(state), language, 1) : XLikelySubtags.trieNext(iter, language, 0);
        if (value2 >= 0) {
            if (!language.isEmpty()) {
                retainOldMask |= 4;
            }
        ...
        if (value2 > 0) {
            ...
            value2 = XLikelySubtags.trieNext(iter, script, 0);
            if (value2 >= 0) {
                if (!script.isEmpty()) {
                    retainOldMask |= 2;
                }
                ...
                if (state == 0L) {
                    ...
                    value2 = XLikelySubtags.trieNext(iter, "", 0);
                    assert (value2 >= 0);
                    state = iter.getState64();
                }
            }
        ...
        if (value2 > 0) {
            ...
            value2 = XLikelySubtags.trieNext(iter, region, 0);
            if (value2 >= 0) {
                if (!region.isEmpty()) {
                    retainOldMask |= 1;
                }
    ...
    LSR minimizeSubtags(String languageIn, String scriptIn, String regionIn, ULocale.Minimize fieldToFavor) {
        ...
        BytesTrie iter = new BytesTrie(this.trie);
        int value2 = XLikelySubtags.trieNext(iter, result.language, 0);
        assert (value2 >= 0);
        if (value2 == 0) {
            value2 = XLikelySubtags.trieNext(iter, "", 0);
            assert (value2 >= 0);
    ...
    public static final class Data {
        ...
        private static UResource.Value getValue(UResource.Table table, String key, UResource.Value value2) {
            if (!table.findValue(key, value2)) {
        ...
        public static Data load() throws MissingResourceException {
            ...
            ICUResourceBundle langInfo = ICUResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", "langInfo", ICUResourceBundle.ICU_DATA_CLASS_LOADER, ICUResourceBundle.OpenType.DIRECT);
            UResource.Value value2 = langInfo.getValueWithFallback("likely");
            UResource.Table likelyTable = value2.getTable();
            if (likelyTable.findValue("languageAliases", value2)) {
                String[] pairs = value2.getStringArray();
            ...
            if (likelyTable.findValue("regionAliases", value2)) {
                String[] pairs = value2.getStringArray();
            ...
            ByteBuffer buffer = Data.getValue(likelyTable, "trie", value2).getBinary();
            byte[] trie = new byte[buffer.remaining()];
            buffer.get(trie);
            String[] lsrSubtags = Data.getValue(likelyTable, "lsrs", value2).getStringArray();
