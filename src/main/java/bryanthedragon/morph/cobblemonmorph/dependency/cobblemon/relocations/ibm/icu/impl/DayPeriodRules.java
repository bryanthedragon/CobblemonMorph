...
public final class DayPeriodRules {
   private static final DayPeriodRules.DayPeriodRulesData DATA = loadData();
   private boolean hasMidnight = false;
   private boolean hasNoon = false;
   private DayPeriodRules.DayPeriod[] dayPeriodForHour = new DayPeriodRules.DayPeriod[24];
   ...
   private static DayPeriodRules.DayPeriodRulesData loadData() {
      DayPeriodRules.DayPeriodRulesData data = new DayPeriodRules.DayPeriodRulesData();
      ICUResourceBundle rb = ICUResourceBundle.getBundleInstance(
         "com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", "dayPeriods", ICUResourceBundle.ICU_DATA_CLASS_LOADER, true
      );
      DayPeriodRules.DayPeriodRulesCountSink countSink = new DayPeriodRules.DayPeriodRulesCountSink(data);
      rb.getAllItemsWithFallback("rules", countSink);
      data.rules = new DayPeriodRules[data.maxRuleSetNum + 1];
      DayPeriodRules.DayPeriodRulesDataSink sink = new DayPeriodRules.DayPeriodRulesDataSink(data);
      rb.getAllItemsWithFallback("", sink);
      return data;
   }

   private int getStartHourForDayPeriod(DayPeriodRules.DayPeriod dayPeriod) throws IllegalArgumentException {
      if (dayPeriod == DayPeriodRules.DayPeriod.MIDNIGHT) {
         ...
         if (this.dayPeriodForHour[0] == dayPeriod && this.dayPeriodForHour[23] == dayPeriod) {
            ...
            for (int ix = 0; ix <= 23; ix++) {
               if (this.dayPeriodForHour[ix] == dayPeriod) {
                  return ix;
               }
            }
         }
   ...
   private static enum CutoffType {
      ...
      private static DayPeriodRules.CutoffType fromStringOrNull(CharSequence str) {
   ...
   public static enum DayPeriod {
      ...
      public static DayPeriodRules.DayPeriod[] VALUES = values();

      private static DayPeriodRules.DayPeriod fromStringOrNull(CharSequence str) {
   ...
   private static class DayPeriodRulesCountSink extends UResource.Sink {
      ...
      private DayPeriodRulesCountSink(DayPeriodRules.DayPeriodRulesData data) {
         this.data = data;
      }
   ...
   private static final class DayPeriodRulesData {
      Map<String, Integer> localesToRuleSetNumMap = new HashMap<>();
   ...
   private static final class DayPeriodRulesDataSink extends UResource.Sink {
         ...
         for (int i = 0; rules.getKeyAndValue(i, key, value); i++) {
            ...
            for (int j = 0; ruleSet.getKeyAndValue(j, key, value); j++) {
               ...
               for (int k = 0; periodDefinition.getKeyAndValue(k, key, value); k++) {
                  if (value.getType() == 0) {
                     DayPeriodRules.CutoffType type = DayPeriodRules.CutoffType.fromStringOrNull(key);
               ...
               for (int kx = 0; kx < this.cutoffs.length; kx++) {
                  this.cutoffs[kx] = 0;
               }
            }

            for (DayPeriodRules.DayPeriod period : this.data.rules[this.ruleSetNum].dayPeriodForHour) {
               if (period == null) {
      ...
      private void addCutoff(DayPeriodRules.CutoffType type, String hourStr) {
         if (type == null) {
            throw new ICUException("Cutoff type not recognized.");
         } else {
            int hour = parseHour(hourStr);
            this.cutoffs[hour] = this.cutoffs[hour] | 1 << type.ordinal();
         }
      }
      ...
      private void setDayPeriodForHoursFromCutoffs() {
         ...
         for (int startHour = 0; startHour <= 24; startHour++) {
            ...
            if ((this.cutoffs[startHour] & 1 << DayPeriodRules.CutoffType.FROM.ordinal()) > 0
               || (this.cutoffs[startHour] & 1 << DayPeriodRules.CutoffType.AFTER.ordinal()) > 0) {
