...
public final class PartyMoveSelectCallbacks {
    ...
    fun create(
        ...
        handler: (ServerPlayer, pokemonInt index, PartySelectPokemonDTO, moveInt index, MoveSelectDTO) -> Unit
    ) {
    ...
    fun handleCallback(ServerPlayer player, UUID uuid, pokemonInt index, moveInt index) {
        val callback = callbacks[player.uuid] ?: return
        callbacks.remove(player.uuid)
        if (callback.uuid != uuid) {
...
public class PartyMoveSelectCallback(
    val UUID uuid = UUID.randomUUID(),
    val pokemon: List<Pair<PartySelectPokemonDTO, List<MoveSelectDTO>>>,
    val cancel: (ServerPlayer) -> Unit = {},
    val handler: (ServerPlayer, pokemonInt index, PartySelectPokemonDTO, moveInt index, MoveSelectDTO) -> Unit
)