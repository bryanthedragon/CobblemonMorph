...
public final class AsyncFunctionBodyNode extends JavaScriptNode {
      ...
      this.atomic(
         () -> {
            if (this.asyncCallTargetInitializationRequired()) {
               ...
               AsyncFunctionBodyNode.AsyncFunctionRootNode asyncRootNode = new AsyncFunctionBodyNode.AsyncFunctionRootNode(
                  this.getContext(), this.functionBody, this.writeAsyncResult, this.readAsyncContext, rootNode.getSourceSection(), rootNode.getName()
               );
      ...
      return this.atomic(
         () -> {
            if (this.resumptionTarget == null) {
               ...
               AsyncFunctionBodyNode.AsyncFunctionRootNode asyncFunctionRoot = (AsyncFunctionBodyNode.AsyncFunctionRootNode)((RootCallTarget)this.resumptionTarget)
                  .getRootNode();
               return create(
                  this.getContext(),
   ...
   public static final class AsyncFunctionRootNode extends JavaScriptRealmBoundaryRootNode implements AsyncRootNode {
      ...
      protected Object executeInRealm(VirtualFrame frame) {
         ...
         } catch (YieldException var16) {
            assert var16.isAwait();
         } catch (AbstractTruffleException var17) {
            this.promiseCapabilityReject(promiseCapability, var17);
         } finally {
            if (enterContext) {
      ...
      public String getName() {
         return this.functionName != null && !this.functionName.isEmpty() ? this.functionName : ":async";
      }
