...
extends JavaScriptBaseNode {
    ...
    static boolean cachedOwnProperty(JSDynamicObject target, String key, int query2, @CachedLibrary(value="target") DynamicObjectLibrary objectLibrary, @Bind(value="objectLibrary.getProperty(target, key)") Property property, @Cached IsCallableNode isCallable, @Cached BranchProfile proxyBranch, @Cached TruffleString.FromJavaStringNode fromJavaStringNode) {
        ...
        if (JSProperty.isAccessor(property)) {
            ...
            if ((query2 & 1) != 0 && accessor.hasGetter()) {
                return true;
            }
            if ((query2 & 2) != 0 && accessor.hasSetter()) {
                return true;
            }
            if ((query2 & 0x20) != 0 && accessor.hasGetter()) {
                return true;
            }
            if ((query2 & 0x40) != 0 && accessor.hasSetter()) {
                return true;
            }
            return (query2 & 0x10) != 0 && JSProperty.isConfigurable(property);
        }
        assert (JSProperty.isData(property));
        if ((query2 & 1) != 0) {
            return true;
        }
        if ((query2 & 2) != 0 && JSProperty.isWritable(property)) {
            return true;
        }
        if ((query2 & 8) != 0) {
            Object value2 = Properties.getOrDefault(objectLibrary, target, tStringKey, Undefined.instance);
            if (JSProperty.isProxy(property)) {
                proxyBranch.enter();
                value2 = ((PropertyProxy)value2).get(target);
            }
            if (isCallable.executeBoolean(value2)) {
                return true;
            }
        }
        return (query2 & 0x10) != 0 && JSProperty.isConfigurable(property);
    ...
    static boolean member(JSDynamicObject target, String key, int query2, @Cached GetPrototypeNode getPrototype, @Cached IsCallableNode isCallable, @Cached IsExtensibleNode isExtensible, @Cached TruffleString.FromJavaStringNode fromJavaStringNode) {
        ...
        if (desc == null) {
            return (query2 & 4) != 0 && isExtensible.executeBoolean(target);
        }
        ...
        if ((query2 & 1) != 0 && readable) {
            return true;
        }
        if ((query2 & 2) != 0 && writable) {
            return true;
        }
        if ((query2 & 0x20) != 0 && readSideEffects) {
            return true;
        }
        if ((query2 & 0x40) != 0 && writeSideEffects) {
            return true;
        }
        if ((query2 & 8) != 0 && desc.isDataDescriptor() && isCallable.executeBoolean(desc.getValue())) {
            return true;
        }
        return (query2 & 0x10) != 0 && desc.getConfigurable();
    }
}
