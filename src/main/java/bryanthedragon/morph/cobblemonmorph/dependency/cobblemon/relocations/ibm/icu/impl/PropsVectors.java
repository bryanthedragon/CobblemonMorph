...
public class PropsVectors {
   ...
   public void compact(PropsVectors.CompactHandler compactor) {
      if (!this.isCompacted) {
         ...
         for (int i = 0; i < this.rows; i++) {
            int startx = this.v[indexArray[i]];
            ...
            if (startx < 1114112) {
               compactor.setRowIndexForRange(startx, limit - 1, count);
            }
         }
   ...
   public IntTrie compactToTrieWithRowIndexes() {
      ...
      return compactor.builder.serialize(new PropsVectors.DefaultGetFoldedValue(compactor.builder), new PropsVectors.DefaultGetFoldingOffset());
   }
