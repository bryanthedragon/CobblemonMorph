...
public class DecimalMatcher implements NumberParseMatcher {
   ...
   private DecimalMatcher(DecimalFormatSymbols symbols, Grouper grouper, int parseFlags) {
      ...
      StaticUnicodeSets.Key decimalKey = StaticUnicodeSets.chooseFrom(
         this.decimalSeparator,
         strictSeparators ? StaticUnicodeSets.Key.STRICT_COMMA : StaticUnicodeSets.Key.COMMA,
         strictSeparators ? StaticUnicodeSets.Key.STRICT_PERIOD : StaticUnicodeSets.Key.PERIOD
      );
      if (decimalKey != null) {
      ...
      if (groupingKey != null && decimalKey != null) {
         ...
         this.leadSet = StaticUnicodeSets.get(
            strictSeparators ? StaticUnicodeSets.Key.DIGITS_OR_ALL_SEPARATORS : StaticUnicodeSets.Key.DIGITS_OR_STRICT_ALL_SEPARATORS
         );
      } else {
   ...
   public boolean match(StringSegment segment, ParsedNumber result, int exponentSign) {
      if (result.seenNumber() && exponentSign == 0) {
         ...
         boolean prevValidSecondaryx = this.validateGroup(prevGroupSepType, prevGroupCount, false);
         boolean currValidPrimaryx = this.validateGroup(currGroupSepType, currGroupCount, true);
         if (!this.requireGroupingMatch) {
            int digitsToRemove = 0;
            if (!prevValidSecondaryx) {
               segment.setOffset(prevGroupOffset);
               digitsToRemove += prevGroupCount;
               digitsToRemove += currGroupCount;
            } else if (!currValidPrimaryx && (prevGroupSepType != 0 || prevGroupCount != 0)) {
               maybeMore = true;
               segment.setOffset(currGroupOffset);
               digitsToRemove += currGroupCount;
            }
            ...
            prevValidSecondaryx = true;
            currValidPrimaryx = true;
         }

         if (currGroupSepType != 2 && (!prevValidSecondaryx || !currValidPrimaryx)) {
            digitsConsumed = null;
         }
   ...
   private boolean validateGroup(int sepType, int count, boolean isPrimary) {
      if (this.requireGroupingMatch) {
         if (sepType == -1) {
            ...
            return isPrimary ? true : count != 0 && count <= this.grouping2;
         } else if (sepType == 1) {
