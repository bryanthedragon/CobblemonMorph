...
import com.cobblemon.mod.relocations.ibm.icu.text.BidiClassifier;
import com.cobblemon.mod.relocations.ibm.icu.text.BidiLine;
import com.cobblemon.mod.relocations.ibm.icu.text.BidiRun;
import com.cobblemon.mod.relocations.ibm.icu.text.BidiWriter;
...
public class Bidi {
    ...
    void verifyRange(int index, int start2, int limit) {
        if (index < start2 || index >= limit) {
            throw new IllegalArgumentException("Value " + index + " is out of range " + start2 + " to " + limit);
        }
    }
    ...
    private byte resolveExplicitLevels() {
        ...
        if ((this.flags & (MASK_EXPLICIT | MASK_ISO)) == 0) {
            ...
            for (int paraIndex = 0; paraIndex < this.paraCount; ++paraIndex) {
                int start3 = paraIndex == 0 ? 0 : this.paras_limit[paraIndex - 1];
                int limit = this.paras_limit[paraIndex];
                level = this.paras_level[paraIndex];
                for (i = start3; i < limit; ++i) {
                    this.levels[i] = level;
    ...
    private void setLevelsOutsideIsolates(int start2, int limit, byte level) {
        int isolateCount = 0;
        for (int k = start2; k < limit; ++k) {
    ...
    private void processPropertySeq(LevState levState, short _prop, int start2, int limit) {
        int k;
        byte level;
        ...
        if (addLevel != 0 || start2 < start0) {
            ...
            if (start2 >= levState.runStart) {
                for (k = start2; k < limit; ++k) {
                    this.levels[k] = level;
                }
            } else {
                this.setLevelsOutsideIsolates(start2, limit, level);
            }
        }
    }
    ...
    private void resolveImplicitLevels(int start2, int limit, short sor, short eor) {
        byte firstStrong;
        byte dirProp;
        int i;
        short stateImp;
        ...
        int nextStrongPos = -1;
        boolean inverseRTL = start2 < this.lastArabicPos && (this.GetParaLevelAt(start2) & 1) > 0 && (this.reorderingMode == 5 || this.reorderingMode == 6);
        levState.startL2EN = -1;
        levState.lastStrongRTL = -1;
        ...
        if (start2 == 0 && this.prologue != null && (lastStrong = this.lastL_R_AL()) != 4) {
            sor = lastStrong;
        }
        if (this.dirProps[start2] == 22) {
            ...
            levState.startON = -1;
            start1 = start2;
            stateImp = this.dirProps[start2] == 17 ? (short)(1 + sor) : (short)0;
            levState.state = 0;
            this.processPropertySeq(levState, sor, start2, start2);
        }
        int start22 = start2;
        block6: for (i = start2; i <= limit; ++i) {
            short gprop;
            if (i >= limit) {
                int k;
                for (k = limit - 1; k > start2 && (Bidi.DirPropFlag(this.dirProps[k]) & MASK_BN_EXPLICIT) != 0; --k) {
                }
            ...
            switch (actionImp) {
                ...
                case 3: {
                    this.processPropertySeq(levState, resProp, start1, start22);
                    this.processPropertySeq(levState, (short)4, start22, i);
                    start1 = i;
                    continue block6;
                }
                case 4: {
        ...
        for (i = limit - 1; i > start2 && (Bidi.DirPropFlag(this.dirProps[i]) & MASK_BN_EXPLICIT) != 0; --i) {
        }
    ...
    void setParaRunsOnly(char[] parmText, byte parmParaLevel) {
        ...
        for (i = oldRunCount - 1; i >= 0; --i) {
            ...
            if (indexOddBit > 0) {
                ...
                start2 = logicalStart + runLength - 1;
                limit = logicalStart;
                step = -1;
            }
            for (j = start2; j != limit; j += step) {
                int insertRemove;
                index = visualMap[j];
                index1 = visualMap[j + step];
                if (this.Bidi_Abs(index - index1) == 1 && saveLevels[index] == saveLevels[index1]) continue;
                this.runs[newI].start = logicalPos = this.Bidi_Min(visualMap[start2], index);
                this.runs[newI].level = (byte)(saveLevels[logicalPos] ^ indexOddBit);
                this.runs[newI].limit = this.runs[i].limit;
                this.runs[i].limit -= this.Bidi_Abs(j - start2) + 1;
            ...
            this.runs[newI].start = logicalPos = this.Bidi_Min(visualMap[start2], visualMap[limit]);
            this.runs[newI].level = (byte)(saveLevels[logicalPos] ^ indexOddBit);
        }
    ...
    public void setPara(char[] chars, byte paraLevel, byte[] embeddingLevels) {
            ...
            default: {
                ...
                if (embeddingLevels == null && this.paraCount <= 1 && (this.flags & DirPropFlagMultiRuns) == 0) {
                    ...
                    do {
                        start2 = limit;
                        level = nextLevel;
                        short sor = start2 > 0 && this.dirProps[start2 - 1] == 7 ? (short)Bidi.GetLRFromLevel(this.GetParaLevelAt(start2)) : eor;
                        ...
                        if ((level & 0xFFFFFF80) == 0) {
                            this.resolveImplicitLevels(start2, limit, sor, eor);
                            continue;
                        }
                        do {
                            int n = start2++;
        ...
        if (this.defaultParaLevel > 0 && (this.reorderingOptions & 1) != 0 && (this.reorderingMode == 5 || this.reorderingMode == 6)) {
            block16: for (int i = 0; i < this.paraCount; ++i) {
                ...
                if (level == 0) continue;
                start2 = i == 0 ? 0 : this.paras_limit[i - 1];
                for (int j = last; j >= start2; --j) {
    ...
    public Bidi setLine(int start2, int limit) {
        this.verifyValidPara();
        this.verifyRange(start2, 0, limit);
        this.verifyRange(limit, 0, this.length + 1);
        if (this.getParagraphIndex(start2) != this.getParagraphIndex(limit - 1)) {
            throw new IllegalArgumentException();
        }
        return BidiLine.setLine(this, start2, limit);
    }
    ...
    public int getRunLevel(int run2) {
        this.verifyValidParaOrLine();
        BidiLine.getRuns(this);
        this.verifyRange(run2, 0, this.runCount);
        this.getLogicalToVisualRunsMap();
        return this.runs[this.logicalToVisualRunsMap[run2]].level;
    }

    public int getRunStart(int run2) {
        this.verifyValidParaOrLine();
        BidiLine.getRuns(this);
        this.verifyRange(run2, 0, this.runCount);
        this.getLogicalToVisualRunsMap();
        return this.runs[this.logicalToVisualRunsMap[run2]].start;
    }

    public int getRunLimit(int run2) {
        this.verifyValidParaOrLine();
        BidiLine.getRuns(this);
        this.verifyRange(run2, 0, this.runCount);
        this.getLogicalToVisualRunsMap();
        int idx = this.logicalToVisualRunsMap[run2];
        int len = idx == 0 ? this.runs[idx].limit : this.runs[idx].limit - this.runs[idx - 1].limit;
        return this.runs[idx].start + len;
    }

    public static boolean requiresBidi(char[] text, int start2, int limit) {
        int RTLMask = 57378;
