...
extends Collator {
    ...
    public RuleBasedCollator setMaxVariable(int group) {
        ...
        CollationSettings defaultSettings = this.getDefaultSettings();
        if (this.settings.readOnly() == defaultSettings && value2 < 0) {
            return this;
        }
        CollationSettings ownedSettings = this.getOwnedSettings();
        if (group == -1) {
        ...
        assert (varTop != 0L);
        ownedSettings.setMaxVariable(value2, defaultSettings.options);
    ...
    @Deprecated
    void internalAddContractions(int c, UnicodeSet set2) {
        new ContractionsAndExpansions(set2, null, null, false).forCodePoint(this.data, c);
    }
    ...
    public int hashCode() {
        ...
        UnicodeSet set2 = this.getTailoredSet();
    ...
    extends UTF16NFDIterator {
        ...
        void setText(Normalizer2Impl nfcImpl, CharSequence seq, int start2) {
            this.reset();
            int spanLimit = nfcImpl.makeFCD(seq, start2, seq.length(), null);
            if (spanLimit == seq.length()) {
                ...
                this.str.append(seq, start2, spanLimit);
                Normalizer2Impl.ReorderingBuffer buffer = new Normalizer2Impl.ReorderingBuffer(nfcImpl, this.str, seq.length() - start2);
    ...
    extends NFDIterator {
        ...
        void setText(CharSequence seq, int start2) {
            this.reset();
            this.s = seq;
            this.pos = start2;
        }
    ...
    extends CollationKeys.SortKeyByteSink {
        ...
        @Override
        protected void AppendBeyondCapacity(byte[] bytes, int start2, int n, int length) {
            if (this.Resize(n, length)) {
                System.arraycopy(bytes, start2, this.buffer_, length, n);
            }
        }
