...
import com.cobblemon.mod.relocations.ibm.icu.text.UnicodeFilter;
...
Freezable<UnicodeSet> {
    ...
    public UnicodeSet(int start2, int end2) {
        this();
        this.add(start2, end2);
    }
    ...
    public UnicodeSet set(int start2, int end2) {
        this.checkFrozen();
        this.clear();
        this.complement(start2, end2);
        return this;
    }
    ...
    private static <T extends Appendable> T _appendToPat(T result, int start2, int end2, boolean escapeUnprintable) {
        UnicodeSet._appendToPat(result, start2, escapeUnprintable);
        if (start2 != end2) {
            if (start2 + 1 != end2 || start2 == 56319) {
                try {
                    result.append('-');
                }
                catch (IOException e) {
            ...
            UnicodeSet._appendToPat(result, end2, escapeUnprintable);
        }
        return result;
    }
    ...
    private <T extends Appendable> T appendNewPattern(T result, boolean escapeUnprintable, boolean includeStrings) {
        try {
            ...
            while (i < limit) {
                int start2 = this.list[i];
                int end2 = this.list[i + 1] - 1;
                if (55296 > end2 || end2 > 56319) {
                    UnicodeSet._appendToPat(result, start2, end2, escapeUnprintable);
                    i += 2;
                    continue;
                }
                int firstLead = i;
                ...
                while (i < limit && (start2 = this.list[i]) <= 57343) {
                    UnicodeSet._appendToPat(result, start2, this.list[i + 1] - 1, escapeUnprintable);
                    i += 2;
                }
    ...
    private static int matchRest(Replaceable text, int start2, int limit, String s) {
        int maxLen;
        int slen = s.length();
        if (start2 < limit) {
            maxLen = limit - start2;
            ...
            for (int i = 1; i < maxLen; ++i) {
                if (text.charAt(start2 + i) == s.charAt(i)) continue;
                return 0;
            }
        } else {
            maxLen = start2 - limit;
            ...
            --slen;
            for (int i = 1; i < maxLen; ++i) {
                if (text.charAt(start2 - i) == s.charAt(slen - i)) continue;
                return 0;
            }
        }
        return maxLen;
    ...
    public int charAt(int index) {
        if (index >= 0) {
            ...
            while (i < len2) {
                int start2;
                int count;
                if (index < (count = this.list[i++] - (start2 = this.list[i++]))) {
                    return start2 + index;
                }
                index -= count;
            }
    ...
    public UnicodeSet add(int start2, int end2) {
        this.checkFrozen();
        return this.add_unchecked(start2, end2);
    }

    public UnicodeSet addAll(int start2, int end2) {
        this.checkFrozen();
        return this.add_unchecked(start2, end2);
    }

    private UnicodeSet add_unchecked(int start2, int end2) {
        if (start2 < 0 || start2 > 0x10FFFF) {
            throw new IllegalArgumentException("Invalid code point U+" + Utility.hex(start2, 6));
        }
        if (end2 < 0 || end2 > 0x10FFFF) {
            throw new IllegalArgumentException("Invalid code point U+" + Utility.hex(end2, 6));
        }
        if (start2 < end2) {
            int limit = end2 + 1;
            if ((this.len & 1) != 0) {
            ...
            this.add(this.range(start2, end2), 2, 0);
        } else if (start2 == end2) {
            this.add(start2);
        }
        return this;
    ...
    public UnicodeSet retain(int start2, int end2) {
        this.checkFrozen();
        if (start2 < 0 || start2 > 0x10FFFF) {
            throw new IllegalArgumentException("Invalid code point U+" + Utility.hex(start2, 6));
        }
        ...
        if (start2 <= end2) {
            this.retain(this.range(start2, end2), 2, 0);
        } else {
            this.clear();
        }
        return this;
    ...
    public UnicodeSet remove(int start2, int end2) {
        this.checkFrozen();
        if (start2 < 0 || start2 > 0x10FFFF) {
            throw new IllegalArgumentException("Invalid code point U+" + Utility.hex(start2, 6));
        }
        ...
        if (start2 <= end2) {
            this.retain(this.range(start2, end2), 2, 2);
        }
        return this;
    }
    ...
    public UnicodeSet complement(int start2, int end2) {
        this.checkFrozen();
        if (start2 < 0 || start2 > 0x10FFFF) {
            throw new IllegalArgumentException("Invalid code point U+" + Utility.hex(start2, 6));
        }
        ...
        if (start2 <= end2) {
            this.xor(this.range(start2, end2), 2, 0);
        }
        this.pat = null;
        return this;
    }
    ...
    public boolean contains(int start2, int end2) {
        if (start2 < 0 || start2 > 0x10FFFF) {
            throw new IllegalArgumentException("Invalid code point U+" + Utility.hex(start2, 6));
        }
        if (end2 < 0 || end2 > 0x10FFFF) {
        ...
        int i = this.findCodePoint(start2);
        return (i & 1) != 0 && end2 < this.list[i];
    }
    ...
    public boolean containsNone(int start2, int end2) {
        if (start2 < 0 || start2 > 0x10FFFF) {
            throw new IllegalArgumentException("Invalid code point U+" + Utility.hex(start2, 6));
        }
        if (end2 < 0 || end2 > 0x10FFFF) {
    ...
    public final boolean containsSome(int start2, int end2) {
        return !this.containsNone(start2, end2);
    }
    ...
    public static String[] toArray(UnicodeSet set2) {
        return UnicodeSet.addAllTo(set2, new String[set2.size()]);
    }
    ...
    private int[] range(int start2, int end2) {
        if (this.rangeList == null) {
            this.rangeList = new int[]{start2, end2 + 1, 0x110000};
        } else {
            this.rangeList[0] = start2;
            this.rangeList[1] = end2 + 1;
        }
    ...
    private void applyFilter(Filter filter, UnicodeSet inclusions) {
        ...
        for (int j = 0; j < limitRange; ++j) {
            int start2 = inclusions.getRangeStart(j);
            int end2 = inclusions.getRangeEnd(j);
            for (int ch = start2; ch <= end2; ++ch) {
    ...
    public UnicodeSet applyIntPropertyValue(int prop, int value2) {
        if (prop == 8192) {
            UnicodeSet inclusions = CharacterPropertiesImpl.getInclusionsForProperty(prop);
            this.applyFilter(new GeneralCategoryMaskFilter(value2), inclusions);
        } else if (prop == 28672) {
            UnicodeSet inclusions = CharacterPropertiesImpl.getInclusionsForProperty(prop);
            this.applyFilter(new ScriptExtensionsFilter(value2), inclusions);
            ...
            UnicodeSet inclusions = CharacterPropertiesImpl.getInclusionsForProperty(prop);
            this.applyFilter(new IntPropertyFilter(prop, value2), inclusions);
        } else {
...
lbl75:
    ...
    private void applyPropertyPattern(RuleCharacterIterator chars, Appendable rebuiltPat, SymbolTable symbols) {
        ...
        int start2 = chars.getCurrentBufferPos();
        ParsePosition pos = new ParsePosition(start2);
        this.applyPropertyPattern(patStr, pos, symbols);
        int length = pos.getIndex() - start2;
        if (length == 0) {
            UnicodeSet.syntaxError(chars, "Invalid property pattern");
        }
        chars.jumpahead(length);
        UnicodeSet.append(rebuiltPat, patStr.substring(start2, pos.getIndex()));
    }

    private static final void addCaseMapping(UnicodeSet set2, int result, StringBuilder full) {
        if (result >= 0) {
            if (result > 31) {
                set2.add(result);
            } else {
                set2.add(full.toString());
                full.setLength(0);
            }
        }
    }
    ...
    public UnicodeSet closeOver(int attribute) {
        ...
        if ((attribute & 6) != 0) {
            ...
            for (int i = 0; i < n; ++i) {
                int cp;
                int start2 = this.getRangeStart(i);
                int end2 = this.getRangeEnd(i);
                if ((attribute & 2) != 0) {
                    for (cp = start2; cp <= end2; ++cp) {
    ...
    public int span(CharSequence s, int start2, SpanCondition spanCondition) {
        int which;
        ...
        if (this.bmpSet != null) {
            return this.bmpSet.span(s, start2, spanCondition, null);
        }
        if (this.stringSpan != null) {
            return this.stringSpan.span(s, start2, spanCondition);
        }
        if (this.hasStrings() && (strSpan = new UnicodeSetStringSpan(this, new ArrayList<String>(this.strings), which = spanCondition == SpanCondition.NOT_CONTAINED ? 33 : 34)).needsStringSpanUTF16()) {
            return strSpan.span(s, start2, spanCondition);
        }
        return this.spanCodePointsAndCount(s, start2, spanCondition, null);
    }

    @Deprecated
    public int spanAndCount(CharSequence s, int start2, SpanCondition spanCondition, OutputInt outCount) {
        if (outCount == null) {
        ...
        if (this.stringSpan != null) {
            return this.stringSpan.spanAndCount(s, start2, spanCondition, outCount);
        }
        if (this.bmpSet != null) {
            return this.bmpSet.span(s, start2, spanCondition, outCount);
        }
        if (this.hasStrings()) {
            ...
            return strSpan.spanAndCount(s, start2, spanCondition, outCount);
        }
        return this.spanCodePointsAndCount(s, start2, spanCondition, outCount);
    }

    private int spanCodePointsAndCount(CharSequence s, int start2, SpanCondition spanCondition, OutputInt outCount) {
        int c;
    ...
    @Deprecated
    public int findIn(CharSequence value2, int fromIndex, boolean findNot) {
        int cp;
        while (fromIndex < value2.length() && this.contains(cp = UTF16.charAt(value2, fromIndex)) == findNot) {
    ...
    @Deprecated
    public int findLastIn(CharSequence value2, int fromIndex, boolean findNot) {
        int cp;
        --fromIndex;
        while (fromIndex >= 0 && this.contains(cp = UTF16.charAt(value2, fromIndex)) == findNot) {
            fromIndex -= UTF16.getCharCount(cp);
        }
    ...
    @Deprecated
    public String stripFrom(CharSequence source, boolean matches2) {
        StringBuilder result = new StringBuilder();
        int pos = 0;
        while (pos < source.length()) {
            int inside = this.findIn(source, pos, !matches2);
            result.append(source.subSequence(pos, inside));
            pos = this.findIn(source, inside, matches2);
        }
    ...
    implements Filter {
        ...
        IntPropertyFilter(int prop, int value2) {
            this.prop = prop;
            this.value = value2;
        }
