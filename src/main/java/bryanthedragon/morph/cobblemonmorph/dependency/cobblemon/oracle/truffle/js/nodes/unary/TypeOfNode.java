...
public abstract class TypeOfNode extends JSUnaryNode {
   ...
   @Specialization
   protected TruffleString doJSProxy(JSProxyObject operand, @Cached("create()") TypeOfNode typeofNode) {
   ...
   protected TruffleString doTruffleObject(Object operand, @CachedLibrary("operand") InteropLibrary interop) {
      ...
      if (interop.isBoolean(operand)) {
         ...
         return !interop.isExecutable(operand) && !interop.isInstantiable(operand) ? JSOrdinary.TYPE_NAME : JSFunction.TYPE_NAME;
      }
   }
   ...
   protected JavaScriptNode copyUninitialized(Set<Class<? extends Tag>> materializedTags) {
      return TypeOfNodeGen.create(cloneUninitialized(this.getOperand(), materializedTags));
   }
}
