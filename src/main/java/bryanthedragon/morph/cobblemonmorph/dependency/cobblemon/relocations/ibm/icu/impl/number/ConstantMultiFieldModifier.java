...
public class ConstantMultiFieldModifier implements Modifier {
   ...
   public ConstantMultiFieldModifier(
      FormattedStringBuilder prefix, FormattedStringBuilder suffix, boolean overwrite, boolean strong, Modifier.Parameters parameters
   ) {
   ...
   public boolean containsField(Field field) {
      ...
      for (int ix = 0; ix < this.suffixFields.length; ix++) {
         if (this.suffixFields[ix] == field) {
            return true;
         }
      }
   ...
   public boolean semanticallyEquivalent(Modifier other) {
      if (!(other instanceof ConstantMultiFieldModifier)) {
         ...
         return this.parameters != null && _other.parameters != null && this.parameters.obj == _other.parameters.obj
            ? true
            : Arrays.equals(this.prefixChars, _other.prefixChars)
               && Arrays.equals(this.prefixFields, _other.prefixFields)
               && Arrays.equals(this.suffixChars, _other.suffixChars)
               && Arrays.equals(this.suffixFields, _other.suffixFields)
               && this.overwrite == _other.overwrite
               && this.strong == _other.strong;
      }
   }
   ...
   public String toString() {
      ...
      return String.format(
         "<ConstantMultiFieldModifier prefix:'%s' suffix:'%s'>", temp.subSequence(0, prefixLength), temp.subSequence(prefixLength, temp.length())
      );
   }
}
