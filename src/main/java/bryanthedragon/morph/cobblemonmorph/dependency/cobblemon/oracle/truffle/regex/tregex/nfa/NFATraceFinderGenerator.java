...
public final class NFATraceFinderGenerator {
    ...
    private NFA run() {
        ...
        for (NFAStateTransition entry : new NFAStateTransition[]{this.originalNFA.getAnchoredEntry()[0], this.originalNFA.getUnAnchoredEntry()[0]}) {
            block1: for (NFAStateTransition t : (NFAStateTransition[])entry.getTarget().getSuccessors()) {
                ...
                while (true) {
                    ...
                    for (NFAState duplicate : this.duplicatedStatesMap[curElement.getTransition().getTarget().getId()]) {
                        ...
                        while (!treeNode.isFinalState()) {
                            ...
                            GroupBoundaries groupBoundaries = ((NFAStateTransition[])treeNode.getSuccessors())[0].getGroupBoundaries();
                            groupBoundaries.applyToResultFactory(result, iResult, this.trackLastGroup);
                            treeNode = ((NFAStateTransition[])treeNode.getSuccessors())[0].getTarget();
                        }
                        treeNode.addPossibleResult(resultID);
                        result.setLength(iResult);
                        PreCalculatedResultFactory existingResult = (PreCalculatedResultFactory)this.resultDeDuplicationMap.get(result);
        ...
        return new NFA(this.originalNFA.getAst(), dummyInitialState, null, null, newAnchoredEntry, newUnAnchoredEntry, this.states, this.stateID, this.transitionID, null, preCalculatedResults);
    }

    private NFAStateTransition createTransition(NFAState source, NFAState target, NFAStateTransition originalTransition, PreCalculatedResultFactory preCalcResult, int preCalcResultIndex) {
        originalTransition.getGroupBoundaries().applyToResultFactory(preCalcResult, preCalcResultIndex, this.trackLastGroup);
        NFAStateTransition copy = new NFAStateTransition((short)this.transitionID.inc(), source, target, originalTransition.getCodePointSet(), originalTransition.getGroupBoundaries());
    ...
    private NFAStateTransition copyEntry(NFAState dummyInitialState, NFAStateTransition originalReverseEntry) {
        return new NFAStateTransition((short)this.transitionID.inc(), this.copy(originalReverseEntry.getSource()), dummyInitialState, originalReverseEntry.getCodePointSet(), GroupBoundaries.getEmptyInstance(this.originalNFA.getAst().getLanguage()));
    }
    ...
    private static final class PathElement {
        ...
        public boolean hasNextTransition() {
            return this.i < ((NFAStateTransition[])this.transition.getTarget().getSuccessors()).length;
        }

        public NFAStateTransition getNextTransition() {
            return ((NFAStateTransition[])this.transition.getTarget().getSuccessors())[this.i++];
        }
    }
}
