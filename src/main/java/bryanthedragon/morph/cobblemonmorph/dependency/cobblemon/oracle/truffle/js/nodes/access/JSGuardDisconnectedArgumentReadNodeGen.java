...
public final class JSGuardDisconnectedArgumentReadNodeGen extends JSGuardDisconnectedArgumentRead implements Introspection.Provider {
   ...
   private JSGuardDisconnectedArgumentReadNodeGen(int index, ReadElementNode readElementNode, JavaScriptNode argumentsArray, TruffleString name) {
      super(index, readElementNode, argumentsArray, name);
   }
      ...
      try {
         ...
         if (argumentsArrayNodeValue instanceof JSArgumentsObject) {
            ...
            if (!JSGuards.isArgumentsDisconnected(argumentsArrayNodeValue_)) {
               ...
               lock.unlock();
               hasLock = false;
               return this.doObject(argumentsArrayNodeValue_, this.unconnected);
            }
            ...
            if (JSGuards.isArgumentsDisconnected(argumentsArrayNodeValue_)) {
               ...
               lock.unlock();
               hasLock = false;
               return this.doObjectDisconnected(argumentsArrayNodeValue_, this.objectDisconnected_wasDisconnected_, this.unconnected);
            }
         }
   ...
   public static JSGuardDisconnectedArgumentRead create(int index, ReadElementNode readElementNode, JavaScriptNode argumentsArray, TruffleString name) {
      return new JSGuardDisconnectedArgumentReadNodeGen(index, readElementNode, argumentsArray, name);
   }
}
