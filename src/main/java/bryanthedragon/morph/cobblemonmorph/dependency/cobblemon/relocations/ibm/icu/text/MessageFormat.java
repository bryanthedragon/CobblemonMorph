...
import com.cobblemon.mod.relocations.ibm.icu.text.PluralFormat;
...
import com.cobblemon.mod.relocations.ibm.icu.text.SelectFormat;
...
extends UFormat {
        ...
        while (true) {
            ...
            if (type == MessagePattern.Part.Type.REPLACE_NUMBER) {
                ...
                if (noArg) {
                    ...
                    PluralSelectorProvider selector2;
                    if (!(arg instanceof Number)) {
                        throw new IllegalArgumentException("'" + arg + "' is not a Number");
                    }
                    ...
                    int subMsgStart = PluralFormat.findSubMessage(this.msgPattern, i, selector2, context, number.doubleValue());
    ...
    private static double parseChoiceArgument(MessagePattern pattern, int partIndex, String source, ParsePosition pos) {
        int start2;
        int furthest = start2 = pos.getIndex();
        ...
        while (pattern.getPartType(partIndex) != MessagePattern.Part.Type.ARG_LIMIT) {
            ...
            if ((len = MessageFormat.matchStringUntilLimitPart(pattern, partIndex += 2, msgLimit = pattern.getLimitPartIndex(partIndex), source, start2)) >= 0 && (newIndex = start2 + len) > furthest) {
                furthest = newIndex;
    ...
    private static final class AppendableWrapper {
        ...
        public void append(CharSequence s, int start2, int limit) {
            try {
                this.app.append(s, start2, limit);
                this.length += limit - start2;
            }
            catch (IOException e) {
                throw new ICUUncheckedIOException(e);
            }
        ...
        public static int append(Appendable result, CharacterIterator iterator) {
            try {
                int start2 = iterator.getBeginIndex();
        ...
        public void formatAndAppend(Format formatter, Object arg) {
            if (this.attributes == null) {
                ...
                formattedArg.first();
                int start2 = formattedArg.getIndex();
                int limit = formattedArg.getEndIndex();
                int offset = prevLength - start2;
                while (start2 < limit) {
                    ...
                    if (map.size() != 0) {
                        for (Map.Entry<AttributedCharacterIterator.Attribute, Object> entry : map.entrySet()) {
                            this.attributes.add(new AttributeAndPosition(entry.getKey(), entry.getValue(), offset + start2, offset + runLimit));
                        }
                    }
                    start2 = runLimit;
                    formattedArg.setIndex(start2);
                }
            }
        }
    ...
    private static final class PluralSelectorContext {
        ...
        private PluralSelectorContext(int start2, String name, Number num, double off) {
