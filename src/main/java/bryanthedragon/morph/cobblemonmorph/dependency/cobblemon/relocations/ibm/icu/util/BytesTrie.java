...
Iterable<Entry> {
    ...
    private static int readValue(byte[] bytes, int pos, int leadByte) {
        int value2 = leadByte < 81 ? leadByte - 16 : (leadByte < 108 ? leadByte - 81 << 8 | bytes[pos] & 0xFF : (leadByte < 126 ? leadByte - 108 << 16 | (bytes[pos] & 0xFF) << 8 | bytes[pos + 1] & 0xFF : (leadByte == 126 ? (bytes[pos] & 0xFF) << 16 | (bytes[pos + 1] & 0xFF) << 8 | bytes[pos + 2] & 0xFF : bytes[pos] << 24 | (bytes[pos + 1] & 0xFF) << 16 | (bytes[pos + 2] & 0xFF) << 8 | bytes[pos + 3] & 0xFF)));
        return value2;
    }
    ...
    private static long findUniqueValueFromBranch(byte[] bytes, int pos, int length, long uniqueValue) {
        ...
        do {
            ...
            int value2 = BytesTrie.readValue(bytes, ++pos, node >> 1);
            pos = BytesTrie.skipValue(pos, node);
            if (isFinal) {
                if (uniqueValue != 0L) {
            ...
            if ((uniqueValue = BytesTrie.findUniqueValue(bytes, pos + value2, uniqueValue)) != 0L) continue;
            return 0L;
        } while (--length > 1);
    ...
    implements java.util.Iterator<Entry> {
        ...
        private int branchNext(int pos, int length) {
            ...
            int value2 = BytesTrie.readValue(this.bytes_, pos, node >> 1);
            pos = BytesTrie.skipValue(pos, node);
