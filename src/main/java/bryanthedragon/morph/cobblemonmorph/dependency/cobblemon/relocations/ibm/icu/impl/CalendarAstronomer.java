...
public class CalendarAstronomer {
   ...
   private static final double DEG_RAD = Math.PI / 180.0;
   ...
   public static final CalendarAstronomer.SolarLongitude VERNAL_EQUINOX = new CalendarAstronomer.SolarLongitude(0.0);
   public static final CalendarAstronomer.SolarLongitude SUMMER_SOLSTICE = new CalendarAstronomer.SolarLongitude(Math.PI / 2);
   public static final CalendarAstronomer.SolarLongitude AUTUMN_EQUINOX = new CalendarAstronomer.SolarLongitude(Math.PI);
   public static final CalendarAstronomer.SolarLongitude WINTER_SOLSTICE = new CalendarAstronomer.SolarLongitude(Math.PI * 3.0 / 2.0);
   ...
   public static final CalendarAstronomer.MoonAge NEW_MOON = new CalendarAstronomer.MoonAge(0.0);
   public static final CalendarAstronomer.MoonAge FIRST_QUARTER = new CalendarAstronomer.MoonAge(Math.PI / 2);
   public static final CalendarAstronomer.MoonAge FULL_MOON = new CalendarAstronomer.MoonAge(Math.PI);
   public static final CalendarAstronomer.MoonAge LAST_QUARTER = new CalendarAstronomer.MoonAge(Math.PI * 3.0 / 2.0);
   private long time;
   ...
   private transient CalendarAstronomer.Equatorial moonPosition = null;
   ...
   public CalendarAstronomer(double longitude, double latitude) {
      this();
      this.fLongitude = normPI(longitude * (Math.PI / 180.0));
      this.fLatitude = normPI(latitude * (Math.PI / 180.0));
   ...
   public double getGreenwichSidereal() {
      if (this.siderealTime == Double.MIN_VALUE) {
         double UT = normalize(this.time / 3600000.0, 24.0);
   ...
   private long lstToUT(double lst) {
      double lt = normalize((lst - this.getSiderealOffset()) * 0.9972695663, 24.0);
   ...
   private long timeOfAngle(CalendarAstronomer.AngleFunc func, double desired, double periodDays, long epsilon, boolean next) {
      ...
      double deltaT = (deltaAngle + (next ? 0.0 : -Math.PI * 2)) * (periodDays * 8.64E7) / (Math.PI * 2);
   ...
   private long riseOrSet(CalendarAstronomer.CoordFunc func, boolean rise, double diameter, double refraction, long epsilon) {
      ...
      long delta = (long)(240.0 * y * (180.0 / Math.PI) / cosD * 1000.0);
   ...
   private static final double normPI(double angle) {
      return normalize(angle + Math.PI, Math.PI * 2) - Math.PI;
   }
   ...
   private double eclipticObliquity() {
      if (this.eclipObliquity == Double.MIN_VALUE) {
         ...
         this.eclipObliquity *= Math.PI / 180.0;
      }
   ...
   private static String radToHms(double angle) {
      ...
      int min = (int)((angle * 3.819718634205488 - hrs) * 60.0);
      int sec = (int)((angle * 3.819718634205488 - hrs - min / 60.0) * 3600.0);
      return Integer.toString(hrs) + "h" + min + "m" + sec + "s";
   }

   private static String radToDms(double angle) {
      int deg = (int)(angle * (180.0 / Math.PI));
      int min = (int)((angle * (180.0 / Math.PI) - deg) * 60.0);
      int sec = (int)((angle * (180.0 / Math.PI) - deg - min / 60.0) * 3600.0);
   ...
   public static final class Ecliptic {
      ...
      public String toString() {
         return Double.toString(this.longitude * (180.0 / Math.PI)) + "," + this.latitude * (180.0 / Math.PI);
      }
   }
   ...
   public static final class Equatorial {
      ...
      @Override
      public String toString() {
         return Double.toString(this.ascension * (180.0 / Math.PI)) + "," + this.declination * (180.0 / Math.PI);
      }
   ...
   public static final class Horizon {
      ...
      @Override
      public String toString() {
         return Double.toString(this.altitude * (180.0 / Math.PI)) + "," + this.azimuth * (180.0 / Math.PI);
      }
   }
