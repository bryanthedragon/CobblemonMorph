...
public abstract class Transcoder implements TranscodingInstruction {
   ...
   protected Transcoder(
      ...
      if (bytes == null) {
         byteArrayHash.put(arrayKey, bytes = ArrayReader.readByteArray("Transcoder_" + arrayKey + "_ByteArray"));
      }

      this.byteArray = bytes;
      int[] ints = wordArrayHash.get(arrayKey);
      if (ints == null) {
         wordArrayHash.put(arrayKey, ints = ArrayReader.readIntArray("Transcoder_" + arrayKey + "_WordArray"));
      }
   ...
   public String toStringFull() {
      ...
      for (int i = 0; i < 20; i++) {
         s = s + (this.intArray[i] & 4294967295L) + ", ";
      }
      ...
      s = s + "  compatibility: " + this.compatibility + "\n";
      return s + "  state size: " + this.stateSize + "\n";
   }
   ...
   static final class GenericTranscoderEntry {
      ...
      Transcoder createTranscoder() {
         return new GenericTranscoder(
            this.source,
            this.destination,
            this.treeStart,
            this.arrayKey,
            this.inputUnitLength,
            this.maxInput,
            this.maxOutput,
            this.compatibility,
            this.stateSize
