...
class Frame {
    ...
    private void pop(int elements2) {
        if (this.outputStackTop >= elements2) {
            this.outputStackTop = (short)(this.outputStackTop - elements2);
        } else {
            this.outputStackStart = (short)(this.outputStackStart - (elements2 - this.outputStackTop));
            this.outputStackTop = 0;
        }
    }
    ...
    private int getInitializedType(SymbolTable symbolTable, int abstractType) {
        if (abstractType == 0x400006 || (abstractType & 0xFFC00000) == 0xC00000) {
            for (int i = 0; i < this.initializationCount; ++i) {
                ...
                int value2 = initializedType & 0xFFFFF;
                if (kind == 0x1000000) {
                    initializedType = dim + this.inputLocals[value2];
                } else if (kind == 0x1400000) {
                    initializedType = dim + this.inputStack[this.inputStack.length - value2];
                }
