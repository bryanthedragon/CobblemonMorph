...
public final class UScript {
   ...
   public static final int getScript(int codepoint) {
      if (codepoint >= 0 & codepoint <= 1114111) {
         int scriptX = UCharacterProperty.INSTANCE.getAdditional(codepoint, 0) & 15728895;
         int codeOrIndex = UCharacterProperty.mergeScriptCodeOrIndex(scriptX);
         if (scriptX < 4194304) {
            return codeOrIndex;
         } else if (scriptX < 8388608) {
            return 0;
         } else {
            return scriptX < 12582912 ? 1 : UCharacterProperty.INSTANCE.m_scriptExtensions_[codeOrIndex];
         }
      } else {
   ...
   public static final boolean hasScript(int c, int sc) {
      int scriptX = UCharacterProperty.INSTANCE.getAdditional(c, 0) & 15728895;
      int codeOrIndex = UCharacterProperty.mergeScriptCodeOrIndex(scriptX);
      if (scriptX < 4194304) {
