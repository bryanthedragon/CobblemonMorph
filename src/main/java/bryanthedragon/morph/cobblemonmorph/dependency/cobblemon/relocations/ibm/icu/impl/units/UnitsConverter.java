...
public class UnitsConverter {
    ...
    static class Factor {
        ...
        private void addPoweredEntity(String poweredEntity) {
            String[] entities2 = poweredEntity.split(Pattern.quote("^"));
            assert (entities2.length == 1 || entities2.length == 2);
            int power = entities2.length == 2 ? Integer.parseInt(entities2[1]) : 1;
            this.addEntity(entities2[0], power);
        }

        private void addEntity(String entity2, int power) {
            if ("ft_to_m".equals(entity2)) {
                this.exponentFtToM += power;
            } else if ("ft2_to_m2".equals(entity2)) {
                this.exponentFtToM += 2 * power;
            } else if ("ft3_to_m3".equals(entity2)) {
                this.exponentFtToM += 3 * power;
            } else if ("in3_to_m3".equals(entity2)) {
                this.exponentFtToM += 3 * power;
                this.factorDen = this.factorDen.multiply(BigDecimal.valueOf(Math.pow(12.0, 3.0)));
            } else if ("gal_to_m3".equals(entity2)) {
            ...
            } else if ("gal_imp_to_m3".equals(entity2)) {
                this.exponentGalImpToM3 += power;
            } else if ("G".equals(entity2)) {
                this.exponentG += power;
            } else if ("gravity".equals(entity2)) {
                this.exponentGravity += power;
            } else if ("lb_to_kg".equals(entity2)) {
                this.exponentLbToKg += power;
            } else if ("glucose_molar_mass".equals(entity2)) {
                this.exponentGlucoseMolarMass += power;
            } else if ("item_per_mole".equals(entity2)) {
                this.exponentItemPerMole += power;
            } else if ("PI".equals(entity2)) {
                this.exponentPi += power;
            } else {
                BigDecimal decimalEntity = new BigDecimal(entity2).pow(power, MathContext.DECIMAL128);
