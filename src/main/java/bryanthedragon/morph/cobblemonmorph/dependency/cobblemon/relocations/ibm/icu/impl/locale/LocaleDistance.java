...
public class LocaleDistance {
    ...
    public void testOnlyPrintDistanceTable() {
        for (Map.Entry<String, Integer> mapping : this.testOnlyGetDistanceTable().entrySet()) {
            String suffix = "";
            int value2 = mapping.getValue();
            if ((value2 & 0x80) != 0) {
                value2 &= 0xFFFFFF7F;
                suffix = " skip script";
            }
            System.out.println(mapping.getKey() + '=' + value2 + suffix);
        }
    }
    ...
    public static final class Data {
        ...
        public static Data load() throws MissingResourceException {
            ...
            UResource.Value value2 = langInfo.getValueWithFallback("match");
            UResource.Table matchTable = value2.getTable();
            ByteBuffer buffer = Data.getValue(matchTable, "trie", value2).getBinary();
            byte[] trie = new byte[buffer.remaining()];
            buffer.get(trie);
            buffer = Data.getValue(matchTable, "regionToPartitions", value2).getBinary();
            ...
            String[] partitions = Data.getValue(matchTable, "partitions", value2).getStringArray();
            if (matchTable.findValue("paradigms", value2)) {
                String[] paradigms = value2.getStringArray();
            ...
            if ((distances = Data.getValue(matchTable, "distances", value2).getIntVector()).length < 4) {
