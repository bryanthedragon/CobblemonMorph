...
public abstract class InputReadNode extends Node {
   ...
   public abstract int execute(Object input, int index, Encodings.Encoding encoding);
   ...
   static int doBoxedCharArray(
      Object input, int index, Encodings.Encoding encoding, @CachedLibrary("input") InteropLibrary inputs, @Cached ToCharNode toCharNode
   ) {
      try {
         return toCharNode.execute(inputs.readArrayElement(input, index));
      } catch (InvalidArrayIndexException | UnsupportedTypeException | UnsupportedMessageException var6) {
