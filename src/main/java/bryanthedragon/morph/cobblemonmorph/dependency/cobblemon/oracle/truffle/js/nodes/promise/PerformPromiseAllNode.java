...
public class PerformPromiseAllNode extends PerformPromiseCombinatorNode {
      ...
      while (true) {
         ...
         if (next == Boolean.FALSE) {
            ...
            if (remainingElementsCount.value == 0) {
               JSDynamicObject valuesArray = JSArray.createConstantObjectArray(this.context, this.getRealm(), values.toArray());
   ...
   protected JSFunctionObject createResolveElementFunction(
      ...
      JSFunctionData functionData = this.context
         .getOrCreateBuiltinFunctionData(JSContext.BuiltinFunctionKey.PromiseAllResolveElement, c -> createResolveElementFunctionImpl(c));
   ...
   private static JSFunctionData createResolveElementFunctionImpl(JSContext context) {
      class PromiseAllResolveElementRootNode extends JavaScriptRootNode implements AsyncHandlerRootNode {
         ...
         public AsyncHandlerRootNode.AsyncStackTraceInfo getAsyncStackTraceInfo(JSFunctionObject handlerFunction) {
            ...
            PerformPromiseAllNode.ResolveElementArgs resolveArgs = (PerformPromiseAllNode.ResolveElementArgs)JSObjectUtil.getHiddenProperty(
               handlerFunction, PerformPromiseAllNode.RESOLVE_ELEMENT_ARGS_KEY
            );
   ...
   static TruffleStackTraceElement createPromiseAllStackTraceElement(int promiseIndex, JSRealm realm) {
      return TruffleStackTraceElement.create(
         new PerformPromiseAllNode.PromiseAllMarkerRootNode(null, JSBuiltin.createSourceSection()),
         (RootCallTarget)JSFunction.getFunctionData(realm.getPromiseAllFunctionObject()).getCallTarget(),
         Truffle.getRuntime()
            .createMaterializedFrame(JSArguments.createOneArg(realm.getPromiseConstructor(), realm.getPromiseAllFunctionObject(), promiseIndex))
      );
   }
