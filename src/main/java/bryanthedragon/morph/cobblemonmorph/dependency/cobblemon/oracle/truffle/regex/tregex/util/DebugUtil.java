...
public class DebugUtil {
   ...
   public static String charToString(int c) {
      if (Constants.WORD_CHARS.contains(c)) {
         ...
         return c <= 65535 ? String.format("\\u%04x", c) : String.format("\\u{%06x}", c);
      }
   }
      ...
      while (m.find()) {
         ...
         switch (c) {
            ...
            default:
               ...
               replacement = String.format("\\\\u%04x", Integer.valueOf(c));
         }
   ...
   public static String randomJsStringFromRanges(SortedListOfRanges ranges, int length) {
      ...
      for (int i = 0; i < length; i++) {
         ...
         if (randChar == 34) {
            ...
            stringBuilder.append("\\\\\\\\");
         } else if (randChar > 65535) {
            stringBuilder.append(String.format("\\u{%06x}", randChar));
         } else if (randChar <= 127 && !Character.isISOControl(randChar)) {
   ...
   public static class Timer {
      ...
      public String elapsedToString() {
         return elapsedToString(this.getElapsed());
      }

      public static String elapsedToString(long elapsed) {
         return String.format("%fms", elapsed / 1000000.0);
      }
   }
}
