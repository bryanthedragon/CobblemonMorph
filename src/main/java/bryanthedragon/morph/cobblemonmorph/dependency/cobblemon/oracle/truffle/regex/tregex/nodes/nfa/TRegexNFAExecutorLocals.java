...
public final class TRegexNFAExecutorLocals extends TRegexExecutorLocals {
   ...
   public void addInitialState(int stateId) {
      ...
      this.curStatesLength = this.curStatesLength + this.frameSize;
   }
   ...
   public int next() {
      this.iCurStates = this.iCurStates + this.frameSize;
   ...
   public void pushSuccessor(NFAStateTransition t, boolean copy) {
      ...
      if (copy) {
         System.arraycopy(
            this.curStates,
            offsetCaptureGroups(this.iCurStates - this.frameSize),
            this.nextStates,
            offsetCaptureGroups(this.nextStatesLength),
            this.frameSize - 1
         );
      } else {
         Arrays.fill(this.nextStates, offsetCaptureGroups(this.nextStatesLength), this.nextStatesLength + this.frameSize, -1);
      }

      t.getGroupBoundaries()
         .apply(this.nextStates, offsetCaptureGroups(this.nextStatesLength), this.offsetLastGroup(this.nextStatesLength), this.getIndex(), this.trackLastGroup);
      this.nextStatesLength = this.nextStatesLength + this.frameSize;
   }
   ...
   public void pushResult(NFAStateTransition t, boolean copy) {
      ...
      if (copy) {
         System.arraycopy(this.curStates, offsetCaptureGroups(this.iCurStates - this.frameSize), this.result, 0, this.result.length);
      } else {
   ...
   public String toString() {
      ...
      for (int ix = 0; ix < this.nextStatesLength; ix += this.frameSize) {
         if (ix > 0) {
            sb.append(",");
         }

         sb.append(this.nextStates[ix]);
      }
