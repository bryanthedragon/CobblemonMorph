...
abstract class BreakpointLocation {
   ...
   static final BreakpointLocation ANY = new BreakpointLocation.BreakpointSourceLocation();
   ...
   static BreakpointLocation create(Object key, SourceElement[] sourceElements, SourceSection sourceSection) {
      return new BreakpointLocation.BreakpointSourceLocation(key, sourceElements, sourceSection);
   }

   static BreakpointLocation create(Object key, SourceElement[] sourceElements, int line, int column) {
      return new BreakpointLocation.BreakpointSourceLocation(key, sourceElements, line, column);
   }

   static BreakpointLocation create(SourceElement[] sourceElements, SuspensionFilter filter) {
      return new BreakpointLocation.BreakpointFilteredLocation(sourceElements, filter);
   }
   ...
   private static final class BreakpointSourceLocation extends BreakpointLocation {
      ...
      Predicate<Source> createSourcePredicate() {
         if (this.key == null) {
            return null;
         } else if (this.key instanceof URI) {
            if (this.key == ANY_SOURCE) {
               ...
               return new Predicate<Source>() {
                  public boolean test(Source s) {
                     URI uri = s.getURI();
                     return uri.isAbsolute() ? sourceUri.equals(uri) : sourceRawPath != null && sourceRawPath.endsWith(uri.getRawPath());
                  }
      ...
      public String toString() {
         ...
         } else if (this.key instanceof Source) {
            keyDescription = "sourceName=" + ((Source)this.key).getName();
         } else if (this.key instanceof URI) {
            keyDescription = "uri=" + ((URI)this.key).toString();
         } else {
            keyDescription = this.key.toString();
         }
