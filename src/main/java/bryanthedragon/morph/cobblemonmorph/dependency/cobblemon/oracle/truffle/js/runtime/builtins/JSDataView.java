...
public final class JSDataView extends JSNonProxy implements JSConstructorFactory.Default, PrototypeSupplier {
   ...
   private static void putGetters(JSRealm realm, JSObject prototype) {
      putGetter(realm, prototype, BUFFER, JSContext.BuiltinFunctionKey.DataViewBuffer, view -> getArrayBuffer(view));
      putGetter(realm, prototype, BYTE_LENGTH, JSContext.BuiltinFunctionKey.DataViewByteLength, view -> typedArrayGetLengthChecked(view));
      putGetter(realm, prototype, BYTE_OFFSET, JSContext.BuiltinFunctionKey.DataViewByteOffset, view -> typedArrayGetOffsetChecked(view));
   }

   public static int typedArrayGetLengthChecked(JSDynamicObject thisObj) {
      if (JSArrayBuffer.isDetachedBuffer(getArrayBuffer(thisObj))) {
   ...
   private static void putGetter(
      JSRealm realm, JSObject prototype, TruffleString name, JSContext.BuiltinFunctionKey key, Function<JSDynamicObject, Object> function
   ) {
      JSFunctionData getterData = realm.getContext()
         .getOrCreateBuiltinFunctionData(key, c -> JSFunctionData.createCallOnly(c, (new JavaScriptRootNode(c.getLanguage(), null, null) {
            @Override
