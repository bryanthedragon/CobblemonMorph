...
public abstract class JSIdenticalNode extends JSCompareNode {
   ...
   public static JavaScriptNode create(JavaScriptNode left, JavaScriptNode right) {
      ...
      } else if (right instanceof JSConstantNode.JSConstantNullNode) {
         return IsNullNode.create(left, false);
      } else if (left instanceof JSConstantNode.JSConstantStringNode) {
         return IsIdenticalStringNode.create((TruffleString)left.execute(null), right, true);
      } else if (right instanceof JSConstantNode.JSConstantStringNode) {
      ...
      } else if (right instanceof JSConstantNode.JSConstantIntegerNode) {
         return IsIdenticalIntegerNode.create((Integer)right.execute(null), left, false);
      } else if (left instanceof JSConstantNode.JSConstantBooleanNode) {
         return IsIdenticalBooleanNode.create((Boolean)left.execute(null), right, true);
      } else if (right instanceof JSConstantNode.JSConstantBooleanNode) {
         return IsIdenticalBooleanNode.create((Boolean)right.execute(null), left, false);
      } else if (left instanceof JSConstantNode.JSConstantUndefinedNode) {
         return IsIdenticalUndefinedNode.create(right, true);
      } else {
         return (JavaScriptNode)(right instanceof JSConstantNode.JSConstantUndefinedNode
            ? IsIdenticalUndefinedNode.create(left, false)
            : JSIdenticalNodeGen.create(left, right, 0));
      }
   }
   ...
   protected boolean doDouble(double a, double b) {
      if (this.type == 0) {
         ...
         if (Double.isNaN(a)) {
            ...
            return a == 0.0 && b == 0.0 ? JSRuntime.isNegativeZero(a) == JSRuntime.isNegativeZero(b) : a == b;
         }
      } else {
         assert this.type == 2;
   ...
   protected static boolean doNullA(
      Object a, Object b, @Cached.Shared("isNullInterop") @CachedLibrary(limit = "InteropLibraryLimit") InteropLibrary nullInterop
   ) {
   ...
   protected static boolean doNullB(
      Object a, Object b, @Cached.Shared("isNullInterop") @CachedLibrary(limit = "InteropLibraryLimit") InteropLibrary nullInterop
   ) {
   ...
   @Specialization(
      guards = {"a.getClass() == cachedClassA", "b.getClass() == cachedClassB", "isJavaNumberType(cachedClassA) != isJavaNumberType(cachedClassB)"},
      limit = "MAX_CLASSES"
   )
   protected static boolean doNumberNotNumberCached(
      Object a, Object b, @Cached("a.getClass()") Class<?> cachedClassA, @Cached("b.getClass()") Class<?> cachedClassB
   ) {
      return false;
   }
   ...
   protected static boolean doForeignObject(Object a, Object b, @CachedLibrary("a") InteropLibrary aInterop, @CachedLibrary("b") InteropLibrary bInterop) {
   ...
   protected JavaScriptNode copyUninitialized(Set<Class<? extends Tag>> materializedTags) {
      return JSIdenticalNodeGen.create(cloneUninitialized(this.getLeft(), materializedTags), cloneUninitialized(this.getRight(), materializedTags), this.type);
   }
}
