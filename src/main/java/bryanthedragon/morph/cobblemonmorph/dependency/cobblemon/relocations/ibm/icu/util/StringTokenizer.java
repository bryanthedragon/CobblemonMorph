...
public final class StringTokenizer implements Enumeration<Object> {
   ...
   public String nextToken() {
      if (this.m_tokenOffset_ >= 0) {
         ...
         String resultx;
         if (tokenlimit < 0) {
            resultx = this.m_source_.substring(this.m_nextOffset_);
            this.m_nextOffset_ = tokenlimit;
         } else {
            resultx = this.m_source_.substring(this.m_nextOffset_, tokenlimit);
   ...
   private int getNextDelimiter(int offset) {
      if (offset >= 0) {
         ...
         if (this.delims == null) {
            ...
            } while (!this.m_delimiters_.contains(c) && ++result < this.m_length_);
         } else {
            do {
               c = UTF16.charAt(this.m_source_, result);
            } while ((c >= this.delims.length || !this.delims[c]) && ++result < this.m_length_);
         }
