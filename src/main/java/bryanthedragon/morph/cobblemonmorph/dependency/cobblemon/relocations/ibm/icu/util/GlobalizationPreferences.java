...
public class GlobalizationPreferences implements Freezable<GlobalizationPreferences> {
   ...
   public ULocale getLocale(int index) {
      ...
      return index >= 0 && index < lcls.size() ? lcls.get(index) : null;
   }
   ...
   public BreakIterator getBreakIterator(int type) {
      if (type < 0 || type >= 5) {
         ...
         return this.breakIterators != null && this.breakIterators[type] != null
            ? (BreakIterator)this.breakIterators[type].clone()
            : this.guessBreakIterator(type);
      }
   }
   ...
   static {
      ...
      for (int i = 0; i < calLocales.length; i++) {
         BitSet bits = available_locales.get(calLocales[i]);
         if (bits == null) {
            bits = new BitSet(6);
            available_locales.put(allLocales[i], bits);
         }
      ...
      for (int i = 0; i < dateLocales.length; i++) {
         BitSet bits = available_locales.get(dateLocales[i]);
         if (bits == null) {
            bits = new BitSet(6);
            available_locales.put(allLocales[i], bits);
         }
      ...
      for (int i = 0; i < numLocales.length; i++) {
         BitSet bits = available_locales.get(numLocales[i]);
         if (bits == null) {
            bits = new BitSet(6);
            available_locales.put(allLocales[i], bits);
         }
      ...
      for (int i = 0; i < collLocales.length; i++) {
         BitSet bits = available_locales.get(collLocales[i]);
         if (bits == null) {
            bits = new BitSet(6);
            available_locales.put(allLocales[i], bits);
         }
      ...
      for (int i = 0; i < brkLocales.length; i++) {
         BitSet bits = available_locales.get(brkLocales[i]);
         bits.set(5);
      }

      language_territory_hack_map = new HashMap<>();
      language_territory_hack = new String[][]{
         {"af", "ZA"},
         {"am", "ET"},
         {"ar", "SA"},
         {"as", "IN"},
      ...
      territory_tzid_hack = new String[][]{
