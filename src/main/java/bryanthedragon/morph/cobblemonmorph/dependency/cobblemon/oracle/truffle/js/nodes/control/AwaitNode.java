...
public class AwaitNode extends AbstractAwaitNode implements ResumableNode.WithIntState {
   ...
   protected void echoInput(VirtualFrame frame, Object value) {
      if (this.materializedInputNode != null) {
         this.materializedInputNode.executeWithTarget(frame, value);
      }
   }

   @Override
   protected JavaScriptNode copyUninitialized(Set<Class<? extends Tag>> materializedTags) {
      JavaScriptNode expressionCopy = cloneUninitialized(this.expression, materializedTags);
      JSReadFrameSlotNode asyncResultCopy = cloneUninitialized(this.readAsyncResultNode, materializedTags);
      JSReadFrameSlotNode asyncContextCopy = cloneUninitialized(this.readAsyncContextNode, materializedTags);
      return create(this.context, this.stateSlot, expressionCopy, asyncContextCopy, asyncResultCopy);
   }
   ...
   public InstrumentableNode materializeInstrumentableNodes(Set<Class<? extends Tag>> materializedTags) {
      if (this.materializationNeeded() && materializedTags.contains(JSTags.ControlFlowBranchTag.class)) {
         ...
         AwaitNode materialized = new AwaitNode(
            this.context,
            this.stateSlot,
