...
import com.cobblemon.mod.relocations.ibm.icu.text.Bidi;
...
final class BidiLine {
    ...
    static void setTrailingWSStart(Bidi bidi) {
        ...
        if (dirProps[start2 - 1] == 7) {
            bidi.trailingWSStart = start2;
            return;
        }
        for (start2 = bidi.length; start2 > 0 && (Bidi.DirPropFlag(dirProps[start2 - 1]) & Bidi.MASK_WS) != 0; --start2) {
        }
        while (start2 > 0 && levels[start2 - 1] == paraLevel) {
            --start2;
        }
        bidi.trailingWSStart = start2;
    }

    static Bidi setLine(Bidi paraBidi, int start2, int limit) {
        Bidi lineBidi = new Bidi();
        lineBidi.originalLength = lineBidi.resultLength = limit - start2;
        lineBidi.length = lineBidi.resultLength;
        int length = lineBidi.resultLength;
        lineBidi.text = new char[length];
        System.arraycopy(paraBidi.text, start2, lineBidi.text, 0, length);
        lineBidi.paraLevel = paraBidi.GetParaLevelAt(start2);
        ...
        System.arraycopy(paraBidi.dirProps, start2, lineBidi.dirProps, 0, length);
        lineBidi.getLevelsMemory(length);
        lineBidi.levels = lineBidi.levelsMemory;
        System.arraycopy(paraBidi.levels, start2, lineBidi.levels, 0, length);
        lineBidi.runCount = -1;
        if (paraBidi.direction != 2) {
            lineBidi.direction = paraBidi.direction;
            lineBidi.trailingWSStart = paraBidi.trailingWSStart <= start2 ? 0 : (paraBidi.trailingWSStart < limit ? paraBidi.trailingWSStart - start2 : length);
        } else {
    ...
    static byte[] getLevels(Bidi bidi) {
        int start2 = bidi.trailingWSStart;
        int length = bidi.length;
        if (start2 != length) {
            Arrays.fill(bidi.levels, start2, length, bidi.paraLevel);
    ...
    static BidiRun getVisualRun(Bidi bidi, int runIndex) {
        int start2 = bidi.runs[runIndex].start;
        byte level = bidi.runs[runIndex].level;
        int limit = runIndex > 0 ? start2 + bidi.runs[runIndex].limit - bidi.runs[runIndex - 1].limit : start2 + bidi.runs[0].limit;
        return new BidiRun(start2, limit, level);
    }
        ...
        if (bidi.direction != 2) {
            ...
            if (runCount == 1 && limit == length) {
                ...
                do {
                    ...
                    runs[runIndex] = new BidiRun(start2, i - start2, level);
                    ++runIndex;
                } while (i < limit);
                if (limit < length) {
    ...
    static int[] reorderLogical(byte[] levels) {
        ...
        block0: do {
            int start2 = 0;
            while (true) {
                if (start2 < levels.length && levels[start2] < maxLevel) {
                    ++start2;
                    continue;
                }
                if (start2 >= levels.length) continue block0;
                int limit = start2;
                while (++limit < levels.length && levels[limit] >= maxLevel) {
                }
                int sumOfSosEos = start2 + limit - 1;
                do {
                    indexMap[start2] = sumOfSosEos - indexMap[start2];
                } while (++start2 < limit);
    ...
    static int[] reorderVisual(byte[] levels) {
        ...
        block0: do {
            ...
            while (true) {
                ...
                for (int end2 = limit - 1; start2 < end2; ++start2, --end2) {
                    int temp = indexMap[start2];
                    indexMap[start2] = indexMap[end2];
                    indexMap[end2] = temp;
                }
                if (limit == levels.length) continue block0;
                start2 = limit + 1;
    ...
    static int getLogicalIndex(Bidi bidi, int visualIndex) {
        ...
        int start2 = runs[i].start;
        if (runs[i].isEvenRun()) {
            if (i > 0) {
                visualIndex -= runs[i - 1].limit;
            }
            return start2 + visualIndex;
        }
        return start2 + runs[i].limit - visualIndex - 1;
    }
