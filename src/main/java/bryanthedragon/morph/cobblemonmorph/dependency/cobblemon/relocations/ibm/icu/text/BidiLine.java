...
final class BidiLine {
      ...
      if (bidi.runCount < 0) {
         ...
         if (bidi.insertPoints.size > 0) {
            for (int ip = 0; ip < bidi.insertPoints.size; ip++) {
               ...
               bidi.runs[runIndex].insertRemove = bidi.runs[runIndex].insertRemove | point.flag;
            }
         }
   ...
   static int getVisualIndex(Bidi bidi, int logicalIndex) {
      ...
      if (bidi.insertPoints.size > 0) {
         ...
         while (true) {
            int length = runs[ix].limit - visualStart;
            int insertRemove = runs[ix].insertRemove;
            if ((insertRemove & 5) > 0) {
               markFound++;
            }

            if (visualIndex < runs[ix].limit) {
         ...
         if (Bidi.IsBidiControlChar(uchar)) {
            ...
            while (true) {
               int lengthx = runs[ix].limit - visualStart;
               int insertRemovex = runs[ix].insertRemove;
               if (visualIndex < runs[ix].limit) {
                  if (insertRemovex == 0) {
                     ...
                     int start;
                     int limit;
                     if (runs[ix].isEvenRun()) {
                        start = runs[ix].start;
                        limit = logicalIndex;
                     } else {
                        start = logicalIndex + 1;
                        limit = runs[ix].start + lengthx;
                     }
   ...
   static int getLogicalIndex(Bidi bidi, int visualIndex) {
      ...
      if (bidi.insertPoints.size > 0) {
         ...
         while (true) {
            int lengthx = runs[i].limit - visualStart;
            int insertRemovex = runs[i].insertRemove;
            if (visualIndex < runs[i].limit - controlFound + insertRemovex) {
               if (insertRemovex == 0) {
                  visualIndex += controlFound;
               } else {
                  int logicalStart = runs[i].start;
                  boolean evenRun = runs[i].isEvenRun();
                  int logicalEnd = logicalStart + lengthx - 1;
      ...
      if (bidi.insertPoints.size > 0) {
         ...
         while (i < runCount) {
            ...
            if (markFound > 0) {
               ...
               for (int jx = logicalStart; jx < logicalLimit; jx++) {
                  indexMap[jx] += markFound;
               }
            }
         ...
         while (i < runCount) {
            int lengthx = runs[i].limit - visualStart;
            int insertRemovex = runs[i].insertRemove;
            if (controlFound - insertRemovex != 0) {
               int logicalStart = runs[i].start;
               boolean evenRun = runs[i].isEvenRun();
               int logicalLimit = logicalStart + lengthx;
               if (insertRemovex == 0) {
                  for (int jx = logicalStart; jx < logicalLimit; jx++) {
                     indexMap[jx] -= controlFound;
                  }
               } else {
                  for (int jx = 0; jx < lengthx; jx++) {
                     int k = evenRun ? logicalStart + jx : logicalLimit - jx - 1;
                     char uchar = bidi.text[k];
                     if (Bidi.IsBidiControlChar(uchar)) {
                        controlFound++;
                        indexMap[k] = -1;
   ...
   static int[] getVisualMap(Bidi bidi) {
      ...
      if (bidi.insertPoints.size > 0) {
         ...
         for (int var31 = runCount - 1; var31 >= 0 && markFound > 0; var31--) {
            int insertRemovex = runs[var31].insertRemove;
            if ((insertRemovex & 10) > 0) {
               indexMap[--k] = -1;
               markFound--;
            }
            ...
            for (int jx = runs[var31].limit - 1; jx >= visualStart && markFound > 0; jx--) {
               indexMap[--k] = indexMap[jx];
            }

            if ((insertRemovex & 5) > 0) {
               indexMap[--k] = -1;
               markFound--;
            }
         }
         ...
         while (i < runCount) {
            int length = runs[i].limit - visualStart;
            int insertRemovexx = runs[i].insertRemove;
            if (insertRemovexx == 0 && k == visualStart) {
               k += length;
            } else if (insertRemovexx == 0) {
               int visualLimit = runs[i].limit;

               for (int jx = visualStart; jx < visualLimit; jx++) {
                  indexMap[k++] = indexMap[jx];
               }
            } else {
               int logicalStart = runs[i].start;
               ...
               for (int jx = 0; jx < length; jx++) {
                  int m = evenRun ? logicalStart + jx : logicalEnd - jx;
   ...
   static int[] invertMap(int[] srcMap) {
      ...
      for (int var8 = 0; var8 < srcLength; var8++) {
         int srcEntryx = srcMap[var8];
         if (srcEntryx >= 0) {
            destMap[srcEntryx] = var8;
         }
      }
