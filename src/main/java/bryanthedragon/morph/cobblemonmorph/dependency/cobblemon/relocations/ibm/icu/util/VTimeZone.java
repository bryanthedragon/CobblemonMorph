...
import com.cobblemon.mod.relocations.ibm.icu.util.RuleBasedTimeZone;
...
extends BasicTimeZone {
    ...
    public void write(Writer writer, long start2) throws IOException {
        TimeZoneRule[] rules = this.tz.getTimeZoneRules(start2);
        ...
        String[] customProperties = null;
        if (this.olsonzid != null && ICU_TZVERSION != null) {
            customProperties = new String[]{"X-TZINFO:" + this.olsonzid + "[" + ICU_TZVERSION + "/Partial@" + start2 + "]"};
        }
    ...
    @Override
    public boolean hasEquivalentTransitions(TimeZone other, long start2, long end2) {
        if (this == other) {
            return true;
        }
        return this.tz.hasEquivalentTransitions(other, start2, end2);
    }
    ...
    @Override
    public TimeZoneRule[] getTimeZoneRules(long start2) {
        return this.tz.getTimeZoneRules(start2);
    }
    ...
    private boolean load(Reader reader) {
        try {
            ...
            while (true) {
                int ch;
                if ((ch = reader.read()) == -1) {
                    if (!start2 || !line.toString().startsWith(ICAL_END_VTIMEZONE)) break;
    ...
    private boolean parse() {
        ...
        for (String line : this.vtzlines) {
            ...
            String value2 = line.substring(valueSep + 1);
            switch (state) {
                case 0: {
                    if (!name.equals(ICAL_BEGIN) || !value2.equals(ICAL_VTIMEZONE)) break;
                    state = 1;
                    break;
                }
                case 1: {
                    ...
                    if (name.equals(ICAL_LASTMOD)) {
                        this.lastmod = new Date(VTimeZone.parseDateTimeString(value2, 0));
                        break;
                    }
                    if (name.equals(ICAL_BEGIN)) {
                        boolean isDST = value2.equals(ICAL_DAYLIGHT);
                        if (value2.equals(ICAL_STANDARD) || isDST) {
                            if (tzid == null) {
                                state = 3;
                                break;
                            }
                ...
                case 2: {
                    ...
                    if (name.equals(ICAL_RDATE)) {
                        ...
                        StringTokenizer st = new StringTokenizer(value2, COMMA);
                    ...
                    try {
                        ...
                        start2 = VTimeZone.parseDateTimeString(dtstart, fromOffset);
                        Date actualStart = null;
                        rule = isRRULE ? VTimeZone.createRuleByRRULE(tzname, rawOffset, dstSavings, start2, dates, fromOffset) : VTimeZone.createRuleByRDATE(tzname, rawOffset, dstSavings, start2, dates, fromOffset);
        ...
        if (finalRuleCount == 1) {
            if (rules.size() == 1) {
                ...
                Date start3 = finalStart = finalRule.getFirstStart(initialRawOffset, initialDSTSavings);
                for (int i = 0; i < rules.size(); ++i) {
                    TimeZoneRule r;
                    Date lastStart;
                    if (finalRuleIdx == i || !(lastStart = (r = (TimeZoneRule)rules.get(i)).getFinalStart(tmpRaw, tmpDST)).after(start3)) continue;
                    start3 = finalRule.getNextStart(lastStart.getTime(), r.getRawOffset(), r.getDSTSavings(), false);
                }
                if (start3 == finalStart) {
                    ...
                    int[] fields = Grego.timeToFields(start3.getTime(), null);
     ...
     */
    private static TimeZoneRule createRuleByRRULE(String tzname, int rawOffset, int dstSavings, long start2, List<String> dates, int fromOffset) {
        long[] until;
        ...
        int[] dfields = Grego.timeToFields(start2 + (long)fromOffset, null);
    ...
    private static int[] parseRRULE(String rrule, long[] until) {
        ...
        block8: while (st.hasMoreTokens()) {
            ...
            String value2 = prop.substring(sep + 1);
            if (attr.equals(ICAL_FREQ)) {
                if (value2.equals(ICAL_YEARLY)) {
                    yearly = true;
                    continue;
                }
                parseError = true;
            ...
            if (attr.equals(ICAL_UNTIL)) {
                try {
                    untilTime = VTimeZone.parseDateTimeString(value2, 0);
                    continue;
                }
            ...
            if (attr.equals(ICAL_BYDAY)) {
                ...
                if (length > 2) {
                    ...
                    try {
                        int n = Integer.parseInt(value2.substring(length - 3, length - 2));
                        if (n == 0 || n > 4) {
                            parseError = true;
                            break;
                        }
                    ...
                    value2 = value2.substring(length - 2);
                }
                for (wday = 0; wday < ICAL_DOW_NAMES.length && !value2.equals(ICAL_DOW_NAMES[wday]); ++wday) {
                }
            ...
            StringTokenizer days = new StringTokenizer(value2, COMMA);
        ...
        if (dayOfMonth == null) {
            ...
            results2 = new int[3 + dayOfMonth.length];
            for (int i = 0; i < dayOfMonth.length; ++i) {
                results2[3 + i] = dayOfMonth[i];
            }
        }
        results2[0] = month;
        results2[1] = dayOfWeek;
        results2[2] = nthDayOfWeek;
        return results2;
    }

    private static TimeZoneRule createRuleByRDATE(String tzname, int rawOffset, int dstSavings, long start2, List<String> dates, int fromOffset) {
        long[] times2;
        if (dates == null || dates.size() == 0) {
            times2 = new long[]{start2};
        } else {
            times2 = new long[dates.size()];
            int idx = 0;
            try {
                for (String date : dates) {
                    times2[idx++] = VTimeZone.parseDateTimeString(date, fromOffset);
                }
            }
            catch (IllegalArgumentException iae) {
                return null;
        ...
        return new TimeArrayTimeZoneRule(tzname, rawOffset, dstSavings, times2, 2);
    }
    ...
    private static String getDateTimeString(long time) {
        ...
        int t = fields[5];
        int hour = t / 3600000;
        int min2 = (t %= 3600000) / 60000;
        int sec = (t %= 60000) / 1000;
        sb.append(VTimeZone.numToString(hour, 2));
        sb.append(VTimeZone.numToString(min2, 2));
    ...
    private static long parseDateTimeString(String str, int offset) {
        ...
        block6: {
            ...
            try {
                ...
                min2 = Integer.parseInt(str.substring(11, 13));
            ...
            if (year >= 0 && month >= 0 && month <= 11 && day >= 1 && day <= maxDayOfMonth && hour >= 0 && hour < 24 && min2 >= 0 && min2 < 60 && sec >= 0 && sec < 60) {
                isValid = true;
            }
        }
        if (!isValid) {
        ...
        time += (long)(hour * 3600000 + min2 * 60000 + sec * 1000);
        if (!isUTC) {
            time -= (long)offset;
        }
        return time;
    ...
    private static int offsetStrToMillis(String str) {
        ...
        block5: {
            block4: {
                ...
                try {
                    ...
                    min2 = Integer.parseInt(str.substring(3, 5));
        ...
        int millis = sign * ((hour * 60 + min2) * 60 + sec) * 1000;
        return millis;
    }
