...
extends MethodVisitor {
    ...
    public void visitMethodInsn(int opcodeAndSource, String owner, String name, String descriptor, boolean isInterface) {
        ...
        if (opcode != 184) {
            ...
            if (opcode == 183 && name.equals("<init>")) {
                int i;
                Object initializedValue = value2 == Opcodes.UNINITIALIZED_THIS ? this.owner : this.uninitializedTypes.get(value2);
                for (i = 0; i < this.locals.size(); ++i) {
                    if (this.locals.get(i) != value2) continue;
                    this.locals.set(i, initializedValue);
                }
                for (i = 0; i < this.stack.size(); ++i) {
                    if (this.stack.get(i) != value2) continue;
    ...
    public void visitLdcInsn(Object value2) {
        ...
        if (value2 instanceof Integer) {
            ...
            if (!(value2 instanceof ConstantDynamic)) throw new IllegalArgumentException();
            this.pushDescriptor(((ConstantDynamic)value2).getDescriptor());
        }
        this.labels = null;
    }
    ...
    public void visitLocalVariable(String name, String descriptor, String signature, Label start2, Label end2, int index) {
        ...
        super.visitLocalVariable(name, descriptor, signature, start2, end2, index);
    }
    ...
    private void pop(int numSlots) {
        ...
        int end2 = size - numSlots;
        for (int i = size - 1; i >= end2; --i) {
            this.stack.remove(i);
        }
    }
