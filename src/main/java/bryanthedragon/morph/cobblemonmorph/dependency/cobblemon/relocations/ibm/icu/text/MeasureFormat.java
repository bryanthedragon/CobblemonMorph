...
import com.cobblemon.mod.relocations.ibm.icu.text.CurrencyFormat;
...
import com.cobblemon.mod.relocations.ibm.icu.text.TimeUnitFormat;
...
extends UFormat {
    ...
    private void formatMeasuresInternal(Appendable appendTo, FieldPosition fieldPosition, Measure ... measures) {
        ...
        String[] results2 = new String[measures.length];
        for (int i = 0; i < measures.length; ++i) {
            results2[i] = i == measures.length - 1 ? this.formatMeasure(measures[i]).toString() : this.formatMeasureInteger(measures[i]).toString();
        }
        ListFormatter.FormattedListBuilder builder = listFormatter.formatImpl(Arrays.asList(results2), false);
    ...
    private void formatMeasuresSlowTrack(ListFormatter listFormatter, Appendable appendTo, FieldPosition fieldPosition, Measure ... measures) {
        ...
        for (int i = 0; i < measures.length; ++i) {
            ...
            results2[i] = result.toString();
        }
        ListFormatter.FormattedListBuilder builder = listFormatter.formatImpl(Arrays.asList(results2), true);
            ...
            switch (c) {
                ...
                case 's': {
                    ...
                    if (i + 1 < pattern.length() && pattern.charAt(i + 1) == c) {
                        fsb.append(numberFormatter2.format(value2), null);
                        ++i;
                        continue block11;
                    }
                    fsb.append(this.numberFormatter.format(value2), null);
                    continue block11;
                }
                case '\'': {
                    if (i + 1 < pattern.length() && pattern.charAt(i + 1) == c) {
