...
public final class TRegexCompiler {
   ...
   public static RegexObject compile(RegexLanguage language, RegexSource source) throws RegexSyntaxException {
      ...
      try {
         ...
         logCompilationTime(source, timer);
         Loggers.LOG_BAILOUT_MESSAGES.fine(() -> var4.getReason() + ": " + source);
         throw var4;
      }
   }
   ...
   @CompilerDirectives.TruffleBoundary
   public static TRegexExecNode.LazyCaptureGroupRegexSearchNode compileLazyDFAExecutor(
      RegexLanguage language, NFA nfa, TRegexExecNode rootNode, boolean allowSimpleCG
   ) {
   ...
   private static void logCompilationTime(RegexSource regexSource, DebugUtil.Timer timer) {
      if (timer != null) {
         Loggers.LOG_TOTAL_COMPILATION_TIME
            .log(Level.FINE, "{0}, {1}", new Object[]{timer.elapsedToString(), DebugUtil.jsStringEscape(regexSource.toString())});
      }
   }
}
