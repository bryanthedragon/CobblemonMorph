...
public final class SimpleFormatterImpl {
    ...
    public static String compileToStringMinMaxArguments(CharSequence pattern, StringBuilder sb, int min2, int max2) {
        int argCount;
        if (min2 <= 2 && 2 <= max2) {
        ...
        if ((argCount = maxArg + 1) < min2) {
            throw new IllegalArgumentException("Fewer than minimum " + min2 + " arguments in pattern \"" + pattern + "\"");
        }
        if (argCount > max2) {
            throw new IllegalArgumentException("More than maximum " + max2 + " arguments in pattern \"" + pattern + "\"");
        }
    ...
    public static String formatRawPattern(String pattern, int min2, int max2, CharSequence ... values) {
        StringBuilder sb = new StringBuilder();
        String compiledPattern = SimpleFormatterImpl.compileToStringMinMaxArguments(pattern, sb, min2, max2);
        sb.setLength(0);
    ...
    public static int formatPrefixSuffix(String compiledPattern, Format.Field field, int start2, int end2, FormattedStringBuilder output) {
        int suffixOffset;
        int argLimit = SimpleFormatterImpl.getArgumentLimit(compiledPattern);
        if (argLimit == 0) {
            return output.splice(start2, end2, compiledPattern, 2, compiledPattern.length(), field);
        }
        assert (argLimit == 1);
        int length = 0;
        if (compiledPattern.charAt(1) != '\u0000') {
            int prefixLength = compiledPattern.charAt(1) - 256;
            length = output.insert(start2, compiledPattern, 2, 2 + prefixLength, field);
        ...
        if (suffixOffset < compiledPattern.length()) {
            ...
            length += output.insert(end2 + length, compiledPattern, 1 + suffixOffset, 1 + suffixOffset + suffixLength, field);
        }
        return length;
    }
