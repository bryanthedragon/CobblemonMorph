...
public final class MarkTakeCommand {

    private const val NAME = "takemark"
    private const val PLAYER = "player"
    private const val SLOT = "slot"
    private const val MARK = "mark"
    private val ALREADY_HAS_EXCEPTION = Dynamic3CommandExceptionType { a, b, c -> commandLang("$NAME.does_not_have", a, b, c).red() }
    ...
    private fun execute(context: CommandContext<CommandSourceStack>, ServerPlayer player) : Int {
        ...
        val takenMessage = commandLang(NAME, mark.getName(), player.name, pokemon.getDisplayName())
        context.source.sendSuccess({ takenMessage }, true)

        if (context.source.player?.equals(player) != true) {
            player.sendSystemMessage(takenMessage)
        }
