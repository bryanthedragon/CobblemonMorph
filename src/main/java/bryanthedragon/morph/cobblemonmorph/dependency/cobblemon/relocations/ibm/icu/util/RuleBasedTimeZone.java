...
public class RuleBasedTimeZone extends BasicTimeZone {
            ...
            if (this.finalRules != null && otherRBTZ.finalRules != null) {
               for (int i = 0; i < this.finalRules.length; i++) {
                  if ((this.finalRules[i] != null || otherRBTZ.finalRules[i] != null)
                     && (this.finalRules[i] == null || otherRBTZ.finalRules[i] == null || !this.finalRules[i].isEquivalentTo(otherRBTZ.finalRules[i]))) {
                     return false;
                  }
               }
                  ...
                  while (true) {
                     ...
                     if (this.finalRules != null) {
                        for (int ix = 0; ix < 2; ix++) {
                           if (this.finalRules[ix] != curRule) {
                              Date d = this.finalRules[ix].getNextStart(lastTransitionTime, curStdOffset, curDstSavings, false);
                              if (d != null) {
                                 long tt = d.getTime();
                                 if (tt < nextTransitionTime) {
                                    nextTransitionTime = tt;
                                    nextRule = this.finalRules[ix];
                                 }
                              }
                           }
                        }
   ...
   private TimeZoneRule findRuleInFinal(long time, boolean local, int NonExistingTimeOpt, int DuplicatedTimeOpt) {
      if (this.finalRules != null && this.finalRules.length == 2 && this.finalRules[0] != null && this.finalRules[1] != null) {
         long base = time;
         if (local) {
            int localDelta = getLocalDelta(
               this.finalRules[1].getRawOffset(),
               this.finalRules[1].getDSTSavings(),
               this.finalRules[0].getRawOffset(),
               this.finalRules[0].getDSTSavings(),
               NonExistingTimeOpt,
               DuplicatedTimeOpt
            );
