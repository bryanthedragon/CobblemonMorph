...
class RBBIRuleScanner {
   ...
   HashMap<String, RBBIRuleScanner.RBBISetTableEl> fSetTable = new HashMap<>();
      ...
      switch (action) {
         ...
         case 4:
            ...
            if (this.fLookAheadRule) {
               ...
               RBBINode catNodex = this.pushNewNode(8);
               this.fNodeStackPtr -= 2;
               catNodex.fLeftChild = thisRule;
               catNodex.fRightChild = endNode;
               this.fNodeStack[this.fNodeStackPtr] = catNodex;
            ...
            if (this.fRB.fTreeRoots[destRules] != null) {
               ...
               RBBINode orNodex = this.pushNewNode(9);
               orNodex.fLeftChild = prevRules;
               prevRules.fParent = orNodex;
               orNodex.fRightChild = thisRule;
               thisRule.fParent = orNodex;
               this.fRB.fTreeRoots[destRules] = orNodex;
            } else {
   ...
   void findSetFor(String s, RBBINode node, UnicodeSet setToAdopt) {
      RBBIRuleScanner.RBBISetTableEl el = this.fSetTable.get(s);
      if (el != null) {
   ...
   void parse() {
      ...
      while (state != 0) {
         ...
         if (this.fRB.fDebugEnv != null && this.fRB.fDebugEnv.indexOf("scan") >= 0) {
            System.out
               .println("char, line, col = ('" + (char)this.fC.fChar + "', " + this.fLineNum + ", " + this.fCharNum + "    state = " + tableEl.fStateName);
         }
         ...
         while (true) {
            ...
            if (tableEl.fCharClass < 127 && !this.fC.fEscaped && tableEl.fCharClass == this.fC.fChar
               || tableEl.fCharClass == 255
               || tableEl.fCharClass == 254 && this.fC.fEscaped
               || tableEl.fCharClass == 253 && this.fC.fEscaped && (this.fC.fChar == 80 || this.fC.fChar == 112)
               || tableEl.fCharClass == 252 && this.fC.fChar == -1) {
               break;
            }
