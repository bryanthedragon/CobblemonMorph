...
public final class CollationData {
    ...
    private void makeReorderRanges(int[] reorder, boolean latinMustMove, UVector32 ranges) {
        ...
        if (specials == 0 && reorder[0] == 25 && !latinMustMove) {
            ...
            assert (index != '\u0000');
            int start2 = this.scriptStarts[index];
            assert (lowStart <= start2);
            skippedReserved = start2 - lowStart;
            lowStart = start2;
        }
        ...
        for (i2 = 1; i2 < this.scriptStarts.length - 1; ++i2) {
            ...
            int start3 = this.scriptStarts[i2];
            if (!hasReorderToEnd && start3 > lowStart) {
                lowStart = start3;
            }
    ...
    private int addLowScriptRange(short[] table, int index, int lowStart) {
        char start2 = this.scriptStarts[index];
        if ((start2 & 0xFF) < (lowStart & 0xFF)) {
            lowStart += 256;
        }
        table[index] = (short)(lowStart >> 8);
        char limit = this.scriptStarts[index + 1];
        lowStart = (lowStart & 0xFF00) + ((limit & 0xFF00) - (start2 & 0xFF00)) | limit & 0xFF;
        return lowStart;
    }
    ...
    private int addHighScriptRange(short[] table, int index, int highLimit) {
        ...
        char start2 = this.scriptStarts[index];
        highLimit = (highLimit & 0xFF00) - ((limit & 0xFF00) - (start2 & 0xFF00)) | start2 & 0xFF;
