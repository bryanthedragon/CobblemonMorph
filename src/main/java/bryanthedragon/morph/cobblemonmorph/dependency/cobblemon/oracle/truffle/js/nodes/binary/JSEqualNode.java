...
public abstract class JSEqualNode extends JSCompareNode {
   ...
   public static JavaScriptNode create(JavaScriptNode left, JavaScriptNode right) {
      ...
      boolean rightIs = right instanceof JSConstantNode.JSConstantUndefinedNode || right instanceof JSConstantNode.JSConstantNullNode;
      if (leftIs) {
         return (JavaScriptNode)(rightIs ? JSConstantNode.createBoolean(true) : JSIsNullOrUndefinedNode.createFromEquals(left, right));
      } else {
         return (JavaScriptNode)(rightIs ? JSIsNullOrUndefinedNode.createFromEquals(left, right) : JSEqualNodeGen.create(left, right));
      }
   }
   ...
   protected boolean doDoubleString(double a, TruffleString b) {
      return doDouble(a, this.stringToDouble(b));
   }
   ...
   protected boolean doBooleanString(boolean a, TruffleString b) {
      return doBooleanDouble(a, this.stringToDouble(b));
   }
   ...
   protected boolean doStringDouble(TruffleString a, double b) {
      return doDouble(this.stringToDouble(a), b);
   }

   @Specialization
   protected boolean doStringBoolean(TruffleString a, boolean b) {
      return doDoubleBoolean(this.stringToDouble(a), b);
   }
   ...
   protected static boolean doLeftNullOrUndefined(
      Object a, Object b, @Cached.Shared("bInterop") @CachedLibrary(limit = "InteropLibraryLimit") InteropLibrary bInterop
   ) {
      return isNullish(b, bInterop);
   }

   @Specialization(guards = "isNullOrUndefined(b)")
   protected static boolean doRightNullOrUndefined(
      Object a, Object b, @Cached.Shared("aInterop") @CachedLibrary(limit = "InteropLibraryLimit") InteropLibrary aInterop
   ) {
   ...
   protected boolean doOverloaded(
      ...
      return a == b ? true : toBooleanNode.executeBoolean(overloadedOperatorNode.execute(a, b));
   }
   ...
   protected boolean doJSObjectVsPrimitive(
      JSObject a,
      Object b,
      @Cached.Shared("bInterop") @CachedLibrary(limit = "InteropLibraryLimit") InteropLibrary bInterop,
      @Cached.Shared("toPrimitive") @Cached("createHintDefault()") JSToPrimitiveNode toPrimitiveNode,
      @Cached.Shared("isPrimitive") @Cached IsPrimitiveNode isPrimitiveNode,
      @Cached.Shared("equal") @Cached JSEqualNode nestedEqualNode
   ) {
      return isNullish(b, bInterop) ? false : nestedEqualNode.executeBoolean(toPrimitiveNode.execute(a), b);
   }
   ...
   protected boolean doJSObjectVsPrimitive(
      Object a,
      JSObject b,
      @Cached.Shared("aInterop") @CachedLibrary(limit = "InteropLibraryLimit") InteropLibrary aInterop,
      @Cached.Shared("toPrimitive") @Cached("createHintDefault()") JSToPrimitiveNode toPrimitiveNode,
      @Cached.Shared("isPrimitive") @Cached IsPrimitiveNode isPrimitiveNode,
      @Cached.Shared("equal") @Cached JSEqualNode nestedEqualNode
   ) {
      return isNullish(a, aInterop) ? false : nestedEqualNode.executeBoolean(a, toPrimitiveNode.execute(b));
   }
   ...
   protected boolean doForeign(
      ...
      } else if (isNullish(a, aInterop)) {
         return isNullish(b, bInterop);
      } else if (!isNullish(b, bInterop)) {
         Object primLeft = isAPrimitive && !isAForeign ? a : toPrimitiveNode.execute(a);
         Object primRight = isBPrimitive && !isBForeign ? b : toPrimitiveNode.execute(b);
   ...
   protected JavaScriptNode copyUninitialized(Set<Class<? extends Tag>> materializedTags) {
      return JSEqualNodeGen.create(cloneUninitialized(this.getLeft(), materializedTags), cloneUninitialized(this.getRight(), materializedTags));
   }
}
