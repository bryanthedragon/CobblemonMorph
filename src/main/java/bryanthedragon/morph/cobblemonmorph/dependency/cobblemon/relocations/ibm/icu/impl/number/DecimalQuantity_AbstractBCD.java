...
import com.cobblemon.mod.relocations.ibm.icu.text.UFieldPosition;
...
public abstract class DecimalQuantity_AbstractBCD implements DecimalQuantity {
   ...
   private static final double[] DOUBLE_MULTIPLIERS = new double[]{
      1.0,
      10.0,
      100.0,
      1000.0,
   ...
   public double getPluralOperand(PluralRules.Operand operand) {
      ...
      switch (operand) {
         case i:
            return this.isNegative() ? -this.toLong(true) : this.toLong(true);
         case f:
   ...
   public int getLowerDisplayMagnitude() {
      ...
      return this.rReqPos < magnitude ? this.rReqPos : magnitude;
   }
   ...
   private void convertToAccurateDouble() {
      ...
      if (dstr.indexOf(69) != -1) {
         ...
         this.scale = this.scale + (2 - dstr.length());
   ...
   public long toLong(boolean truncateIfOverflow) {
      ...
      for (int magnitude = upperMagnitude; magnitude >= 0; magnitude--) {
         result = result * 10L + this.getDigitPos(magnitude - this.scale - this.exponent);
      }
   ...
   private void roundToMagnitude(int magnitude, MathContext mathContext, boolean nickel) {
      ...
      if ((position > 0 || this.isApproximate || nickel && trailingDigit != 0 && trailingDigit != 5) && this.precision != 0) {
         ...
         if (!this.isApproximate) {
            ...
            int px = safeSubtract(position, 2);
               ...
               if (leadingDigit != 4 || nickel && trailingDigit != 2 && trailingDigit != 7) {
                  if (leadingDigit != 5 || nickel && trailingDigit != 2 && trailingDigit != 7) {
                     if (leadingDigit != 9 || nickel && trailingDigit != 4 && trailingDigit != 9) {
                        ...
                        for (section = -2; px >= minP; px--) {
                           if (this.getDigitPos(px) != 9) {
                              section = 3;
                              break;
                           }
                        }
         ...
         boolean isEven = nickel
            ? trailingDigit < 2 || trailingDigit > 7 || trailingDigit == 2 && section != 3 || trailingDigit == 7 && section == 3
            : trailingDigit % 2 == 0;
   ...
   public boolean equals(Object other) {
      if (this == other) {
         ...
         boolean basicEquals = this.scale == _other.scale
            && this.precision == _other.precision
            && this.flags == _other.flags
            && this.lReqPos == _other.lReqPos
            && this.rReqPos == _other.rReqPos
            && this.isApproximate == _other.isApproximate;
         if (!basicEquals) {
            return false;
