            ...
            try {
               ...
               if (insnType != 8 && insnType != 15 && insnType != 14) {
                  ...
                  if (insnNode instanceof JumpInsnNode) {
                     ...
                     for (int ix = 0; ix < lookupSwitchInsn.labels.size(); ix++) {
                        LabelNode label = lookupSwitchInsn.labels.get(ix);
                     ...
                     for (int ix = 0; ix < tableSwitchInsn.labels.size(); ix++) {
                        LabelNode label = tableSwitchInsn.labels.get(ix);
                        currentFrame.initJumpTarget(insnOpcode, label);
                        targetInsnIndex = this.insnList.indexOf(label);
                        this.merge(targetInsnIndex, currentFrame, subroutine);
                        this.newControlFlowEdge(insnIndex, targetInsnIndex);
