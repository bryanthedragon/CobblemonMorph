...
public abstract class Calendar implements Serializable, Cloneable, Comparable<Calendar> {
   ...
   private static final ICUCache<String, Calendar.PatternData> PATTERN_CACHE = new SimpleCache<>();
   ...
   private static final int[][] LIMITS = new int[][]{
      ...
      {0, 0, 59, 59},
      {0, 0, 999, 999},
      {-43200000, -43200000, 43200000, 43200000},
      {0, 0, 3600000, 3600000},
      new int[0],
      {1, 1, 7, 7},
      new int[0],
      {-2130706432, -2130706432, 2130706432, 2130706432},
      {0, 0, 86399999, 86399999},
      {0, 0, 1, 1}
   };
   ...
   private static final Calendar.WeekDataCache WEEK_DATA_CACHE = new Calendar.WeekDataCache();
   protected static final int RESOLVE_REMAP = 32;
   static final int[][][] DATE_PRECEDENCE = new int[][][]{
      {{5}, {3, 7}, {4, 7}, {8, 7}, {3, 18}, {4, 18}, {8, 18}, {6}, {37, 1}, {35, 17}}, {{3}, {4}, {8}, {40, 7}, {40, 18}}
   };
   ...
   public boolean isEquivalentTo(Calendar other) {
      return this.getClass() == other.getClass()
         ...
         && this.getRepeatedWallTimeOption() == other.getRepeatedWallTimeOption()
         && this.getSkippedWallTimeOption() == other.getSkippedWallTimeOption();
   }

   @Override
   public int hashCode() {
      return (this.lenient ? 1 : 0)
         | this.firstDayOfWeek << 1
         | this.minimalDaysInFirstWeek << 4
         | this.repeatedWallTime << 7
         | this.skippedWallTime << 9
         | this.zone.hashCode() << 11;
   }
      ...
      if (amount != 0) {
         ...
         switch (field) {
            ...
            case 2:
               ...
               return;
            case 3:
               int dowx = this.internalGet(7) - this.getFirstDayOfWeek();
               if (dowx < 0) {
                  dowx += 7;
               }

               int fdy = (dowx - this.internalGet(6) + 1) % 7;
               if (fdy < 0) {
                  fdy += 7;
               }
               ...
               int ldy = (yearLen - this.internalGet(6) + dowx) % 7;
               int limitx = yearLen + 7 - ldy;
               int gapx = limitx - startx;
               int day_of_year = (this.internalGet(6) + amount * 7 - startx) % gapx;
               if (day_of_year < 0) {
                  day_of_year += gapx;
               }
            ...
            case 4:
               ...
               return;
            case 6:
               long deltax = amount * 86400000L;
               long min2 = this.time - (this.internalGet(6) - 1) * 86400000L;
               int yearLength = this.getActualMaximum(6);
               this.time = (this.time + deltax - min2) % (yearLength * 86400000L);
               if (this.time < 0L) {
            ...
            case 18:
               ...
               long min2 = this.time - leadDays * 86400000L;
            ...
            case 8:
               ...
               long min2 = this.time - preWeeks * 604800000L;
            ...
            case 11:
               ...
               this.setTimeInMillis(start + 3600000L * ((long)newHour - oldHour));
               return;
            case 15:
            case 16:
            default:
      ...
      if (timeStyle < -1 || timeStyle > 3) {
         ...
         if (timeStyle >= 0 && dateStyle >= 0) {
            pattern = SimpleFormatterImpl.formatRawPattern(
               patternData.getDateTimePattern(dateStyle), 2, 2, patternData.patterns[timeStyle], patternData.patterns[dateStyle + 4]
            );
   ...
   private static Calendar.PatternData getPatternData(ULocale locale, String calType) {
      ...
      if (baseLocID.length() > 0 && !baseLocID.equals("und")) {
         ...
         if (validLoc != baseLoc) {
            ...
            if (baseReg.length() > 0 && !baseReg.equals(validLoc.getCountry()) || !baseLoc.getLanguage().equals(validLoc.getLanguage())) {
               for (DateTimePatternGenerator dtpg = DateTimePatternGenerator.getInstanceNoStdPat(locale); i < 4; i++) {
      ...
      return new Calendar.PatternData(dateTimePatterns, dateTimePatternsOverrides);
   }
   ...
   public int fieldDifference(Date when, int field) {
      ...
      if (startMs < targetMs) {
         ...
         while (true) {
            this.setTimeInMillis(startMs);
            this.add(field, max);
            long msxx = this.getTimeInMillis();
            if (msxx == targetMs) {
               return max;
            }
            ...
            if (msxx < targetMs) {
               while (min - max > 1) {
                  int tx = min + (max - min) / 2;
                  this.setTimeInMillis(startMs);
                  this.add(field, tx);
                  long msxxx = this.getTimeInMillis();
                  if (msxxx == targetMs) {
                     return tx;
                  }
   ...
   public Calendar.WeekData getWeekData() {
      return new Calendar.WeekData(
         this.firstDayOfWeek, this.minimalDaysInFirstWeek, this.weekendOnset, this.weekendOnsetMillis, this.weekendCease, this.weekendCeaseMillis
      );
   }
   ...
   private static Calendar.WeekData getWeekDataForRegionInternal(String region) {
      if (region == null) {
         region = "001";
      }
      ...
      return new Calendar.WeekData(wdi[0], wdi[1], wdi[2], wdi[3], wdi[4], wdi[5]);
   }
   ...
   private void setWeekData(String region) {
      ...
      Calendar.WeekData wdata = WEEK_DATA_CACHE.getInstance(region, region);
   ...
   protected int computeZoneOffset(long millis, int millisInDay) {
      ...
      if (this.zone instanceof BasicTimeZone) {
         ...
         if (!sawRecentNegativeShift && this.skippedWallTime == 1) {
            long tgmtx = wall - (offsets[0] + offsets[1]);
            this.zone.getOffset(tgmtx, false, offsets);
         }
      }
   ...
   static class PatternData {
      ...
      private static Calendar.PatternData make(Calendar cal, ULocale loc) {
         ...
         Calendar.PatternData patternData = Calendar.PATTERN_CACHE.get(key);
         if (patternData == null) {
            try {
               patternData = Calendar.getPatternData(loc, calType);
            } catch (MissingResourceException var6) {
               patternData = new Calendar.PatternData(Calendar.DEFAULT_PATTERNS, null);
            }
   ...
   public static final class WeekData {
      ...
      @Override
      public int hashCode() {
         return ((((this.firstDayOfWeek * 37 + this.minimalDaysInFirstWeek) * 37 + this.weekendOnset) * 37 + this.weekendOnsetMillis) * 37 + this.weekendCease)
               * 37
            + this.weekendCeaseMillis;
      }
      ...
      public boolean equals(Object other) {
         if (this == other) {
            return true;
         } else if (!(other instanceof Calendar.WeekData)) {
            return false;
         } else {
            Calendar.WeekData that = (Calendar.WeekData)other;
            return this.firstDayOfWeek == that.firstDayOfWeek
               && this.minimalDaysInFirstWeek == that.minimalDaysInFirstWeek
               && this.weekendOnset == that.weekendOnset
               && this.weekendOnsetMillis == that.weekendOnsetMillis
               && this.weekendCease == that.weekendCease
               && this.weekendCeaseMillis == that.weekendCeaseMillis;
         }
      }
   ...
   private static class WeekDataCache extends SoftCache<String, Calendar.WeekData, String> {
      private WeekDataCache() {
      }

      protected Calendar.WeekData createInstance(String key, String data) {
