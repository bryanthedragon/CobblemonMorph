...
import com.cobblemon.mod.relocations.ibm.icu.util.BuddhistCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.ChineseCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.CopticCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.DangiCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.EthiopicCalendar;
...
import com.cobblemon.mod.relocations.ibm.icu.util.IndianCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.IslamicCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.JapaneseCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.PersianCalendar;
import com.cobblemon.mod.relocations.ibm.icu.util.TaiwanCalendar;
...
Comparable<Calendar> {
    ...
    public static final String[] getKeywordValuesForLocale(String key, ULocale locale, boolean commonlyUsed) {
        ...
        UResourceBundle rb = UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", "supplementalData", ICUResourceBundle.ICU_DATA_CLASS_LOADER);
    ...
    public final void set(int field, int value2) {
    ...
    private static int gregoYearFromIslamicStart(int year) {
        ...
        if (year >= 1397) {
            ...
            shift2 = 2 * cycle + (offset >= 33 ? 1 : 0);
        } else {
            int cycle = (year - 1396) / 67 - 1;
            int offset = -(year - 1396) % 67;
            shift2 = 2 * cycle + (offset <= 33 ? 1 : 0);
        }
        return year + 579 - shift2;
    }
    ...
    public void roll(int field, int amount) {
        ...
        switch (field) {
            ...
            case 14: 
            case 21: {
                int min2 = this.getActualMinimum(field);
                int max2 = this.getActualMaximum(field);
                int gap = max2 - min2 + 1;
                int value2 = this.internalGet(field) + amount;
                value2 = (value2 - min2) % gap;
                if (value2 < 0) {
                    value2 += gap;
                }
                this.set(field, value2 += min2);
                return;
            }
            case 10: 
            case 11: {
                long start2 = this.getTimeInMillis();
                int oldHour = this.internalGet(field);
                int max3 = this.getMaximum(field);
                int newHour = (oldHour + amount) % (max3 + 1);
                if (newHour < 0) {
                    newHour += max3 + 1;
                }
                this.setTimeInMillis(start2 + 3600000L * ((long)newHour - (long)oldHour));
                return;
            }
            case 2: {
                int max4 = this.getActualMaximum(2);
                int mon = (this.internalGet(2) + amount) % (max4 + 1);
                if (mon < 0) {
                    mon += max4 + 1;
                }
                this.set(2, mon);
            ...
            case 4: {
                ...
                int start3 = 7 - fdm < this.getMinimalDaysInFirstWeek() ? 8 - fdm : 1 - fdm;
                int monthLen = this.getActualMaximum(5);
                int ldm = (monthLen - this.internalGet(5) + dow) % 7;
                int limit = monthLen + 7 - ldm;
                int gap = limit - start3;
                int day_of_month = (this.internalGet(5) + amount * 7 - start3) % gap;
                if (day_of_month < 0) {
                    day_of_month += gap;
                }
                if ((day_of_month += start3) < 1) {
                    day_of_month = 1;
                }
            ...
            case 3: {
                ...
                int start4 = 7 - fdy < this.getMinimalDaysInFirstWeek() ? 8 - fdy : 1 - fdy;
                int yearLen = this.getActualMaximum(6);
                int ldy = (yearLen - this.internalGet(6) + dow) % 7;
                int limit = yearLen + 7 - ldy;
                int gap = limit - start4;
                int day_of_year = (this.internalGet(6) + amount * 7 - start4) % gap;
                if (day_of_year < 0) {
                    day_of_year += gap;
                }
                if ((day_of_year += start4) < 1) {
                    day_of_year = 1;
                }
    ...
    protected void pinField(int field) {
        int max2 = this.getActualMaximum(field);
        int min2 = this.getActualMinimum(field);
        if (this.fields[field] > max2) {
            this.set(field, max2);
        } else if (this.fields[field] < min2) {
            this.set(field, min2);
        }
    }
        ...
        if (startMs < (targetMs = when.getTime())) {
            ...
            block13: {
                ...
                while (true) {
                    ...
                    if (max2 >= Integer.MAX_VALUE) break;
                    min2 = max2;
                    if ((max2 <<= 1) >= 0) continue;
                    max2 = Integer.MAX_VALUE;
                }
                throw new RuntimeException();
            }
            while (max2 - min2 > 1) {
                int t = min2 + (max2 - min2) / 2;
                this.setTimeInMillis(startMs);
                this.add(field, t);
                long ms = this.getTimeInMillis();
                if (ms == targetMs) {
    ...
    public void setTimeZone(TimeZone value2) {
        this.zone = value2;
        this.areFieldsSet = false;
    }
    ...
    public void setFirstDayOfWeek(int value2) {
        if (this.firstDayOfWeek != value2) {
            if (value2 < 1 || value2 > 7) {
                throw new IllegalArgumentException("Invalid day of week");
            }
            this.firstDayOfWeek = value2;
            this.areFieldsSet = false;
        }
    }
    ...
    public void setMinimalDaysInFirstWeek(int value2) {
        if (value2 < 1) {
            value2 = 1;
        } else if (value2 > 7) {
            value2 = 7;
        }
        if (this.minimalDaysInFirstWeek != value2) {
            this.minimalDaysInFirstWeek = value2;
            this.areFieldsSet = false;
        }
    }
    ...
    protected final void validateField(int field, int min2, int max2) {
        int value2 = this.fields[field];
        if (value2 < min2 || value2 > max2) {
            throw new IllegalArgumentException(this.fieldName(field) + '=' + value2 + ", valid range=" + min2 + ".." + max2);
        }
    }
    ...
    protected final void internalSet(int field, int value2) {
