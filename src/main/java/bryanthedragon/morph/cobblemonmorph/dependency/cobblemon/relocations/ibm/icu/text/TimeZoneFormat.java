...
import com.cobblemon.mod.relocations.ibm.icu.impl.TextTrieMap;
import com.cobblemon.mod.relocations.ibm.icu.impl.TimeZoneGenericNames;
...
public class TimeZoneFormat extends UFormat implements Freezable<TimeZoneFormat>, Serializable {
   ...
   private static final TimeZoneFormat.GMTOffsetPatternType[] PARSE_GMT_OFFSET_TYPES = new TimeZoneFormat.GMTOffsetPatternType[]{
   ...
   private static final EnumSet<TimeZoneNames.NameType> ALL_SIMPLE_NAME_TYPES = EnumSet.of(
   ...
   private static final EnumSet<TimeZoneGenericNames.GenericNameType> ALL_GENERIC_NAME_TYPES = EnumSet.of(
      TimeZoneGenericNames.GenericNameType.LOCATION, TimeZoneGenericNames.GenericNameType.LONG, TimeZoneGenericNames.GenericNameType.SHORT
   );
   ...
   protected TimeZoneFormat(ULocale locale) {
      ...
      try {
         ICUResourceBundle bundle = (ICUResourceBundle)ICUResourceBundle.getBundleInstance(
            "com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b/zone", locale
         );
   ...
   public EnumSet<TimeZoneFormat.ParseOption> getDefaultParseOptions() {
      if (this._parseAllStyles && this._parseTZDBNames) {
         ...
         return this._parseTZDBNames ? EnumSet.of(TimeZoneFormat.ParseOption.TZ_DATABASE_ABBREVIATIONS) : EnumSet.noneOf(TimeZoneFormat.ParseOption.class);
      }
   }
   ...
   public TimeZone parse(
      TimeZoneFormat.Style style, String text, ParsePosition pos, EnumSet<TimeZoneFormat.ParseOption> options, Output<TimeZoneFormat.TimeType> timeType
   ) {
      if (timeType == null) {
      ...
      boolean fallbackLocalizedGMT = style == TimeZoneFormat.Style.SPECIFIC_LONG
         || style == TimeZoneFormat.Style.GENERIC_LONG
      ...
      boolean parseTZDBAbbrev = options == null
         ? this.getDefaultParseOptions().contains(TimeZoneFormat.ParseOption.TZ_DATABASE_ABBREVIATIONS)
         : options.contains(TimeZoneFormat.ParseOption.TZ_DATABASE_ABBREVIATIONS);
      switch (style) {
         case GENERIC_LOCATION:
         case GENERIC_LONG:
         case GENERIC_SHORT:
         ...
         case SPECIFIC_SHORT:
            ...
            if (parseTZDBAbbrev && style == TimeZoneFormat.Style.SPECIFIC_SHORT) {
               ...
               if (tzdbNameMatches != null) {
                  ...
                  for (TimeZoneNames.MatchInfo matchx : tzdbNameMatches) {
                     if (startIdx + matchx.matchLength() > parsedPos) {
                        tzdbNameMatch = matchx;
                        parsedPos = startIdx + matchx.matchLength();
                     }
                  }
         ...
         case ZONE_ID:
            ...
            String idxx = parseZoneID(text, tmpPos);
            if (tmpPos.getErrorIndex() == -1) {
               pos.setIndex(tmpPos.getIndex());
               return TimeZone.getTimeZone(idxx);
            }
            break;
         case ZONE_ID_SHORT:
            tmpPos.setIndex(startIdx);
            tmpPos.setErrorIndex(-1);
            String idx = parseShortZoneID(text, tmpPos);
            if (tmpPos.getErrorIndex() == -1) {
               pos.setIndex(tmpPos.getIndex());
               return TimeZone.getTimeZone(idx);
            }
            break;
         case EXEMPLAR_LOCATION:
            tmpPos.setIndex(startIdx);
         ...
         case LOCALIZED_GMT:
            tmpPos.setIndex(startIdx);
            tmpPos.setErrorIndex(-1);
            int offsetxx = this.parseOffsetLocalizedGMT(text, tmpPos);
            if (tmpPos.getErrorIndex() == -1) {
               pos.setIndex(tmpPos.getIndex());
               return this.getTimeZoneForOffset(offsetxx);
            }
         ...
         case LOCALIZED_GMT_SHORT:
            tmpPos.setIndex(startIdx);
            tmpPos.setErrorIndex(-1);
            int offsetxx = this.parseOffsetShortLocalizedGMT(text, tmpPos);
            if (tmpPos.getErrorIndex() == -1) {
               pos.setIndex(tmpPos.getIndex());
               return this.getTimeZoneForOffset(offsetxx);
            }
         ...
         case ISO_EXTENDED_FULL:
            tmpPos.setIndex(startIdx);
            tmpPos.setErrorIndex(-1);
            int offsetx = this.parseOffsetISO8601(text, tmpPos);
            if (tmpPos.getErrorIndex() == -1) {
               pos.setIndex(tmpPos.getIndex());
               return this.getTimeZoneForOffset(offsetx);
            }
            break;
         case ISO_BASIC_LOCAL_SHORT:
         case ISO_BASIC_LOCAL_FIXED:
      ...
      if (parsedPos <= startIdx) {
         ...
         if (parsedPos < maxPos && ((evaluated & 128) == 0 || (evaluated & 256) == 0)) {
            tmpPos.setIndex(startIdx);
            tmpPos.setErrorIndex(-1);
            Output<Boolean> hasDigitOffsetx = new Output<>(false);
            int offsetx = parseOffsetISO8601(text, tmpPos, false, hasDigitOffsetx);
            if (tmpPos.getErrorIndex() == -1) {
               if (tmpPos.getIndex() == maxPos || hasDigitOffsetx.value) {
                  pos.setIndex(tmpPos.getIndex());
                  return this.getTimeZoneForOffset(offsetx);
               }

               if (parsedPos < tmpPos.getIndex()) {
                  parsedOffset = offsetx;
                  parsedID = null;
         ...
         if (parsedPos < maxPos && (evaluated & TimeZoneFormat.Style.LOCALIZED_GMT.flag) == 0) {
            ...
            tmpPos.setErrorIndex(-1);
            Output<Boolean> hasDigitOffsetx = new Output<>(false);
            int offsetx = this.parseOffsetLocalizedGMT(text, tmpPos, false, hasDigitOffsetx);
            if (tmpPos.getErrorIndex() == -1) {
               if (tmpPos.getIndex() == maxPos || hasDigitOffsetx.value) {
                  pos.setIndex(tmpPos.getIndex());
                  return this.getTimeZoneForOffset(offsetx);
         ...
         if (parsedPos < maxPos && (evaluated & TimeZoneFormat.Style.LOCALIZED_GMT_SHORT.flag) == 0) {
            ...
            tmpPos.setErrorIndex(-1);
            Output<Boolean> hasDigitOffsetx = new Output<>(false);
            int offsetx = this.parseOffsetLocalizedGMT(text, tmpPos, true, hasDigitOffsetx);
            if (tmpPos.getErrorIndex() == -1) {
               if (tmpPos.getIndex() == maxPos || hasDigitOffsetx.value) {
                  pos.setIndex(tmpPos.getIndex());
                  return this.getTimeZoneForOffset(offsetx);
         ...
         boolean parseAllStyles = options == null
            ? this.getDefaultParseOptions().contains(TimeZoneFormat.ParseOption.ALL_STYLES)
            : options.contains(TimeZoneFormat.ParseOption.ALL_STYLES);
         if (parseAllStyles) {
            if (parsedPos < maxPos) {
               Collection<TimeZoneNames.MatchInfo> specificMatchesx = this._tznames.find(text, startIdx, ALL_SIMPLE_NAME_TYPES);
               TimeZoneNames.MatchInfo specificMatch = null;
               int matchPos = -1;
               if (specificMatchesx != null) {
                  for (TimeZoneNames.MatchInfo matchxx : specificMatchesx) {
                     if (startIdx + matchxx.matchLength() > matchPos) {
                        specificMatch = matchxx;
                        matchPos = startIdx + matchxx.matchLength();
                     }
                  }
               }
            ...
            if (parseTZDBAbbrev && parsedPos < maxPos && (evaluated & TimeZoneFormat.Style.SPECIFIC_SHORT.flag) == 0) {
               ...
               int matchPosx = -1;
               if (tzdbNameMatches != null) {
                  for (TimeZoneNames.MatchInfo matchxxx : tzdbNameMatches) {
                     if (startIdx + matchxxx.matchLength() > matchPosx) {
                        tzdbNameMatch = matchxxx;
                        matchPosx = startIdx + matchxxx.matchLength();
                     }
                  }

                  if (parsedPos < matchPosx) {
                     parsedPos = matchPosx;
            ...
            if (parsedPos < maxPos && (evaluated & TimeZoneFormat.Style.ZONE_ID.flag) == 0) {
               tmpPos.setIndex(startIdx);
               tmpPos.setErrorIndex(-1);
               String idxxx = parseZoneID(text, tmpPos);
            ...
            if (parsedPos < maxPos && (evaluated & TimeZoneFormat.Style.ZONE_ID_SHORT.flag) == 0) {
               tmpPos.setIndex(startIdx);
               tmpPos.setErrorIndex(-1);
               String idxxx = parseShortZoneID(text, tmpPos);
               if (tmpPos.getErrorIndex() == -1 && parsedPos < tmpPos.getIndex()) {
                  parsedPos = tmpPos.getIndex();
                  parsedID = idxxx;
                  parsedTimeType = TimeZoneFormat.TimeType.UNKNOWN;
   ...
   public final TimeZone parse(String text, ParsePosition pos) {
      return this.parse(TimeZoneFormat.Style.GENERIC_LOCATION, text, pos, EnumSet.of(TimeZoneFormat.ParseOption.ALL_STYLES), null);
   }
   ...
   private String formatOffsetISO8601(int offset, boolean isBasic, boolean useUtcIndicator, boolean isShort, boolean ignoreSeconds) {
      ...
      if (!useUtcIndicator || absOffset >= 1000 && (!ignoreSeconds || absOffset >= 60000)) {
         ...
         if (absOffset >= 86400000) {
            ...
            for (int idxx = 0; idxx <= lastIdx; idxx++) {
               if (sep != null && idxx != 0) {
                  buf.append(sep);
               }
   ...
   private String formatSpecific(
      TimeZone tz, TimeZoneNames.NameType stdType, TimeZoneNames.NameType dstType, long date, Output<TimeZoneFormat.TimeType> timeType
   ) {
      ...
      String name = isDaylight
         ? this.getTimeZoneNames().getDisplayName(ZoneMeta.getCanonicalCLDRID(tz), dstType, date)
         : this.getTimeZoneNames().getDisplayName(ZoneMeta.getCanonicalCLDRID(tz), stdType, date);
   ...
   private int parseOffsetFields(String text, int start, boolean isShort, int[] parsedLen) {
      ...
      if (outLen > 0 && this._abuttingOffsetHoursAndMinutes) {
         ...
         for (TimeZoneFormat.GMTOffsetPatternType gmtPatType : PARSE_GMT_OFFSET_TYPES) {
            Object[] itemsx = this._gmtOffsetPatternItems[gmtPatType.ordinal()];

            assert itemsx != null;

            tmpLen = this.parseOffsetFieldsWithPattern(text, start, itemsx, true, fields);
            if (tmpLen > 0) {
   ...
   private int parseAbuttingOffsetFields(String text, int start, int[] parsedLen) {
      ...
      if (numDigits == 0) {
         ...
         int offset;
         for (offset = 0; numDigits > 0; numDigits--) {
            int hour = 0;
            int min = 0;
            int sec = 0;
   ...
   private static int parseAbuttingAsciiOffsetFields(
      String text, ParsePosition pos, TimeZoneFormat.OffsetFields minFields, TimeZoneFormat.OffsetFields maxFields, boolean fixedHourWidth
   ) {
      int start = pos.getIndex();
   ...
   private static int parseAsciiOffsetFields(
      String text, ParsePosition pos, char sep, TimeZoneFormat.OffsetFields minFields, TimeZoneFormat.OffsetFields maxFields
   ) {
      int start = pos.getIndex();
            ...
            if (this._gmtPattern == null) {
               ...
               if (tmpGmtOffsetPatterns == null) {
                  ...
                  if (tmpGmtOffsetPatterns.length == 4) {
                     ...
                     this._gmtOffsetPatterns[TimeZoneFormat.GMTOffsetPatternType.POSITIVE_H.ordinal()] = truncateOffsetPattern(
                        this._gmtOffsetPatterns[TimeZoneFormat.GMTOffsetPatternType.POSITIVE_HM.ordinal()]
                     );
                     this._gmtOffsetPatterns[TimeZoneFormat.GMTOffsetPatternType.NEGATIVE_H.ordinal()] = truncateOffsetPattern(
                        this._gmtOffsetPatterns[TimeZoneFormat.GMTOffsetPatternType.NEGATIVE_HM.ordinal()]
                     );
                  } else {
