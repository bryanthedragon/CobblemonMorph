...
Serializable {
    ...
    private int parseOffsetLocalizedGMT(String text, ParsePosition pos, boolean isShort, Output<Boolean> hasDigitOffset) {
        ...
        offset = this.parseOffsetLocalizedGMTPattern(text, start2, isShort, parsedLength);
        if (parsedLength[0] > 0) {
            if (hasDigitOffset != null) {
                hasDigitOffset.value = true;
            }
            pos.setIndex(start2 + parsedLength[0]);
            return offset;
        }
        offset = this.parseOffsetDefaultLocalizedGMT(text, start2, parsedLength);
        if (parsedLength[0] > 0) {
            if (hasDigitOffset != null) {
                hasDigitOffset.value = true;
            }
        ...
        if (text.regionMatches(true, start2, this._gmtZeroFormat, 0, this._gmtZeroFormat.length())) {
            pos.setIndex(start2 + this._gmtZeroFormat.length());
            return 0;
        }
        for (String defGMTZero : ALT_GMT_STRINGS) {
            if (!text.regionMatches(true, start2, defGMTZero, 0, defGMTZero.length())) continue;
            pos.setIndex(start2 + defGMTZero.length());
            return 0;
        }
        pos.setErrorIndex(start2);
        return 0;
    }

    private int parseOffsetLocalizedGMTPattern(String text, int start2, boolean isShort, int[] parsedLen) {
        int idx = start2;
        int offset = 0;
        ...
        parsedLen[0] = parsed ? idx - start2 : 0;
        return offset;
    }

    private int parseOffsetFields(String text, int start2, boolean isShort, int[] parsedLen) {
        int outLen = 0;
        int offset = 0;
        int sign = 1;
        ...
        for (GMTOffsetPatternType gmtPatType : PARSE_GMT_OFFSET_TYPES) {
            ...
            assert (items != null);
            outLen = this.parseOffsetFieldsWithPattern(text, start2, items, false, fields);
        ...
        if (outLen > 0 && this._abuttingOffsetHoursAndMinutes) {
            ...
            for (GMTOffsetPatternType gmtPatType : PARSE_GMT_OFFSET_TYPES) {
                Object[] items = this._gmtOffsetPatternItems[gmtPatType.ordinal()];
                assert (items != null);
                tmpLen = this.parseOffsetFieldsWithPattern(text, start2, items, true, fields);
    ...
    private int parseOffsetFieldsWithPattern(String text, int start2, Object[] patternItems, boolean forceSingleHourDigit, int[] fields) {
    ...
    private int parseOffsetDefaultLocalizedGMT(String text, int start2, int[] parsedLen) {
        int parsed;
        int offset;
        block8: {
            int sign;
    ...
    private int parseDefaultOffsetFields(String text, int start2, char separator, int[] parsedLen) {
        ...
        if (len[0] != 0 && (idx += len[0]) + 1 < max2 && text.charAt(idx) == separator) {
            min2 = this.parseOffsetFieldWithLocalizedDigits(text, idx + 1, 2, 2, 0, 59, len);
            if (len[0] != 0 && (idx += 1 + len[0]) + 1 < max2 && text.charAt(idx) == separator) {
        ...
        parsedLen[0] = idx - start2;
        return hour * 3600000 + min2 * 60000 + sec * 1000;
    }

    private int parseAbuttingOffsetFields(String text, int start2, int[] parsedLen) {
        int MAXDIGITS = 6;
        ...
        while (numDigits > 0) {
            ...
            switch (numDigits) {
                ...
                case 3: {
                    hour = digits[0];
                    min2 = digits[1] * 10 + digits[2];
                    break;
                }
                case 4: {
                    hour = digits[0] * 10 + digits[1];
                    min2 = digits[2] * 10 + digits[3];
                    break;
                }
                case 5: {
                    hour = digits[0];
            ...
            if (hour <= 23 && min2 <= 59 && sec <= 59) {
                offset = hour * 3600000 + min2 * 60000 + sec * 1000;
    ...
    private int parseOffsetFieldWithLocalizedDigits(String text, int start2, int minDigits, int maxDigits, int minVal, int maxVal, int[] parsedLen) {
        int tmpVal;
        int digit;
        int idx;
        parsedLen[0] = 0;
        ...
        for (idx = start2; idx < text.length() && numDigits < maxDigits && (digit = this.parseSingleLocalizedDigit(text, idx, digitLen)) >= 0 && (tmpVal = decVal * 10 + digit) <= maxVal; ++numDigits, idx += digitLen[0]) {
            decVal = tmpVal;
        }
    ...
    private int parseSingleLocalizedDigit(String text, int start2, int[] len) {
        int digit = -1;
        len[0] = 0;
        if (start2 < text.length()) {
            int cp = Character.codePointAt(text, start2);
    ...
    private static int parseOffsetISO8601(String text, ParsePosition pos, boolean extendedOnly, Output<Boolean> hasDigitOffset) {
        ...
        if ((start2 = pos.getIndex()) >= text.length()) {
            pos.setErrorIndex(start2);
            return 0;
        }
        char firstChar = text.charAt(start2);
        if (Character.toUpperCase(firstChar) == ISO8601_UTC.charAt(0)) {
            pos.setIndex(start2 + 1);
            return 0;
        }
        if (firstChar == '+') {
            sign = 1;
        ...
        ParsePosition posOffset = new ParsePosition(start2 + 1);
        int offset = TimeZoneFormat.parseAsciiOffsetFields(text, posOffset, ':', OffsetFields.H, OffsetFields.HMS);
        if (posOffset.getErrorIndex() == -1 && !extendedOnly && posOffset.getIndex() - start2 <= 3) {
            ParsePosition posBasic = new ParsePosition(start2 + 1);
    ...
    private static int parseAbuttingAsciiOffsetFields(String text, ParsePosition pos, OffsetFields minFields, OffsetFields maxFields, boolean fixedHourWidth) {
        ...
        int numDigits = 0;
        for (int idx = start2; numDigits < digits.length && idx < text.length() && (digit = ASCII_DIGITS.indexOf(text.charAt(idx))) >= 0; ++numDigits, ++idx) {
        ...
        pos.setIndex(start2 + numDigits);
        return ((hour * 60 + min2) * 60 + sec) * 1000;
    }
    ...
    private static int parseAsciiOffsetFields(String text, ParsePosition pos, char sep, OffsetFields minFields, OffsetFields maxFields) {
        ...
        int fieldIdx = 0;
        for (int idx = start2; idx < text.length() && fieldIdx <= maxFields.ordinal(); ++idx) {
            int digit;
    ...
    private static String parseZoneID(String text, ParsePosition pos) {
        ...
        if (ZONE_ID_TRIE == null) {
            ...
            // MONITORENTER : com.cobblemon.mod.relocations.ibm.icu.text.TimeZoneFormat.class
            if (ZONE_ID_TRIE == null) {
                String[] ids;
