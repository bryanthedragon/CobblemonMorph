...
public final class JavaScriptTranslator extends GraalJSTranslator {
   ...
   public static ScriptNode translateScript(
      NodeFactory factory, JSContext context, Source source, boolean isParentStrict, String prologue, String epilogue, List<String> argumentNames
   ) {
   ...
   private static ScriptNode translateScript(
      ...
      FunctionNode parserFunctionNode = GraalJSParserHelper.parseScript(
         context, source, context.getParserOptions().putStrict(isParentStrict), isEval, evalInFunction, parentScope, prologue, epilogue, argumentNames
      );
   ...
   private static Source applyExplicitSourceURL(Source source, FunctionNode parserFunctionNode) {
      ...
      if (explicitURL != null) {
         boolean internal = source.isInternal() || explicitURL.startsWith("internal:");
   ...
   public static ScriptNode translateFunction(
      NodeFactory factory, JSContext context, Environment env, Source source, boolean isParentStrict, FunctionNode rootNode
   ) {
      return translateFunction(factory, context, env, source, 0, isParentStrict, rootNode);
   }

   public static ScriptNode translateFunction(
      NodeFactory factory, JSContext context, Environment env, Source source, int prologLength, boolean isParentStrict, FunctionNode rootNode
   ) {
