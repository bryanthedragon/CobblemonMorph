...
abstract class RootBlock(settings: Properties) : Block(settings), BonemealableBlock, ShearableBlock {
    ...
    override fun isRandomlyTicking(BlockState state) = true
    ...
    fun hasReachedSpreadCap(Level world, (BlockPos pos): Boolean {
        var nearby = 0
    ...
    override fun canSurvive(BlockState state, Level worldReader, (BlockPos pos): Boolean = this.canGoOn(state, world, pos) { true }
    ...
    override fun isValidBonemealTarget(Level worldReader, (BlockPos pos, BlockState state) = this.canSpread(world, pos, state)

    override fun isBonemealSuccess(Level world, random: RandomSource, (BlockPos pos, BlockState state) = this.canSpread(world, pos, state)
    ...
    fun spreadFrom(ServerLevel world, (BlockPos pos, random: RandomSource) {
     ...
     */
    protected fun canSpread(Level worldReader, (BlockPos pos, BlockState state): Boolean = this.possibleDirections.any { direction ->
     ...
     */
    protected fun canSpreadTo(BlockState state, Level worldReader, (BlockPos pos): Boolean {
     ...
     */
    protected fun canGoOn(BlockState state, Level worldReader, (BlockPos pos, ceilingValidator: (ceiling: BlockState) -> Boolean): Boolean {
        val up = pos.above()
