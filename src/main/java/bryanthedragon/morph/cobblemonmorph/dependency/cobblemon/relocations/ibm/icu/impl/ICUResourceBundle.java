...
import com.cobblemon.mod.relocations.ibm.icu.impl.ICUResourceBundleImpl;
import com.cobblemon.mod.relocations.ibm.icu.impl.ICUResourceBundleReader;
import com.cobblemon.mod.relocations.ibm.icu.impl.SoftCache;
import com.cobblemon.mod.relocations.ibm.icu.impl.URLHandler;
...
extends UResourceBundle {
    ...
    public void getAllChildrenWithFallback(String path, final UResource.Sink sink) throws MissingResourceException {
        ...
        extends UResource.Sink {
            ...
            public void put(UResource.Key key, UResource.Value value2, boolean noFallback) {
                UResource.Table itemsTable = value2.getTable();
                int i = 0;
                while (itemsTable.getKeyAndValue(i, key, value2)) {
                    if (value2.getType() == 3) {
                        String aliasPath = value2.getAliasString();
                        ...
                        sink.put(key, value2, noFallback);
                    }
                    ++i;
                }
            }
    ...
    public static Set<String> getFullLocaleNameSet() {
        return ICUResourceBundle.getFullLocaleNameSet("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ICU_DATA_CLASS_LOADER);
    }
    ...
    public static Set<String> getAvailableLocaleNameSet() {
        return ICUResourceBundle.getAvailableLocaleNameSet("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ICU_DATA_CLASS_LOADER);
    }
    ...
    public static final ULocale[] getAvailableULocales() {
        return ICUResourceBundle.getAvailableULocales("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ICU_DATA_CLASS_LOADER, ULocale.AvailableType.DEFAULT);
    }

    public static final ULocale[] getAvailableULocales(ULocale.AvailableType type) {
        return ICUResourceBundle.getAvailableULocales("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ICU_DATA_CLASS_LOADER, type);
    }
    ...
    public static final Locale[] getAvailableLocales() {
        return ICUResourceBundle.getAvailableLocales("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ICU_DATA_CLASS_LOADER, ULocale.AvailableType.DEFAULT);
    }

    public static final Locale[] getAvailableLocales(ULocale.AvailableType type) {
        return ICUResourceBundle.getAvailableLocales("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ICU_DATA_CLASS_LOADER, type);
    }
    ...
    private static Set<String> createFullLocaleNameSet(String baseName, ClassLoader loader) {
        ...
        if (!skipScan.equalsIgnoreCase("true")) {
            String folder;
            ICUResourceBundle.addBundleBaseNamesFromClassLoader(bn, loader, set2);
            if (baseName.startsWith("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b") && (folder = baseName.length() == "com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b".length() ? "" : (baseName.charAt("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b".length()) == '/' ? baseName.substring("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b".length() + 1) : null)) != null) {
                ICUBinary.addBaseNamesInFileFolder(folder, ".res", set2);
            }
            set2.remove(ICU_RESOURCE_INDEX);
        ...
        if (set2.isEmpty()) {
            ...
            ICUResourceBundle.addLocaleIDsFromListFile(bn, loader, set2);
        }
        if (set2.isEmpty()) {
            ICUResourceBundle.addLocaleIDsFromIndexBundle(baseName, loader, set2);
        }
        set2.remove("root");
        set2.add(ULocale.ROOT.toString());
...
lbl32:
    ...
    private static void getResPathKeys(String path, int num, String[] keys, int start2) {
        int j;
        if (num == 0) {
            return;
        }
        ...
        while (true) {
            j = path.indexOf(47, i);
            assert (j >= i);
            keys[start2++] = path.substring(i, j);
            if (num == 2) {
    ...
    extends UResource.Sink {
        ...
        @Override
        public void put(UResource.Key key, UResource.Value value2, boolean noFallback) {
            UResource.Table resIndexTable = value2.getTable();
            int i = 0;
            while (resIndexTable.getKeyAndValue(i, key, value2)) {
                block6: {
                    ...
                    UResource.Table availableLocalesTable = value2.getTable();
                    ULocale[] locales = new ULocale[availableLocalesTable.getSize()];
                    int j = 0;
                    while (availableLocalesTable.getKeyAndValue(j, key, value2)) {
