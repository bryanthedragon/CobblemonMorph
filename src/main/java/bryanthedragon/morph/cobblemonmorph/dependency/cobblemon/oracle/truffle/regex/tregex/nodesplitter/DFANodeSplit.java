...
import com.oracle.truffle.regex.tregex.nodesplitter.DominatorTree;
...
implements StateIndex<GraphNode> {
    ...
    private boolean splitLoops(GraphNode topNode, Set<GraphNode> set2) throws DFANodeSplitBailoutException {
        boolean crossEdge = false;
        for (GraphNode child : topNode.getDomChildren(this)) {
            if (!set2.isEmpty() && !set2.contains(child) || !this.splitLoops(child, set2)) continue;
            crossEdge = true;
        }
        if (crossEdge) {
            this.handleIrChildren(topNode, set2);
        }
    ...
    private void handleIrChildren(GraphNode topNode, Set<GraphNode> set2) throws DFANodeSplitBailoutException {
        ...
        for (GraphNode graphNode : topNode.getDomChildren(this)) {
            if (this.isDone(graphNode) || !set2.isEmpty() && !set2.contains(graphNode)) continue;
            this.scc1(dfsList, graphNode, set2, topNode.getDomTreeDepth());
        }
        ...
        for (Set set3 : sccList) {
            if (set3.size() <= 1) continue;
            this.handleScc(topNode, set3);
        }
    }

    private void scc1(ArrayDeque<GraphNode> dfsList, GraphNode curNode, Set<GraphNode> set2, int level) {
        this.setDone(curNode);
        for (GraphNode child : curNode.getSuccessors(this)) {
            if (this.isDone(child) || child.getDomTreeDepth() <= level || !set2.isEmpty() && !set2.contains(child)) continue;
            this.scc1(dfsList, child, set2, level);
        }
    ...
    private void handleScc(GraphNode topNode, Set<GraphNode> scc) throws DFANodeSplitBailoutException {
        ...
        for (GraphNode n : scc) {
            if (n.getDomTreeDepth() != topNode.getDomTreeDepth() + 1) continue;
    ...
    private void splitSCC(GraphNode headerNode, Set<GraphNode> scc) throws DFANodeSplitBailoutException {
        for (GraphNode n : scc) {
            if (n.getHeader() == headerNode) continue;
            if (this.nextId == 4000) {
        ...
        for (GraphNode g : new ArrayList<GraphNode>(scc)) {
            if (g.getHeader() == headerNode) continue;
            scc.add(g.getCopy());
            g.clearCopy();
        }
    }
    ...
    private static GraphNode chooseNode(Set<GraphNode> msed) {
        ...
        for (GraphNode n : msed) {
            if (n.getWeight() <= maxWeight) continue;
