...
package bryanthedragon.morph.cobblemonmorph.dependency.cobblemon.relocations.ibm.icu.lang;
...
import com.cobblemon.mod.relocations.ibm.icu.lang.UCharacterEnums;
import com.cobblemon.mod.relocations.ibm.icu.lang.UCharacterNameIterator;
...
UCharacterEnums.ECharacterDirection {
    ...
    public static int digit(int ch, int radix) {
        if (2 <= radix && radix <= 36) {
            int value2 = UCharacter.digit(ch);
            if (value2 < 0) {
                value2 = UCharacterProperty.getEuropeanDigit(ch);
            }
            return value2 < radix ? value2 : -1;
        }
        return -1;
    }
    ...
    public static String getPropertyValueName(int property, int value2, int nameChoice) {
        if ((property == 4098 || property == 4112 || property == 4113) && value2 >= UCharacter.getIntPropertyMinValue(4098) && value2 <= UCharacter.getIntPropertyMaxValue(4098) && nameChoice >= 0 && nameChoice < 2) {
            try {
                return UPropertyAliases.INSTANCE.getPropertyValueName(property, value2, nameChoice);
            }
    ...
    public static int codePointCount(CharSequence text, int start2, int limit) {
        if (start2 < 0 || limit < start2 || limit > text.length()) {
            throw new IndexOutOfBoundsException("start (" + start2 + ") or limit (" + limit + ") invalid or out of range 0, " + text.length());
        }
        int len = limit - start2;
        block0: while (limit > start2) {
            char ch = text.charAt(--limit);
            while (ch >= '\udc00' && ch <= '\udfff' && limit > start2) {
    ...
    public static int codePointCount(char[] text, int start2, int limit) {
        if (start2 < 0 || limit < start2 || limit > text.length) {
            throw new IndexOutOfBoundsException("start (" + start2 + ") or limit (" + limit + ") invalid or out of range 0, " + text.length);
        }
        int len = limit - start2;
    ...
    public static int offsetByCodePoints(char[] text, int start2, int count, int index, int codePointOffset) {
        int limit = start2 + count;
        if (start2 < 0 || limit < start2 || limit > text.length || index < start2 || index > limit) {
            throw new IndexOutOfBoundsException("index ( " + index + ") out of range " + start2 + ", " + limit + " in array 0, " + text.length);
        }
        if (codePointOffset < 0) {
            while (++codePointOffset <= 0) {
                char ch = text[--index];
                if (index < start2) {
                    throw new IndexOutOfBoundsException("index ( " + index + ") < start (" + start2 + ")");
                }
                while (ch >= '\udc00' && ch <= '\udfff' && index > start2) {
    ...
    implements ValueIterator {
        ...
        @Override
        public void setRange(int start2, int limit) {
        }
    }
    ...
    extends Character.Subset {
        ...
        private static final UnicodeBlock[] BLOCKS_ = new UnicodeBlock[321];
