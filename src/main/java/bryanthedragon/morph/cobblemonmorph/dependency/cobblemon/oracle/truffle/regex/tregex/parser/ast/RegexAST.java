...
JsonConvertible {
    ...
    public void createNFAHelperNodes(RegexASTSubtreeRootNode rootNode) {
        ...
        MatchFound end2 = new MatchFound();
        rootNode.setMatchFound(end2);
    ...
    public GroupBoundaries createGroupBoundaries(TBitSet updateIndices, TBitSet clearIndices, int lastGroup) {
        ...
        if (this.groupBoundariesDeduplicationMap.containsKey(lookup)) {
            return (GroupBoundaries)this.groupBoundariesDeduplicationMap.get(lookup);
        }
    ...
    public List<SourceSection> getSourceSections(RegexASTNode node) {
        return this.getOptions().isDumpAutomataWithSourceSections() ? (List)this.sourceSections.get(node) : null;
    }
    ...
    private List<SourceSection> getOrCreateSourceSections(RegexASTNode node) {
        ArrayList sections = (ArrayList)this.sourceSections.get(node);
        if (sections == null) {
    ...
    public boolean canTransformToDFA() {
        ...
        return this.getNumberOfNodes() <= 4000 && this.getNumberOfCaptureGroups() <= 127 && !this.getProperties().hasBackReferences() && !this.getProperties().hasLargeCountedRepetitions() && !this.getProperties().hasNegativeLookAheadAssertions() && !this.getProperties().hasNonLiteralLookBehindAssertions() && !this.getProperties().hasNegativeLookBehindAssertions() && !this.getRoot().hasQuantifiers() && !this.getProperties().hasAtomicGroups() && couldCalculateLastGroup;
    }
