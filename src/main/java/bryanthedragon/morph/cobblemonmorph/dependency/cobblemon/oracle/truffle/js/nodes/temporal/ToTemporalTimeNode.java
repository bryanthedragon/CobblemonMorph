...
public abstract class ToTemporalTimeNode extends JavaScriptBaseNode {
   ...
   public abstract JSDynamicObject executeDynamicObject(Object value, TemporalUtil.Overflow overflowParam);

   @Specialization
   protected JSDynamicObject toTemporalTime(
      Object item,
      TemporalUtil.Overflow overflowParam,
      @Cached("create()") IsObjectNode isObjectNode,
      @Cached("create()") JSToStringNode toStringNode,
      @Cached("create(ctx)") GetTemporalCalendarWithISODefaultNode getTemporalCalendarNode
   ) {
      TemporalUtil.Overflow overflow = overflowParam == null ? TemporalUtil.Overflow.CONSTRAIN : overflowParam;
      ...
      if (this.isObjectProfile.profile(isObjectNode.executeBoolean(item))) {
         ...
         if (this.isPlainDateTimeProfile.profile(JSTemporalPlainDateTime.isJSTemporalPlainDateTime(itemObj))) {
            ...
            return JSTemporalPlainTime.create(
               this.ctx, dt.getHour(), dt.getMinute(), dt.getSecond(), dt.getMillisecond(), dt.getMicrosecond(), dt.getNanosecond(), this.errorBranch
            );
         }
      ...
      return JSTemporalPlainTime.create(
         this.ctx,
         TemporalUtil.dtoi(result2.getHours()),
         TemporalUtil.dtoi(result2.getMinutes()),
         TemporalUtil.dtoi(result2.getSeconds()),
         TemporalUtil.dtoi(result2.getMilliseconds()),
         TemporalUtil.dtoi(result2.getMicroseconds()),
         TemporalUtil.dtoi(result2.getNanoseconds()),
         this.errorBranch
      );
   }
}
