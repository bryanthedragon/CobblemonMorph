      ...
      if (position == this.line.length()) {
         ...
         while (position < this.line.length()) {
            ...
            if (c != '\\') {
               ...
               if (cpAndLength < 0) {
                  throw new RuntimeException("Invalid escape at " + this.reader.describePosition() + 58 + position);
               }
         ...
         if (quote != 0) {
            throw new RuntimeException("Unterminated quote at " + this.reader.describePosition() + 58 + startpos);
         } else {
            return position;
         }
      }
