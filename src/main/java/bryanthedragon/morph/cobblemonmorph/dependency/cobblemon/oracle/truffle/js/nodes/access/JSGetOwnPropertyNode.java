...
public abstract class JSGetOwnPropertyNode extends JavaScriptBaseNode {
   ...
   @Node.Child
   private JSGetOwnPropertyNode.GetPropertyProxyValueNode getPropertyProxyValueNode;
   ...
   protected PropertyDescriptor getOwnPropertyString(JSDynamicObject thisObj, Object key, @Cached("createBinaryProfile()") ConditionProfile stringCaseProfile) {
      assert JSRuntime.isPropertyKey(key);

      Property prop = thisObj.getShape().getProperty(key);
      PropertyDescriptor desc = this.ordinaryGetOwnProperty(thisObj, prop);
      return stringCaseProfile.profile(desc == null) ? JSString.stringGetIndexProperty(thisObj, key) : desc;
   }

   @Specialization(
      guards = {"allowCaching", "cachedJSClass != null", "propertyKeyEquals(equalsNode, cachedPropertyKey, propertyKey)", "cachedShape == thisObj.getShape()"},
      assumptions = "cachedShape.getValidAssumption()",
      limit = "3"
   )
   PropertyDescriptor cachedOrdinary(
      JSDynamicObject thisObj,
      Object propertyKey,
   ...
   PropertyDescriptor uncachedOrdinary(
      JSDynamicObject thisObj,
      Object propertyKey,
      @Cached @Cached.Shared("usesOrdinaryGetOwnProperty") JSGetOwnPropertyNode.UsesOrdinaryGetOwnPropertyNode usesOrdinaryGetOwnProperty
   ) {
      assert JSRuntime.isPropertyKey(propertyKey) && JSObject.getJSClass(thisObj).usesOrdinaryGetOwnProperty();
   ...
   static JSClass getJSClassIfOrdinary(JSDynamicObject obj) {
      ...
      return jsclass.usesOrdinaryGetOwnProperty() ? jsclass : null;
   }
   ...
   private Object getDataPropertyValue(JSDynamicObject thisObj, Property prop) {
      ...
      return JSProperty.isProxy(prop) ? this.getPropertyProxyValue(thisObj, value) : value;
   }
   ...
   static PropertyDescriptor generic(
      JSDynamicObject thisObj,
      Object key,
   ...
   public abstract static class GetPropertyProxyValueNode extends JavaScriptBaseNode {
      ...
      static Object doCached(JSDynamicObject obj, Object propertyProxy, @Cached("propertyProxy.getClass()") Class<?> cachedClass) {
   ...
   public abstract static class UsesOrdinaryGetOwnPropertyNode extends JavaScriptBaseNode {
      ...
      static boolean doCached(Object jsclass, @Cached("asJSClass(jsclass)") JSClass cachedJSClass) {
