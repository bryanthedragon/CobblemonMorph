...
public final class CaseMapImpl {
    ...
    private static final void appendUnchanged(CharSequence src, int start2, int length, Appendable dest, int options, Edits edits) throws IOException {
        if (length > 0) {
            if (edits != null) {
            ...
            dest.append(src, start2, start2 + length);
        }
    }
    ...
    private static <A extends Appendable> int maybeTitleDutchIJ(CharSequence src, int c, int start2, int segmentLimit, A dest, int options, Edits edits) throws IOException {
        int cp;
        int bit;
        assert (start2 < segmentLimit);
        int index = start2;
        ...
        CaseMapImpl.appendUnchanged(src, start2, unchanged1, dest, options, edits);
        start2 += unchanged1;
        if (doTitleJ) {
            dest.append('J');
            if (edits != null) {
        ...
        CaseMapImpl.appendUnchanged(src, start2, unchanged2, dest, options, edits);
        assert (start2 + unchanged2 == index);
        return index;
    }
