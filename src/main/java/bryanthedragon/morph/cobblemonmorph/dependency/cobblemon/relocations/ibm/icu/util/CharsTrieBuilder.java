...
extends StringTrieBuilder {
    ...
    public CharsTrieBuilder add(CharSequence s, int value2) {
        this.addImpl(s, value2);
        return this;
    }
    ...
    @Override
    @Deprecated
    protected int writeValueAndType(boolean hasValue, int value2, int node) {
        int length;
        if (!hasValue) {
            return this.write(node);
        }
        if (value2 < 0 || value2 > 0xFDFFFF) {
            this.intUnits[0] = 32704;
            this.intUnits[1] = (char)(value2 >> 16);
            this.intUnits[2] = (char)value2;
            length = 3;
        } else if (value2 <= 255) {
            this.intUnits[0] = (char)(value2 + 1 << 6);
            length = 1;
        } else {
            this.intUnits[0] = (char)(16448 + (value2 >> 10 & 0x7FC0));
            this.intUnits[1] = (char)value2;
            length = 2;
        }
