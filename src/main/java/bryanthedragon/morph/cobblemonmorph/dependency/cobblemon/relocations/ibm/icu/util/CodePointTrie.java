...
extends CodePointMap {
    ...
    private static final int maybeFilterValue(int value2, int trieNullValue, int nullValue, CodePointMap.ValueFilter filter) {
        if (value2 == trieNullValue) {
            value2 = nullValue;
        } else if (filter != null) {
    ...
    @Override
    public final boolean getRange(int start2, CodePointMap.ValueFilter filter, CodePointMap.Range range) {
        if (start2 < 0 || 0x10FFFF < start2) {
            return false;
        }
        if (start2 >= this.highStart) {
            int di = this.dataLength - 2;
            int value2 = this.data.getFromIndex(di);
            if (filter != null) {
                value2 = filter.apply(value2);
            }
        ...
        do {
            ...
            if (c <= 65535 && (type == Type.FAST || c <= 4095)) {
                ...
                if (i3Block == prevI3Block && c - start2 >= 512) {
                    assert ((c & 0x1FF) == 0);
                    c += 512;
                    continue;
                }
            ...
            do {
                ...
                if (block == prevBlock && c - start2 >= dataBlockLength) {
                ...
                if (haveValue) {
                    if (trieValue2 != trieValue) {
                        if (filter == null || CodePointTrie.maybeFilterValue(trieValue2, this.nullValue, nullValue, filter) != value3) {
                            range.set(start2, c - 1, value3);
                            return true;
                        }
                    ...
                    trieValue = trieValue2;
                    value3 = CodePointTrie.maybeFilterValue(trieValue2, this.nullValue, nullValue, filter);
                    haveValue = true;
                }
                while ((++c & dataMask) != 0) {
        ...
        c = CodePointTrie.maybeFilterValue(highValue, this.nullValue, nullValue, filter) != value3 ? --c : 0x10FFFF;
        range.set(start2, c, value3);
        return true;
    }
