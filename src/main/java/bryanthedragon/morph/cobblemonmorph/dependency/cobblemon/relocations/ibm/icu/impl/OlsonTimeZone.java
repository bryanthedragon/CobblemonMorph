...
extends BasicTimeZone {
        ...
        if ((double)current < this.finalStartMillis) {
            ...
            if (bDst) {
                ...
                if (currentRules.length == 3 && currentRules[1] instanceof AnnualTimeZoneRule && currentRules[2] instanceof AnnualTimeZoneRule) {
                    ...
                    stz.setStartRule(start2.getRuleMonth(), start2.getRuleWeekInMonth(), start2.getRuleDayOfWeek(), start2.getRuleMillisInDay());
                    stz.setEndRule(end2.getRuleMonth(), end2.getRuleWeekInMonth(), end2.getRuleDayOfWeek(), end2.getRuleMillisInDay());
    ...
    public boolean useDaylightTime() {
        ...
        int[] fields = Grego.timeToFields(current, null);
        long start2 = Grego.fieldsToDay(fields[0], 0, 1) * 86400L;
        long limit = Grego.fieldsToDay(fields[0] + 1, 0, 1) * 86400L;
        for (int i = 0; i < this.transitionCount && this.transitionTimes64[i] < limit; ++i) {
            if ((this.transitionTimes64[i] < start2 || this.dstOffsetAt(i) == 0) && (this.transitionTimes64[i] <= start2 || i <= 0 || this.dstOffsetAt(i - 1) == 0)) continue;
            return true;
        }
        return false;
    }
    ...
    private synchronized void initTransitionRules() {
        ...
        if (this.transitionCount > 0) {
            ...
            if (transitionIdx != this.transitionCount) {
                int typeIdx;
                long[] times2 = new long[this.transitionCount];
                for (typeIdx = 0; typeIdx < this.typeCount; ++typeIdx) {
                    ...
                    System.arraycopy(times2, 0, startTimes, 0, nTimes);
