...
extends JavaScriptBaseNode {
    ...
    protected static JSDynamicObject doNullOrUndefined(JSDynamicObject target, Object value2, Object[] excludedItems, boolean withExcluded) {
        return target;
    }

    @Specialization(guards={"isJSObject(source)"})
    protected static JSDynamicObject copyDataProperties(JSDynamicObject target, JSDynamicObject source, Object[] excludedItems, boolean withExcluded, @Cached(value="create(context)") ReadElementNode getNode, @Cached(value="create(false)") JSGetOwnPropertyNode getOwnProperty, @Cached ListSizeNode listSize, @Cached ListGetNode listGet, @Cached JSClassProfile classProfile, @Cached TruffleString.EqualNode equalsNode) {
            ...
            try {
                if (this.context.getContextOptions().hasForeignHashProperties() && objInterop.hasHashEntries(from)) {
                    ...
                    while (true) {
                        ...
                        Object value2 = arrayInterop.readArrayElement(entry, 1L);
                        ...
                        JSRuntime.createDataPropertyOrThrow(target, stringKey, importValue.executeWithTarget(value2));
                    }
                }
                if (objInterop.hasMembers(from)) {
                    ...
                    for (long i = 0L; i < length; ++i) {
                        ...
                        if (CopyDataPropertiesNode.isExcluded(withExcluded, excludedItems, stringKey, equalsNode)) continue;
                        Object value3 = objInterop.readMember(from, Strings.toJavaString(stringKey));
                        JSRuntime.createDataPropertyOrThrow(target, stringKey, importValue.executeWithTarget(value3));
                    }
                }
            }
