...
public class ICUCurrencyMetaInfo extends CurrencyMetaInfo {
   ...
   public ICUCurrencyMetaInfo() {
      ICUResourceBundle bundle = (ICUResourceBundle)ICUResourceBundle.getBundleInstance(
         "com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b/curr", "supplementalData", ICUResourceBundle.ICU_DATA_CLASS_LOADER
      );
   ...
   public List<CurrencyMetaInfo.CurrencyInfo> currencyInfo(CurrencyMetaInfo.CurrencyFilter filter) {
      return this.collect(new ICUCurrencyMetaInfo.InfoCollector(), filter);
   }

   @Override
   public List<String> currencies(CurrencyMetaInfo.CurrencyFilter filter) {
      return this.collect(new ICUCurrencyMetaInfo.CurrencyCollector(), filter);
   }

   @Override
   public List<String> regions(CurrencyMetaInfo.CurrencyFilter filter) {
      return this.collect(new ICUCurrencyMetaInfo.RegionCollector(), filter);
   }
   ...
   public CurrencyMetaInfo.CurrencyDigits currencyDigits(String isoCode, Currency.CurrencyUsage currencyPurpose) {
      ...
      if (currencyPurpose == Currency.CurrencyUsage.CASH) {
         ...
         return currencyPurpose == Currency.CurrencyUsage.STANDARD
            ? new CurrencyMetaInfo.CurrencyDigits(data[0], data[1])
            : new CurrencyMetaInfo.CurrencyDigits(data[0], data[1]);
      }
   }
   ...
   private long getDate(ICUResourceBundle b, long defaultValue, boolean endOfDay) {
      if (b == null) {
         ...
         return (long)values[0] << 32 | values[1] & 4294967295L;
      }
   }
   ...
   private static class CurrencyCollector implements ICUCurrencyMetaInfo.Collector<String> {
      private final ICUCurrencyMetaInfo.UniqueList<String> result = ICUCurrencyMetaInfo.UniqueList.create();
   ...
   private static class InfoCollector implements ICUCurrencyMetaInfo.Collector<CurrencyMetaInfo.CurrencyInfo> {
      private List<CurrencyMetaInfo.CurrencyInfo> result = new ArrayList<>();
   ...
   private static class UniqueList<T> {
      ...
      private static <T> ICUCurrencyMetaInfo.UniqueList<T> create() {
         return new ICUCurrencyMetaInfo.UniqueList<>();
      }
