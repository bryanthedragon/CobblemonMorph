...
public class PoisonBadlyStatus : PersistentStatus(
      MiscUtilsKt.cobblemonResource("poisonbadly"), "tox", "cobblemon.status.poisonbadly.apply", "cobblemon.status.poison.cure", new IntRange(180, 300)
   ) {
      ...
      if (!pokemon.isFainted() && random.nextInt(15) == 0) {
         pokemon.setCurrentHealth(
            pokemon.getCurrentHealth()
               - Math.max(1, (int)Math.rint((double)pokemon.getHp() * 0.1)) * (if (pokemon.getAbility().getTemplate().getName() == "poisonheal") -1 else 1)
         );
