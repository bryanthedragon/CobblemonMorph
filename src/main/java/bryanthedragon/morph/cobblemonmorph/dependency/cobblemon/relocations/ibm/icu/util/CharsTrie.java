...
Iterable<Entry> {
    ...
    private static int readValue(CharSequence chars, int pos, int leadUnit) {
        int value2 = leadUnit < 16384 ? leadUnit : (leadUnit < Short.MAX_VALUE ? leadUnit - 16384 << 16 | chars.charAt(pos) : chars.charAt(pos) << 16 | chars.charAt(pos + 1));
        return value2;
    }
    ...
    private static int readNodeValue(CharSequence chars, int pos, int leadUnit) {
        ...
        int value2 = leadUnit < 16448 ? (leadUnit >> 6) - 1 : (leadUnit < 32704 ? (leadUnit & 0x7FC0) - 16448 << 10 | chars.charAt(pos) : chars.charAt(pos) << 16 | chars.charAt(pos + 1));
        return value2;
    }
    ...
    private static long findUniqueValueFromBranch(CharSequence chars, int pos, int length, long uniqueValue) {
        ...
        do {
            ...
            int value2 = CharsTrie.readValue(chars, ++pos, node &= Short.MAX_VALUE);
            pos = CharsTrie.skipValue(pos, node);
            if (isFinal) {
                if (uniqueValue != 0L) {
                    if (value2 == (int)(uniqueValue >> 1)) continue;
                    return 0L;
                }
                uniqueValue = (long)value2 << 1 | 1L;
                continue;
            }
            if ((uniqueValue = CharsTrie.findUniqueValue(chars, pos + value2, uniqueValue)) != 0L) continue;
            return 0L;
        } while (--length > 1);
    ...
    private static long findUniqueValue(CharSequence chars, int pos, long uniqueValue) {
        ...
        while (true) {
            ...
            boolean isFinal = (node & 0x8000) != 0;
            int value2 = isFinal ? CharsTrie.readValue(chars, pos, node & Short.MAX_VALUE) : CharsTrie.readNodeValue(chars, pos, node);
            if (uniqueValue != 0L) {
                if (value2 != (int)(uniqueValue >> 1)) {
                    return 0L;
                }
            } else {
                uniqueValue = (long)value2 << 1 | 1L;
    ...
    implements java.util.Iterator<Entry> {
        ...
        private int branchNext(int pos, int length) {
            ...
            int value2 = CharsTrie.readValue(this.chars_, pos, node &= Short.MAX_VALUE);
            pos = CharsTrie.skipValue(pos, node);
