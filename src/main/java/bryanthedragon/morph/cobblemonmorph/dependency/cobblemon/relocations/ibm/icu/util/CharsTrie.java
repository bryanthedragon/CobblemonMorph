...
public final class CharsTrie implements Cloneable, Iterable<CharsTrie.Entry> {
   ...
   public long getState64() {
      return (long)this.remainingMatchLength_ << 32 | this.pos_;
   }
   ...
   public BytesTrie.Result firstForCodePoint(int cp) {
      return cp <= 65535
         ? this.first(cp)
         : (this.first(UTF16.getLeadSurrogate(cp)).hasNext() ? this.next(UTF16.getTrailSurrogate(cp)) : BytesTrie.Result.NO_MATCH);
   }
   ...
   public BytesTrie.Result nextForCodePoint(int cp) {
      return cp <= 65535
         ? this.next(cp)
         : (this.next(UTF16.getLeadSurrogate(cp)).hasNext() ? this.next(UTF16.getTrailSurrogate(cp)) : BytesTrie.Result.NO_MATCH);
   }
   ...
   public int getValue() {
      ...
      return (leadUnit & 32768) != 0 ? readValue(this.chars_, pos, leadUnit & 32767) : readNodeValue(this.chars_, pos, leadUnit);
   }
   ...
   private BytesTrie.Result branchNext(int pos, int length, int inUnit) {
      ...
      if ((node & 32768) != 0) {
         ...
         pos += delta;
         int var13 = this.chars_.charAt(pos);
         result = var13 >= '@' ? valueResults_[var13 >> 15] : BytesTrie.Result.NO_VALUE;
      }
      ...
      while (true) {
         ...
         if (node < 64) {
            ...
            if (inUnit == this.chars_.charAt(pos++)) {
               ...
               this.pos_ = pos;
               char var7;
               return length < 0 && (var7 = this.chars_.charAt(pos)) >= '@' ? valueResults_[var7 >> 15] : BytesTrie.Result.NO_VALUE;
            }
            break;
         }
   ...
   private static long findUniqueValueFromBranch(CharSequence chars, int pos, int length, long uniqueValue) {
      ...
      do {
         int var10001 = ++pos;
         pos++;
         int node = chars.charAt(var10001);
         boolean isFinal = (node & 32768) != 0;
         node &= 32767;
      ...
      while (true) {
         while (node >= 48) {
            if (node < 64) {
               ...
               int value;
               if (isFinal) {
                  value = readValue(chars, pos, node & 32767);
               } else {
   ...
   public static final class Iterator implements java.util.Iterator<CharsTrie.Entry> {
      private CharSequence chars_;
      private int pos_;
      ...
      private CharsTrie.Entry entry_ = new CharsTrie.Entry();
      ...
      public CharsTrie.Entry next() {
         ...
         if (pos < 0) {
            ...
            int length = (int)top;
            pos = (int)(top >> 32);
            this.str_.setLength(length & 65535);
            length >>>= 16;
            if (length > 1) {
      ...
      private int branchNext(int pos, int length) {
         while (length > 5) {
            this.stack_.add((long)CharsTrie.skipDelta(this.chars_, ++pos) << 32 | length - (length >> 1) << 16 | this.str_.length());
            length >>= 1;
         ...
         this.stack_.add((long)pos << 32 | length - 1 << 16 | this.str_.length());
