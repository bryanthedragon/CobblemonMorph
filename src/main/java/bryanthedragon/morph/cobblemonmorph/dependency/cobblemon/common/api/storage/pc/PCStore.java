...
public class PCStore(
    final override val UUID uuid, val name: MutableComponent) : PokemonStore<PCPosition>() {
    ...
    fun addObserver(ServerPlayer player) {
        observingUUIDs.add(player.uuid);
        sendTo(player);
    }
    fun removeObserver(UUID playerID) {
    ...
    override boolean isValidPosition(PCPosition position) {
    ...
    fun resize(Int newSize, boolean lockNewSize = false, overflowHandler: (Pokemon) -> Unit = ::relocateEvictedBoxPokemon) {
        if (newSize <= 0) {
    ...
    fun removeListOfBoxes(List<PCBox> boxList, boolean lockNewSize = false, overflowHandler: (Pokemon) -> Unit = ::relocateEvictedBoxPokemon) {
        this.lockedSize = lockNewSize;
    ...
    override CompoundTag saveToNBT(CompoundTag nbt, RegistryAccess registryAccess) {
        nbt.putShort(DataKeys.STORE_BOX_COUNT, boxes.size.toShort());
        nbt.putBoolean(DataKeys.STORE_BOX_COUNT_LOCKED, lockedSize);
        boxes.forEachIndexed { index, box -> nbt.put(DataKeys.STORE_BOX + index, box.saveToNBT(CompoundTag(), registryAccess))};
    ...
    override fun loadFromNBT(CompoundTag nbt, RegistryAccess registryAccess): PokemonStore<PCPosition> {
    ...
    override  JsonObject saveToJSON(JsonObject json, RegistryAccess registryAccess registryAccess) {
    ...
    override PokemonStore<PCPosition> loadFromJSON(JsonObject json, RegistryAccess registryAccess) {
    ...
    override boolean remove(Pokemon pokemon) {
    ...
    override fun swap(PCPosition position1, PCPosition position2) {
    ...
    override fun loadPositionFromNBT(CompoundTag nbt): StoreCoordinates<PCPosition> {
        return StoreCoordinates(this, PCPosition(nbt.getShort(DataKeys.STORE_BOX).toInt(), nbt.getByte(DataKeys.STORE_SLOT).toInt()));
    }

    override fun savePositionToNBT(position: PCPosition, CompoundTag nbt) {
    ...
    fun clearPC() { 
        boxes.forEach { box -> box.getNonEmptySlots().forEach{remove(PCPosition(box.boxNumber, it.key))}};
    }
    ...
    boolean unlockWallpaper(ResourceLocation wallpaper, boolean playSound = true) {
            ...
            CobblemonEvents.WALLPAPER_UNLOCKED_EVENT.postThen(
                ...
                ifSucceeded = {
                    if (unlockedWallpapers.add(wallpaper)) {
                        ...
                        pcChangeObservable.emit(Unit);
                        getObservingPlayers().forEach { player -> player.sendPacket(UnlockPCBoxWallpaperPacket(unlockableWallpaper.texture));
                            if (event.shouldNotify) {
                                val toast = Toast(title = lang("wallpaper_unlocked"), description = unlockableWallpaper.displayName?.let { "\"".text().add(it.asTranslated()).add("\"") } ?: lang("unknown_wallpaper") , icon = ItemStack(CobblemonItems.PC));
