...
implements Comparable<ByteArrayWrapper> {
    ...
    public final ByteArrayWrapper set(byte[] src, int start2, int limit) {
        this.size = 0;
        this.append(src, start2, limit);
        return this;
    }

    public final ByteArrayWrapper append(byte[] src, int start2, int limit) {
        int len = limit - start2;
        this.ensureCapacity(this.size + len);
        ByteArrayWrapper.copyBytes(src, start2, this.bytes, this.size, len);
        this.size += len;
        return this;
    }
