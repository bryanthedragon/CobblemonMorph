...
class ParserContextFunctionNode extends ParserContextBaseNode {
   ...
   private List<Entry<VarNode, Scope>> hoistedVarDeclarations;
   private List<Entry<VarNode, Scope>> hoistableBlockFunctionDeclarations;
   ...
   public boolean hasArrowEval() {
      return this.getFlag(134217728) != 0;
   }
   ...
   public boolean isMethod() {
      return this.getFlag(1048576) != 0;
   }

   public boolean isClassConstructor() {
      return this.getFlag(2097152) != 0;
   }

   public boolean isDerivedConstructor() {
      return this.getFlag(4194304) != 0;
   }
   ...
   private boolean addParameterBinding(IdentNode bindingIdentifier) {
      ...
      Symbol paramSymbol = new Symbol(bindingIdentifier.getNameTS(), 17 | (!tdz ? 1024 : 0));
   ...
   public boolean isAsync() {
      return this.getFlag(33554432) != 0;
   }
   ...
   public boolean isGenerator() {
      return this.getFlag(16777216) != 0;
   }
   ...
   private boolean needsArguments() {
      return this.getFlag(40) != 0 && this.getFlag(1073807616) == 0;
   }
   ...
   private void putFunctionSymbolIfAbsent(TruffleString bindingName, int symbolFlags) {
      if (this.hasParameterExpressions()) {
         ...
         if (!parameterScope.hasSymbol(bindingName.toJavaStringUncached()) && !this.bodyScope.hasSymbol(bindingName.toJavaStringUncached())) {
            parameterScope.putSymbol(new Symbol(bindingName, 1 | symbolFlags | 1024));
         }
      } else if (!this.bodyScope.hasSymbol(bindingName.toJavaStringUncached())) {
         this.bodyScope.putSymbol(new Symbol(bindingName, 4 | symbolFlags | 1024));
      }
   }
   ...
   public void finishBodyScope(Function<TruffleString, TruffleString> stringIntern) {
      ...
      if (!this.isScriptOrModule()) {
         if (this.needsArguments()) {
            this.putFunctionSymbolIfAbsent(stringIntern.apply(Parser.ARGUMENTS_NAME), 2097152);
         }
         ...
         if (!this.isArrow()) {
            boolean needsThisForEval = this.hasEval() || this.hasArrowEval();
            ...
            if (this.usesSuper() || this.isMethod() && needsThisForEval) {
               this.putFunctionSymbolIfAbsent(stringIntern.apply(TokenType.SUPER.getNameTS()), 33554432);
            }

            if (this.usesNewTarget() || needsThisForEval) {
               this.putFunctionSymbolIfAbsent(stringIntern.apply(Parser.NEW_TARGET_NAME), 67108864);
            }
         }
   ...
   public boolean isClassStaticBlock() {
      return this.getFlag(1073741824) != 0;
   }
   ...
   public void recordHoistedVarDeclaration(final VarNode varDecl, final Scope scope) {
      ...
      this.hoistedVarDeclarations.add(new SimpleImmutableEntry<>(varDecl, scope));
   }
   ...
   public VarNode verifyHoistedVarDeclarations() {
      if (!this.hasHoistedVarDeclarations()) {
         ...
         for (Entry<VarNode, Scope> entry : this.hoistedVarDeclarations) {
   ...
   public void recordHoistableBlockFunctionDeclaration(final VarNode functionDeclaration, final Scope scope) {
      ...
      this.hoistableBlockFunctionDeclarations.add(new SimpleImmutableEntry<>(functionDeclaration, scope));
   }

   public void declareHoistedBlockFunctionDeclarations() {
      if (this.hoistableBlockFunctionDeclarations != null) {
         label45:
         for (Entry<VarNode, Scope> entry : this.hoistableBlockFunctionDeclarations) {
