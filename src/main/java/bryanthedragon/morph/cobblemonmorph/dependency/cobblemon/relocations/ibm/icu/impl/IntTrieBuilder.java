...
extends TrieBuilder {
    ...
    public boolean setValue(int ch, int value2) {
        if (this.m_isCompacted_ || ch > 0x10FFFF || ch < 0) {
            return false;
        }
        ...
        this.m_data_[block + (ch & 0x1F)] = value2;
        return true;
    }
    ...
    public boolean setRange(int start2, int limit, int value2, boolean overwrite) {
        int block;
        if (this.m_isCompacted_ || start2 < 0 || start2 > 0x10FFFF || limit < 0 || limit > 0x110000 || start2 > limit) {
            return false;
        }
        if (start2 == limit) {
            return true;
        ...
        if ((start2 & 0x1F) != 0) {
            int block2 = this.getDataBlock(start2);
            if (block2 < 0) {
                return false;
            }
            int nextStart = start2 + 32 & 0xFFFFFFE0;
            if (nextStart <= limit) {
                this.fillBlock(block2, start2 & 0x1F, 32, value2, overwrite);
                start2 = nextStart;
            } else {
                this.fillBlock(block2, start2 & 0x1F, limit & 0x1F, value2, overwrite);
                return true;
            }
        }
        int rest = limit & 0x1F;
        limit &= 0xFFFFFFE0;
        int repeatBlock = 0;
        if (value2 != this.m_initialValue_) {
            repeatBlock = -1;
        }
        while (start2 < limit) {
            block = this.m_index_[start2 >> 5];
            if (block > 0) {
                this.fillBlock(block, 0, 32, value2, overwrite);
            } else if (this.m_data_[-block] != value2 && (block == 0 || overwrite)) {
                if (repeatBlock >= 0) {
                    this.m_index_[start2 >> 5] = -repeatBlock;
                } else {
                    repeatBlock = this.getDataBlock(start2);
                    if (repeatBlock < 0) {
                        return false;
                    }
                    this.m_index_[start2 >> 5] = -repeatBlock;
                    this.fillBlock(repeatBlock, 0, 32, value2, true);
                }
            }
            start2 += 32;
        }
        if (rest > 0) {
            block = this.getDataBlock(start2);
            if (block < 0) {
                return false;
            }
            this.fillBlock(block, 0, rest, value2, overwrite);
        }
        return true;
    }
    ...
    private void compact(boolean overlap) {
        ...
        int start2 = newStart = 32;
        while (start2 < this.m_dataLength_) {
            if (this.m_map_[start2 >>> 5] < 0) {
                start2 += 32;
                continue;
            }
            if (start2 >= overlapStart && (i = IntTrieBuilder.findSameDataBlock(this.m_data_, newStart, start2, overlap ? 4 : 32)) >= 0) {
                this.m_map_[start2 >>> 5] = i;
                start2 += 32;
                continue;
            }
            if (overlap && start2 >= overlapStart) {
                for (i = 28; i > 0 && !IntTrieBuilder.equal_int(this.m_data_, newStart - i, start2, i); i -= 4) {
                }
            } else {
                i = 0;
            }
            if (i > 0) {
                this.m_map_[start2 >>> 5] = newStart - i;
                start2 += i;
                for (i = 32 - i; i > 0; --i) {
                    this.m_data_[newStart++] = this.m_data_[start2++];
                }
                continue;
            }
            if (newStart < start2) {
                this.m_map_[start2 >>> 5] = newStart;
                for (i = 32; i > 0; --i) {
                    this.m_data_[newStart++] = this.m_data_[start2++];
                }
                continue;
    ...
    private final void fold(TrieBuilder.DataManipulate manipulate) {
        ...
        while (c < 0x110000) {
            if (index[c >> 5] != 0) {
                int value2;
                if ((value2 = manipulate.getFoldedValue(c &= 0xFFFFFC00, (block = IntTrieBuilder.findSameIndexBlock(index, indexLength, c >> 5)) + 32)) != this.getValue(UTF16.getLeadSurrogate(c))) {
                    if (!this.setValue(UTF16.getLeadSurrogate(c), value2)) {
    ...
    private void fillBlock(int block, int start2, int limit, int value2, boolean overwrite) {
        limit += block;
        block += start2;
        if (overwrite) {
            while (block < limit) {
                this.m_data_[block++] = value2;
            }
        } else {
            while (block < limit) {
