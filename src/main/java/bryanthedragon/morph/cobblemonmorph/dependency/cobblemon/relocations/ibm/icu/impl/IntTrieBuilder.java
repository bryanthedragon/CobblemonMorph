...
public class IntTrieBuilder extends TrieBuilder {
      ...
      if (!this.m_isCompacted_ && start >= 0 && start <= 1114111 && limit >= 0 && limit <= 1114112 && start <= limit) {
         if (start == limit) {
            ...
            for (; start < limit; start += 32) {
               int blockx = this.m_index_[start >> 5];
               if (blockx > 0) {
                  this.fillBlock(blockx, 0, 32, value, overwrite);
               } else if (this.m_data_[-blockx] != value && (blockx == 0 || overwrite)) {
                  if (repeatBlock >= 0) {
            ...
            if (rest > 0) {
               int blockx = this.getDataBlock(start);
               if (blockx < 0) {
                  return false;
               }

               this.fillBlock(blockx, 0, rest, value, overwrite);
            }
   ...
   private void compact(boolean overlap) {
      if (!this.m_isCompacted_) {
         ...
         for (int ix = 0; ix < this.m_indexLength_; ix++) {
            this.m_index_[ix] = this.m_map_[Math.abs(this.m_index_[ix]) >>> 5];
         }
