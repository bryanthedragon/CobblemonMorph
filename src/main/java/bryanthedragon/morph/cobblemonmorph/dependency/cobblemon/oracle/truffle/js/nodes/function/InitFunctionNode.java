...
public class InitFunctionNode extends JavaScriptBaseNode {
   ...
   protected InitFunctionNode(
      ...
      this.nameFlags = nameAttributes | 16;
   ...
   protected InitFunctionNode(JSFunctionData functionData) {
      this(
         ...
         functionData.hasStrictFunctionProperties(),
         functionData.isConstructor(),
         functionData.isBound(),
   ...
   public static InitFunctionNode create(
      JSContext context, boolean strictProperties, boolean isConstructor, boolean isBound, boolean isGenerator, boolean prototypeNotWritable
   ) {
   ...
   public final JSFunctionObject execute(JSFunctionObject function, int length, TruffleString name) {
      ...
      assert JSFunction.getFunctionData(function).isBound() || length == JSFunction.LENGTH_PROXY.get(function);
      ...
      if (this.context.getEcmaScriptVersion() >= 6) {
         if (!this.strictProperties) {
            if (this.context.isOptionV8CompatibilityMode()) {
               Properties.putConstant(this.setArgumentsNode, function, JSFunction.ARGUMENTS, JSFunction.ARGUMENTS_PROXY, this.argumentsCallerFlags | 16);
               Properties.putConstant(this.setCallerNode, function, JSFunction.CALLER, JSFunction.CALLER_PROXY, this.argumentsCallerFlags | 16);
            } else {
