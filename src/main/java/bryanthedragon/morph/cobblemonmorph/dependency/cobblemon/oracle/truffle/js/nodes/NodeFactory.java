...
public class NodeFactory {
   ...
   public JavaScriptNode createUnary(NodeFactory.UnaryOperation operation, JavaScriptNode operand) {
      switch (operation) {
   ...
   public JavaScriptNode createLocalVarInc(
      NodeFactory.UnaryOperation operation, JSFrameSlot frameSlot, boolean hasTemporalDeadZone, ScopeFrameNode scopeFrameNode
   ) {
      switch (operation) {
   ...
   public JavaScriptNode createDual(JSContext context, JavaScriptNode left, JavaScriptNode right) {
      return left instanceof EmptyNode ? right : this.createBinary(context, NodeFactory.BinaryOperation.DUAL, left, right);
   }
   ...
   private static JavaScriptNode createBinaryIdentical(JavaScriptNode left, JavaScriptNode right) {
      ...
      return node != null ? node : JSIdenticalNode.create(left, right);
   }

   private static JavaScriptNode createBinaryEqual(JavaScriptNode left, JavaScriptNode right) {
      JavaScriptNode node = createIdenticalSpecial(left, right);
      return node != null ? node : JSEqualNode.create(left, right);
   }
   ...
   private static JavaScriptNode createIdenticalSpecial(JavaScriptNode left, JavaScriptNode right) {
      if (left instanceof TypeOfNode && right instanceof JSConstantNode.JSConstantStringNode) {
         ...
         return right instanceof TypeOfNode && left instanceof JSConstantNode.JSConstantStringNode
            ? JSTypeofIdenticalNode.create(((TypeOfNode)right).getOperand(), (JSConstantNode.JSConstantStringNode)left)
            : null;
      }
   }
   ...
   public JavaScriptNode createGlobalScopeTDZCheck(JSContext context, TruffleString name, boolean checkTDZ) {
      return !checkTDZ ? this.createGlobalScope(context) : GlobalScopeNode.createWithTDZCheck(context, name);
   }

   public JavaScriptNode createGlobalVarWrapper(
      TruffleString varName, JavaScriptNode defaultDelegate, JavaScriptNode dynamicScope, JSTargetableNode scopeAccessNode
   ) {
   ...
   public final JavaScriptNode createClearFrameSlots(ScopeFrameNode scope, int[] slots, int from, int to) {
      return isIndexRange(slots, from, to)
         ? this.createClearFrameSlotRange(scope, slots[from], slots[to - 1] + 1)
         : this.createClearFrameSlots(scope, from == 0 && to == slots.length ? slots : Arrays.copyOfRange(slots, from, to));
   }
   ...
   public JavaScriptNode createFunctionCall(JSContext context, JavaScriptNode expression, JavaScriptNode[] arguments) {
      if (!(expression instanceof PropertyNode)
         && !(expression instanceof ReadElementNode)
         && !(expression instanceof WithVarWrapperNode)
         && !(expression instanceof PrivateFieldGetNode)
         && !(expression instanceof OptionalChainNode.ShortCircuitTargetableNode)
         && !(expression instanceof OptionalChainNode.OptionalTargetableNode)) {
         if (expression instanceof JSTargetableWrapperNode) {
            ...
            if (expression instanceof GlobalPropertyNode) {
               ((GlobalPropertyNode)expression).setMethod();
            } else if (expression instanceof GlobalScopeVarWrapperNode) {
               ((GlobalScopeVarWrapperNode)expression).setMethod();
            }

            return JSFunctionCallNode.createCall(expression, target, arguments, false, false);
         }
      } else {
   ...
   public JavaScriptNode createAccessCallee(int level) {
      return (JavaScriptNode)(level == 0 ? AccessFunctionNode.create() : AccessLevelFunctionNode.create(level));
   }
   ...
   public WriteElementNode createWriteElementNode(
      JavaScriptNode targetNode, JavaScriptNode indexNode, JavaScriptNode valueNode, JSContext context, boolean isStrict
   ) {
   ...
   public WriteElementNode createCompoundWriteElementNode(
      JavaScriptNode targetNode, JavaScriptNode indexNode, JavaScriptNode valueNode, JSWriteFrameSlotNode writeIndex, JSContext context, boolean isStrict
   ) {
   ...
   public WritePropertyNode createWriteProperty(
      JavaScriptNode target, TruffleString name, JavaScriptNode rhs, JSContext context, boolean isStrict, boolean isGlobal, boolean verifyHasProperty
   ) {
   ...
   public FunctionRootNode createFunctionRootNode(
   ...
   public FunctionRootNode createModuleRootNode(
      ...
      if (linkBody == evalBody) {
         ...
         linkRoot = FunctionRootNode.create(
            linkBody, frameDescriptor, functionData, sourceSection, Strings.concat(internalFunctionName, Evaluator.MODULE_LINK_SUFFIX)
         );
         evalRoot = FunctionRootNode.create(
            evalBody,
            JavaScriptRootNode.MODULE_DUMMY_FRAMEDESCRIPTOR,
            functionData,
            sourceSection,
   ...
   public JSFunctionExpressionNode createFunctionExpressionLexicalThis(
      JSFunctionData function, FunctionRootNode functionNode, JSFrameSlot blockScopeSlot, JavaScriptNode thisNode
   ) {
   ...
   public ObjectLiteralNode.ObjectLiteralMemberNode createAccessorMember(
      TruffleString keyName, boolean isStatic, boolean enumerable, JavaScriptNode getter, JavaScriptNode setter
   ) {
      return ObjectLiteralNode.newAccessorMember(keyName, isStatic, enumerable, getter, setter);
   }

   public ObjectLiteralNode.ObjectLiteralMemberNode createDataMember(
      TruffleString keyName, boolean isStatic, boolean enumerable, JavaScriptNode value, boolean isField
   ) {
   ...
   public ObjectLiteralNode.ObjectLiteralMemberNode createComputedDataMember(
      JavaScriptNode key, boolean isStatic, boolean enumerable, JavaScriptNode value, boolean isField, boolean isAnonymousFunctionDefinition
   ) {
   ...
   public ObjectLiteralNode.ObjectLiteralMemberNode createComputedAccessorMember(
      JavaScriptNode key, boolean isStatic, boolean enumerable, JavaScriptNode getter, JavaScriptNode setter
   ) {
   ...
   public JavaScriptNode createClassDefinition(
      ...
      return ClassDefinitionNode.create(
         context,
         constructorFunction,
         classHeritage,
         members,
   ...
   public JavaScriptNode createAwait(
      JSContext context, JSFrameSlot stateSlot, JavaScriptNode expression, JSReadFrameSlotNode asyncContextNode, JSReadFrameSlotNode asyncResultNode
   ) {
   ...
   public JavaScriptNode createYield(
      ...
      return yieldStar
         ? YieldNode.createYieldStar(context, stateSlot.getIndex(), expression, yieldValue, returnNode, writeYieldResultNode)
         : YieldNode.createYield(context, stateSlot.getIndex(), expression, yieldValue, returnNode, writeYieldResultNode);
   }
   ...
   public JavaScriptNode createAsyncGeneratorYieldStar(
      ...
      return AsyncGeneratorYieldNode.createYieldStar(
         context, stateSlot.getIndex(), expression, asyncContextNode, asyncResultNode, returnNode, iteratorTempSlot.getIndex()
      );
   }
   ...
   public JavaScriptNode createBlockScope(
      ...
      JSFrameSlot blockScopeSlot,
      FrameDescriptor blockFrameDescriptor,
   ...
   public JavaScriptNode createIteratorGetNextValue(
      JSContext context, JavaScriptNode iterator, JavaScriptNode doneNode, boolean setDoneOnError, boolean readValue
   ) {
   ...
   public JavaScriptNode createAsyncIteratorNext(
      JSContext context, JSFrameSlot stateSlot, JavaScriptNode createReadNode, JSReadFrameSlotNode asyncContextNode, JSReadFrameSlotNode asyncResultNode
   ) {
   ...
   public JavaScriptNode createWithVarWrapper(
      TruffleString propertyName, JavaScriptNode withTarget, JSTargetableNode withAccessNode, JavaScriptNode globalDelegate
   ) {
   ...
   public JavaScriptNode createGuardDisconnectedArgumentWrite(
      int index, WriteElementNode argumentsArrayAccess, JavaScriptNode argumentsArray, JavaScriptNode rhs, JSFrameSlot slot
   ) {
   ...
   public JavaScriptNode createTopLevelAsyncModuleBody(
      JSContext context, JavaScriptNode moduleBody, JSWriteFrameSlotNode asyncResult, JSWriteFrameSlotNode writeAsyncContextNode
   ) {
   ...
   public JavaScriptNode createResolveStarImport(
      JSContext context, JavaScriptNode moduleNode, Module.ModuleRequest moduleRequest, JSWriteFrameSlotNode writeLocalNode
   ) {
      return ResolveStarImportNode.create(context, moduleNode, moduleRequest, writeLocalNode);
   }

   public JavaScriptNode createResolveNamedImport(
      JSContext context, JavaScriptNode moduleNode, Module.ModuleRequest moduleRequest, TruffleString importName, JSWriteFrameSlotNode writeLocalNode
   ) {
   ...
   public JavaScriptNode createRestObject(JSContext context, JavaScriptNode source, JavaScriptNode excludedNames) {
      JavaScriptNode restObj = ObjectLiteralNode.create(context, ObjectLiteralNode.ObjectLiteralMemberNode.EMPTY);
   ...
   public ObjectLiteralNode.ObjectLiteralMemberNode createPrivateFieldMember(
      JavaScriptNode keyNode, boolean isStatic, JavaScriptNode valueNode, JSWriteFrameSlotNode writePrivateNode
   ) {
   ...
   public JavaScriptNode createDebugVarWrapper(
      TruffleString varName, JavaScriptNode defaultDelegate, JavaScriptNode dynamicScope, JSTargetableNode scopeAccessNode
   ) {
