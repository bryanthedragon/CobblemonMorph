...
public final class Period {
    ...
    public Period inFuture(boolean future2) {
        return this.setFuture(future2);
    }
    ...
    private Period(int limit, boolean future2, float count, TimeUnit unit) {
        this.timeLimit = (byte)limit;
        this.inFuture = future2;
    ...
    private Period setTimeUnitValue(TimeUnit unit, float value2) {
        if (value2 < 0.0f) {
            throw new IllegalArgumentException("value: " + value2);
        }
        return this.setTimeUnitInternalValue(unit, (int)(value2 * 1000.0f) + 1);
    }

    private Period setTimeUnitInternalValue(TimeUnit unit, int value2) {
        byte ord = unit.ordinal;
        if (this.counts[ord] != value2) {
            int[] newCounts = new int[this.counts.length];
            for (int i = 0; i < this.counts.length; ++i) {
                newCounts[i] = this.counts[i];
            }
            newCounts[ord] = value2;
            return new Period(this.timeLimit, this.inFuture, newCounts);
        }
        return this;
    }

    private Period setFuture(boolean future2) {
        if (this.inFuture != future2) {
            return new Period(this.timeLimit, future2, this.counts);
        }
        return this;
    }
