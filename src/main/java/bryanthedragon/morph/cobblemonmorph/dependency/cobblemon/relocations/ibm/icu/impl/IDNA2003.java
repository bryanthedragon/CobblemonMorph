...
import com.cobblemon.mod.relocations.ibm.icu.text.StringPrep;
   ...
   public static StringBuffer convertToASCII(UCharacterIterator src, int options) throws StringPrepParseException {
      ...
      if (poLen == 0) {
         ...
         for (int j = 0; j < poLen; j++) {
            int var15 = processOut.charAt(j);
            if (var15 > 127) {
               srcIsASCII = false;
            } else if (!isLDHChar(var15)) {
               srcIsLDH = false;
               failPos = j;
            }
         }
         ...
         if (!useSTD3ASCIIRules || srcIsLDH && processOut.charAt(0) != '-' && processOut.charAt(processOut.length() - 1) != '-') {
            ...
            throw new StringPrepParseException(
               "The input does not conform to the STD 3 ASCII rules", 5, processOut.toString(), failPos > 0 ? failPos - 1 : failPos
            );
