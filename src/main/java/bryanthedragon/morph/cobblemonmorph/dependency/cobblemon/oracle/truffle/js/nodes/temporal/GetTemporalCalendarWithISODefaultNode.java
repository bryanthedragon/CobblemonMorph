...
public abstract class GetTemporalCalendarWithISODefaultNode extends JavaScriptBaseNode {
   ...
   protected JSDynamicObject getTemporalCalendarWithISODefault(Object item, @Cached BranchProfile errorBranch) {
      if (this.isCalendarProfile.profile(item instanceof TemporalCalendar)) {
         ...
         return (JSDynamicObject)(this.isNullishProfile.profile(calendar == Undefined.instance)
            ? TemporalUtil.getISO8601Calendar(this.ctx, this.getRealm(), errorBranch)
            : this.toTemporalCalendar(calendar));
      }
   }
