...
implements MultiplierProducer {
    ...
    extends UResource.Sink {
        ...
        public void put(UResource.Key key, UResource.Value value2, boolean isRoot) {
            UResource.Table powersOfTenTable = value2.getTable();
            int i3 = 0;
            while (powersOfTenTable.getKeyAndValue(i3, key, value2)) {
                byte magnitude = (byte)(key.length() - 1);
                byte multiplier = this.data.multipliers[magnitude];
                assert (magnitude < 15);
                UResource.Table pluralVariantsTable = value2.getTable();
                int i4 = 0;
                while (pluralVariantsTable.getKeyAndValue(i4, key, value2)) {
                    ...
                    if (this.data.patterns[CompactData.getIndex(magnitude, plural)] == null) {
                        int numZeros;
                        String patternString = value2.toString();
