...
public final class UTF16 {
    ...
    public static int charAt(char[] source, int start2, int limit, int offset16) {
        if ((offset16 += start2) < start2 || offset16 >= limit) {
    ...
    public static int bounds(char[] source, int start2, int limit, int offset16) {
        if ((offset16 += start2) < start2 || offset16 >= limit) {
            throw new ArrayIndexOutOfBoundsException(offset16);
        }
        ...
        if (UTF16.isSurrogate(ch)) {
            ...
            } else if (--offset16 >= start2 && UTF16.isLeadSurrogate(source[offset16])) {
                return 5;
            }
        }
        return 1;
    ...
    public static String valueOf(char[] source, int start2, int limit, int offset16) {
        switch (UTF16.bounds(source, start2, limit, offset16)) {
            case 2: {
                return new String(source, start2 + offset16, 2);
            }
            case 5: {
                return new String(source, start2 + offset16 - 1, 2);
            }
        }
        return new String(source, start2 + offset16, 1);
    }
    ...
    public static int findOffsetFromCodePoint(char[] source, int start2, int limit, int offset32) {
        int count;
        int result = start2;
        if (offset32 > limit - start2) {
    ...
    public static int findCodePointOffset(char[] source, int start2, int limit, int offset16) {
        if ((offset16 += start2) > limit) {
            throw new StringIndexOutOfBoundsException(offset16);
        }
        int result = 0;
        boolean hadLeadSurrogate = false;
        for (int i = start2; i < offset16; ++i) {
            char ch = source[i];
            if (hadLeadSurrogate && UTF16.isTrailSurrogate(ch)) {
                hadLeadSurrogate = false;
                continue;
    ...
    public static int countCodePoint(char[] source, int start2, int limit) {
        if (source == null || source.length == 0) {
            return 0;
        }
        return UTF16.findCodePointOffset(source, start2, limit, limit - start2);
    }
    ...
    public static int moveCodePointOffset(char[] source, int start2, int limit, int offset16, int shift32) {
        int count;
        int size = source.length;
        int result = offset16 + start2;
        if (start2 < 0 || limit < start2) {
        ...
        if (shift32 > 0) {
            ...
            if (result + shift32 < start2) {
                throw new StringIndexOutOfBoundsException(result);
            }
            for (count = -shift32; count > 0 && --result >= start2; --count) {
                char ch = source[result];
                if (!UTF16.isTrailSurrogate(ch) || result <= start2 || !UTF16.isLeadSurrogate(source[result - 1])) continue;
                --result;
            }
        }
        if (count != 0) {
    ...
    public static boolean hasMoreCodePointsThan(String source, int number) {
        ...
        while (length != 0) {
            ...
            if (UTF16.isLeadSurrogate(source.charAt(start2++)) && start2 != length && UTF16.isTrailSurrogate(source.charAt(start2))) {
                ++start2;
                if (--maxsupplementary <= 0) {
                    return false;
                }
    ...
    public static boolean hasMoreCodePointsThan(char[] source, int start2, int limit, int number) {
        int length = limit - start2;
        if (length < 0 || start2 < 0 || limit < 0) {
        ...
        while (length != 0) {
            ...
            if (UTF16.isLeadSurrogate(source[start2++]) && start2 != limit && UTF16.isTrailSurrogate(source[start2])) {
                ++start2;
                if (--maxsupplementary <= 0) {
                    return false;
                }
