...
extends JSBuiltinsContainer.SwitchEnum<RegExpBuiltin> {
    ...
    extends JSRegExpStaticResultPropertyNode {
        ...
        TruffleString rightContext(VirtualFrame frame, @Cached TruffleString.SubstringByteIndexNode substringNode) {
            ...
            if (this.resultAccessor.isMatch(result)) {
                int end2 = this.resultAccessor.captureGroupEnd(result, 0);
                return Strings.substring(this.getContext(), substringNode, this.getInput(), end2);
            }
    ...
    extends JSRegExpStaticResultPropertyNode {
        ...
        TruffleString leftContext(VirtualFrame frame, @Cached TruffleString.SubstringByteIndexNode substringNode) {
            ...
            Object result = this.getResultNode.execute();
            if (this.resultAccessor.isMatch(result)) {
                int start2 = this.resultAccessor.captureGroupStart(result, 0);
    ...
    extends JSRegExpStaticResultPropertyNode {
        ...
        TruffleString lastParen(VirtualFrame frame, @Cached TruffleString.SubstringByteIndexNode substringNode) {
            ...
            Object result = this.getResultNode.execute();
            if (this.resultAccessor.isMatch(result) && (groupNumber = this.compiledRegexAccessor.groupCount(realm.getStaticRegexResultCompiledRegex()) - 1) > 0 && (start2 = this.resultAccessor.captureGroupStart(result, groupNumber)) >= 0) {
                return Strings.substring(this.getContext(), substringNode, this.getInput(), start2, this.resultAccessor.captureGroupEnd(result, groupNumber) - start2);
            }
            return Strings.EMPTY_STRING;
        }
    }
    ...
    extends JSRegExpStaticResultPropertyNode {
        ...
        TruffleString getGroup(VirtualFrame frame, @Cached TruffleString.SubstringByteIndexNode substringNode) {
            ...
            RegExpBuiltins.checkStaticRegexResultPropertyGet(this.getContext(), realm, JSFrameUtil.getThisObj(frame));
            Object result = this.getResultNode.execute();
            if (this.resultAccessor.isMatch(result) && this.compiledRegexAccessor.groupCount(realm.getStaticRegexResultCompiledRegex()) > this.groupNumber && (start2 = this.resultAccessor.captureGroupStart(result, this.groupNumber)) >= 0) {
                return Strings.substring(this.getContext(), substringNode, this.getInput(), start2, this.resultAccessor.captureGroupEnd(result, this.groupNumber) - start2);
            }
            return Strings.EMPTY_STRING;
        }
    }
    ...
    extends JSBuiltinNode {
        ...
        boolean getMultilineEager(@Cached(value="createGetResultNode()") GetStaticRegExpResultNode getResultNode, @Cached(value="create()") TRegexUtil.TRegexResultAccessor resultAccessor) {
            ...
            if (!this.getContext().isOptionNashornCompatibilityMode() && resultAccessor.isMatch(result)) {
                return this.multilineAccessor.get(compiledRegex);
            }
            return false;
        }
