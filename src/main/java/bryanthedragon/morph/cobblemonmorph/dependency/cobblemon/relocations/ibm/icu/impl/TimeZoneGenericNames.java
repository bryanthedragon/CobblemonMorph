...
Freezable<TimeZoneGenericNames> {
            ...
            if (offsets[1] == 0) {
                ...
                if (tz instanceof BasicTimeZone) {
                    ...
                    if (before != null && date - before.getTime() < 15897600000L && before.getFrom().getDSTSavings() != 0) {
                        ...
                        TimeZoneTransition after2 = btz.getNextTransition(date, false);
                        if (after2 != null && after2.getTime() - date < 15897600000L && after2.getTo().getDSTSavings() != 0) {
                            useStandard = false;
                        }
                    }
                } else {
    ...
    private synchronized String formatPattern(Pattern pat, String ... args) {
        ...
        if (this._patternFormatters[idx = pat.ordinal()] == null) {
            String patText;
            try {
                ICUResourceBundle bundle = (ICUResourceBundle)ICUResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b/zone", this._locale);
    ...
    public GenericMatchInfo findBestMatch(String text, int start2, EnumSet<GenericNameType> genericTypes) {
        Collection<GenericMatchInfo> localMatches;
        if (text == null || text.length() == 0 || start2 < 0 || start2 >= text.length()) {
            throw new IllegalArgumentException("bad input text or range");
        }
        GenericMatchInfo bestMatch = null;
        Collection<TimeZoneNames.MatchInfo> tznamesMatches = this.findTimeZoneNames(text, start2, genericTypes);
        if (tznamesMatches != null) {
            ...
            if (longestMatch != null && (bestMatch = this.createGenericMatchInfo(longestMatch)).matchLength() == text.length() - start2 && bestMatch.timeType != TimeZoneFormat.TimeType.STANDARD) {
                return bestMatch;
            }
        }
        if ((localMatches = this.findLocal(text, start2, genericTypes)) != null) {
    ...
    public Collection<GenericMatchInfo> find(String text, int start2, EnumSet<GenericNameType> genericTypes) {
        if (text == null || text.length() == 0 || start2 < 0 || start2 >= text.length()) {
            throw new IllegalArgumentException("bad input text or range");
        }
        Collection<GenericMatchInfo> results2 = this.findLocal(text, start2, genericTypes);
        Collection<TimeZoneNames.MatchInfo> tznamesMatches = this.findTimeZoneNames(text, start2, genericTypes);
        if (tznamesMatches != null) {
            for (TimeZoneNames.MatchInfo match : tznamesMatches) {
                if (results2 == null) {
                    results2 = new LinkedList<GenericMatchInfo>();
                }
                results2.add(this.createGenericMatchInfo(match));
            }
        }
        return results2;
    }
    ...
    private Collection<TimeZoneNames.MatchInfo> findTimeZoneNames(String text, int start2, EnumSet<GenericNameType> types) {
        ...
        if (!nameTypes.isEmpty()) {
            tznamesMatches = this._tznames.find(text, start2, nameTypes);
        }
        return tznamesMatches;
    }

    private synchronized Collection<GenericMatchInfo> findLocal(String text, int start2, EnumSet<GenericNameType> types) {
        GenericNameSearchHandler handler = new GenericNameSearchHandler(types);
        this._gnamesTrie.find(text, start2, handler);
        if (handler.getMaxMatchLen() == text.length() - start2 || this._gnamesTrieFullyLoaded) {
