...
public final class StringSearch extends SearchIterator {
   ...
   public StringSearch(String pattern, CharacterIterator target, RuleBasedCollator collator, BreakIterator breakiter) {
      ...
      if (collator.getNumericCollation()) {
         ...
         this.pattern_ = new StringSearch.Pattern(pattern);
   ...
   private int initializePatternPCETable() {
      ...
      int offset = 0;
      int result = 0;
      StringSearch.CollationPCE iter = new StringSearch.CollationPCE(coleiter);
   ...
   private boolean initTextProcessedIter() {
      if (this.textProcessedIter_ == null) {
         this.textProcessedIter_ = new StringSearch.CollationPCE(this.textIter_);
      } else {
            ...
            if (targLev2 != patLev2) {
               if (targLev2 == 0) {
                  ...
                  return patLev2 != CE_LEVEL2_BASE
                        && (compareType != SearchIterator.ElementComparisonType.ANY_BASE_WEIGHT_IS_WILDCARD || targLev2 != CE_LEVEL2_BASE)
                     ? 0
                     : -1;
               }
            } else {
               ...
               if (targLev3 == patLev3) {
                  ...
                  return patLev3 != CE_LEVEL3_BASE
                        && (compareType != SearchIterator.ElementComparisonType.ANY_BASE_WEIGHT_IS_WILDCARD || targLev3 != CE_LEVEL3_BASE)
                     ? 0
                     : -1;
               }
            }
   ...
   private boolean search(int startIdx, StringSearch.Match m) {
      if (this.pattern_.CELength_ != 0 && startIdx >= this.search_.beginIndex() && startIdx <= this.search_.endIndex()) {
         ...
         StringSearch.CEBuffer ceb = new StringSearch.CEBuffer(this);
         int targetIx = 0;
         StringSearch.CEI targetCEI = null;
         int mStart = -1;
         int mLimit = -1;
         targetIx = 0;
         ...
         while (true) {
            ...
            long patCE = 0L;
            StringSearch.CEI firstCEI = ceb.get(targetIx);
            if (firstCEI == null) {
            ...
            if (found || targetCEI != null && targetCEI.ce_ == -1L) {
               ...
               StringSearch.CEI lastCEI = ceb.get(targetIx + targetIxOffset - 1);
               mStart = firstCEI.lowIndex_;
               int minLimit = lastCEI.lowIndex_;
               StringSearch.CEI nextCEI = null;
               int maxLimit;
               if (this.search_.elementComparisonType_ == SearchIterator.ElementComparisonType.STANDARD_ELEMENT_COMPARISON) {
                  ...
                  while (true) {
                     ...
                     int ceMatchx = compareCE64s(nextCEI.ce_, patCE, this.search_.elementComparisonType_);
                     if (ceMatchx == 0 || ceMatchx == 2) {
                        found = false;
                        break;
                     }
               ...
               boolean allowMidclusterMatch = this.breakIterator == null
                  && (nextCEI.ce_ >>> 32 & 4294901760L) != 0L
                  && maxLimit >= lastCEI.highIndex_
                  && nextCEI.highIndex_ > maxLimit
                  && (
                     this.nfd_.hasBoundaryBefore(codePointAt(this.targetText, maxLimit))
                        || this.nfd_.hasBoundaryAfter(codePointBefore(this.targetText, maxLimit))
                  );
               mLimit = maxLimit;
         ...
         throw new IllegalArgumentException(
            "search(" + startIdx + ", m) - expected position to be between " + this.search_.beginIndex() + " and " + this.search_.endIndex()
         );
      }
   }
   ...
   private boolean searchBackwards(int startIdx, StringSearch.Match m) {
      if (this.pattern_.CELength_ != 0 && startIdx >= this.search_.beginIndex() && startIdx <= this.search_.endIndex()) {
         ...
         while (true) {
            found = true;
            StringSearch.CEI lastCEI = ceb.getPrevious(targetIx);
            if (lastCEI == null) {
            ...
            if (found || targetCEI != null && targetCEI.ce_ == -1L) {
               ...
               StringSearch.CEI firstCEI = ceb.getPrevious(targetIx + this.pattern_.PCELength_ - 1 + targetIxOffset);
               mStart = firstCEI.lowIndex_;
               if (!this.isBreakBoundary(mStart)) {
                  found = false;
               }
               ...
               int minLimit = lastCEI.lowIndex_;
               if (targetIx > 0) {
                  StringSearch.CEI nextCEI = ceb.getPrevious(targetIx - 1);
                  if (nextCEI.lowIndex_ == nextCEI.highIndex_ && nextCEI.ce_ != -1L) {
                     found = false;
                  }

                  int maxLimit = nextCEI.lowIndex_;
                  mLimit = nextCEI.lowIndex_;
                  boolean allowMidclusterMatch = this.breakIterator == null
                     && (nextCEI.ce_ >>> 32 & 4294901760L) != 0L
                     && maxLimit >= lastCEI.highIndex_
                     && nextCEI.highIndex_ > maxLimit
         ...
         throw new IllegalArgumentException(
            "searchBackwards(" + startIdx + ", m) - expected position to be between " + this.search_.beginIndex() + " and " + this.search_.endIndex()
         );
      }
   }
   ...
   private boolean handleNextCommonImpl() {
      ...
      StringSearch.Match match = new StringSearch.Match();
   ...
   private static class CEBuffer {
      ...
      CEBuffer(StringSearch ss) {
         ...
         if (ss.initTextProcessedIter()) {
            this.buf_ = new StringSearch.CEI[this.bufSize_];
         }
      }
      ...
      StringSearch.CEI get(int index) {
         ...
         if (index >= this.firstIx_ && index < this.limitIx_) {
            ...
            StringSearch.CollationPCE.Range range = new StringSearch.CollationPCE.Range();
            if (this.buf_[i] == null) {
               this.buf_[i] = new StringSearch.CEI();
            }
   ...
   private static class CollationPCE {
      ...
      private StringSearch.CollationPCE.PCEBuffer pceBuffer_ = new StringSearch.CollationPCE.PCEBuffer();
      ...
      public long previousProcessed(StringSearch.CollationPCE.Range range) {
         ...
         while (this.pceBuffer_.empty()) {
            StringSearch.CollationPCE.RCEBuffer rceb = new StringSearch.CollationPCE.RCEBuffer();
            ...
            while (!rceb.empty()) {
               StringSearch.CollationPCE.RCEI rcei = rceb.get();
         ...
         if (this.pceBuffer_.empty()) {
            ...
            StringSearch.CollationPCE.PCEI pcei = this.pceBuffer_.get();
            if (range != null) {
      ...
      private static final class PCEBuffer {
         private StringSearch.CollationPCE.PCEI[] buffer_ = new StringSearch.CollationPCE.PCEI[16];
         ...
         void put(long ce, int ixLow, int ixHigh) {
            if (this.bufferIndex_ >= this.buffer_.length) {
               StringSearch.CollationPCE.PCEI[] newBuffer = new StringSearch.CollationPCE.PCEI[this.buffer_.length + 8];
               System.arraycopy(this.buffer_, 0, newBuffer, 0, this.buffer_.length);
               this.buffer_ = newBuffer;
            }

            this.buffer_[this.bufferIndex_] = new StringSearch.CollationPCE.PCEI();
         ...
         StringSearch.CollationPCE.PCEI get() {
      ...
      private static final class RCEBuffer {
         private StringSearch.CollationPCE.RCEI[] buffer_ = new StringSearch.CollationPCE.RCEI[16];
         private int bufferIndex_ = 0;
         ...
         void put(int ce, int ixLow, int ixHigh) {
            if (this.bufferIndex_ >= this.buffer_.length) {
               StringSearch.CollationPCE.RCEI[] newBuffer = new StringSearch.CollationPCE.RCEI[this.buffer_.length + 8];
               System.arraycopy(this.buffer_, 0, newBuffer, 0, this.buffer_.length);
               this.buffer_ = newBuffer;
            }

            this.buffer_[this.bufferIndex_] = new StringSearch.CollationPCE.RCEI();
            this.buffer_[this.bufferIndex_].ce_ = ce;
            this.buffer_[this.bufferIndex_].low_ = ixLow;
            this.buffer_[this.bufferIndex_].high_ = ixHigh;
            this.bufferIndex_++;
