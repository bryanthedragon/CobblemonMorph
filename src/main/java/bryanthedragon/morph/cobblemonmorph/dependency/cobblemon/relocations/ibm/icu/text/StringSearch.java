...
import com.cobblemon.mod.relocations.ibm.icu.text.SearchIterator;
...
extends SearchIterator {
    ...
    public void reset() {
        ...
        if (this.toShift_ != (shift2 = this.collator_.isAlternateHandlingShifted())) {
            this.toShift_ = shift2;
            sameCollAttribute = false;
        }
    ...
    private static int[] addToIntArray(int[] destination, int offset, int value2, int increments) {
        ...
        destination[offset] = value2;
        return destination;
    }

    private static long[] addToLongArray(long[] destination, int offset, int destinationlength, long value2, int increments) {
    ...
    private boolean checkIdentical(int start2, int end2) {
        String patternstr;
        if (this.strength_ != 15) {
            return true;
        }
        String textstr = StringSearch.getString(this.targetText, start2, end2 - start2);
    ...
    private static final String getString(CharacterIterator text, int start2, int length) {
        StringBuilder result = new StringBuilder(length);
        int offset = text.getIndex();
        text.setIndex(start2);
