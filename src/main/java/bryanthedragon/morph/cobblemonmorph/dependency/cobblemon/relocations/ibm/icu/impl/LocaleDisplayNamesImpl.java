...
extends LocaleDisplayNames {
    ...
    private String localeDisplayNameInternal(ULocale locale) {
        ...
        if ((keys = locale.getKeywords()) != null) {
            while (keys.hasNext()) {
                ...
                String value2 = locale.getKeywordValue(key);
                ...
                String valueDisplayName = this.keyValueDisplayName(key, value2, true);
                if (valueDisplayName == null) {
                    return null;
                }
                if (!(valueDisplayName = valueDisplayName.replace(this.formatOpenParen, this.formatReplaceOpenParen).replace(this.formatCloseParen, this.formatReplaceCloseParen)).equals(value2)) {
    ...
    private String keyValueDisplayName(String key, String value2, boolean skipAdjust) {
        String keyValueName = null;
        if (key.equals("currency")) {
            keyValueName = this.currencyDisplayInfo.getName(AsciiUtil.toUpperString(value2));
            if (keyValueName == null) {
                keyValueName = value2;
            }
        } else {
            String tmp;
            if (this.nameLength == DisplayContext.LENGTH_SHORT && (tmp = this.langData.get("Types%short", key, value2)) != null && !tmp.equals(value2)) {
                keyValueName = tmp;
            }
            if (keyValueName == null) {
                keyValueName = this.langData.get("Types", key, value2);
            }
        }
    ...
    @Override
    public String keyValueDisplayName(String key, String value2) {
        return this.keyValueDisplayName(key, value2, false);
    }
    ...
    static class RegionDataTables {
        static final DataTables impl = DataTables.load("com.cobblemon.mod.relocations.ibm.icu.impl.ICURegionDataTables");
    ...
    static class LangDataTables {
        static final DataTables impl = DataTables.load("com.cobblemon.mod.relocations.ibm.icu.impl.ICULangDataTables");
    ...
    extends UResource.Sink {
        ...
        public void put(UResource.Key key, UResource.Value value2, boolean noFallback) {
            UResource.Table contextsTable = value2.getTable();
            int i = 0;
            while (contextsTable.getKeyAndValue(i, key, value2)) {
                int[] intVector;
                CapitalizationContextUsage usage = (CapitalizationContextUsage)((Object)contextUsageTypeMap.get(key.toString()));
                if (usage != null && (intVector = value2.getIntVector()).length >= 2) {
                    int titlecaseInt;
