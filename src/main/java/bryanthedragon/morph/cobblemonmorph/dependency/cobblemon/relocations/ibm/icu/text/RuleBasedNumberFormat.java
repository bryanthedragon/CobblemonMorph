...
import com.cobblemon.mod.relocations.ibm.icu.text.RbnfLenientScannerProvider;
...
extends NumberFormat {
    ...
    private void init(String description, String[][] localizations) {
        ...
        for (int curRuleSet = 0; curRuleSet < this.ruleSets.length; ++curRuleSet) {
            NFRuleSet ruleSet;
            p = descBuf.indexOf(";%", start2);
            if (p < 0) {
                p = descBuf.length() - 1;
            }
            ruleSetDescriptions[curRuleSet] = descBuf.substring(start2, p + 1);
    ...
    private StringBuilder stripWhitespace(String description) {
        ...
        int start2 = 0;
        while (start2 < descriptionLength) {
            while (start2 < descriptionLength && PatternProps.isWhiteSpace(description.charAt(start2))) {
                ++start2;
            }
            if (start2 < descriptionLength && description.charAt(start2) == ';') {
                ++start2;
                continue;
            }
            int p = description.indexOf(59, start2);
            if (p == -1) {
                result.append(description.substring(start2));
                break;
            }
            if (p >= descriptionLength) break;
            result.append(description.substring(start2, p + 1));
            start2 = p + 1;
        }
        return result;
    }
