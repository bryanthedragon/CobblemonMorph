...
PrototypeSupplier {
    ...
    public static JSArrayObject createConstant(JSContext context, JSRealm realm, Object[] elements2) {
        return JSArray.create(context, realm, ScriptArray.createConstantArray(elements2), elements2, elements2.length);
    }
    ...
    public static JSArrayObject create(JSContext context, JSRealm realm, ScriptArray arrayType, Object array, ArrayAllocationSite site, long length, int usedLength, int indexOffset, int arrayOffset, int holeCount) {
        return JSArray.createImpl(context, realm, arrayType, array, site, length, usedLength, indexOffset, arrayOffset, holeCount);
    }

    private static JSArrayObject createImpl(JSContext context, JSRealm realm, ScriptArray arrayType, Object array, ArrayAllocationSite site, long length, int usedLength, int indexOffset, int arrayOffset, int holeCount) {
    ...
    public JSDynamicObject createPrototype(JSRealm realm, JSFunctionObject ctor) {
        ...
        JSArrayObject arrayPrototype = JSArrayObject.createEmpty(protoShape, ConstantEmptyPrototypeArray.createConstantEmptyPrototypeArray());
        ...
        if (ctx.getEcmaScriptVersion() >= 6) {
            JSObjectUtil.putDataProperty(ctx, arrayPrototype, Symbol.SYMBOL_ITERATOR, JSDynamicObject.getOrNull(arrayPrototype, Strings.VALUES), JSAttributes.getDefaultNotEnumerable());
    ...
    private static boolean isSorted(List<TruffleString> list) {
        for (int i = 0; i < list.size() - 1; ++i) {
            if (list.get(i).compareCharsUTF16Uncached(list.get(i + 1)) <= 0) continue;
            return false;
        }
        return true;
    }
    ...
    public static boolean setLength(JSDynamicObject store, Object value2) {
        long arrLength = 0L;
        arrLength = value2 instanceof Integer && (Integer)value2 >= 0 ? (long)((Integer)value2).intValue() : JSArray.toArrayLengthOrRangeError(value2);
        assert (arrLength >= 0L);
    ...
    public static JSArrayObject createLazyRegexArray(JSContext context, JSRealm realm, int length, Object regexResult, TruffleString input, JSDynamicObject groups, JSDynamicObject indicesGroups) {
        assert (JSRuntime.isRepresentableAsUnsignedInt(length));
        JSArrayObject obj = JSArray.createLazyRegexArray(context, realm, length);
        ...
        if (context.isOptionRegexpMatchIndices()) {
            JSArrayObject indices = JSArray.createLazyRegexIndicesArray(context, realm, length, regexResult, indicesGroups);
    ...
    private static JSArrayObject createLazyRegexIndicesArray(JSContext context, JSRealm realm, int length, Object regexResult, JSDynamicObject indicesGroups) {
        assert (JSRuntime.isRepresentableAsUnsignedInt(length));
        Object[] array = new Object[length];
        JSArrayObject obj = JSArray.create(context, realm, LazyRegexResultIndicesArray.createLazyRegexResultIndicesArray(), array, length);
        JSObjectUtil.putHiddenProperty(obj, LAZY_REGEX_RESULT_ID, regexResult);
    ...
    extends PropertyProxy {
        ...
        public boolean set(JSDynamicObject store, Object value2) {
            assert (JSArray.isJSArray(store));
            return JSArray.setLength(store, value2);
        }
    }
}
