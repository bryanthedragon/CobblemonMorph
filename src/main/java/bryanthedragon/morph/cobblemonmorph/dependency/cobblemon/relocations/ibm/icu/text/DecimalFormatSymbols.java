...
Serializable {
    ...
    public void setPlusSign(char plus2) {
        this.plusSign = plus2;
        this.plusString = String.valueOf(plus2);
    }
    ...
    extends UResource.Sink {
        ...
        public void put(UResource.Key key, UResource.Value value2, boolean noFallback) {
            UResource.Table symbolsTable = value2.getTable();
            int j = 0;
            while (symbolsTable.getKeyAndValue(j, key, value2)) {
                for (int i = 0; i < SYMBOL_KEYS.length; ++i) {
                    if (!key.contentEquals(SYMBOL_KEYS[i])) continue;
                    if (this.numberElements[i] != null) break;
                    this.numberElements[i] = value2.toString();
                    break;
                }
                ++j;
            }
