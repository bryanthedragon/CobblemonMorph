...
public abstract class TRegexExecutorNode extends TRegexExecutorBaseNode {
   ...
   public int inputReadAndDecode(TRegexExecutorLocals locals, int index) {
      if (this.getEncoding() == Encodings.UTF_16) {
         ...
         if (this.inputUTF16IsHighSurrogate(c) && this.inputHasNext(locals, locals.getNextIndex())) {
         ...
         return c;
      } else if (this.getEncoding() == Encodings.UTF_8) {
         int c = this.inputReadRaw(locals);
         if (c < 128) {
            locals.setNextIndex(this.inputIncRaw(index));
            return c;
            ...
            if (this.isForward()) {
               ...
               switch (nBytes) {
                  case 4:
                     codepoint = codepoint << 6 | this.inputReadRaw(locals, locals.getNextIndex()) & 63;
                     locals.setNextIndex(this.inputIncRaw(locals.getNextIndex()));
                  case 3:
                     codepoint = codepoint << 6 | this.inputReadRaw(locals, locals.getNextIndex()) & 63;
                     locals.setNextIndex(this.inputIncRaw(locals.getNextIndex()));
         ...
         assert this.getEncoding() == Encodings.UTF_16_RAW
            || this.getEncoding() == Encodings.UTF_32
            || this.getEncoding() == Encodings.LATIN_1
            || this.getEncoding() == Encodings.BYTES
            || this.getEncoding() == Encodings.ASCII;
   ...
   protected void inputSkipIntl(TRegexExecutorLocals locals, boolean forward) {
      if (this.isUTF16()) {
         ...
         if (Encodings.Encoding.UTF16.isHighSurrogate(c, forward)
            && this.inputHasNext(locals, forward)
            && Encodings.Encoding.UTF16.isLowSurrogate(this.inputReadRaw(locals, forward), forward)) {
            this.inputIncRaw(locals, forward);
         }
      } else if (this.isUTF8()) {
         int c;
         if (forward) {
            c = this.inputReadRaw(locals, true);
            if (c < 128) {
               this.inputIncRaw(locals, true);
            } else {
               this.getBMPProfile().enter();
               this.inputIncRaw(locals, inputUTF8NumberOfLeadingOnes(c), true);
            }
         } else {
            do {
               c = this.inputReadRaw(locals, false);
               this.inputIncRaw(locals, false);
            } while (this.inputHasNext(locals, false) && inputUTF8IsTrailingByte(c));
         }
      } else {
         assert this.getEncoding() == Encodings.UTF_16_RAW
            || this.getEncoding() == Encodings.UTF_32
   ...
   public int inputIncRaw(int index) {
      return inputIncRaw(index, 1, this.isForward());
   }

   public int inputIncRaw(int index, int offset) {
      return inputIncRaw(index, offset, this.isForward());
   }
   ...
   public void inputIncNextIndexRaw(TRegexExecutorLocals locals, int offset) {
      locals.setNextIndex(inputIncRaw(locals.getIndex(), offset, this.isForward()));
   }
   ...
   public int countUpTo(TRegexExecutorLocals locals, int max, int nCodePoints) {
      ...
      if (nCodePoints <= 0) {
         ...
         int index;
         for (index = locals.getIndex(); locals.getIndex() < max && i < nCodePoints; i++) {
   ...
   public int rewindUpTo(TRegexExecutorLocals locals, int min, int nCodePoints) {
      ...
      if (nCodePoints <= 0) {
         ...
         int i;
         for (i = 0; locals.getIndex() > min && i < nCodePoints; i++) {
