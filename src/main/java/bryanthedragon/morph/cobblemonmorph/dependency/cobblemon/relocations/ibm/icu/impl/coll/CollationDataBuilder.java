...
package bryanthedragon.morph.cobblemonmorph.dependency.cobblemon.mod.relocations.ibm.icu.impl.coll;
...
import com.cobblemon.mod.relocations.ibm.icu.impl.coll.CollationFastLatinBuilder;
...
import com.cobblemon.mod.relocations.ibm.icu.impl.coll.UVector64;
...
final class CollationDataBuilder {
    ...
    void optimize(UnicodeSet set2) {
        ...
        UnicodeSetIterator iter = new UnicodeSetIterator(set2);
    ...
    protected int encodeExpansion(long[] ces, int start2, int length) {
        int j;
        int i;
        long first = ces[start2];
        ...
        block0: for (i = 0; i <= ce64sMax; ++i) {
            ...
            j = 1;
            while (j != length) {
                if (this.ce64s.elementAti(i + j) != ces[start2 + j]) continue block0;
                ++j;
            }
        ...
        for (j = 0; j < length; ++j) {
            this.ce64s.addElement(ces[start2 + j]);
        }
        return Collation.makeCE32FromTagIndexAndLength(6, i, length);
    }

    protected int encodeExpansion32(int[] newCE32s, int start2, int length) {
        int j;
        int i;
        int first = newCE32s[start2];
        ...
        block0: for (i = 0; i <= ce32sMax; ++i) {
            ...
            j = 1;
            while (j != length) {
                if (this.ce32s.elementAti(i + j) != newCE32s[start2 + j]) continue block0;
                ++j;
            }
        ...
        for (j = 0; j < length; ++j) {
            this.ce32s.addElement(newCE32s[start2 + j]);
        }
        return Collation.makeCE32FromTagIndexAndLength(5, i, length);
    }
    ...
    protected int copyFromBaseCE32(int c, int ce32, boolean withContext) {
        ...
        switch (Collation.tagFromCE32(ce32)) {
            ...
            case 8: {
                ...
                ConditionalCE32 head5 = new ConditionalCE32("", 0);
                StringBuilder context = new StringBuilder("\u0000");
                if (Collation.isContractionCE32(ce32)) {
                    index = this.copyContractionsFromBaseCE32(context, c, ce32, head5);
                } else {
                    ce32 = this.copyFromBaseCE32(c, ce32, true);
                    head5.next = index = this.addConditionalCE32(context.toString(), ce32);
                }
                ...
                ce32 = CollationDataBuilder.makeBuilderContextCE32(head5.next);
                this.contextChars.add(c);
                break;
            }
            case 9: {
                ...
                ConditionalCE32 head6 = new ConditionalCE32("", 0);
                StringBuilder context = new StringBuilder("\u0000");
                this.copyContractionsFromBaseCE32(context, c, ce32, head6);
                ce32 = CollationDataBuilder.makeBuilderContextCE32(head6.next);
                this.contextChars.add(c);
                break;
            }
            case 12: {
    ...
    private static void enumRangeForCopy(int start2, int end2, int value2, CopyHelper helper) {
        if (value2 != -1 && value2 != 192) {
            helper.copyRangeCE32(start2, end2, value2);
        }
    }
    ...
    protected int buildContext(ConditionalCE32 head5) {
        assert (!head5.hasContext());
        assert (head5.next >= 0);
        CharsTrieBuilder prefixBuilder = new CharsTrieBuilder();
        CharsTrieBuilder contractionBuilder = new CharsTrieBuilder();
        ConditionalCE32 cond = head5;
        while (true) {
            int ce32;
            assert (cond == head5 || cond.hasContext());
        ...
        assert (head5.defaultCE32 != 1);
        int index = this.addContextTrie(head5.defaultCE32, prefixBuilder);
        if (index > 524287) {
            throw new IndexOutOfBoundsException("too many context-sensitive mappings");
        }
    ...
    protected int getCEs(CharSequence s, int start2, long[] ces, int cesLength) {
        ...
        return this.collIter.fetchCEs(s, start2, ces, cesLength);
    }
    ...
    extends CollationIterator {
        ...
        int fetchCEs(CharSequence str, int start2, long[] ces, int cesLength) {
    ...
    private static final class CopyHelper {
        ...
        void copyRangeCE32(int start2, int end2, int ce32) {
            ce32 = this.copyCE32(ce32);
            this.dest.trie.setRange(start2, end2, ce32, true);
            if (CollationDataBuilder.isBuilderContextCE32(ce32)) {
                this.dest.contextChars.add(start2, end2);
            }
        }
