...
import com.cobblemon.mod.relocations.ibm.icu.impl.Trie2Writable;
...
import com.cobblemon.mod.relocations.ibm.icu.util.CharsTrie;
import com.cobblemon.mod.relocations.ibm.icu.util.CharsTrieBuilder;
import com.cobblemon.mod.relocations.ibm.icu.util.StringTrieBuilder;
...
final class CollationDataBuilder {
   ...
   protected ArrayList<CollationDataBuilder.ConditionalCE32> conditionalCE32s;
      ...
      if (cesLength < 0 || cesLength > 31) {
         ...
         if (cesLength == 2) {
            ...
            if ((ce0 & 72057594037862655L) == 83886080L && (ce1 & -4278190081L) == 1280L && p0 != 0L) {
               return (int)p0 | ((int)ce0 & 0xFF00) << 8 | (int)ce1 >> 16 & 0xFF00 | 192 | 4;
            }
         }
      ...
      if (s.length() == 0) {
         ...
         if (!hasContext) {
            if (!isBuilderContextCE32(oldCE32)) {
               ...
               CollationDataBuilder.ConditionalCE32 cond = this.getConditionalCE32ForCE32(oldCE32);
               cond.builtCE32 = 1;
               cond.ce32 = ce32;
            }
         } else {
            ...
            while (true) {
               ...
               CollationDataBuilder.ConditionalCE32 nextCond = this.getConditionalCE32(next);
   ...
   void copyFrom(CollationDataBuilder src, CollationDataBuilder.CEModifier modifier) {
      if (!this.isMutable()) {
         ...
         CollationDataBuilder.CopyHelper helper = new CollationDataBuilder.CopyHelper(src, this, modifier);
   ...
   protected int addConditionalCE32(String context, int ce32) {
      ...
      if (index > 524287) {
         ...
         CollationDataBuilder.ConditionalCE32 cond = new CollationDataBuilder.ConditionalCE32(context, ce32);
   ...
   protected static int encodeOneCEAsCE32(long ce) {
      ...
      if ((ce & 281470698455295L) == 0L) {
         ...
         return p == 0L && (t & 0xFF) == 0 ? Collation.makeLongSecondaryCE32(lower32) : 1;
      }
   }
   ...
   protected int encodeExpansion(long[] ces, int start, int length) {
      ...
      int ix = this.ce64s.size();
      if (ix > 524287) {
         throw new IndexOutOfBoundsException("too many mappings");
      } else {
         for (int jx = 0; jx < length; jx++) {
            this.ce64s.addElement(ces[start + jx]);
         }

         return Collation.makeCE32FromTagIndexAndLength(6, ix, length);
      }
   }
   ...
   protected int encodeExpansion32(int[] newCE32s, int start, int length) {
      ...
      int ix = this.ce32s.size();
      if (ix > 524287) {
         throw new IndexOutOfBoundsException("too many mappings");
      } else {
         for (int jx = 0; jx < length; jx++) {
            this.ce32s.addElement(newCE32s[start + jx]);
         }

         return Collation.makeCE32FromTagIndexAndLength(5, ix, length);
      }
   }
      ...
      if (!Collation.isSpecialCE32(ce32)) {
         ...
         switch (Collation.tagFromCE32(ce32)) {
            ...
            case 8: {
               ...
               CollationDataBuilder.ConditionalCE32 head = new CollationDataBuilder.ConditionalCE32("", 0);
               ...
               CollationDataBuilder.ConditionalCE32 cond = this.getConditionalCE32(index);
            ...
            case 9:
               if (!withContext) {
                  int indexx = Collation.indexFromCE32(ce32);
                  ce32 = this.base.getCE32FromContexts(indexx);
                  return this.copyFromBaseCE32(c, ce32, false);
               }
   ...
   protected boolean getJamoCE32s(int[] jamoCE32s) {
      ...
      if (anyJamoAssigned && needToCopyFromBase) {
         for (int j = 0; j < 67; j++) {
            if (jamoCE32s[j] == 192) {
               int jamox = jamoCpFromIndex(j);
               jamoCE32s[j] = this.copyFromBaseCE32(jamox, this.base.getCE32(jamox), true);
            }
         }
      }
      ...
      if (!this.isMutable()) {
         ...
         if (this.getJamoCE32s(jamoCE32s)) {
            ...
            for (int ix = 0; ix < 19; ix++) {
               int ce32 = hangulCE32;
               if (!isAnyJamoVTSpecial && !Collation.isSpecialCE32(jamoCE32s[ix])) {
                  ce32 = hangulCE32 | 256;
               }
   ...
   protected void buildContexts() {
      ...
      while (iter.next()) {
         ...
         CollationDataBuilder.ConditionalCE32 cond = this.getConditionalCE32ForCE32(ce32);
   ...
   protected int buildContext(CollationDataBuilder.ConditionalCE32 head) {
      ...
      CollationDataBuilder.ConditionalCE32 cond = head;

      while ($assertionsDisabled || cond == head || cond.hasContext()) {
         int prefixLength = cond.prefixLength();
         StringBuilder prefix = new StringBuilder().append(cond.context, 0, prefixLength + 1);
         String prefixString = prefix.toString();
         CollationDataBuilder.ConditionalCE32 firstCond = cond;
         CollationDataBuilder.ConditionalCE32 lastCond = cond;
   ...
   protected int getCEs(CharSequence s, int start, long[] ces, int cesLength) {
      if (this.collIter == null) {
         this.collIter = new CollationDataBuilder.DataBuilderCollationIterator(this, new CollationData(this.nfcImpl));
         if (this.collIter == null) {
            return 0;
         }
      }
   ...
   private static final class CopyHelper {
      ...
      CopyHelper(CollationDataBuilder s, CollationDataBuilder d, CollationDataBuilder.CEModifier m) {
         this.src = s;
         this.dest = d;
         this.modifier = m;
      }
         ...
         if (!Collation.isSpecialCE32(ce32)) {
            ...
            if (tag == 5) {
               ...
               for (int ix = 0; ix < length; ix++) {
                  long srcCE = srcCEs[srcIndex + ix];
                  long ce = this.modifier.modifyCE(srcCE);
                  if (ce == 4311744768L) {
                     if (isModified) {
                        this.modifiedCEs[ix] = srcCE;
                     }
                  } else {
                     if (!isModified) {
                        for (int j = 0; j < ix; j++) {
               ...
               CollationDataBuilder.ConditionalCE32 cond = this.src.getConditionalCE32ForCE32(ce32);
               ...
               while (cond.next >= 0) {
                  ...
                  CollationDataBuilder.ConditionalCE32 prevDestCond = this.dest.getConditionalCE32(destIndex);
   ...
   private static final class DataBuilderCollationIterator extends CollationIterator {
      ...
      protected int getCE32FromBuilderData(int ce32) {
         ...
         if ((ce32 & 256) != 0) {
            ...
            CollationDataBuilder.ConditionalCE32 cond = this.builder.getConditionalCE32ForCE32(ce32);
            if (cond.builtCE32 == 1) {
               try {
