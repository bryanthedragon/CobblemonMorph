...
public final class ICUResourceBundleReader {
   ...
   private static final ICUResourceBundleReader.IsAcceptable IS_ACCEPTABLE = new ICUResourceBundleReader.IsAcceptable();
   ...
   private static final ICUResourceBundleReader.Array EMPTY_ARRAY = new ICUResourceBundleReader.Array();
   private static final ICUResourceBundleReader.Table EMPTY_TABLE = new ICUResourceBundleReader.Table();
   ...
   static ICUResourceBundleReader getReader(String baseName, String localeID, ClassLoader root) {
      ICUResourceBundleReader.ReaderCacheKey info = new ICUResourceBundleReader.ReaderCacheKey(baseName, localeID);
      ...
      if (indexLength <= 4) {
         ...
         if (dataLength >= 1 + indexLength << 2 && dataLength >= (bundleTop = this.getIndexesInt(3)) << 2) {
            ...
            if (indexLength > 5) {
               ...
               this.poolStringIndexLimit |= (att & 61440) << 12;
            ...
            if (keysTop > keysBottom) {
               if (this.isPoolBundle) {
                  ...
                  ((Buffer)this.bytes).position(keysBottom << 2);
               } else {
            ...
            if (indexLength > 6) {
               ...
               if (_16BitTop > keysTop) {
                  ...
                  ((Buffer)this.bytes).position(keysTop << 2);
                  this.b16BitUnits = this.bytes.asCharBuffer();
                  ((Buffer)this.b16BitUnits).limit(num16BitUnits);
            ...
            if (!this.isPoolBundle || this.b16BitUnits.length() > 1) {
               this.resourceCache = new ICUResourceBundleReader.ResourceCache(maxOffset);
            }
   ...
   private String getKey16String(int keyOffset) {
      return keyOffset < this.localKeyLimit
         ? makeKeyStringFromBytes(this.keyBytes, keyOffset)
         : makeKeyStringFromBytes(this.poolBundleReader.keyBytes, keyOffset - this.localKeyLimit);
   }

   private String getKey32String(int keyOffset) {
      return keyOffset >= 0 ? makeKeyStringFromBytes(this.keyBytes, keyOffset) : makeKeyStringFromBytes(this.poolBundleReader.keyBytes, keyOffset & 2147483647);
   }
   ...
   private void setKeyFromKey32(int keyOffset, UResource.Key key) {
      if (keyOffset >= 0) {
         ...
         key.setBytes(this.poolBundleReader.keyBytes, keyOffset & 2147483647);
      }
   }

   private int compareKeys(CharSequence key, char keyOffset) {
      return keyOffset < this.localKeyLimit
         ? ICUBinary.compareKeys(key, this.keyBytes, keyOffset)
         : ICUBinary.compareKeys(key, this.poolBundleReader.keyBytes, keyOffset - this.localKeyLimit);
   }

   private int compareKeys32(CharSequence key, int keyOffset) {
      return keyOffset >= 0
         ? ICUBinary.compareKeys(key, this.keyBytes, keyOffset)
         : ICUBinary.compareKeys(key, this.poolBundleReader.keyBytes, keyOffset & 2147483647);
   }
      ...
      if (offset != 0) {
         if (res == offset) {
            ...
            return this.getInt(offset) == 3
               && this.bytes.getChar(offset + 4) == 8709
               && this.bytes.getChar(offset + 6) == 8709
               && this.bytes.getChar(offset + 8) == 8709;
         }
   ...
   private boolean isStringV2NoInheritanceMarker(int offset) {
      ...
      if (first == 8709) {
         ...
         return first != 56323
            ? false
            : this.b16BitUnits.charAt(offset + 1) == 8709 && this.b16BitUnits.charAt(offset + 2) == 8709 && this.b16BitUnits.charAt(offset + 3) == 8709;
      }
   }
   ...
   ICUResourceBundleReader.Array getArray(int res) {
      ...
      if (!URES_IS_ARRAY(type)) {
         ...
         if (offset == 0) {
            ...
            Object value = this.resourceCache.get(res);
            if (value != null) {
               return (ICUResourceBundleReader.Array)value;
            } else {
               ICUResourceBundleReader.Array array = (ICUResourceBundleReader.Array)(type == 8
                  ? new ICUResourceBundleReader.Array32(this, offset)
                  : new ICUResourceBundleReader.Array16(this, offset));
               return (ICUResourceBundleReader.Array)this.resourceCache.putIfAbsent(res, array, 0);
            }
         }
      }
   }
   ...
   ICUResourceBundleReader.Table getTable(int res) {
      ...
      if (!URES_IS_TABLE(type)) {
         ...
         if (offset == 0) {
            ...
            Object value = this.resourceCache.get(res);
            if (value != null) {
               return (ICUResourceBundleReader.Table)value;
            } else {
               ICUResourceBundleReader.Table table;
               int size;
               if (type == 2) {
                  table = new ICUResourceBundleReader.Table1632(this, offset);
                  size = table.getSize() * 2;
               } else if (type == 5) {
                  table = new ICUResourceBundleReader.Table16(this, offset);
                  size = table.getSize() * 2;
               } else {
                  table = new ICUResourceBundleReader.Table32(this, offset);
                  size = table.getSize() * 4;
               }

               return (ICUResourceBundleReader.Table)this.resourceCache.putIfAbsent(res, table, size);
            }
         }
      }
   }
   ...
   static class Array extends ICUResourceBundleReader.Container implements UResource.Array {
      @Override
      public boolean getValue(int i, UResource.Value value) {
         if (0 <= i && i < this.size) {
            ICUResourceBundleReader.ReaderValue readerValue = (ICUResourceBundleReader.ReaderValue)value;
   ...
   private static final class Array16 extends ICUResourceBundleReader.Array {
      @Override
   ...
   private static final class Array32 extends ICUResourceBundleReader.Array {
      @Override
      int getContainerResource(ICUResourceBundleReader reader, int index) {
   ...
   private static class ReaderCache extends SoftCache<ICUResourceBundleReader.ReaderCacheKey, ICUResourceBundleReader, ClassLoader> {
      private ReaderCache() {
      }

      protected ICUResourceBundleReader createInstance(ICUResourceBundleReader.ReaderCacheKey key, ClassLoader loader) {
         String fullName = ICUResourceBundleReader.getFullName(key.baseName, key.localeID);

         try {
            ByteBuffer inBytes;
            if (key.baseName != null && key.baseName.startsWith("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b")) {
               String itemPath = fullName.substring("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b".length() + 1);
   ...
   private static class ReaderCacheKey {
      ...
      public boolean equals(Object obj) {
         if (this == obj) {
            return true;
         } else if (!(obj instanceof ICUResourceBundleReader.ReaderCacheKey)) {
            return false;
         } else {
            ICUResourceBundleReader.ReaderCacheKey info = (ICUResourceBundleReader.ReaderCacheKey)obj;
   ...
   static class ReaderValue extends UResource.Value {
      ...
      public UResource.Array getArray() {
         ICUResourceBundleReader.Array array = this.reader.getArray(this.res);
         if (array == null) {
            throw new UResourceTypeMismatchException("");
         } else {
            return array;
      ...
      public UResource.Table getTable() {
         ICUResourceBundleReader.Table table = this.reader.getTable(this.res);
         if (table == null) {
            throw new UResourceTypeMismatchException("");
         } else {
            return table;
      ...
      private String[] getStringArray(ICUResourceBundleReader.Array array) {
   ...
   private static final class ResourceCache {
      ...
      synchronized Object putIfAbsent(int res, Object item, int size) {
         if (this.length >= 0) {
            ...
            this.rootLevel = new ICUResourceBundleReader.ResourceCache.Level(this.levelBitsList, 0);
      ...
      private static final class Level {
         ...
         Object get(int key) {
            ...
            if (k == key) {
               ...
               if (k == 0) {
                  ICUResourceBundleReader.ResourceCache.Level level = (ICUResourceBundleReader.ResourceCache.Level)this.values[index];
                  if (level != null) {
         ...
         Object putIfAbsent(int key, Object item, int size) {
            ...
            if (k == key) {
               ...
               ICUResourceBundleReader.ResourceCache.Level level = new ICUResourceBundleReader.ResourceCache.Level(
                  this.levelBitsList >> 4, this.shift + (this.levelBitsList & 15)
               );
   ...
   static class Table extends ICUResourceBundleReader.Container implements UResource.Table {
