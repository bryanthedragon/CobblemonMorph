...
public final class CollationElementIterator {
    ...
    static int getMaxExpansion(Map<Integer, Integer> maxExpansions, int order) {
        ...
        if (maxExpansions != null && (max2 = maxExpansions.get(order)) != null) {
            return max2;
        }
        if ((order & 0xC0) == 192) {
            return 2;
    ...
    implements ContractionsAndExpansions.CESink {
        ...
        @Override
        public void handleExpansion(long[] ces, int start2, int length) {
            Integer oldCount;
            if (length <= 1) {
                return;
            }
            ...
            for (int i = 0; i < length; ++i) {
                count += CollationElementIterator.ceNeedsTwoParts(ces[start2 + i]) ? 2 : 1;
            }
            long ce = ces[start2 + length - 1];
            long p = ce >>> 32;
            int lower32 = (int)ce;
