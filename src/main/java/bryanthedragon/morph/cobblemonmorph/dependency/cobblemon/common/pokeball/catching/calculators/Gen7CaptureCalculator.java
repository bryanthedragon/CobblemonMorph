...
PokedexProgressCaptureMultiplierProvider {
    ...
    public CaptureContext processCapture(@NotNull LivingEntity thrower, @NotNull EmptyPokeBallEntity pokeBallEntity, @NotNull PokemonEntity target) {
        ...
        if (Intrinsics.areEqual((Object)pokeBall, (Object)PokeBalls.INSTANCE.getHEAVY_BALL())) {
        ...
        float modifiedCatchRate = ((Number)pokeBall.getCatchRateModifier().behavior(thrower, pokemon).getMutator().invoke((Object)Float.valueOf((3.0f * (float)pokemon.getHp() - 2.0f * (float)pokemon.getCurrentHealth()) * 4096.0f * (float)darkGrass * rate), (Object)Float.valueOf(ballBonus))).floatValue() / (3.0f * (float)pokemon.getHp()) * bonusStatus;
