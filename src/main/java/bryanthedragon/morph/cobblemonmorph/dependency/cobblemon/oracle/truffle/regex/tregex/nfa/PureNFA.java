...
public final class PureNFA implements StateIndex<PureNFAState> {
   ...
   public PureNFA(
      RegexASTSubtreeRootNode astSubRoot, PureNFAState[] states, Counter.ThresholdCounter stateIDCounter, Counter.ThresholdCounter transitionIDCounter
   ) {
   ...
   public RegexASTSubtreeRootNode getASTSubtree(RegexAST ast) {
      return this.isRoot() ? ast.getRoot().getSubTreeParent() : ast.getSubtrees().get(this.globalSubTreeId);
   }
   ...
   public int getNumberOfEntryPoints() {
      return this.getDummyInitialState().getSuccessors().length / 2;
   }

   public PureNFATransition getAnchoredEntry() {
      return this.getDummyInitialState().getSuccessors()[0];
   }

   public PureNFATransition getUnAnchoredEntry() {
      return this.getDummyInitialState().getSuccessors()[1];
   }
   ...
   public PureNFATransition getReverseAnchoredEntry() {
      return this.getDummyInitialState().getPredecessors()[0];
   }

   public PureNFATransition getReverseUnAnchoredEntry() {
      return this.getDummyInitialState().getPredecessors()[1];
   }
   ...
   public CodePointSet getMergedInitialStateCharSet(RegexAST ast, CompilationBuffer compilationBuffer) {
      ...
      return mergeInitialStateMatcher(ast, this, acc) ? acc.toCodePointSet() : null;
   }
      ...
      return Json.obj(
         Json.prop(
            ...
            Arrays.stream(this.states)
               .map(
                  x -> (JsonConvertible)(x != null && x != this.getDummyInitialState() && (!x.isAnchoredFinalState() || x.hasPredecessors())
                     ? x.toJson(ast)
                     : Json.nullValue())
               )
         ),
         Json.prop(
            "transitions",
            Arrays.stream(this.transitions)
               .map(x -> (JsonConvertible)(x != null && x.getSource() != this.getDummyInitialState() ? x.toJson(ast) : Json.nullValue()))
         ),
         Json.prop("anchoredEntry", Json.array(Json.val(this.getAnchoredInitialState().getId()))),
         Json.prop("unAnchoredEntry", Json.array(Json.val(this.getUnAnchoredInitialState().getId())))
      );
   }
}
