...
implements SCSU {
    ...
    public static byte[] compress(char[] buffer, int start2, int limit) {
        UnicodeCompressor comp = new UnicodeCompressor();
        int len = Math.max(4, 3 * (limit - start2) + 1);
        byte[] temp = new byte[len];
        int byteCount = comp.compress(buffer, start2, limit, null, temp, 0, len);
