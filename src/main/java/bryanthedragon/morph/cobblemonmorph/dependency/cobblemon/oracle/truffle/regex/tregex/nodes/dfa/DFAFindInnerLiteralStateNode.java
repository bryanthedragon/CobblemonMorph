...
extends DFAAbstractStateNode {
    ...
    int executeInnerLiteralSearch(TRegexDFAExecutorLocals locals, TRegexDFAExecutorNode executor, boolean tString) {
        return executor.getIndexOfStringNode().execute(locals.getInput(), locals.getIndex(), executor.getMaxIndex(locals), this.innerLiteral.getLiteralContent(tString), this.innerLiteral.getMaskContent(tString), executor.getEncoding());
    }
