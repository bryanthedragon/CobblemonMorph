...
public final class UPropertyAliases {
    ...
    private int findProperty(int property) {
        ...
        for (int numRanges = this.valueMaps[0]; numRanges > 0; --numRanges) {
            int start2 = this.valueMaps[i];
            int limit = this.valueMaps[i + 1];
            i += 2;
            if (property < start2) break;
            if (property < limit) {
                return i + (property - start2) * 2;
            }
            i += (limit - start2) * 2;
        }
        return 0;
    ...
    private int findPropertyValueNameGroup(int valueMapIndex, int value2) {
        if (valueMapIndex == 0) {
            return 0;
        }
        int n = ++valueMapIndex;
        ...
        if (numRanges < 16) {
            while (numRanges > 0) {
                int start2 = this.valueMaps[valueMapIndex];
                int limit = this.valueMaps[valueMapIndex + 1];
                valueMapIndex += 2;
                if (value2 >= start2) {
                    if (value2 < limit) {
                        return this.valueMaps[valueMapIndex + value2 - start2];
                    }
                    valueMapIndex += limit - start2;
                    --numRanges;
                    continue;
                }
                break;
            ...
            while (value2 >= (v = this.valueMaps[valueMapIndex])) {
                if (value2 == v) {
    ...
    public String getPropertyValueName(int property, int value2, int nameChoice) {
        int valueMapIndex = this.findProperty(property);
        if (valueMapIndex == 0) {
            throw new IllegalArgumentException("Invalid property enum " + property + " (0x" + Integer.toHexString(property) + ")");
        }
        int nameGroupOffset = this.findPropertyValueNameGroup(this.valueMaps[valueMapIndex + 1], value2);
