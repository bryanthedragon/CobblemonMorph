...
public class PatternStringUtils {
    ...
    public static String propertiesToPatternString(DecimalFormatProperties properties2) {
        StringBuilder sb = new StringBuilder();
        int dosMax = 100;
        int grouping1 = Math.max(0, Math.min(properties2.getGroupingSize(), dosMax));
        int grouping2 = Math.max(0, Math.min(properties2.getSecondaryGroupingSize(), dosMax));
        boolean useGrouping = properties2.getGroupingUsed();
        int paddingWidth = Math.min(properties2.getFormatWidth(), dosMax);
        Padder.PadPosition paddingLocation = properties2.getPadPosition();
        String paddingString = properties2.getPadString();
        int minInt = Math.max(0, Math.min(properties2.getMinimumIntegerDigits(), dosMax));
        int maxInt = Math.min(properties2.getMaximumIntegerDigits(), dosMax);
        int minFrac = Math.max(0, Math.min(properties2.getMinimumFractionDigits(), dosMax));
        int maxFrac = Math.min(properties2.getMaximumFractionDigits(), dosMax);
        int minSig = Math.min(properties2.getMinimumSignificantDigits(), dosMax);
        int maxSig = Math.min(properties2.getMaximumSignificantDigits(), dosMax);
        boolean alwaysShowDecimal = properties2.getDecimalSeparatorAlwaysShown();
        boolean currencyAsDecimal = properties2.getCurrencyAsDecimal();
        int exponentDigits = Math.min(properties2.getMinimumExponentDigits(), dosMax);
        boolean exponentShowPlusSign = properties2.getExponentSignAlwaysShown();
        AffixPatternProvider affixes = PropertiesAffixPatternProvider.forProperties(properties2);
        ...
        BigDecimal roundingInterval = properties2.getRoundingIncrement();
