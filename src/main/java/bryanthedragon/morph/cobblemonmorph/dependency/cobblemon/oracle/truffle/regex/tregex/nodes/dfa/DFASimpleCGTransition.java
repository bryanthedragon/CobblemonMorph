...
public final class DFASimpleCGTransition implements JsonConvertible {
   ...
   public static DFASimpleCGTransition create(NFAStateTransition t, boolean fullClear) {
      if (t != null && (fullClear || !t.getGroupBoundaries().isEmpty())) {
         ...
         return new DFASimpleCGTransition(
            t.getGroupBoundaries().isEmpty() ? EMPTY_ARRAY : t.getGroupBoundaries().updatesToByteArray(),
            fullClear ? FULL_CLEAR_ARRAY : t.getGroupBoundaries().clearsToByteArray(),
            t.getGroupBoundaries().getLastGroup()
         );
      } else {
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         return true;
      } else if (!(obj instanceof DFASimpleCGTransition)) {
         return false;
      } else {
   ...
   public JsonValue toJson() {
      return Json.obj(
         Json.prop("indexUpdates", DFACaptureGroupPartialTransition.IndexOperation.groupBoundariesToJsonObject(this.indexUpdates)),
         Json.prop("indexClears", DFACaptureGroupPartialTransition.IndexOperation.groupBoundariesToJsonObject(this.indexClears))
      );
   }
}
