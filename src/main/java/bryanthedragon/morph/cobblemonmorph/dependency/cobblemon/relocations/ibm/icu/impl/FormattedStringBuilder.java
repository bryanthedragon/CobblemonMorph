...
Appendable {
    ...
    public int insert(int index, CharSequence sequence, int start2, int end2, Object field) {
        int count = end2 - start2;
        int position = this.prepareForInsert(index, count);
        for (int i = 0; i < count; ++i) {
            this.chars[position + i] = sequence.charAt(start2 + i);
    ...
    public CharSequence subSequence(int start2, int end2) {
        assert (start2 >= 0);
        assert (end2 <= this.length);
        assert (end2 >= start2);
        FormattedStringBuilder other = new FormattedStringBuilder(this);
        other.zero = this.zero + start2;
        other.length = end2 - start2;
        return other;
    }

    public String subString(int start2, int end2) {
        if (start2 < 0 || end2 > this.length || end2 < start2) {
            throw new IndexOutOfBoundsException();
        }
        return new String(this.chars, start2 + this.zero, end2 - start2);
    }
    ...
    @Override
    public Appendable append(CharSequence csq, int start2, int end2) {
        assert (this.appendableField != null);
        this.insert(this.length - this.appendOffset, csq, start2, end2, this.appendableField);
        return this;
    }
