...
import com.cobblemon.mod.relocations.ibm.icu.util.RangeDateRule;
import com.cobblemon.mod.relocations.ibm.icu.util.SimpleDateRule;
...
extends Holiday {
    ...
    private static DateRule rangeRule(int startYear, int endYear, DateRule rule) {
        ...
        if (startYear != 0) {
            GregorianCalendar start2 = new GregorianCalendar(startYear, 0, 1);
            rangeRule.add(start2.getTime(), rule);
        } else {
            rangeRule.add(rule);
        }
        if (endYear != 0) {
            Date end2 = new GregorianCalendar(endYear, 11, 31).getTime();
            rangeRule.add(end2, null);
        }
        return rangeRule;
    }
}
