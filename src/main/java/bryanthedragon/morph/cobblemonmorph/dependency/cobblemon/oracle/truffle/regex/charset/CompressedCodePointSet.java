...
public final class CompressedCodePointSet {
   ...
   public static CompressedCodePointSet create(ImmutableSortedListOfIntRanges cps, CompilationBuffer compilationBuffer) {
      ...
      if (size < 3) {
         ...
         for (int i = 0; i < size; i++) {
            ...
            if (BitSets.highByte(cps.getHi(i)) != curPlane) {
               if (lowestOCP != i) {
         ...
         if (bitSets.isEmpty()) {
            ...
            return new CompressedCodePointSet(ranges.toArray(), bitSets.toArray(NO_BITSETS));
         }
      }
   }
   ...
   private static void processRanges(
      ImmutableSortedListOfIntRanges cps, IntRangesBuffer ranges, ObjectArrayBuffer<long[]> bitSets, int iMin, int iMax, int iMaxBS, int curPlane
   ) {
      if (iMaxBS - iMin >= 3) {
   ...
   private static void addRanges(ImmutableSortedListOfIntRanges cps, ObjectArrayBuffer<long[]> bitSets, IntRangesBuffer ranges, int iMinArg, int iMax) {
      if (iMinArg != iMax) {
         int iMin = iMinArg;
         if (!bitSets.isEmpty() && bitSets.peek() != null && BitSets.highByte(cps.getLo(iMinArg)) == BitSets.highByte(ranges.getMax())) {
            ranges.appendRangeAllowAdjacent(ranges.getMax() + 1, cps.getHi(iMinArg));
            iMin = iMinArg + 1;
         }
   ...
   private static void addBitSet(
      ImmutableSortedListOfIntRanges cps, ObjectArrayBuffer<long[]> bitSets, IntRangesBuffer ranges, int iMin, int iMax, int curPlane
   ) {
      assert iMax - iMin > 1;
   ...
   public boolean equals(Object obj) {
      if (this == obj) {
         ...
         return !(obj instanceof CompressedCodePointSet)
            ? false
            : Arrays.equals(this.ranges, ((CompressedCodePointSet)obj).ranges) && Arrays.deepEquals(this.bitSets, ((CompressedCodePointSet)obj).bitSets);
      }
   }
      ...
      if (this.bitSets == null) {
         ...
         for (int i = 0; i < this.ranges.length; i += 2) {
            if (this.bitSets[i / 2] == null) {
               ...
               sb.append("[range: ")
                  .append(Range.toString(this.ranges[i], this.ranges[i + 1]))
                  .append(", bs: ")
