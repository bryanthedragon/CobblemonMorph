...
import com.cobblemon.mod.relocations.ibm.icu.impl.UCharacterNameReader;
...
public final class UCharacterName {
    ...
    public int getGroupLengths(int index, char[] offsets, char[] lengths) {
        ...
        while (i < 32) {
            ...
            for (int shift2 = 4; shift2 >= 0; shift2 -= 4) {
                n = (byte)(b >> shift2 & 0xF);
    ...
    public void getCharNameCharacters(UnicodeSet set2) {
        this.convert(this.m_nameSet_, set2);
    }

    public void getISOCommentCharacters(UnicodeSet set2) {
        this.convert(this.m_ISOCommentSet_, set2);
    }
    ...
    private static void add(int[] set2, char ch) {
        int n = ch >>> 5;
        set2[n] = set2[n] | 1 << (ch & 0x1F);
    }

    private static boolean contains(int[] set2, char ch) {
        return (set2[ch >>> 5] & 1 << (ch & 0x1F)) != 0;
    }

    private static int add(int[] set2, String str) {
        int result = str.length();
        for (int i = result - 1; i >= 0; --i) {
            UCharacterName.add(set2, str.charAt(i));
        }
        return result;
    }

    private static int add(int[] set2, StringBuffer str) {
        int result = str.length();
        for (int i = result - 1; i >= 0; --i) {
            UCharacterName.add(set2, str.charAt(i));
        }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private int[] addGroupName(int offset, int length, byte[] tokenlength, int[] set2) {
        int resultnlength = 0;
        int resultplength = 0;
        while (resultplength < length) {
            ...
            if (tlength == 0) {
                ...
                synchronized (stringBuffer) {
                    this.m_utilStringBuffer_.setLength(0);
                    UCharacterUtility.getNullTermByteSubString(this.m_utilStringBuffer_, this.m_tokenstring_, token);
                    tlength = (byte)UCharacterName.add(set2, this.m_utilStringBuffer_);
                }
    ...
    private void convert(int[] set2, UnicodeSet uset) {
        uset.clear();
        if (!this.initNameSetsLengths()) {
            return;
        }
        for (char c = '\u00ff'; c > '\u0000'; c = (char)(c - '\u0001')) {
            if (!UCharacterName.contains(set2, c)) continue;
            uset.add(c);
        }
    }
    ...
    static final class AlgorithmName {
         ...
         * WARNING - Removed try catching itself - possible behaviour change.
         */
        int add(int[] set2, int maxlength) {
            int length = UCharacterName.add(set2, this.m_prefix_);
            switch (this.m_type_) {
                case 0: {
                    ...
                    for (int i = this.m_variant_ - 1; i > 0; --i) {
                        ...
                        for (int factor = this.m_factor_[i]; factor > 0; --factor) {
                            ...
                            synchronized (stringBuffer) {
                                ...
                                UCharacterName.add(set2, this.m_utilStringBuffer_);
