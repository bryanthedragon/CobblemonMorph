...
public abstract class JSArrayElementIndexNode extends JavaScriptBaseNode {
   ...
   protected static ScriptArray getArrayTypeIfArray(JSDynamicObject object, boolean isArray) {
      return !isArray ? null : getArrayType(object);
   }

   protected final boolean isSuitableForEnumBasedProcessingUsingOwnKeys(Object object, long length) {
      return length > 10000L
         && !JSArrayBufferView.isJSArrayBufferView(object)
         && !JSProxy.isJSProxy(object)
         && (
            JSArray.isJSArray(object) && this.context.getArrayPrototypeNoElementsAssumption().isValid()
               || !JSDynamicObject.isJSDynamicObject(object)
   ...
   protected static final boolean isSuitableForEnumBasedProcessing(Object object, long length) {
      if (length > 10000L && JSDynamicObject.isJSDynamicObject(object)) {
         ...
         while (!JSArrayBufferView.isJSArrayBufferView(chainObject) && !JSProxy.isJSProxy(chainObject)) {
            chainObject = JSObject.getPrototype(chainObject);
            if (chainObject == Null.instance) {
               return true;
            }
         }
