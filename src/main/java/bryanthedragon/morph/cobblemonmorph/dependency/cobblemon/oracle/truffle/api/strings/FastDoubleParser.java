...
final class FastDoubleParser {
   ...
   static double parseDouble(Node location, AbstractTruffleString a, Object arrayA, int strideA, int off, int len, BranchProfile errorProfile) throws TruffleString.NumberFormatException {
      ...
      if (index == endIndex) {
         ...
         int ch = TStringOps.readValue(a, arrayA, strideA, index);
         boolean isNegative = ch == 45;
         if (isNegative || ch == 43) {
            index++;
            ch = index < endIndex ? TStringOps.readValue(a, arrayA, strideA, index) : 0;
            if (ch == 0) {
               errorProfile.enter();
               throw NumberConversion.numberFormatException(a, off, len, TruffleString.NumberFormatException.Reason.LONE_SIGN);
            }
         }
   ...
   private static int tryToParseEightDigits(long value) {
      long val = value - 3472328296227680304L;
      long l = (value + 5063812098665367110L | val) & -9187201950435737472L;
      if (l != 0L) {
         return -1;
      } else {
   ...
   private static double parseInfinity(
      Node location, AbstractTruffleString a, Object arrayA, int strideA, int curIndex, int endIndex, boolean negative, int off, BranchProfile errorProfile
   ) throws TruffleString.NumberFormatException {
      if (curIndex + 7 < endIndex && regionMatches(location, a, arrayA, strideA, curIndex, TStringConstants.getInfinity(a.encoding()))) {
         int index = skipWhitespace(a, arrayA, strideA, curIndex + 8, endIndex);
         if (index < endIndex) {
            errorProfile.enter();
            throw NumberConversion.numberFormatException(a, off, endIndex - off, TruffleString.NumberFormatException.Reason.INVALID_CODEPOINT);
         } else {
   ...
   private static double parseNaN(
      Node location, AbstractTruffleString a, Object arrayA, int strideA, int curIndex, int endIndex, int off, BranchProfile errorProfile
   ) throws TruffleString.NumberFormatException {
      if (curIndex + 2 < endIndex && regionMatches(location, a, arrayA, strideA, curIndex, TStringConstants.getNaN(a.encoding()))) {
         int index = skipWhitespace(a, arrayA, strideA, curIndex + 3, endIndex);
         if (index < endIndex) {
            errorProfile.enter();
            throw NumberConversion.numberFormatException(a, off, endIndex - off, TruffleString.NumberFormatException.Reason.INVALID_CODEPOINT);
         } else {
   ...
   private static double parseRestOfDecimalFloatLiteral(
      ...
      for (ch = 0; index < endIndex; index++) {
         ch = TStringOps.readValue(a, arrayA, strideA, index);
         if (isDigit(ch)) {
            digits = 10L * digits + ch - 48L;
         } else {
            if (ch != 46) {
               break;
            }
            ...
            if (virtualIndexOfPoint != -1) {
               errorProfile.enter();
               throw NumberConversion.numberFormatException(a, startIndex, len, TruffleString.NumberFormatException.Reason.MULTIPLE_DECIMAL_POINTS);
            }
            ...
            if (strideA == 0) {
               while (index < endIndex - 9) {
                  ...
                  digits = digits * 100000000L + parsed;
                  index += 8;
               }
            }
         }
      ...
      int digitCount;
      if (virtualIndexOfPoint == -1) {
         digitCount = index - curIndex;
         virtualIndexOfPoint = index;
      } else {
         digitCount = index - curIndex - 1;
         exponent = virtualIndexOfPoint - index + 1;
      }

      long expNumber = 0L;
      boolean hasExponent = ch == 101 || ch == 69;
      if (hasExponent) {
         index++;
         ch = index < endIndex ? TStringOps.readValue(a, arrayA, strideA, index) : 0;
         boolean negExp = ch == 45;
         if (negExp || ch == 43) {
            index++;
            ch = index < endIndex ? TStringOps.readValue(a, arrayA, strideA, index) : 0;
         }
         ...
         if (!isDigit(ch)) {
            errorProfile.enter();
            throw NumberConversion.numberFormatException(a, startIndex, len, TruffleString.NumberFormatException.Reason.INVALID_CODEPOINT);
         }

         do {
            if (expNumber < 10000000L) {
         ...
         exponent = (int)(exponent + expNumber);
      }
      ...
      if (index >= endIndex && (hasLeadingZero || digitCount != 0)) {
         ...
         if (digitCount > 19) {
            ...
            for (index = curIndex; index < indexAfterDigits; index++) {
               ch = TStringOps.readValue(a, arrayA, strideA, index);
               if (ch == 46) {
                  skipCountInTruncatedDigits++;
               } else {
                  if (Long.compareUnsigned(digits, 1000000000000000000L) >= 0) {
                     break;
                  }
         ...
         double result = FastDoubleParser.FastDoubleMath.decFloatLiteralToDouble(
            index, isNegative, digits, exponent, virtualIndexOfPoint, expNumber, isDigitsTruncated, skipCountInTruncatedDigits
         );
         return Double.isNaN(result) ? parseViaJavaString(location, a, arrayA, strideA, startIndex, len) : result;
      } else {
         errorProfile.enter();
         throw NumberConversion.numberFormatException(a, startIndex, len, TruffleString.NumberFormatException.Reason.EMPTY);
      }
   }
   ...
   private static double parseRestOfHexFloatingPointLiteral(
      ...
      if (curIndex >= endIndex) {
         errorProfile.enter();
         throw NumberConversion.numberFormatException(a, startIndex, len, TruffleString.NumberFormatException.Reason.MALFORMED_HEX_ESCAPE);
      } else {
         long digits = 0L;
         int exponent = 0;
         int virtualIndexOfPoint = -1;
         ...
         for (ch = 0; index < endIndex; index++) {
            ch = TStringOps.readValue(a, arrayA, strideA, index);
            int hexValue = ch > 127 ? -1 : CHAR_TO_HEX_MAP[ch];
            if (hexValue >= 0) {
         ...
         if (virtualIndexOfPoint == -1) {
            ...
            digitCount = index - curIndex - 1;
            exponent = Math.min(virtualIndexOfPoint - index + 1, 10000000) * 4;
         }

         long expNumber = 0L;
         boolean hasExponent = ch == 112 || ch == 80;
         if (hasExponent) {
            index++;
            ch = index < endIndex ? TStringOps.readValue(a, arrayA, strideA, index) : 0;
            boolean negExp = ch == 45;
         ...
         index = skipWhitespace(a, arrayA, strideA, index, endIndex);
         if (index >= endIndex && (digitCount != 0 || TStringOps.readValue(a, arrayA, strideA, virtualIndexOfPoint) == 46) && hasExponent) {
            int skipCountInTruncatedDigits = 0;
            boolean isDigitsTruncated;
            if (digitCount > 16) {
               digits = 0L;
            ...
            double d = FastDoubleParser.FastDoubleMath.hexFloatLiteralToDouble(
               index, isNegative, digits, (long)exponent, virtualIndexOfPoint, expNumber, isDigitsTruncated, skipCountInTruncatedDigits
            );
   ...
   private static int skipWhitespace(AbstractTruffleString a, Object arrayA, int strideA, int startIndex, int endIndex) {
      ...
      while (index < endIndex && TStringOps.readValue(a, arrayA, strideA, index) <= 32) {
         index++;
      }
   ...
   private static final class FastDoubleMath {
      ...
         2877803288514593168L,
         3597254110643241460L,
         9108253656731439729L,
         1080972517029761926L,
         5962901664714590312L,
         -6381430974388925822L,
         -8600080377420466543L,
         7696643601933968437L,
         397432465562684739L,
         -4363290727450709942L,
         8380944645968776284L,
         1252808770606194547L,
         -8440366555225904216L,
         7896285879677171346L,
         -3964700705685699529L,
         2133748077373825698L,
         2667185096717282123L,
         3333981370896602653L,
         6695424375237764562L,
         8369280469047205703L,
         -3373457468973156583L,
         -9025939945749304721L,
         7164319141522920715L,
         4343712908476262990L,
         7326506586225052273L,
         9158133232781315341L,
         2224294504121868368L,
         -7833187971778608078L,
         -568112927868484289L,
         3901544858591782542L,
         -4479063491021217767L,
         -5598829363776522209L,
         -2386850686293264857L,
         1628122660560806833L,
         -8205795374004271538L,
         -1033872180650563614L,
         -5904026244240592421L,
         -5995859411864064215L,
         1728547772024695539L,
         -2451001303396518480L,
         5385653213018257806L,
         -7102991539009341455L,
         -8878739423761676819L,
         3674159897003727796L,
         4592699871254659745L,
         1129188820640936778L,
         3011586022114279438L,
         8376168546070237202L,
         -7976533391121755114L,
         1932195658189984910L,
         -6808127464117294671L,
         -3898473311719230434L,
         9092669226243950738L,
         -2469221522477225289L,
         6136845133758244197L,
         -3082000819042179233L,
         -8464187042230111945L,
         3254824252494523781L,
         -7189106879045698445L,
         -8986383598807123057L,
         2602078556773259891L,
         -1359087822460813040L,
         -849429889038008150L,
         -5673473379724898091L,
         -2480155706228734710L,
         -3855940325606653146L,
         -208239388580928528L,
         -4871985254153548564L,
         -3044990783845967853L,
         5417133557047315992L,
         -2451955090545630818L,
         -3838314940804713213L,
         4425478360848884291L,
         920161932633717460L,
         2880944217109767365L,
         -5622191765467566602L,
         6807318348447705459L,
         -2662955059861265944L,
         -7940379843253970334L,
         8521269269642088699L,
         -6203421752542164323L,
         6080780864604458308L,
         -6234081974526590827L,
         5327070802775656541L,
         6658838503469570676L,
         8323548129336963345L,
         -4021154456019173717L,
         -5026443070023967147L,
         2940318199324816875L,
         8755227902219092403L,
         -2891023177508298209L,
         -8225464990312760665L,
         -5670145219463562927L,
         7985374283903742931L,
         758345818024902856L,
         -3663753745896259334L,
         -9207375118826243940L,
         -2285846861678029117L,
         1754377441329851508L,
         1096485900831157192L,
         -3241078642388441414L,
         5172023733869224041L,
         5538357842881958977L,
         -2300424733252327086L,
         6347841120289366950L,
         6273243709394548296L
      };
      ...
      static double decFloatLiteralToDouble(
         ...
         if (digits == 0L) {
            ...
            if (isDigitsTruncated) {
               long exponentOfTruncatedDigits = virtualIndexOfPoint - index + skipCountInTruncatedDigits + expNumber;
               if (-325L <= exponentOfTruncatedDigits && exponentOfTruncatedDigits <= 308L) {
                  double withoutRounding = tryDecToDoubleWithFastAlgorithm(isNegative, digits, (int)exponentOfTruncatedDigits);
                  double roundedUp = tryDecToDoubleWithFastAlgorithm(isNegative, digits + 1L, (int)exponentOfTruncatedDigits);
            ...
            } else if (-325 <= exponent && exponent <= 308) {
      ...
      private static FastDoubleParser.FastDoubleMath.Value128 fullMultiplication(long x, long y) {
         ...
         return new FastDoubleParser.FastDoubleMath.Value128(p11 + (middle >>> 32) + (p01 >>> 32), middle << 32 | p00 & 4294967295L);
      }

      private static double hexFloatLiteralToDouble(
         int index,
         boolean isNegative,
         long digits,
         long exponent,
         ...
         if (digits == 0L) {
            ...
            double outDouble;
            if (isDigitsTruncated) {
               long truncatedExponent = (virtualIndexOfPoint - index + skipCountInTruncatedDigits) * 4L + expNumber;
               if (-1022L <= truncatedExponent && truncatedExponent <= 1023L) {
                  double withoutRounding = tryHexToDoubleWithFastAlgorithm(isNegative, digits, (int)truncatedExponent);
                  double roundedUp = tryHexToDoubleWithFastAlgorithm(isNegative, digits + 1L, (int)truncatedExponent);
               ...
               outDouble = Double.NaN;
            } else if (-1022L <= exponent && exponent <= 1023L) {
      ...
      private static double tryDecToDoubleWithFastAlgorithm(boolean isNegative, long curDigits, int power) {
         if (curDigits == 0L || power < -399) {
            return isNegative ? -0.0 : 0.0;
         } else if (power > 380) {
            return isNegative ? Double.NEGATIVE_INFINITY : Double.POSITIVE_INFINITY;
         } else if (-22 <= power && power <= 22 && Long.compareUnsigned(curDigits, 9007199254740991L) <= 0) {
            double d = curDigits;
            if (power < 0) {
               d /= powerOfTen[-power];
            } else {
            ...
            long factorMantissa = MANTISSA_64[power - -325];
            long exponent = (217706 * power >> 16) + 1024 + 63;
            int lz = Long.numberOfLeadingZeros(curDigits);
            long digits = curDigits << lz;
            FastDoubleParser.FastDoubleMath.Value128 product = fullMultiplication(digits, factorMantissa);
            long lower = product.low;
            long upper = product.high;
            if ((upper & 511L) == 511L && Long.compareUnsigned(lower + digits, lower) < 0) {
               long factorMantissaLow = MANTISSA_128[power - -325];
               ...
               if (Long.compareUnsigned(productMiddle, lower) < 0) {
                  productHigh = upper + 1L;
               }

               if (productMiddle + 1L == 0L && (productHigh & 511L) == 511L && productLow + Long.compareUnsigned(digits, productLow) < 0L) {
                  return Double.NaN;
               }
            ...
            long mantissa = upper >>> (int)(upperbit + 9L);
            lz += (int)(1L ^ upperbit);
            if ((upper & 511L) != 511L && ((upper & 511L) != 0L || (mantissa & 3L) != 1L)) {
               mantissa = ++mantissa >>> 1;
               if (mantissa >= 9007199254740992L) {
                  mantissa = 4503599627370496L;
                  lz--;
               }

               mantissa &= -4503599627370497L;
               long realExponent = exponent - lz;
               if (realExponent >= 1L && realExponent <= 2046L) {
                  long bits = mantissa | realExponent << 52 | (isNegative ? Long.MIN_VALUE : 0L);
      ...
      private static double tryHexToDoubleWithFastAlgorithm(boolean isNegative, long digits, int power) {
         if (digits == 0L || power < -1076) {
            return isNegative ? -0.0 : 0.0;
         } else if (power > 1023) {
            return isNegative ? Double.NEGATIVE_INFINITY : Double.POSITIVE_INFINITY;
         } else if (Long.compareUnsigned(digits, 9007199254740991L) <= 0) {
            double d = digits;
            d *= Math.scalb(1.0, power);
            if (isNegative) {
               d = -d;
            }
