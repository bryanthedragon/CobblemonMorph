...
import com.cobblemon.mod.relocations.ibm.icu.text.SCSU;
...
implements SCSU {
    ...
    public static char[] decompress(byte[] buffer, int start2, int limit) {
        UnicodeDecompressor comp = new UnicodeDecompressor();
        int len = Math.max(2, 2 * (limit - start2));
        char[] temp = new char[len];
        int charCount = comp.decompress(buffer, start2, limit, null, temp, 0, len);
