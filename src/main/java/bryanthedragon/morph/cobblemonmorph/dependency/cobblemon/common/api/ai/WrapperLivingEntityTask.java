 ...
 */
public class WrapperLivingEntityTask<T : LivingEntity>(val task: BehaviorControl<T>, val clazz: Class<T>) : BehaviorControl<LivingEntity> {
    final class Companion {
        inline fun <reified E : LivingEntity> BehaviorControl<E>.wrapped(): BehaviorControl<LivingEntity> {
            return WrapperLivingEntityTask(this, E.class)
        }
    }
    override fun tryStart(ServerLevel level, LivingEntity entity, gameTime: Long): Boolean {
    ...
    override fun tickOrStop(ServerLevel level, LivingEntity entity, gameTime: Long) {
        task.tickOrStop(level, entity as T, gameTime)
    }

    override fun getStatus() = task.status
    override fun doStop(ServerLevel level, LivingEntity entity, gameTime: Long) = task.doStop(level, entity as T, gameTime)
}