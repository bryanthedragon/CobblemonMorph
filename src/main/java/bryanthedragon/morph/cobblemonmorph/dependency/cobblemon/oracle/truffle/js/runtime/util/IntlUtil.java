...
import bryanthedragon.morph.cobblemonmorph.dependency.cobblemon.relocations.ibm.icu.text.CaseMap;
import bryanthedragon.morph.cobblemonmorph.dependency.cobblemon.relocations.ibm.icu.text.Collator;
import bryanthedragon.morph.cobblemonmorph.dependency.cobblemon.relocations.ibm.icu.text.CurrencyMetaInfo;
...
public final class IntlUtil {
   ...
   private static final Set<String> SANCTIONED_SIMPLE_UNIT_IDENTIFIERS = new HashSet<>(
      Arrays.asList(
         "acre",
         "bit",
         "byte",
   ...
   public static Locale bestAvailableLocale(JSContext context, Locale locale) {
      ...
      while (!isAvailableLocale(context, candidate)) {
      ...
      for (String locale : requestedLocales) {
         ...
         if (availableLocale != null) {
            ...
            if (unicodeExtension != null) {
               availableLocale = new Builder().setLocale(availableLocale).setExtension('u', unicodeExtension).build();
            }
   ...
   private static Set<Locale> initAvailableLocales() {
      ...
      try {
         ...
         throw Errors.createICU4JDataError(var5);
      }
   }
   ...
   public static String defaultNumberingSystemName(JSContext context, Locale locale) {
      return context.isOptionV8CompatibilityMode() && "ar".equals(locale.toLanguageTag()) ? "latn" : NumberingSystem.getInstance(locale).getName();
   }
   ...
   public static boolean isWellFormedCurrencyCode(String currency) {
      return currency.length() == 3
         && UTS35Validator.isAlpha(currency.charAt(0))
         && UTS35Validator.isAlpha(currency.charAt(1))
         && UTS35Validator.isAlpha(currency.charAt(2));
   }
   ...
   public static String validateAndCanonicalizeLanguageTag(String languageTag) {
      try {
         if (!UTS35Validator.isWellFormedUnicodeBCP47LocaleIdentifier(languageTag)) {
            ...
            if (!extensions.isEmpty()) {
               ...
               Locale loc = new Builder().setLanguageTag(languageTag).build();
      ...
      } catch (IllformedLocaleException var12) {
         throw Errors.createRangeError(var12.getMessage());
      }
   }
   ...
   public static String normalizeCAType(String type) {
      if ("gregorian".equals(type)) {
         ...
         return "islamicc".equals(type) ? "islamic-civil" : type;
      }
   }
   ...
   private static String normalizeRGType(String type) {
      if ("cn11".equals(type)) {
         ...
         return "no23".equals(type) ? "no50" : type;
      }
   }
   ...
   private static String normalizeTransformedExtension(String extension) {
      ...
      for (int i = 0; i < extension.length() - 1; i++) {
         if (seenDash
            && UTS35Validator.isAlpha(extension.charAt(i))
            && UTS35Validator.isDigit(extension.charAt(i + 1))
            && (i + 2 == extension.length() || extension.charAt(i + 2) == '-')) {
            if (lastKey == null) {
   ...
   public static Locale selectedLocaleStripped(JSContext ctx, String[] locales) {
      return selectedLocale(ctx, locales).stripExtensions();
   }
   ...
   public static JSObject makePart(JSContext context, JSRealm realm, String type, String value, String unit, String source) {
      ...
      if (source != null) {
         JSObject.set(p, KEY_SOURCE, Strings.fromJavaString(source));
      }
   ...
   public static TimeZone getICUTimeZone(String tzId, JSContext context) {
      ...
      return (TimeZone)(context.getContextOptions().hasZoneRulesBasedTimeZones()
         ? new ZoneRulesBasedTimeZone(tzId, ZoneRulesProvider.getRules(tzId, false))
   ...
   public static TimeZone getICUTimeZone(ZoneId zoneId, JSContext context) {
      return (TimeZone)(context.getContextOptions().hasZoneRulesBasedTimeZones()
         ? new ZoneRulesBasedTimeZone(zoneId.getId(), zoneId.getRules())
         : TimeZone.getTimeZone(toICUTimeZoneId(zoneId)));
   }
   ...
   public static String[] availableCalendars(ULocale locale, boolean commonlyUsed) {
      ...
      for (String calendar : calendars) {
         if (!"unknown".equals(calendar)) {
            calendars[length++] = normalizeCAType(calendar);
         }
      }
   ...
   public static String[] availableCollations(ULocale locale, boolean commonOnly) {
      String[] collations;
      if (locale == null) {
         collations = Collator.getKeywordValues("collation");
      } else {
         collations = Collator.getKeywordValuesForLocale("collation", locale, commonOnly);
      }

      int length = 0;

      for (String element : collations) {
         if (!"search".equals(element) && !"standard".equals(element)) {
   ...
   public static String[] availableNumberingSystems(JSContext context) {
      ...
      Arrays.sort((Object[])numberingSystems);
