...
public class TextTrieMap<V> {
    ...
    public Iterator<V> get(CharSequence text, int start2) {
        return this.get(text, start2, null);
    }

    public Iterator<V> get(CharSequence text, int start2, Output output) {
        LongestMatchHandler handler = new LongestMatchHandler();
        this.find(text, start2, handler, output);
        if (output != null) {
    ...
    private static char[] subArray(char[] array, int start2) {
        if (start2 == 0) {
            return array;
        }
        char[] sub = new char[array.length - start2];
        System.arraycopy(array, start2, sub, 0, sub.length);
        return sub;
    }

    private static char[] subArray(char[] array, int start2, int limit) {
        if (start2 == 0 && limit == array.length) {
            return array;
        }
        char[] sub = new char[limit - start2];
        System.arraycopy(array, start2, sub, 0, limit - start2);
        return sub;
    }
    ...
    private class Node {
        ...
        public void add(CharIterator chitr, V value2) {
            StringBuilder buf = new StringBuilder();
            while (chitr.hasNext()) {
                buf.append(chitr.next());
            }
            this.add(TextTrieMap.toCharArray(buf), 0, value2);
        }
        ...
        private void add(char[] text, int offset, V value2) {
            if (text.length == offset) {
                this._values = this.addValue(this._values, value2);
                return;
            }
            if (this._children == null) {
                this._children = new LinkedList<Node>();
                Node child = new Node(TextTrieMap.subArray(text, offset), this.addValue(null, value2), null);
            ...
            while (litr.hasNext()) {
                ...
                if (matchLen == next._text.length) {
                    next.add(text, offset + matchLen, value2);
                } else {
                    next.split(matchLen);
                    next.add(text, offset + matchLen, value2);
                }
            ...
            litr.add(new Node(TextTrieMap.subArray(text, offset), this.addValue(null, value2), null));
        }
        ...
        private List<V> addValue(List<V> list, V value2) {
            if (list == null) {
                list = new LinkedList();
            }
            list.add(value2);
