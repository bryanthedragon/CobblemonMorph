...
public final class SetIteratorPrototypeBuiltins extends JSBuiltinsContainer.SwitchEnum<SetIteratorPrototypeBuiltins.SetIteratorPrototype> {
   ...
   protected SetIteratorPrototypeBuiltins() {
      super(JSSet.ITERATOR_PROTOTYPE_NAME, SetIteratorPrototypeBuiltins.SetIteratorPrototype.class);
   }

   protected Object createNode(
      JSContext context, JSBuiltin builtin, boolean construct, boolean newTarget, SetIteratorPrototypeBuiltins.SetIteratorPrototype builtinEnum
   ) {
      switch (builtinEnum) {
         case next:
            return SetIteratorPrototypeBuiltinsFactory.SetIteratorNextNodeGen.create(context, builtin, args().withThis().createArgumentNodes(context));
         default:
            return null;
      }
   }
   ...
   public abstract static class SetIteratorNextNode extends JSBuiltinNode {
      ...
      protected JSDynamicObject doSetIterator(VirtualFrame frame, JSDynamicObject iterator) {
         Object set = this.getIteratedObjectNode.getValue(iterator);
         if (this.detachedProf.profile(set == Undefined.instance)) {
   ...
   public static enum SetIteratorPrototype implements BuiltinEnum<SetIteratorPrototypeBuiltins.SetIteratorPrototype> {
      next(0);
