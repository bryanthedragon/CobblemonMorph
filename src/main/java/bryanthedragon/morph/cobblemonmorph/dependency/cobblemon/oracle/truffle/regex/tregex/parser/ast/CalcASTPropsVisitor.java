...
public class CalcASTPropsVisitor extends DepthFirstTraversalRegexASTVisitor {
   ...
   protected void leave(Group group) {
      ...
      if (group.isDead()) {
         ...
         int maxPath = 0;
         int flags = (group.isLoop() ? 4096 : 0) | 50;

         for (Sequence s : group.getAlternatives()) {
            if (!s.isDead()) {
               flags = flags & (s.getFlags(50) | -51) | s.getFlags(258060);
         ...
         if (group.hasQuantifier()) {
            ...
            if (this.ast.getOptions().getFlavor().canHaveEmptyLoopIterations() || (flags & 229376) != 0) {
   ...
   protected void leave(AtomicGroup atomicGroup) {
      ...
      leaveSubtreeRootNode(atomicGroup, 258110);
   ...
   private void leaveLookAroundAssertion(LookAroundAssertion assertion) {
      ...
      leaveSubtreeRootNode(assertion, assertion.isNegated() ? 258060 : (assertion.isLookBehindAssertion() ? 258062 : 258110));
   }
