...
public final class UResource {
    ...
    Comparable<Key> {
        ...
        @Override
        public Key subSequence(int start2, int end2) {
            assert (0 <= start2 && start2 < this.length);
            assert (start2 <= end2 && end2 <= this.length);
            return new Key(this.bytes, this.offset + start2, end2 - start2);
        }
        ...
        private String internalSubString(int start2, int end2) {
            StringBuilder sb = new StringBuilder(end2 - start2);
        ...
        public String substring(int start2) {
            assert (0 <= start2 && start2 < this.length);
            return this.internalSubString(start2, this.length);
        }

        public String substring(int start2, int end2) {
            assert (0 <= start2 && start2 < this.length);
            assert (start2 <= end2 && end2 <= this.length);
            return this.internalSubString(start2, end2);
        }
        ...
        private boolean regionMatches(int start2, CharSequence cs, int n) {
            for (int i = 0; i < n; ++i) {
                if (this.bytes[this.offset + start2 + i] == cs.charAt(i)) continue;
                return false;
            }
            return true;
        }
        ...
        public boolean regionMatches(int start2, CharSequence cs) {
            int csLength = cs.length();
            return csLength == this.length - start2 && this.regionMatches(start2, cs, csLength);
        }
