...
import bryanthedragon.morph.cobblemonmorph.dependency.cobblemon.relocations.ibm.icu.text.LocaleDisplayNames;
...
public final class JSDisplayNames extends JSNonProxy implements JSConstructorFactory.WithFunctions, PrototypeSupplier {
   ...
   public static JSDisplayNamesObject create(JSContext context, JSRealm realm) {
      JSDisplayNames.InternalState state = new JSDisplayNames.InternalState();
   ...
   public static void setupInternalState(
      JSContext ctx, JSDisplayNames.InternalState state, String[] locales, String optStyle, String optType, String optFallback, String optLanguageDisplay
   ) {
      ...
      state.languageDisplay = "language".equals(optType) ? optLanguageDisplay : null;
   ...
   private static ULocale convertOldISOCodes(Locale locale) {
      ...
      String var2 = locale.getLanguage();
      switch (var2) {
         case "iw":
   ...
   public static JSDynamicObject resolvedOptions(JSContext context, JSRealm realm, JSDynamicObject displayNamesObject) {
      JSDisplayNames.InternalState state = getInternalState(displayNamesObject);
   ...
   public static Object of(JSDynamicObject displayNamesObject, String code) {
      ...
      switch (type) {
         ...
         case "calendar":
            ...
            result = displayNames.keyValueDisplayName("calendar", displayNamesFriendlyCalendar(code.toLowerCase()));
            break;
         case "dateTimeField":
         ...
         case "currency":
            ...
            if ("none".equals(state.fallback) && upperCaseCode.equals(result)) {
               result = null;
            }
            break;
         default:
   ...
   public static JSDisplayNames.InternalState getInternalState(JSDynamicObject displayNamesObject) {
   ...
   private static String displayNamesFriendlyCalendar(String calendar) {
      if ("gregory".equals(calendar)) {
         ...
         return "ethioaa".equals(calendar) ? "ethiopic-amete-alem" : calendar;
      }
   }
      ...
      JSObject toResolvedOptionsObject(JSContext context, JSRealm realm) {
         ...
         if (this.languageDisplay != null) {
            JSObjectUtil.defineDataProperty(
               context, result, IntlUtil.KEY_LANGUAGE_DISPLAY, Strings.fromJavaString(this.languageDisplay), JSAttributes.getDefault()
            );
         }
