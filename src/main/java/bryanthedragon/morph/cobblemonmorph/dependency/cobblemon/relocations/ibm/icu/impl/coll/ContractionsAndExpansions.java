...
public final class ContractionsAndExpansions {
    ...
    private void enumCnERange(int start2, int end2, int ce32, ContractionsAndExpansions cne) {
        if (cne.checkTailored != 0) {
            if (cne.checkTailored < 0) {
                if (ce32 == 192) {
                    return;
                }
                cne.tailored.add(start2, end2);
            } else if (start2 == end2) {
                if (cne.tailored.contains(start2)) {
                    return;
                }
            } else if (cne.tailored.containsSome(start2, end2)) {
                if (cne.ranges == null) {
                    cne.ranges = new UnicodeSet();
                }
                cne.ranges.set(start2, end2).removeAll(cne.tailored);
        ...
        cne.handleCE32(start2, end2, ce32);
    }
    ...
    private void handleCE32(int start2, int end2, int ce32) {
        while (true) {
            if ((ce32 & 0xFF) < 192) {
                if (this.sink != null) {
            ...
            switch (Collation.tagFromCE32(ce32)) {
                ...
                case 4: {
                    ...
                    if (this.unreversedPrefix.length() == 0) {
                        this.addExpansions(start2, end2);
                    }
                    return;
                }
                case 5: {
                ...
                case 8: {
                    this.handlePrefixes(start2, end2, ce32);
                    return;
                }
                case 9: {
                    this.handleContractions(start2, end2, ce32);
                    return;
                }
                case 10: {
    ...
    private void handlePrefixes(int start2, int end2, int ce32) {
        int index = Collation.indexFromCE32(ce32);
        ce32 = this.data.getCE32FromContexts(index);
        this.handleCE32(start2, end2, ce32);
        if (!this.addPrefixes) {
            return;
        }
        for (CharsTrie.Entry e : new CharsTrie(this.data.contexts, index + 2)) {
            this.setPrefix(e.chars);
            this.addStrings(start2, end2, this.contractions);
            this.addStrings(start2, end2, this.expansions);
            this.handleCE32(start2, end2, e.value);
        }
        this.resetPrefix();
    }

    void handleContractions(int start2, int end2, int ce32) {
        int index = Collation.indexFromCE32(ce32);
        if ((ce32 & 0x100) != 0) {
    ...
    void addExpansions(int start2, int end2) {
        if (this.unreversedPrefix.length() == 0 && this.suffix == null) {
            if (this.expansions != null) {
                this.expansions.add(start2, end2);
            }
        } else {
            this.addStrings(start2, end2, this.expansions);
        }
    ...
    void addStrings(int start2, int end2, UnicodeSet set2) {
        if (set2 == null) {
            return;
        }
        StringBuilder s = new StringBuilder(this.unreversedPrefix);
        do {
            s.appendCodePoint(start2);
            if (this.suffix != null) {
