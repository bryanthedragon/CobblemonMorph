...
public final class ZoneMeta {
    ...
    private static synchronized String[] getZoneIDs() {
        if (ZONEIDS == null) {
            try {
                UResourceBundle top = UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ZONEINFORESNAME, ICUResourceBundle.ICU_DATA_CLASS_LOADER);
    ...
    private static int getZoneIndex(String zid) {
        ...
        if (all.length > 0) {
            ...
            while (lastMid != (mid = (start2 + limit) / 2)) {
                lastMid = mid;
    ...
    private static String findCLDRCanonicalID(String tzid) {
        ...
        try {
            UResourceBundle keyTypeData = UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", "keyTypeData", ICUResourceBundle.ICU_DATA_CLASS_LOADER);
    ...
    public static UResourceBundle openOlsonResource(UResourceBundle top, String id) {
        ...
        if (zoneIdx >= 0) {
            try {
                ...
                if (top == null) {
                    top = UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ZONEINFORESNAME, ICUResourceBundle.ICU_DATA_CLASS_LOADER);
                }
    ...
    static boolean parseCustomID(String id, int[] fields) {
        ...
        if (id != null && id.length() > "GMT".length() && id.toUpperCase(Locale.ENGLISH).startsWith("GMT")) {
            ...
            if (pos.getIndex() == start2) {
                return false;
            }
            hour = n.intValue();
            if (pos.getIndex() < id.length()) {
                if (pos.getIndex() - start2 > 2 || id.charAt(pos.getIndex()) != ':') {
                    return false;
                }
                pos.setIndex(pos.getIndex() + 1);
    ...
    public static SimpleTimeZone getCustomTimeZone(int offset) {
        ...
        int min2 = (tmp /= 60) % 60;
        int hour = tmp / 60;
        String zid = ZoneMeta.formatCustomID(hour, min2, sec, negative);
        return new SimpleTimeZone(offset, zid);
    }

    static String formatCustomID(int hour, int min2, int sec, boolean negative) {
    ...
    extends SoftCache<String, OlsonTimeZone, String> {
        ...
        protected OlsonTimeZone createInstance(String key, String data) {
            ...
            try {
                UResourceBundle top = UResourceBundle.getBundleInstance("com/cobblemon/mod/relocations/ibm/icu/impl/data/icudt71b", ZoneMeta.ZONEINFORESNAME, ICUResourceBundle.ICU_DATA_CLASS_LOADER);
