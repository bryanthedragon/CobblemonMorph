...
public final class ListFormatter {
    ...
    @Deprecated
    public ListFormatter(String two, String start2, String middle, String end2) {
        this(ListFormatter.compilePattern(two, new StringBuilder()), ListFormatter.compilePattern(start2, new StringBuilder()), ListFormatter.compilePattern(middle, new StringBuilder()), ListFormatter.compilePattern(end2, new StringBuilder()), null);
    }

    private ListFormatter(String two, String start2, String middle, String end2, ULocale locale) {
        this.start = start2;
        this.middle = middle;
        this.locale = locale;
        this.patternHandler = this.createPatternHandler(two, end2);
    }
    ...
    private PatternHandler createPatternHandler(String two, String end2) {
        if (this.locale != null) {
            String language = this.locale.getLanguage();
            if (language.equals("es")) {
                boolean twoIsY = two.equals(compiledY);
                boolean endIsY = end2.equals(compiledY);
                if (twoIsY || endIsY) {
                    return new ContextualHandler(changeToE, twoIsY ? compiledE : two, two, endIsY ? compiledE : end2, end2);
                }
                boolean twoIsO = two.equals(compiledO);
                boolean endIsO = end2.equals(compiledO);
                if (twoIsO || endIsO) {
                    return new ContextualHandler(changeToU, twoIsO ? compiledU : two, two, endIsO ? compiledU : end2, end2);
                }
            } else if (language.equals("he") || language.equals("iw")) {
                boolean twoIsVav = two.equals(compiledVav);
                boolean endIsVav = end2.equals(compiledVav);
                if (twoIsVav || endIsVav) {
                    return new ContextualHandler(changeToVavDash, twoIsVav ? compiledVavDash : two, two, endIsVav ? compiledVavDash : end2, end2);
                }
            }
        }
        return new StaticHandler(two, end2);
    }
    ...
    static class FormattedListBuilder {
        ...
        public FormattedListBuilder(Object start2, boolean needsFields) {
            this.needsFields = needsFields;
            this.string.setAppendableField(Field.LITERAL);
            this.appendElement(start2, 0);
        }
    ...
    implements PatternHandler {
        ...
        StaticHandler(String two, String end2) {
            this.twoPattern = two;
            this.endPattern = end2;
        }
