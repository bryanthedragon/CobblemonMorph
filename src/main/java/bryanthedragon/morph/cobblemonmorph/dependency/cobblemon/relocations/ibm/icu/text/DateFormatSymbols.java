...
Cloneable {
    ...
    extends UResource.Sink {
        ...
        public void put(UResource.Key key, UResource.Value value2, boolean noFallback) {
            ...
            UResource.Table calendarData = value2.getTable();
            int i = 0;
            while (calendarData.getKeyAndValue(i, key, value2)) {
                String keyString = key.toString();
                AliasType aliasType = this.processAliasFromValue(keyString, value2);
                    ...
                    if (aliasType == AliasType.DIFFERENT_CALENDAR) {
                        ...
                        if (keyString.startsWith("AmPmMarkers")) {
                            if (!keyString.endsWith("%variant") && !this.arrays.containsKey(keyString)) {
                                String[] dataArray = value2.getStringArray();
                            ...
                            this.processResource(keyString, key, value2);
                        }
                    }
                }
                ++i;
        ...
        protected void processResource(String path, UResource.Key key, UResource.Value value2) {
            ...
            while (table.getKeyAndValue(i, key, value2)) {
                if (!key.endsWith("%variant")) {
                    ...
                    if (value2.getType() == 0) {
                        ...
                        stringMap.put(keyString, value2.getString());
                    } else {
                        assert (stringMap == null);
                        ...
                        if ((!currentPath.startsWith("cyclicNameSets") || "cyclicNameSets/years/format/abbreviated".startsWith(currentPath) || "cyclicNameSets/zodiacs/format/abbreviated".startsWith(currentPath) || "cyclicNameSets/dayParts/format/abbreviated".startsWith(currentPath)) && !this.arrays.containsKey(currentPath) && !this.maps.containsKey(currentPath)) {
                            AliasType aliasType = this.processAliasFromValue(currentPath, value2);
                            if (aliasType == AliasType.SAME_CALENDAR) {
                                ...
                                } else if (value2.getType() == 2) {
                                    this.processResource(currentPath, key, value2);
                                }
                            }
                        }
                    }
        ...
        private AliasType processAliasFromValue(String currentRelativePath, UResource.Value value2) {
            if (value2.getType() == 3) {
                int typeLimit;
