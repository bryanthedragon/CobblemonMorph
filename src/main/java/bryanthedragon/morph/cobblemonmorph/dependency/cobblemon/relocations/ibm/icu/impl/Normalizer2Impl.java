...
public final class Normalizer2Impl {
    ...
    public void addLcccChars(UnicodeSet set2) {
        int start2 = 0;
        CodePointMap.Range range = new CodePointMap.Range();
        while (this.normTrie.getRange(start2, CodePointMap.RangeOption.FIXED_LEAD_SURROGATES, 1, null, range)) {
            int fcd16;
            int end2 = range.getEnd();
            int norm16 = range.getValue();
            if (norm16 > 64512 && norm16 != 65024) {
                set2.add(start2, end2);
            } else if (this.minNoNoCompNoMaybeCC <= norm16 && norm16 < this.limitNoNo && (fcd16 = this.getFCD16(start2)) > 255) {
                set2.add(start2, end2);
            }
            start2 = end2 + 1;
        }
    ...
    public void addPropertyStarts(UnicodeSet set2) {
        int start2 = 0;
        CodePointMap.Range range = new CodePointMap.Range();
        while (this.normTrie.getRange(start2, CodePointMap.RangeOption.FIXED_LEAD_SURROGATES, 1, null, range)) {
            int end2 = range.getEnd();
            ...
            set2.add(start2);
            if (start2 != end2 && this.isAlgorithmicNoNo(value2) && (value2 & 6) > 2) {
                int prevFCD16 = this.getFCD16(start2);
                while (++start2 <= end2) {
                    int fcd16 = this.getFCD16(start2);
    ...
    public void addCanonIterPropertyStarts(UnicodeSet set2) {
        this.ensureCanonIterData();
        int start2 = 0;
        CodePointMap.Range range = new CodePointMap.Range();
        while (this.canonIterData.getRange(start2, segmentStarterMapper, range)) {
            set2.add(start2);
            start2 = range.getEnd() + 1;
        }
    }
    ...
    public boolean getCanonStartSet(int c, UnicodeSet set2) {
        int canonValue = this.canonIterData.get(c) & Integer.MAX_VALUE;
        if (canonValue == 0) {
            return false;
        }
        set2.clear();
        int value2 = canonValue & 0x1FFFFF;
        if ((canonValue & 0x200000) != 0) {
            set2.addAll(this.canonStartSets.get(value2));
        } else if (value2 != 0) {
            set2.add(value2);
        }
        if ((canonValue & 0x40000000) != 0) {
            ...
            if (norm16 == 2) {
                ...
                set2.add(syllable, syllable + 588 - 1);
            } else {
                this.addComposites(this.getCompositionsList(norm16), set2);
            }
        }
        return true;
    }
    ...
    private void addComposites(int list, UnicodeSet set2) {
        char firstUnit;
        do {
            int compositeAndFwd;
            ...
            if ((compositeAndFwd & '\u0001') != 0) {
                this.addComposites(this.getCompositionsListForComposite(this.getRawNorm16(composite)), set2);
            }
            set2.add(composite);
    ...
    private boolean hasCompBoundaryAfter(CharSequence s, int start2, int p, boolean onlyContiguous) {
        return start2 == p || this.hasCompBoundaryAfter(Character.codePointBefore(s, p), onlyContiguous);
    }
    ...
    private int getPreviousTrailCC(CharSequence s, int start2, int p) {
        if (start2 == p) {
            return 0;
        }
    ...
    private void addToStartSet(MutableCodePointTrie mutableTrie, int origin, int decompLead) {
        ...
        if ((canonValue & 0x3FFFFF) == 0 && origin != 0) {
            ...
            if ((canonValue & 0x200000) == 0) {
                ...
                set2 = new UnicodeSet();
                this.canonStartSets.add(set2);
                if (firstOrigin != 0) {
                    set2.add(firstOrigin);
                }
            } else {
                set2 = this.canonStartSets.get(canonValue & 0x1FFFFF);
            }
            set2.add(origin);
        }
    }
    ...
    implements Appendable {
        ...
        public boolean equals(CharSequence s, int start2, int limit) {
            return UTF16Plus.equal(this.str, 0, this.str.length(), s, start2, limit);
        }
        ...
        public void append(CharSequence s, int start2, int limit, boolean isNFD, int leadCC, int trailCC) {
            if (start2 == limit) {
                return;
            }
            if (this.lastCC <= leadCC || leadCC == 0) {
                if (trailCC <= 1) {
                    this.reorderStart = this.str.length() + (limit - start2);
                } else if (leadCC <= 1) {
                    this.reorderStart = this.str.length() + 1;
                }
                this.str.append(s, start2, limit);
                this.lastCC = trailCC;
            } else {
                int c = Character.codePointAt(s, start2);
                start2 += Character.charCount(c);
                this.insert(c, leadCC);
                while (start2 < limit) {
                    leadCC = (start2 += Character.charCount(c = Character.codePointAt(s, start2))) < limit ? (isNFD ? Normalizer2Impl.getCCFromYesOrMaybe(this.impl.getNorm16(c)) : this.impl.getCC(this.impl.getNorm16(c))) : trailCC;
        ...
        @Override
        public ReorderingBuffer append(CharSequence s, int start2, int limit) {
            if (start2 != limit) {
                this.str.append(s, start2, limit);
                this.lastCC = 0;
                this.reorderStart = this.str.length();
        ...
        public ReorderingBuffer flushAndAppendZeroCC(CharSequence s, int start2, int limit) {
            if (this.appIsStringBuilder) {
                this.str.append(s, start2, limit);
                this.reorderStart = this.str.length();
            } else {
                try {
                    this.app.append(this.str).append(s, start2, limit);
                    this.str.setLength(0);
                    this.reorderStart = 0;
                }
                catch (IOException e) {
