...
final class DominatorTree {
   ...
   boolean dom(GraphNode a, GraphNode b) {
      for (int dom = this.doms[b.getPostOrderIndex()]; a.getPostOrderIndex() != dom; dom = this.doms[dom]) {
   ...
   private void buildDominatorTree() {
      if (this.doms.length < this.graph.size()) {
         int newLength = this.doms.length * 2;

         while (newLength < this.graph.size()) {
            newLength *= 2;
         }
      ...
      while (changed) {
         ...
         for (int i = this.postOrder.size() - 1; i >= 0; i--) {
            ...
            if (b != this.graph.getStart()) {
               ...
               for (GraphNode px : b.getPredecessors()) {
                  if (px != selectedPredecessor && this.doms[px.getPostOrderIndex()] != -1) {
                     newIDom = this.intersect(px.getPostOrderIndex(), newIDom);
                  }
               }

               if (this.doms[b.getPostOrderIndex()] != newIDom) {
      ...
      for (int ix = 0; ix < this.graph.size(); ix++) {
         GraphNode dominator = this.postOrder.get(this.doms[ix]);
         GraphNode child = this.postOrder.get(ix);
         if (dominator != child) {
