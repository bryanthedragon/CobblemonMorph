...
JsonConvertible {
    ...
    public NFA(RegexAST ast, NFAState dummyInitialState, NFAStateTransition[] anchoredEntry, NFAStateTransition[] unAnchoredEntry, NFAStateTransition reverseAnchoredEntry, NFAStateTransition reverseUnAnchoredEntry, Collection<NFAState> states, Counter.ThresholdCounter stateIDCounter, Counter.ThresholdCounter transitionIDCounter, NFAStateTransition initialLoopBack, PreCalculatedResultFactory[] preCalculatedResults) {
        ...
        for (NFAState s : states) {
            ...
            if (s.getSuccessors() == null) continue;
            ...
            for (NFAStateTransition t : (NFAStateTransition[])s.getPredecessors()) {
    ...
    public boolean hasReverseUnAnchoredEntry() {
        return this.reverseUnAnchoredEntry != null && ((NFAStateTransition[])this.reverseUnAnchoredEntry.getSource().getPredecessors()).length > 0;
    }
    ...
    public boolean isDead() {
        return this.anchoredEntry != null ? NFA.allDead(this.anchoredEntry) : this.reverseAnchoredEntry.getSource().isDead(false) && this.reverseUnAnchoredEntry.getSource().isDead(false);
    }
    ...
    private static boolean allDead(NFAStateTransition[] entries) {
        ...
        for (NFAStateTransition t : entries) {
            if (t.getTarget().isDead(true)) continue;
            return false;
        }
        return true;
    }
    ...
    public void setInitialLoopBack(boolean enable) {
        if (((NFAStateTransition[])this.getUnAnchoredInitialState().getSuccessors()).length == 0) {
            return;
        }
        NFAState loopbackState = this.initialLoopBack.getSource();
        NFAStateTransition lastInitTransition = ((NFAStateTransition[])loopbackState.getSuccessors())[((NFAStateTransition[])loopbackState.getSuccessors()).length - 1];
        if (enable) {
    ...
    public boolean isFixedCodePointWidth() {
        ...
        for (NFAState state : this.states) {
            if (state == null || this.ast.getEncoding().isFixedCodePointWidth(state.getCharSet())) continue;
