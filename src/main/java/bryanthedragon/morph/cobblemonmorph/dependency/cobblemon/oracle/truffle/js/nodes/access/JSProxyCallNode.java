...
public abstract class JSProxyCallNode extends JavaScriptBaseNode {
   ...
   protected JSProxyCallNode(JSContext context, boolean isNew, boolean isNewTarget) {
      this.callNode = !isNew && !isNewTarget ? JSFunctionCallNode.createCall() : JSFunctionCallNode.createNewTarget();
      this.callTrapNode = JSFunctionCallNode.createCall();
      this.trapGetter = GetMethodNode.create(context, !isNewTarget && !isNew ? JSProxy.APPLY : JSProxy.CONSTRUCT);
   ...
   protected Object doConstruct(Object[] arguments, @Cached IsConstructorNode isConstructor) {
      ...
      if (!isConstructor.executeBoolean(JSProxy.getTarget(proxy))) {
         ...
         if (this.pxTrapFunProfile.profile(pxTrapFun == Undefined.instance)) {
            return JSDynamicObject.isJSDynamicObject(pxTarget)
               ? this.callNode.executeCall(JSArguments.createWithNewTarget(JSFunction.CONSTRUCT, pxTarget, newTarget, constructorArguments))
               : JSInteropUtil.construct(pxTarget, constructorArguments);
         } else {
