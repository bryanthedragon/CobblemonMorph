...
final class BidiWriter {
    ...
    private static String doWriteForward(char[] text, int start2, int limit, int options) {
        return BidiWriter.doWriteForward(new String(text, start2, limit - start2), options);
    }
    ...
    static String doWriteReverse(char[] text, int start2, int limit, int options) {
        return BidiWriter.writeReverse(new String(text, start2, limit - start2), options);
    }
        ...
        if ((options & 0x10) == 0) {
            if ((options & 4) == 0) {
                ...
                for (int run3 = 0; run3 < runCount; ++run3) {
                    char uc;
                    BidiRun bidiRun = bidi.getVisualRun(run3);
                    int markFlag = 0;
                    markFlag = bidi.runs[run3].insertRemove;
                    if (markFlag < 0) {
                        markFlag = 0;
                    }
                    if (bidiRun.isEvenRun()) {
            ...
            int run4 = runCount;
            while (--run4 >= 0) {
                BidiRun bidiRun = bidi.getVisualRun(run4);
                if (bidiRun.isEvenRun()) {
            ...
            int run5 = runCount;
            while (--run5 >= 0) {
                BidiRun bidiRun = bidi.getVisualRun(run5);
                if (bidiRun.isEvenRun()) {
