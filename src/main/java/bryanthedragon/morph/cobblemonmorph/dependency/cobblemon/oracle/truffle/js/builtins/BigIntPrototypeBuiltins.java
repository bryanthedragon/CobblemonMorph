...
public final class BigIntPrototypeBuiltins extends JSBuiltinsContainer.SwitchEnum<BigIntPrototypeBuiltins.BigIntPrototype> {
   ...
   protected BigIntPrototypeBuiltins() {
      super(JSBigInt.PROTOTYPE_NAME, BigIntPrototypeBuiltins.BigIntPrototype.class);
   }

   protected Object createNode(JSContext context, JSBuiltin builtin, boolean construct, boolean newTarget, BigIntPrototypeBuiltins.BigIntPrototype builtinEnum) {
      switch (builtinEnum) {
         case toLocaleString:
            if (context.isOptionIntl402()) {
               return BigIntPrototypeBuiltinsFactory.JSBigIntToLocaleStringIntlNodeGen.create(
                  context, builtin, args().withThis().fixedArgs(2).createArgumentNodes(context)
               );
            }

            return BigIntPrototypeBuiltinsFactory.JSBigIntToLocaleStringNodeGen.create(context, builtin, args().withThis().createArgumentNodes(context));
         case toString:
            return BigIntPrototypeBuiltinsFactory.JSBigIntToStringNodeGen.create(context, builtin, args().withThis().fixedArgs(1).createArgumentNodes(context));
         case valueOf:
            return BigIntPrototypeBuiltinsFactory.JSBigIntValueOfNodeGen.create(context, builtin, args().withThis().createArgumentNodes(context));
         default:
            return null;
      }
   }

   public static enum BigIntPrototype implements BuiltinEnum<BigIntPrototypeBuiltins.BigIntPrototype> {
      toLocaleString(0),
      toString(0),
      valueOf(0);
   ...
   public abstract static class JSBigIntToLocaleStringIntlNode extends BigIntPrototypeBuiltins.JSBigIntOperation {
      @Node.Child
   ...
   public abstract static class JSBigIntToLocaleStringNode extends BigIntPrototypeBuiltins.JSBigIntOperation {
      ...
      protected TruffleString toLocaleStringJSBigInt(JSDynamicObject thisObj) {
         return toLocaleStringImpl(this.getBigIntValue(thisObj));
      }
   ...
   public abstract static class JSBigIntToStringNode extends BigIntPrototypeBuiltins.JSBigIntOperation {
   ...
   public abstract static class JSBigIntValueOfNode extends BigIntPrototypeBuiltins.JSBigIntOperation {
