...
public class UnicodeSetStringSpan {
    ...
    public UnicodeSetStringSpan(UnicodeSet set2, ArrayList<String> setStrings, int which) {
        int spanLength;
        this.spanSet = new UnicodeSet(0, 0x10FFFF);
        this.strings = setStrings;
        this.all = which == 127;
        this.spanSet.retainAll(set2);
        if (0 != (which & 1)) {
    ...
    public int span(CharSequence s, int start2, UnicodeSet.SpanCondition spanCondition) {
        if (spanCondition == UnicodeSet.SpanCondition.NOT_CONTAINED) {
            return this.spanNot(s, start2, null);
        }
        int spanLimit = this.spanSet.span(s, start2, UnicodeSet.SpanCondition.CONTAINED);
        if (spanLimit == s.length()) {
            return spanLimit;
        }
        return this.spanWithStrings(s, start2, spanLimit, spanCondition);
    }

    private synchronized int spanWithStrings(CharSequence s, int start2, int spanLimit, UnicodeSet.SpanCondition spanCondition) {
        int initSize = 0;
        ...
        int spanLength = spanLimit - start2;
        int stringsLength = this.strings.size();
        while (true) {
            int i;
            if (spanCondition == UnicodeSet.SpanCondition.CONTAINED) {
    ...
    public int spanAndCount(CharSequence s, int start2, UnicodeSet.SpanCondition spanCondition, OutputInt outCount) {
        int maxInc;
        if (spanCondition == UnicodeSet.SpanCondition.NOT_CONTAINED) {
            return this.spanNot(s, start2, outCount);
        }
        if (spanCondition == UnicodeSet.SpanCondition.CONTAINED) {
            return this.spanContainedAndCount(s, start2, outCount);
        }
        int stringsLength = this.strings.size();
        int length = s.length();
        int pos = start2;
        int count = 0;
        for (int rest = length - start2; rest != 0; rest -= maxInc) {
    ...
    private synchronized int spanContainedAndCount(CharSequence s, int start2, OutputInt outCount) {
        int minOffset;
        ...
        int pos = start2;
        int count = 0;
        for (int rest = length - start2; rest != 0; rest -= minOffset) {
            int cpLength = UnicodeSetStringSpan.spanOne(this.spanSet, s, pos, rest);
            if (cpLength > 0) {
    ...
    private int spanNot(CharSequence s, int start2, OutputInt outCount) {
        int cpLength;
        int length = s.length();
        int pos = start2;
        int rest = length - start2;
        int stringsLength = this.strings.size();
        int count = 0;
        do {
            int spanLimit;
    ...
    private static boolean matches16(CharSequence s, int start2, String t, int length) {
        int end2 = start2 + length;
        while (length-- > 0) {
            if (s.charAt(--end2) == t.charAt(length)) continue;
            return false;
        }
        return true;
    }

    static boolean matches16CPB(CharSequence s, int start2, int limit, String t, int tlength) {
        return !(!UnicodeSetStringSpan.matches16(s, start2, t, tlength) || 0 < start2 && Character.isHighSurrogate(s.charAt(start2 - 1)) && Character.isLowSurrogate(s.charAt(start2)) || start2 + tlength < limit && Character.isHighSurrogate(s.charAt(start2 + tlength - 1)) && Character.isLowSurrogate(s.charAt(start2 + tlength)));
    }

    static int spanOne(UnicodeSet set2, CharSequence s, int start2, int length) {
        char c2;
        char c = s.charAt(start2);
        if (c >= '\ud800' && c <= '\udbff' && length >= 2 && UTF16.isTrailSurrogate(c2 = s.charAt(start2 + 1))) {
            int supplementary = Character.toCodePoint(c, c2);
            return set2.contains(supplementary) ? 2 : -2;
        }
        return set2.contains(c) ? 1 : -1;
    }

    static int spanOneBack(UnicodeSet set2, CharSequence s, int length) {
        char c2;
