...
import com.cobblemon.mod.relocations.ibm.icu.impl.number.DecimalQuantity_AbstractBCD;
...
extends DecimalQuantity_AbstractBCD {
    ...
    @Override
    protected void setDigitPos(int position, byte value2) {
        assert (position >= 0);
        if (this.usingBytes) {
            this.ensureCapacity(position + 1);
            this.bcdBytes[position] = value2;
        } else if (position >= 16) {
            this.switchStorage();
            this.ensureCapacity(position + 1);
            this.bcdBytes[position] = value2;
        } else {
            int shift2 = position * 4;
            this.bcdLong = this.bcdLong & (15L << shift2 ^ 0xFFFFFFFFFFFFFFFFL) | (long)value2 << shift2;
        }
    }
    ...
    protected BigDecimal bcdToBigDecimal() {
        ...
        long tempLong = 0L;
        for (int shift2 = this.precision - 1; shift2 >= 0; --shift2) {
            tempLong = tempLong * 10L + (long)this.getDigitPos(shift2);
        }
