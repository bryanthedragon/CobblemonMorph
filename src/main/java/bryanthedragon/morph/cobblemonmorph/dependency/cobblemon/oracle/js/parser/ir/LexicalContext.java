...
public class LexicalContext {
   ...
   public LexicalContextNode replace(final LexicalContextNode oldNode, final LexicalContextNode newNode) {
      for (int i = this.sp - 1; i >= 0; i--) {
         if (this.stack[i] == oldNode) {
            assert i == this.sp - 1 : "violation of contract - we always expect to find the replacement node on top of the lexical context stack: "
               + newNode
               + " has "
               + this.stack[i + 1].getClass()
               + " above it";
   ...
   public Scope getCurrentScope() {
      LexicalContext.NodeIterator<LexicalContextScope> iterator = new LexicalContext.NodeIterator<>(LexicalContextScope.class);
      return iterator.hasNext() ? iterator.next().getScope() : null;
   }

   public ClassNode getCurrentClass() {
      LexicalContext.NodeIterator<ClassNode> iterator = new LexicalContext.NodeIterator<>(ClassNode.class);
      return iterator.hasNext() ? iterator.next() : null;
   }

   public boolean inModule() {
      Iterator<FunctionNode> functions = this.getFunctions();
   ...
   private class NodeIterator<T extends LexicalContextNode> implements Iterator<T> {
      private int index = LexicalContext.this.sp - 1;
      private T next;
