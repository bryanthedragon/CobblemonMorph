...
final class InstrumentAccessor extends Accessor {
   ...
   static final class InstrumentImpl extends Accessor.InstrumentSupport {
      @Override
      public Object createInstrumentationHandler(
      ...
      public OptionDescriptors describeEngineOptions(Object instrumentationHandler, Object key, String requiredGroup) {
         InstrumentationHandler.InstrumentClientInstrumenter instrumenter = (InstrumentationHandler.InstrumentClientInstrumenter)((InstrumentationHandler)instrumentationHandler)
            .instrumenterMap
            .get(key);
      ...
      private static OptionDescriptors validateOptions(
         String requiredGroup, InstrumentationHandler.InstrumentClientInstrumenter instrumenter, OptionDescriptors descriptors
      ) {
         if (descriptors == null) {
            ...
            for (OptionDescriptor descriptor : descriptors) {
               if (!descriptor.getName().equals(requiredGroup) && !descriptor.getName().startsWith(groupPlusDot)) {
                  throw new IllegalArgumentException(
                     String.format(
                        "Illegal option prefix in name '%s' specified for option described by instrument '%s'. The option prefix must match the id of the instrument '%s'.",
                        descriptor.getName(),
                        instrumenter.instrument.getClass().getName(),
                        requiredGroup
                     )
                  );
               }
      ...
      public void collectEnvServices(Set<Object> collectTo, Object polyglotLanguageContext, TruffleLanguage<?> language) {
         InstrumentationHandler instrumentationHandler = (InstrumentationHandler)InstrumentAccessor.engineAccess()
            .getInstrumentationHandler(polyglotLanguageContext);
      ...
      public Object invokeContextLocalFactory(Object factory, TruffleContext truffleContext) {
         ...
         if (result == null) {
            throw new IllegalStateException(
               String.format("%s.create is not allowed to return null.", TruffleInstrument.ContextLocalFactory.class.getSimpleName())
            );
         } else {
            return result;
         }
      ...
      public Object invokeContextThreadLocalFactory(Object factory, TruffleContext truffleContext, Thread t) {
         ...
         if (result == null) {
            throw new IllegalStateException(
               String.format("%s.create is not allowed to return null.", TruffleInstrument.ContextThreadLocalFactory.class.getSimpleName())
            );
         } else {
            return result;
         }
