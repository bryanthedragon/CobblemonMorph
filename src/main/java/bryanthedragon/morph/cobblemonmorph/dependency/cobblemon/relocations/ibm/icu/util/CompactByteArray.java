...
implements Cloneable {
    ...
    @Deprecated
    public void setElementAt(char index, byte value2) {
        if (this.isCompact) {
            this.expand();
        }
        this.values[index] = value2;
        this.touchBlock(index >> 7, value2);
    }

    @Deprecated
    public void setElementAt(char start2, char end2, byte value2) {
        if (this.isCompact) {
            this.expand();
        }
        for (int i = start2; i <= end2; ++i) {
            this.values[i] = value2;
            this.touchBlock(i >> 7, value2);
        }
    }
    ...
    private void expand() {
        if (this.isCompact) {
            ...
            for (i = 0; i < 65536; ++i) {
                byte value2;
                tempArray[i] = value2 = this.elementAt((char)i);
                this.touchBlock(i >> 7, value2);
            }
            for (i = 0; i < 512; ++i) {
                this.indices[i] = (char)(i << 7);
