...
public class TemporalInstantFunctionBuiltins extends JSBuiltinsContainer.SwitchEnum<TemporalInstantFunctionBuiltins.TemporalInstantFunction> {
   ...
   protected TemporalInstantFunctionBuiltins() {
      super(JSTemporalInstant.CLASS_NAME, TemporalInstantFunctionBuiltins.TemporalInstantFunction.class);
   }

   protected Object createNode(
      JSContext context, JSBuiltin builtin, boolean construct, boolean newTarget, TemporalInstantFunctionBuiltins.TemporalInstantFunction builtinEnum
   ) {
      switch (builtinEnum) {
         case from:
            return TemporalInstantFunctionBuiltinsFactory.JSTemporalInstantFromNodeGen.create(
               context, builtin, args().fixedArgs(1).createArgumentNodes(context)
            );
         case fromEpochSeconds:
            return TemporalInstantFunctionBuiltinsFactory.JSTemporalInstantFromEpochNodeGen.create(
               context, builtin, 1000000000L, true, args().fixedArgs(1).createArgumentNodes(context)
            );
         case fromEpochMilliseconds:
            return TemporalInstantFunctionBuiltinsFactory.JSTemporalInstantFromEpochNodeGen.create(
               context, builtin, 1000000L, true, args().fixedArgs(1).createArgumentNodes(context)
            );
         case fromEpochMicroseconds:
            return TemporalInstantFunctionBuiltinsFactory.JSTemporalInstantFromEpochNodeGen.create(
               context, builtin, 1000L, false, args().fixedArgs(1).createArgumentNodes(context)
            );
         case fromEpochNanoseconds:
            return TemporalInstantFunctionBuiltinsFactory.JSTemporalInstantFromEpochNodeGen.create(
               context, builtin, 1L, false, args().fixedArgs(1).createArgumentNodes(context)
            );
         case compare:
            return TemporalInstantFunctionBuiltinsFactory.JSTemporalInstantCompareNodeGen.create(
   ...
   public abstract static class JSTemporalInstantCompareNode extends JSBuiltinNode {
      ...
      @Specialization
      protected int compare(Object obj1, Object obj2, @Cached("create(getContext())") ToTemporalInstantNode toTemporalInstantNode) {
   ...
   public abstract static class JSTemporalInstantFromNode extends JSBuiltinNode {
      ...
      @Specialization
      protected JSDynamicObject from(Object item, @Cached("create(getContext())") ToTemporalInstantNode toTemporalInstantNode) {
         return TemporalUtil.isTemporalInstant(item)
            ? JSTemporalInstant.create(this.getContext(), this.getRealm(), ((JSTemporalInstantObject)item).getNanoseconds())
            : toTemporalInstantNode.execute(item);
      }
   }

   public static enum TemporalInstantFunction implements BuiltinEnum<TemporalInstantFunctionBuiltins.TemporalInstantFunction> {
      from(1),
      fromEpochSeconds(1),
