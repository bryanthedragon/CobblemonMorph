...
@TruffleLanguage.Registration(name="REGEX", id="regex", characterMimeTypes={"application/tregex"}, version="0.1", contextPolicy=TruffleLanguage.ContextPolicy.SHARED, internal=true, interactive=false, website="https://github.com/oracle/graal/tree/master/regex")
...
extends TruffleLanguage<RegexContext> {
    ...
    protected CallTarget parse(TruffleLanguage.ParsingRequest parsingRequest) {
        return RootNode.createConstantNode(this.createRegexObject(RegexLanguage.createRegexSource(parsingRequest.getSource()))).getCallTarget();
    }
    ...
    private static RegexSource createRegexSource(Source source) {
        ...
        if (optBuilder.getFlavor() == ECMAScriptFlavor.INSTANCE && !optBuilder.isUtf16ExplodeAstralSymbols() && optBuilder.getEncoding() == Encodings.UTF_16_RAW && flags.indexOf(117) >= 0) {
