...
import com.cobblemon.mod.relocations.ibm.icu.text.RBBIRuleBuilder;
...
extends BreakIterator {
    ...
    public int first() {
        ...
        this.fText.first();
        int start2 = this.fText.getIndex();
        if (!this.fBreakCache.seek(start2)) {
            this.fBreakCache.populateNear(start2);
        }
        this.fBreakCache.current();
        assert (this.fPosition == start2);
    ...
    class BreakCache {
        ...
        boolean seek(int pos) {
            ...
            int min2 = this.fStartBufIdx;
            int max2 = this.fEndBufIdx;
            while (min2 != max2) {
                int probe = (min2 + max2 + (min2 > max2 ? 128 : 0)) / 2;
                if (this.fBoundaries[probe = this.modChunkSize(probe)] > pos) {
                    max2 = probe;
                    continue;
                }
                min2 = this.modChunkSize(probe + 1);
            }
            assert (this.fBoundaries[max2] > pos);
            this.fBufIdx = this.modChunkSize(max2 - 1);
            this.fTextIdx = this.fBoundaries[this.fBufIdx];
