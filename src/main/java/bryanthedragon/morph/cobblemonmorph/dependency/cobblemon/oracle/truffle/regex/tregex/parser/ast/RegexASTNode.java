...
public abstract class RegexASTNode implements JsonConvertible {
   ...
   protected static JsonValue astNodeId(RegexASTNode astNode) {
      return (JsonValue)(astNode == null ? Json.nullValue() : Json.val(astNode.id));
   }
   ...
   protected JsonObject toJson(String typeName) {
      return Json.obj(
         ...
         Json.prop("parent", astNodeId(this.parent)),
         Json.prop("minPath", this.minPath),
         Json.prop("isPrefix", this.isPrefix()),
         Json.prop("isDead", this.isDead())
      );
   }
}
