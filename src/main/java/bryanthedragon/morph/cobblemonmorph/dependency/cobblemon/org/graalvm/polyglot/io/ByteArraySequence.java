...
final class ByteArraySequence implements ByteSequence {
      ...
      if (this == obj) {
            ...
            if (this.length != other.length) {
               ...
               if (thisHash != 0 && otherHash != 0 && thisHash != otherHash) {
                  ...
                  for (int i = 0; i < this.length; i++) {
                     if (this.buffer[this.start + i] != other.buffer[otherStart + i]) {
                        return false;
                     }
                  }
         ...
         if (this.length != other.length()) {
            ...
            for (int ix = 0; ix < this.length; ix++) {
               if (this.buffer[this.start + ix] != other.byteAt(ix)) {
                  return false;
               }
            }
   ...
   public int hashCode() {
      ...
      if (h == 0 && this.length > 0) {
         ...
         int i;
         for (i = this.start; i + 3 < end; i += 4) {
            int h0 = this.buffer[i + 0] & 255;
            int h1 = this.buffer[i + 1] & '\uff00';
