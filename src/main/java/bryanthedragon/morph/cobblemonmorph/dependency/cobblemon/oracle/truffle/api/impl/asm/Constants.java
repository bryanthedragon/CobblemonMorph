   ...
   static boolean isWhitelisted(String internalName) {
      if (!internalName.startsWith("com/oracle/truffle/api/impl/asm/")) {
         ...
         return internalName.contains("Test$")
            || Pattern.matches("com/oracle/truffle/api/impl/asm/util/Trace" + member + "Visitor(\\$.*)?", internalName)
            || Pattern.matches("com/oracle/truffle/api/impl/asm/util/Check" + member + "Adapter(\\$.*)?", internalName);
      }
   }
