...
public final class JavaScriptLanguage extends TruffleLanguage<JSRealm> {
   ...
   private static final OptionKey<?>[] PREINIT_CONTEXT_PATCHABLE_OPTIONS = new OptionKey[]{
   ...
   public CallTarget parse(TruffleLanguage.ParsingRequest parsingRequest) {
      ...
      return context.isOptionParseOnly()
         ? createEmptyScript(context).getCallTarget()
         : new JavaScriptLanguage.ParsedProgramRoot(this, context, program).getCallTarget();
   }

   public static CallTarget getParsedProgramCallTarget(RootNode rootNode) {
      return ((JavaScriptLanguage.ParsedProgramRoot)rootNode).directCallNode.getCallTarget();
   }
   ...
   protected ExecutableNode parse(TruffleLanguage.InlineParsingRequest request) throws Exception {
      ...
      return new ExecutableNode(this) {
         @Node.Child
         private JavaScriptNode expression = this.insert(JavaScriptLanguage.parseInlineScript(context, source, requestFrame, strict, locationNode));
         @Node.Child
   ...
   private static ScriptNode parseScript(JSContext context, Source code, String prolog, String epilog, boolean strict, List<String> argumentNames) {
      ...
      ScriptNode var9;
      try {
         var9 = context.getEvaluator().parseScript(context, code, prolog, epilog, strict, argumentNames.isEmpty() ? null : argumentNames);
      } finally {
         if (profileTime) {
   ...
   protected static JavaScriptNode parseInlineScript(JSContext context, Source code, MaterializedFrame lexicalContextFrame, boolean strict, Node locationNode) {
      ...
      JavaScriptNode var8;
      try {
         var8 = context.getEvaluator().parseInlineScript(context, code, lexicalContextFrame, strict, locationNode);
      } finally {
         if (profileTime) {
            context.getTimeProfiler().printElapsed(startTime, "parsing " + code.getName());
         }
   ...
   private synchronized JSContext initLanguageContext(TruffleLanguage.Env env) {
      ...
      if (curContext != null) {
         ...
         JSContext newContext = this.newJSContext(env);
         this.languageContext = newContext;
         return newContext;
      }
   }
      ...
      if (!preinitOptions.hasSetOptions() && !options.hasSetOptions()) {
         ...
         for (OptionDescriptor descriptor : options.getDescriptors()) {
            ...
            if ((preinitOptions.hasBeenSet(key) || options.hasBeenSet(key))
               && !ignoredOptions.contains(key)
               && !preinitOptions.get(key).equals(options.get(key))) {
               return false;
            }
         }
   ...
   private static void ensureErrorClassesInitialized() {
      if (!JSConfig.SubstrateVM) {
         try {
            ...
            throw Errors.shouldNotReachHere(var1);
         }
      }
   }
   ...
   private final class ParsedProgramRoot extends RootNode {
      ...
      public Object execute(VirtualFrame frame) {
         ...
         try {
            ...
            var5 = this.exportValueNode.execute(result);
         } finally {
