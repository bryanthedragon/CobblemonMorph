...
JSFunctionData.CallTargetInitializer {
    ...
    public JSBuiltin(TruffleString containerName, TruffleString functionName, Object key, int length, int attributeFlags, int ecmaScriptVersion, boolean annexB, BuiltinNodeFactory functionNodeFactory, BuiltinNodeFactory constructorNodeFactory, BuiltinNodeFactory newTargetConstructorFactory) {
        ...
        this.attributeFlags = (byte)(attributeFlags | JSBuiltin.detectAccessor(functionName));
        this.annexB = annexB;
    ...
    public boolean isSetter() {
        return (this.attributeFlags & 0x10) != 0;
    }
    ...
    private static boolean isAllowedKey(Object key) {
        ...
        if (Strings.isTString(key)) {
            ...
            return !Strings.isEmpty(tsk) && (!Strings.endsWith(tsk, Strings.UNDERSCORE) || Strings.equals(tsk, Strings.$_)) && !Strings.startsWith(tsk, Strings.UNDERSCORE) || Strings.startsWith(tsk, Strings.UNDERSCORE_2) && Strings.endsWith(tsk, Strings.UNDERSCORE_2);
        }
        return false;
    }
    ...
    private static int detectAccessor(TruffleString functionName) {
        ...
        if (Strings.startsWith(functionName, Strings.SET_SPC)) {
            return 16;
        }
        return 0;
    }
