...
public final class WithTargetNode extends JavaScriptNode {
   ...
   public Object execute(VirtualFrame frame) {
      ...
      if (this.withObjectHasProperty.hasProperty(target) && this.isPropertyScopable(target)) {
         ...
         return this.context.isOptionNashornCompatibilityMode()
               && this.hasNoSuchProperty(target, false)
               && !this.globalObjectHasProperty.hasProperty(this.getRealm().getGlobalObject())
            ? target
            : Undefined.instance;
      }
   }
   ...
   private boolean isPropertyScopable(Object target) {
      if (this.context.getEcmaScriptVersion() >= 6) {
         ...
         if (JSRuntime.isObject(unscopables)) {
            boolean blocked = this.toBoolean.executeBoolean(this.unscopablesGetProperty.getValue(unscopables));
            if (blocked) {
               return false;
            }
         }
   ...
   private boolean hasNoSuchProperty(Object thisTruffleObj, boolean isMethod) {
      if (JSRuntime.isObject(thisTruffleObj)) {
         ...
         if (!isMethod && !this.context.getNoSuchPropertyUnusedAssumption().isValid() && JSObject.hasOwnProperty(thisObj, JSObject.NO_SUCH_PROPERTY_NAME)
            || isMethod && !this.context.getNoSuchMethodUnusedAssumption().isValid() && JSObject.hasOwnProperty(thisObj, JSObject.NO_SUCH_METHOD_NAME)) {
   ...
   protected JavaScriptNode copyUninitialized(Set<Class<? extends Tag>> materializedTags) {
      return new WithTargetNode(this.context, (TruffleString)this.withObjectHasProperty.getKey(), cloneUninitialized(this.withVariable, materializedTags));
   }
}
