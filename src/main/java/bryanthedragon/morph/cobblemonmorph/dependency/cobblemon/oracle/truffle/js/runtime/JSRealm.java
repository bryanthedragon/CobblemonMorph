...
public class JSRealm {
    ...
    protected JSRealm(JSContext context, TruffleLanguage.Env env, JSRealm parentRealm) {
        ...
        this.scriptEngineImportScope = context.getContextOptions().isScriptEngineGlobalScopeImport() ? JSOrdinary.createWithNullPrototypeInit(context) : null;
        ...
        this.commonJSRequireCache = context.getContextOptions().isCommonJSRequire() ? new HashMap<TruffleFile, JSDynamicObject>() : null;
        if (context.getContextOptions().isWebAssembly()) {
            Object wasmMemSetGrowCallback;
            if (!this.isWasmAvailable()) {
                Object msg = "WebAssembly API enabled but wasm language cannot be accessed! Did you forget to set the --polyglot flag?";
                if (JSConfig.SubstrateVM) {
                    msg = (String)msg + " In native mode, you might have to rebuild libpolyglot with 'gu rebuild-images libpolyglot'.";
                }
    ...
    private void createTemplateRegistry() {
        if (this.templateRegistry == null) {
            this.templateRegistry = new WeakHashMap<Object, JSDynamicObject>();
        }
    }
    ...
    private JSDynamicObject createThrowerFunction() {
        ...
        JSFunctionObject thrower = JSFunction.create(this, this.context.throwerFunctionData);
    ...
    public void setupGlobals() {
        ...
        JSObjectUtil.putDataProperty(this.context, global, (Object)Strings.NAN, Double.NaN);
        JSObjectUtil.putDataProperty(this.context, global, (Object)Strings.INFINITY, Double.POSITIVE_INFINITY);
        JSObjectUtil.putDataProperty(this.context, global, (Object)Undefined.NAME, Undefined.instance);
        ...
        block4: for (JSErrorType jSErrorType : JSErrorType.errorTypes()) {
            switch (jSErrorType) {
                ...
                case RuntimeError: {
                    ...
                    JSObjectUtil.putDataProperty(this.context, this.webAssemblyObject, Strings.fromJavaString(jSErrorType.name()), this.getErrorConstructor(jSErrorType), JSAttributes.getDefaultNotEnumerable());
                    continue block4;
                }
                case AggregateError: {
                    if (this.context.getEcmaScriptVersion() < 12) continue block4;
                    this.putGlobalProperty(Strings.fromJavaString(jSErrorType.name()), this.getErrorConstructor(jSErrorType));
                    continue block4;
                }
                default: {
                    this.putGlobalProperty(Strings.fromJavaString(jSErrorType.name()), this.getErrorConstructor(jSErrorType));
        ...
        for (Enum enum_ : TypedArray.factories(this.context)) {
            this.putGlobalProperty(((TypedArrayFactory)enum_).getName(), this.getArrayBufferViewConstructor((TypedArrayFactory)enum_));
        }
        ...
        if (this.context.getEcmaScriptVersion() >= 6) {
            ...
            this.promiseAllFunctionObject = (JSDynamicObject)JSObject.get((JSDynamicObject)this.getPromiseConstructor(), Strings.ALL);
        }
    ...
    private void addCommonJSGlobals() {
        if (this.getContext().getContextOptions().isCommonJSRequire()) {
            ...
            JSFunctionObject requireFunction = this.lookupFunction(GlobalBuiltins.GLOBAL_COMMONJS_REQUIRE_EXTENSIONS, Strings.REQUIRE_PROPERTY_NAME);
            JSFunctionObject resolveFunction = this.lookupFunction(GlobalBuiltins.GLOBAL_COMMONJS_REQUIRE_EXTENSIONS, Strings.RESOLVE_PROPERTY_NAME);
            ...
            JSFunctionObject dirnameGetter = this.lookupFunction(GlobalBuiltins.GLOBAL_COMMONJS_REQUIRE_EXTENSIONS, GlobalCommonJSRequireBuiltins.GlobalRequire.dirnameGetter.getName());
            JSObject.defineOwnProperty(this.getGlobalObject(), Strings.DIRNAME_VAR_NAME, PropertyDescriptor.createAccessor(dirnameGetter, Undefined.instance, false, false));
            JSFunctionObject filenameGetter = this.lookupFunction(GlobalBuiltins.GLOBAL_COMMONJS_REQUIRE_EXTENSIONS, GlobalCommonJSRequireBuiltins.GlobalRequire.filenameGetter.getName());
            JSObject.defineOwnProperty(this.getGlobalObject(), Strings.FILENAME_VAR_NAME, PropertyDescriptor.createAccessor(filenameGetter, Undefined.instance, false, false));
            JSFunctionObject moduleGetter = this.lookupFunction(GlobalBuiltins.GLOBAL_COMMONJS_REQUIRE_EXTENSIONS, GlobalCommonJSRequireBuiltins.GlobalRequire.globalModuleGetter.getName());
            JSObject.defineOwnProperty(this.getGlobalObject(), Strings.MODULE_PROPERTY_NAME, PropertyDescriptor.createAccessor(moduleGetter, Undefined.instance, false, false));
            JSFunctionObject exportsGetter = this.lookupFunction(GlobalBuiltins.GLOBAL_COMMONJS_REQUIRE_EXTENSIONS, GlobalCommonJSRequireBuiltins.GlobalRequire.globalExportsGetter.getName());
            JSObject.defineOwnProperty(this.getGlobalObject(), Strings.EXPORTS_PROPERTY_NAME, PropertyDescriptor.createAccessor(exportsGetter, Undefined.instance, false, false));
    ...
    private void putGraalObject() {
        ...
        JSObjectUtil.putDataProperty(this.context, graalObject, Strings.IS_GRAAL_RUNTIME, JSFunction.create(this, JSRealm.isGraalRuntimeFunction(this.context)), flags);
    ...
    private static JSFunctionData setUnhandledPromiseRejectionHandlerFunction(JSContext context) {
        return context.getOrCreateBuiltinFunctionData(JSContext.BuiltinFunctionKey.SetUnhandledPromiseRejectionHandler, c -> JSFunctionData.createCallOnly(c, new JavaScriptRootNode(c.getLanguage(), null, null){
    ...
    private static JSFunctionData isGraalRuntimeFunction(JSContext context) {
        return context.getOrCreateBuiltinFunctionData(JSContext.BuiltinFunctionKey.IsGraalRuntime, c -> JSFunctionData.createCallOnly(context, new JavaScriptRootNode(context.getLanguage(), null, null){
    ...
    private void putGlobalProperty(TruffleString key, Object value2) {
        this.putGlobalProperty(key, value2, JSAttributes.getDefaultNotEnumerable());
    }

    private void putGlobalProperty(Object key, Object value2, int attributes) {
        JSObjectUtil.putDataProperty(this.getContext(), this.getGlobalObject(), key, value2, attributes);
    }

    private void putProperty(JSDynamicObject receiver, Object key, Object value2) {
        JSObjectUtil.putDataProperty(this.getContext(), receiver, key, value2, JSAttributes.getDefaultNotEnumerable());
    }
    ...
    private void setupJavaInterop() {
        ...
        if (this.getEnv() != null && this.getEnv().isHostLookupAllowed() && JSContextOptions.JAVA_PACKAGE_GLOBALS.getValue(this.getEnv().getOptions()).booleanValue()) {
    ...
    private void setupPolyglot() {
        ...
        if (this.getContext().isOptionDebugBuiltin()) {
            ...
            JSObjectUtil.putDataProperty(this.context, polyglotObject, Strings.EVAL_FILE, this.lookupFunction(PolyglotBuiltins.INTERNAL_BUILTINS, Strings.EVAL_FILE), JSAttributes.getDefaultNotEnumerable());
        }
    ...
    private void addScriptingGlobals() {
        ...
        if (this.getContext().getParserOptions().isScripting()) {
            ...
            JSObjectUtil.putDataProperty(this.context, timezoneObj, Strings.CAPS_ID, Strings.fromJavaString(timezone), JSAttributes.configurableEnumerableWritable());
            ...
            JSArrayObject arguments = JSArray.createConstant(this.context, this, Strings.constantArray(this.getEnv().getApplicationArguments()));
            ...
            for (Map.Entry<String, String> entry : sysenv.entrySet()) {
                JSObjectUtil.putDataProperty(this.context, envObj, Strings.fromJavaString(entry.getKey()), Strings.fromJavaString(entry.getValue()), JSAttributes.configurableEnumerableWritable());
            }
    ...
    public void setArguments(TruffleString[] arguments) {
        JSObjectUtil.defineDataProperty(this.context, this.getGlobalObject(), ARGUMENTS_NAME, JSArray.createConstant(this.context, this, arguments), this.context.isOptionV8CompatibilityModeInContextInit() ? JSAttributes.getDefault() : JSAttributes.getDefaultNotEnumerable());
    }
    ...
    private synchronized void createModuleLoader() {
        if (this.moduleLoader == null) {
            this.moduleLoader = this.context.getContextOptions().isCommonJSRequire() ? NpmCompatibleESModuleLoader.create(this) : DefaultESModuleLoader.create(this);
        }
    }
    ...
    public boolean joinStackPush(Object o, BranchProfile growProfile) {
        ...
        for (int i = 0; i < this.joinStack.size(); ++i) {
            ...
            if (!(interop == null ? o == element : interop.isIdentical(o, element, InteropLibrary.getFactory().getUncached(element)))) continue;
            return false;
        }
    ...
    extends PropertyProxy {
        ...
        public boolean set(JSDynamicObject store, Object value2) {
            RealmSharedPropertyProxy.topLevelRealm().v8RealmShared = value2;
            return true;
        }
