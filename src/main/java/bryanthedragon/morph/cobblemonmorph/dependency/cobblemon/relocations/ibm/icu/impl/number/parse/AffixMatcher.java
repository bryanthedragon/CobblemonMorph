...
public class AffixMatcher implements NumberParseMatcher {
   ...
   private static boolean isInteresting(AffixPatternProvider patternInfo, IgnorablesMatcher ignorables, int parseFlags) {
      ...
      return 0 != (parseFlags & 256)
         ...
         || AffixUtils.containsType(posSuffixString, -2)
         || AffixUtils.containsType(posSuffixString, -1)
         || AffixUtils.containsType(negSuffixString, -2)
         || AffixUtils.containsType(negSuffixString, -1);
   }
   ...
   public static void createMatchers(
      ...
      if (isInteresting(patternInfo, ignorables, parseFlags)) {
         ...
         for (PatternStringUtils.PatternSignType type : PatternStringUtils.PatternSignType.VALUES) {
            if ((type != PatternStringUtils.PatternSignType.POS || 0 == (parseFlags & 1024))
               && (type != PatternStringUtils.PatternSignType.POS_SIGN || 0 != (parseFlags & 1024))) {
   ...
   public void postProcess(ParsedNumber result) {
      if (matched(this.prefix, result.prefix) && matched(this.suffix, result.suffix)) {
         ...
         result.flags = result.flags | this.flags;
         if (this.prefix != null) {
