...
public final class BMPSet {
    ...
    private static void set32x64Bits(int[] table, int start2, int limit) {
        assert (64 == table.length);
        int lead = start2 >> 6;
        int trail = start2 & 0x3F;
        int bits = 1 << lead;
        if (start2 + 1 == limit) {
            int n = trail;
    ...
    private void initBits() {
        ...
        do {
            ...
            } while (start2 < limit && start2 < 256);
        } while (limit <= 256);
        while (start2 < 2048) {
            BMPSet.set32x64Bits(this.table7FF, start2, limit <= 2048 ? limit : 2048);
            if (limit > 2048) {
                start2 = 2048;
                break;
            }
        ...
        while (start2 < 65536) {
            ...
            if (start2 < limit) {
                if (0 != (start2 & 0x3F)) {
                    int n = (start2 >>= 6) & 0x3F;
                    this.bmpBlockBits[n] = this.bmpBlockBits[n] | 65537 << (start2 >> 6);
                    minStart = start2 = start2 + 1 << 6;
                }
                if (start2 < limit) {
                    if (start2 < (limit & 0xFFFFFFC0)) {
                        BMPSet.set32x64Bits(this.bmpBlockBits, start2 >> 6, limit >> 6);
                    }
                    if (0 != (limit & 0x3F)) {
                        int n = (limit >>= 6) & 0x3F;
