...
public final class BMPSet {
   ...
   public final int spanBack(CharSequence s, int limit, UnicodeSet.SpanCondition spanCondition) {
      if (UnicodeSet.SpanCondition.NOT_CONTAINED != spanCondition) {
         ...
         while (true) {
            char cx = s.charAt(--limit);
            if (cx <= 255) {
               if (this.latin1Contains[cx]) {
                  break;
               }
            } else if (cx <= 2047) {
               if ((this.table7FF[cx & '?'] & 1 << (cx >> 6)) != 0) {
                  break;
               }
            } else {
               char c2;
               if (cx >= '\ud800' && cx >= '\udc00' && 0 != limit && (c2 = s.charAt(limit - 1)) >= '\ud800' && c2 < '\udc00') {
                  int supplementary = Character.toCodePoint(c2, cx);
                  if (this.containsSlow(supplementary, this.list4kStarts[16], this.list4kStarts[17])) {
                     break;
                  }
                  ...
                  int lead = cx >> '\f';
                  int twoBits = this.bmpBlockBits[cx >> 6 & 63] >> lead & 65537;
                  if (twoBits <= 1 ? twoBits != 0 : this.containsSlow(cx, this.list4kStarts[lead], this.list4kStarts[lead + 1])) {
                     break;
                  }
               }
            }
   ...
   private void initBits() {
      ...
      while (start < 65536) {
         ...
         if (start < limit) {
            if (0 != (start & 63)) {
               int var5 = start >> 6;
               this.bmpBlockBits[var5 & 63] = this.bmpBlockBits[var5 & 63] | 65537 << (var5 >> 6);
               start = var5 + 1 << 6;
               minStart = start;
            }
            ...
            if (start < limit) {
               ...
               if (0 != (limit & 63)) {
                  int var6 = limit >> 6;
                  this.bmpBlockBits[var6 & 63] = this.bmpBlockBits[var6 & 63] | 65537 << (var6 >> 6);
                  limit = var6 + 1 << 6;
                  minStart = limit;
               }
            }
