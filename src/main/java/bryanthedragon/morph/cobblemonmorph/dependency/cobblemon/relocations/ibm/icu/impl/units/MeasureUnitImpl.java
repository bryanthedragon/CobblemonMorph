...
public class MeasureUnitImpl {
   ...
   public ArrayList<MeasureUnitImpl.MeasureUnitImplWithIndex> extractIndividualUnitsWithIndices() {
      ArrayList<MeasureUnitImpl.MeasureUnitImplWithIndex> result = new ArrayList<>();
      if (this.getComplexity() != MeasureUnit.Complexity.MIXED) {
         result.add(new MeasureUnitImpl.MeasureUnitImplWithIndex(0, this.copy()));
         return result;
      } else {
         int i = 0;

         for (SingleUnitImpl singleUnit : this.getSingleUnits()) {
            result.add(new MeasureUnitImpl.MeasureUnitImplWithIndex(i++, new MeasureUnitImpl(singleUnit)));
         }
   ...
   public void serialize() {
      if (this.getSingleUnits().size() != 0) {
         if (this.complexity == MeasureUnit.Complexity.COMPOUND) {
            Collections.sort(this.getSingleUnits(), new MeasureUnitImpl.SingleUnitComparator());
         }
   ...
   public static enum CompoundPart {
      ...
      public static MeasureUnitImpl.CompoundPart getCompoundPartFromTrieIndex(int trieIndex) {
   ...
   public static enum InitialCompoundPart {
      ...
      public static MeasureUnitImpl.InitialCompoundPart getInitialCompoundPartFromTrieIndex(int trieIndex) {
   ...
   static class MeasureUnitImplWithIndexComparator implements Comparator<MeasureUnitImpl.MeasureUnitImplWithIndex> {
      private MeasureUnitImpl.MeasureUnitImplComparator measureUnitImplComparator;

      public MeasureUnitImplWithIndexComparator(ConversionRates conversionRates) {
         this.measureUnitImplComparator = new MeasureUnitImpl.MeasureUnitImplComparator(conversionRates);
      }

      public int compare(MeasureUnitImpl.MeasureUnitImplWithIndex o1, MeasureUnitImpl.MeasureUnitImplWithIndex o2) {
   ...
   public static class UnitsParser {
      ...
      public static MeasureUnitImpl parseForIdentifier(String identifier) {
         if (identifier != null && !identifier.isEmpty()) {
            MeasureUnitImpl.UnitsParser parser = new MeasureUnitImpl.UnitsParser(identifier);
            return parser.parse();
         } else {
            return null;
         }
      ...
      private SingleUnitImpl nextSingleUnit() {
         ...
         MeasureUnitImpl.UnitsParser.Token token = this.nextToken();
         if (atStart) {
            ...
            MeasureUnitImpl.CompoundPart compoundPart = MeasureUnitImpl.CompoundPart.getCompoundPartFromTrieIndex(token.getMatch());
            switch (compoundPart) {
               case PER:
                  if (this.fSawAnd) {
      ...
      private MeasureUnitImpl.UnitsParser.Token nextToken() {
         ...
         if (match < 0) {
            ...
            this.fIndex = previ;
            return new MeasureUnitImpl.UnitsParser.Token(match);
         }
      }
      ...
      static class Token {
         ...
         public MeasureUnitImpl.InitialCompoundPart getInitialCompoundPart() {
            assert this.type == MeasureUnitImpl.UnitsParser.Token.Type.TYPE_INITIAL_COMPOUND_PART
               && this.fMatch == MeasureUnitImpl.InitialCompoundPart.INITIAL_COMPOUND_PART_PER.getTrieIndex();
