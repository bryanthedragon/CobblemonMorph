...
public abstract class InitializeNumberFormatNode extends JavaScriptBaseNode {
   ...
   protected InitializeNumberFormatNode(JSContext context) {
      ...
      this.getCurrencyDisplayOption = GetStringOptionNode.create(
         context, IntlUtil.KEY_CURRENCY_DISPLAY, new String[]{"code", "symbol", "narrowSymbol", "name"}, "symbol"
      );
      ...
      this.getNotationOption = GetStringOptionNode.create(
         context, IntlUtil.KEY_NOTATION, new String[]{"standard", "scientific", "engineering", "compact"}, "standard"
      );
      this.getCompactDisplayOption = GetStringOptionNode.create(context, IntlUtil.KEY_COMPACT_DISPLAY, new String[]{"short", "long"}, "short");
      this.getUseGroupingOption = GetStringOrBooleanOptionNode.create(
         context, IntlUtil.KEY_USE_GROUPING, new String[]{"min2", "auto", "always"}, "always", false, null
      );
      this.getSignDisplayOption = GetStringOptionNode.create(
         context, IntlUtil.KEY_SIGN_DISPLAY, new String[]{"auto", "never", "always", "exceptZero", "negative"}, "auto"
      );
      this.getRoundingIncrementOption = GetNumberOptionNode.create(context, IntlUtil.KEY_ROUNDING_INCREMENT);
      this.getTrailingZeroDisplayOption = GetStringOptionNode.create(
         context, IntlUtil.KEY_TRAILING_ZERO_DISPLAY, new String[]{"auto", "stripIfInteger"}, "auto"
      );
      this.getRoundingModeOption = GetStringOptionNode.create(
         context,
         IntlUtil.KEY_ROUNDING_MODE,
         new String[]{"ceil", "floor", "expand", "trunc", "halfCeil", "halfFloor", "halfExpand", "halfTrunc", "halfEven"},
         "halfExpand"
      );
   ...
   public JSDynamicObject initializeNumberFormat(JSDynamicObject numberFormatObj, Object localesArg, Object optionsArg) {
      try {
         ...
         if ("currency".equals(style)) {
            int cDigits = JSNumberFormat.currencyDigits(this.context, state.getCurrency());
            mnfdDefault = cDigits;
            mxfdDefault = cDigits;
         } else {
            mnfdDefault = 0;
         ...
         if (!isValidRoundingIncrement(roundingIncrement)) {
            ...
            if (roundingIncrement != 1) {
               ...
               if (state.getMinimumFractionDigits() != state.getMaximumFractionDigits()) {
                  this.errorBranch.enter();
            ...
            Object useGrouping = this.getUseGroupingOption.executeValue(options);
            if (useGrouping == null) {
         ...
         this.errorBranch.enter();
         throw Errors.createICU4JDataError(var20);
      }
   }
