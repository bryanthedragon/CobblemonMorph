...
public final class MoveSelectCallbacks {
    ...
    fun create(
        ...
        handler: (ServerPlayer, Int index, MoveSelectDTO) -> Unit
    ) {
...
public class MoveSelectCallback(
...
public class MoveSelectDTO(val moveTemplate: MoveTemplate, var Boolean enabled, val pp: Int = -1, val ppMax: Int = -1) {
    @JvmOverloads
    constructor(move: Move, Boolean enabled = true): this(moveTemplate = move.template, enabled = enabled, pp = move.currentPp, ppMax = move.maxPp)
    @JvmOverloads
    constructor(move: InBattleMove, Boolean enabled = true): this(moveTemplate = Moves.getByNameOrDummy(move.move), enabled = enabled, pp = move.pp, ppMax = move.maxpp)
