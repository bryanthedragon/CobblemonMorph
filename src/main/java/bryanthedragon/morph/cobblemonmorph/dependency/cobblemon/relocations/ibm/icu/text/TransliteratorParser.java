...
import com.cobblemon.mod.relocations.ibm.icu.text.FunctionReplacer;
import com.cobblemon.mod.relocations.ibm.icu.text.Normalizer;
import com.cobblemon.mod.relocations.ibm.icu.text.Quantifier;
import com.cobblemon.mod.relocations.ibm.icu.text.RuleBasedTransliterator;
import com.cobblemon.mod.relocations.ibm.icu.text.StringMatcher;
import com.cobblemon.mod.relocations.ibm.icu.text.StringReplacer;
import com.cobblemon.mod.relocations.ibm.icu.text.SymbolTable;
import com.cobblemon.mod.relocations.ibm.icu.text.TransliterationRule;
import com.cobblemon.mod.relocations.ibm.icu.text.Transliterator;
import com.cobblemon.mod.relocations.ibm.icu.text.TransliteratorIDParser;
...
class TransliteratorParser {
    ...
    private int parseRule(String rule, int pos, int limit) {
        ...
        if ((pos = left.parse(rule, pos, limit, this)) == limit || OPERATORS.indexOf(operator = rule.charAt(--pos)) < 0) {
            TransliteratorParser.syntaxError("No operator pos=" + pos, rule, start2);
        }
        ...
        if (pos < limit) {
            if (rule.charAt(--pos) == ';') {
                ...
                TransliteratorParser.syntaxError("Unquoted operator", rule, start2);
            }
        }
        if (operator == 61) {
            if (this.undefinedVariableName == null) {
                TransliteratorParser.syntaxError("Missing '$' or duplicate definition", rule, start2);
            }
            if (left.text.length() != 1 || left.text.charAt(0) != this.variableLimit) {
                TransliteratorParser.syntaxError("Malformed LHS", rule, start2);
            }
            if (left.anchorStart || left.anchorEnd || right.anchorStart || right.anchorEnd) {
                TransliteratorParser.syntaxError("Malformed variable def", rule, start2);
            }
            int n = right.text.length();
            char[] value2 = new char[n];
            right.text.getChars(0, n, value2, 0);
            this.variableNames.put(this.undefinedVariableName, value2);
            this.variableLimit = (char)(this.variableLimit + '\u0001');
            return pos;
        }
        if (this.undefinedVariableName != null) {
            TransliteratorParser.syntaxError("Undefined variable $" + this.undefinedVariableName, rule, start2);
        }
        ...
        for (i = 0; i < this.segmentStandins.length(); ++i) {
            ...
            TransliteratorParser.syntaxError("Internal error", rule, start2);
        }
        ...
        if (right.ante >= 0 || right.post >= 0 || left.cursor >= 0 || right.cursorOffset != 0 && right.cursor < 0 || right.anchorStart || right.anchorEnd || !left.isValidInput(this) || !right.isValidOutput(this) || left.ante > left.post) {
            TransliteratorParser.syntaxError("Malformed rule", rule, start2);
        }
    ...
    private void setVariableRange(int start2, int end2) {
        if (start2 > end2 || start2 < 0 || end2 > 65535) {
            throw new IllegalIcuArgumentException("Invalid variable range " + start2 + ", " + end2);
        }
        this.curData.variablesBase = (char)start2;
        if (this.dataVector.size() == 0) {
            this.variableNext = (char)start2;
            this.variableLimit = (char)(end2 + 1);
        }
    }

    private void checkVariableRange(int ch, String rule, int start2) {
        if (ch >= this.curData.variablesBase && ch < this.variableLimit) {
            TransliteratorParser.syntaxError("Variable range character in rule", rule, start2);
        }
    }
    ...
    static final void syntaxError(String msg, String rule, int start2) {
        int end2 = TransliteratorParser.ruleEnd(rule, start2, rule.length());
        throw new IllegalIcuArgumentException(msg + " in \"" + Utility.escape(rule.substring(start2, end2)) + '\"');
    }

    static final int ruleEnd(String rule, int start2, int limit) {
        int end2 = Utility.quotedIndexOf(rule, start2, limit, ";");
        if (end2 < 0) {
            end2 = limit;
        }
        return end2;
    ...
    private final char parseSet(String rule, ParsePosition pos) {
        UnicodeSet set2 = new UnicodeSet(rule, pos, this.parseData);
        if (this.variableNext >= this.variableLimit) {
            throw new RuntimeException("Private use variables exhausted");
        }
        set2.compact();
        return this.generateStandInFor(set2);
    }
    ...
    private static class RuleHalf {
        ...
        public int parse(String rule, int pos, int limit, TransliteratorParser parser) {
            ...
            if (this.cursorOffset > 0 && this.cursor != this.cursorOffsetPos) {
                TransliteratorParser.syntaxError("Misplaced |", rule, start2);
            }
            return pos;
        }
        ...
        private int parseSection(String rule, int pos, int limit, TransliteratorParser parser, StringBuffer buf, UnicodeSet illegal, boolean isSegment) {
            ...
            block19: while (pos < limit) {
                ...
                if (TransliteratorParser.HALF_ENDERS.indexOf(c) >= 0) {
                    if (!isSegment) break;
                    TransliteratorParser.syntaxError("Unclosed segment", rule, start2);
                    break;
                }
                if (this.anchorEnd) {
                    TransliteratorParser.syntaxError("Malformed variable reference", rule, start2);
                }
                ...
                if (c == '\\') {
                    int cpAndLength;
                    if (pos == limit) {
                        TransliteratorParser.syntaxError("Trailing backslash", rule, start2);
                    }
                    if ((cpAndLength = Utility.unescapeAndLengthAt(rule, pos)) < 0) {
                        TransliteratorParser.syntaxError("Malformed escape", rule, start2);
                    }
                    int escaped = Utility.cpFromCodePointAndLength(cpAndLength);
                    pos += Utility.lengthFromCodePointAndLength(cpAndLength);
                    parser.checkVariableRange(escaped, rule, start2);
                ...
                if (c == '\'') {
                    ...
                    while (true) {
                        if (iq < 0) {
                            TransliteratorParser.syntaxError("Unterminated quote", rule, start2);
                        }
                    ...
                    for (iq = quoteStart; iq < quoteLimit; ++iq) {
                        parser.checkVariableRange(buf.charAt(iq), rule, start2);
                    }
                    continue;
                }
                parser.checkVariableRange(c, rule, start2);
                if (illegal.contains(c)) {
                    TransliteratorParser.syntaxError("Illegal character '" + c + '\'', rule, start2);
                }
                switch (c) {
                    case '^': {
                        ...
                        TransliteratorParser.syntaxError("Misplaced anchor start", rule, start2);
                        break;
                    }
                    case '(': {
                        int segmentNumber;
                    ...
                    case '\u2206': {
                        ...
                        if (single == null || !Utility.parseChar(rule, iref, '(')) {
                            TransliteratorParser.syntaxError("Invalid function", rule, start2);
                        }
                        if ((t = single.getInstance()) == null) {
                            TransliteratorParser.syntaxError("Invalid function ID", rule, start2);
                        }
                        int bufSegStart = buf.length();
                    ...
                    case '?': {
                        ...
                        if (isSegment && buf.length() == bufStart) {
                            TransliteratorParser.syntaxError("Misplaced quantifier", rule, start2);
                            break;
                        }
                    ...
                    case '{': {
                        if (this.ante >= 0) {
                            TransliteratorParser.syntaxError("Multiple ante contexts", rule, start2);
                        }
                    ...
                    case '}': {
                        if (this.post >= 0) {
                            TransliteratorParser.syntaxError("Multiple post contexts", rule, start2);
                        }
                    ...
                    case '|': {
                        if (this.cursor >= 0) {
                            TransliteratorParser.syntaxError("Multiple cursors", rule, start2);
                        }
                    ...
                    case '@': {
                        if (this.cursorOffset < 0) {
                            if (buf.length() > 0) {
                                TransliteratorParser.syntaxError("Misplaced " + c, rule, start2);
                            }
                            --this.cursorOffset;
                            break;
                        }
                    ...
                    default: {
                        if (!(c < '!' || c > '~' || c >= '0' && c <= '9' || c >= 'A' && c <= 'Z' || c >= 'a' && c <= 'z')) {
                            TransliteratorParser.syntaxError("Unquoted " + c, rule, start2);
                        }
                        buf.append(c);
                    }
                }
    ...
    implements SymbolTable {
        ...
        public String parseReference(String text, ParsePosition pos, int limit) {
            int start2;
            int i;
            for (i = start2 = pos.getIndex(); i < limit; ++i) {
            ...
            pos.setIndex(i);
            return text.substring(start2, i);
        }
