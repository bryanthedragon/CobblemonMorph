...
public final class RegexASTBuilder {
    ...
    public void addBackReference(Token.BackReference token) {
        ...
        } else if (RegexASTBuilder.isNestedBackReference(backReference)) {
    ...
    public void addQuantifier(Token.Quantifier quantifier) {
        ...
        if (this.curSequence.size() > 1 && (prevTerm = this.curSequence.getTerms().get(this.curSequence.size() - 2)).isQuantifiableTerm() && (prev = prevTerm.asQuantifiableTerm()).hasQuantifier() && this.curTerm.asQuantifiableTerm().equalsSemantic(prev, true)) {
            long max2;
            this.removeCurTerm();
            long min2 = (long)prev.getQuantifier().getMin() + (long)quantifier.getMin();
            long l = max2 = prev.getQuantifier().isInfiniteLoop() || quantifier.isInfiniteLoop() ? -1L : (long)prev.getQuantifier().getMax() + (long)quantifier.getMax();
            ...
            this.setQuantifier(prev, Token.createQuantifier((int)min2, (int)max2, prev.getQuantifier().isGreedy() || quantifier.isGreedy()));
        }
    }
    ...
    private void optimizeGroup() {
        RegexASTBuilder.sortAlternatives(this.curGroup);
    ...
    private static void sortAlternatives(Group group) {
        ...
        while (begin + 1 < group.size()) {
            int end2 = RegexASTBuilder.findSingleCharAlternatives(group, begin);
            if (end2 > begin + 1) {
                group.getAlternatives().subList(begin, end2).sort(Comparator.comparingInt(a -> a.getFirstTerm().asCharacterClass().getCharSet().getMin()));
                begin = end2;
                continue;
            }
            ++begin;
    ...
    private void mergeCommonPrefixes(Group group) {
        ...
        int begin = 0;
        while (begin + 1 < group.size()) {
            int end2 = RegexASTBuilder.findMatchingAlternatives(group, begin);
            if (end2 < 0 || !isDeterministicVisitor.isDeterministic(group.getAlternatives().get(begin).getFirstTerm())) {
                ++begin;
                continue;
            }
            ...
            lastEnd = end2;
            int prefixSize = 1;
            while (RegexASTBuilder.alternativesAreEqualAt(group, begin, end2, prefixSize) && isDeterministicVisitor.isDeterministic(group.getAlternatives().get(begin).get(prefixSize))) {
                ++prefixSize;
            }
