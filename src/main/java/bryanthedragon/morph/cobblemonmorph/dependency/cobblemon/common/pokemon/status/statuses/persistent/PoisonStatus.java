...
extends PersistentStatus {
    ...
    public void onSecondPassed(@NotNull ServerPlayer player, @NotNull Pokemon pokemon, @NotNull Random random) {
        ...
        if (!pokemon.isFainted() && random.nextInt(15) == 0) {
            pokemon.setCurrentHealth(pokemon.getCurrentHealth() - Math.max(1, (int)Math.rint((double)pokemon.getHp() * 0.05)) * (Intrinsics.areEqual((Object)pokemon.getAbility().getTemplate().getName(), (Object)"poisonheal") ? -1 : 1));
