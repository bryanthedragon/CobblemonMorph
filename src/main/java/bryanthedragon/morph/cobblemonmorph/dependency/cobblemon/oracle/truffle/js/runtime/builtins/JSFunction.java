...
public final class JSFunction extends JSNonProxy {
   ...
   public static final SourceSection BUILTIN_SOURCE_SECTION = createBuiltinSourceSection("<builtin>");
   ...
   public static final PropertyProxy PROTOTYPE_PROXY = new JSFunction.ClassPrototypeProxyProperty();
   public static final PropertyProxy LENGTH_PROXY = new JSFunction.FunctionLengthPropertyProxy();
   public static final PropertyProxy NAME_PROXY = new JSFunction.FunctionNamePropertyProxy();
   public static final PropertyProxy ARGUMENTS_PROXY = new JSFunction.ArgumentsProxyProperty();
   public static final PropertyProxy CALLER_PROXY = new JSFunction.CallerProxyProperty();
   ...
   public static JSFunctionObject createWithPrototype(
      JSFunctionFactory factory, JSRealm realm, JSFunctionData functionData, MaterializedFrame enclosingFrame, JSDynamicObject prototype
   ) {
   ...
   private static JSFunctionObject createWithPrototype(
      JSFunctionFactory factory, JSFunctionData functionData, MaterializedFrame enclosingFrame, Object classPrototype, JSRealm realm, JSDynamicObject prototype
   ) {
   ...
   public static JSFunctionObject createBound(
      JSContext context, JSRealm realm, JSFunctionData functionData, JSDynamicObject boundTargetFunction, Object boundThis, Object[] boundArguments
   ) {
   ...
   public static Object call(JSFunctionObject functionObject, Object thisObject, Object[] argumentValues) {
      assert isJSFunction(functionObject);
   ...
   public static Object call(Object[] jsArguments) {
      assert isJSFunction(JSArguments.getFunctionObject(jsArguments));
   ...
   public static Object construct(JSFunctionObject functionObject, Object[] argumentValues) {
      assert isJSFunction(functionObject) && isConstructor((JSDynamicObject)functionObject);
   ...
   public static JSDynamicObject bind(JSRealm realm, JSFunctionObject thisFnObj, Object thisArg, Object[] boundArguments) {
      assert isJSFunction(thisFnObj);

      JSContext context = realm.getContext();
      JSDynamicObject proto = JSObject.getPrototype(thisFnObj);
      JSDynamicObject boundFunction = boundFunctionCreate(
         context,
         thisFnObj,
         thisArg,
         boundArguments,
      ...
      long length = 0L;
      boolean targetHasLength = JSObject.hasOwnProperty(thisFnObj, LENGTH);
      boolean mustSetLength = true;
      if (targetHasLength) {
         Object targetLen = JSObject.get(thisFnObj, LENGTH);
         if (JSRuntime.isNumber(targetLen)) {
            long targetLenInt = JSRuntime.toInteger(targetLen);
            length = Math.max(0L, targetLenInt - boundArguments.length);
   ...
   public static JSDynamicObject boundFunctionCreate(
      JSContext context,
      JSFunctionObject boundTargetFunction,
      Object boundThis,
      Object[] boundArguments,
      JSDynamicObject proto,
      ConditionProfile isConstructorProfile,
      ConditionProfile isAsyncProfile,
      ConditionProfile setProtoProfile,
      Node node
   ) {
      CompilerAsserts.partialEvaluationConstant(context);
      JSFunctionData targetFunctionData = getFunctionData(boundTargetFunction);
      ...
      JSDynamicObject boundFunction = createBound(context, realm, boundFunctionData, boundTargetFunction, boundThis, boundArguments);
   ...
   private static JSDynamicObject createPrototype(JSDynamicObject constructor) {
      ...
      if (!functionData.isGenerator()) {
         ...
         return functionData.isAsync()
            ? JSOrdinary.createWithRealm(context, context.getAsyncGeneratorObjectFactory(), realm)
            : JSOrdinary.createWithRealm(context, context.getGeneratorObjectFactory(), realm);
      }
   }
   ...
   public static RootNode createBoundRootNode(JSContext context, boolean construct, boolean newTarget) {
      if (newTarget) {
         return new JSFunction.BoundConstructNewTargetRootNode(context);
      } else {
         return (RootNode)(construct ? new JSFunction.BoundConstructRootNode(context) : new JSFunction.BoundRootNode(context));
      }
   }
   ...
   public static JSFunctionObject createFunctionPrototype(JSRealm realm, JSDynamicObject objectPrototype) {
      ...
      JSFunctionObject proto = JSFunctionObject.create(
         protoShape, createEmptyFunctionData(context), JSFrameUtil.NULL_MATERIALIZED_FRAME, realm, CLASS_PROTOTYPE_PLACEHOLDER
      );
   ...
   public static JSFunctionObject createNamedEmptyFunction(JSRealm realm, TruffleString name) {
      return create(realm, createNamedEmptyFunctionData(realm.getContext(), name));
   }

   public static JSFunctionObject createEmptyFunction(JSRealm realm) {
      return create(realm, createEmptyFunctionData(realm.getContext()));
   }
   ...
   public static Shape makeFunctionShape(JSContext context, JSDynamicObject prototype, boolean isGenerator, boolean isAsync) {
      ...
      if (isGenerator) {
         initialShape = Shape.newBuilder(initialShape)
            .addConstantProperty(isAsync ? ASYNC_GENERATOR_FUNCTION_MARKER_ID : GENERATOR_FUNCTION_MARKER_ID, null, 0)
            .build();
      }
   ...
   public static JSFunctionObject createFunctionConstructor(JSRealm realm) {
      ...
      JSObjectUtil.putDataProperty(
         ctx, functionConstructor, JSObject.PROTOTYPE, realm.getFunctionPrototype(), JSAttributes.notConfigurableNotEnumerableNotWritable()
      );
   ...
   public TruffleString toDisplayStringImpl(JSDynamicObject obj, boolean allowSideEffects, ToDisplayStringFormat format, int depth) {
      RootNode rn = ((RootCallTarget)getCallTarget(obj)).getRootNode();
      SourceSection ssect = rn.getSourceSection();
      TruffleString source;
      if (ssect == null || !ssect.isAvailable() || ssect.getSource().isInternal()) {
         source = Strings.concatAll(Strings.FUNCTION_SPC, getName(obj), Strings.FUNCTION_NATIVE_CODE_BODY);
      } else if (depth >= format.getMaxDepth()) {
         source = Strings.concatAll(Strings.FUNCTION_SPC, getName(obj), Strings.FUNCTION_BODY_DOTS);
      } else if (ssect.getCharacters().length() > 200) {
   ...
   public static JSObject createGeneratorFunctionPrototype(JSRealm realm, JSDynamicObject constructor) {
      ...
      JSObjectUtil.putDataProperty(
         ctx, prototype, JSObject.PROTOTYPE, createGeneratorPrototype(realm, prototype), JSAttributes.configurableNotEnumerableNotWritable()
      );
   ...
   public static JSObject createAsyncIteratorPrototype(JSRealm realm) {
      ...
      JSFunctionData functionData = realm.getContext()
         .getOrCreateBuiltinFunctionData(
            JSContext.BuiltinFunctionKey.FunctionAsyncIterator,
            c -> JSFunctionData.createCallOnly(context, (new JavaScriptRootNode(context.getLanguage(), null, null) {
               @Override
      ...
      JSFunctionObject asyncIterator = create(realm, functionData);
   ...
   public static JSObject createAsyncGeneratorFunctionPrototype(JSRealm realm, JSDynamicObject constructor) {
      ...
      JSObjectUtil.putDataProperty(ctx, prototype, JSObject.CONSTRUCTOR, constructor, JSAttributes.configurableNotEnumerableNotWritable());
      JSObjectUtil.putDataProperty(
         ctx, prototype, JSObject.PROTOTYPE, createAsyncGeneratorPrototype(realm, prototype), JSAttributes.configurableNotEnumerableNotWritable()
      );
   ...
   public static JSDynamicObject createOrdinaryHasInstanceFunction(JSRealm realm) {
      ...
      return create(
         realm,
         ctx.getOrCreateBuiltinFunctionData(
            JSContext.BuiltinFunctionKey.OrdinaryHasInstance,
            c -> JSFunctionData.createCallOnly(c, new InstanceofNode.OrdinaryHasInstanceRootNode(c).getCallTarget(), 1, ORDINARY_HAS_INSTANCE)
         )
      );
   }
   ...
   public static boolean isBuiltinThatShouldNotAppearInStackTrace(JSRealm realm, JSDynamicObject function) {
      return function == realm.getApplyFunctionObject()
         || function == realm.getCallFunctionObject()
         || function == realm.getReflectApplyFunctionObject()
   ...
   public static boolean isStrictBuiltin(JSDynamicObject function, JSRealm realm) {
      JSFunctionData functionData = getFunctionData(function);
   ...
   public static final class ArgumentsProxyProperty extends PropertyProxy {
      ...
      public Object get(JSDynamicObject thiz) {
         ...
         assert !JSFunction.getFunctionData(thisFunction).hasStrictFunctionProperties()
            && JSFunction.getFunctionData(thisFunction).getContext().isOptionV8CompatibilityMode();
      ...
      private static Object createArguments(JSFunctionObject thisFunction) {
         ...
         return Truffle.getRuntime().iterateFrames(new FrameInstanceVisitor<Object>() {
            ...
            public Object visitFrame(FrameInstance frameInstance) {
               ...
               if (rootNode instanceof FunctionRootNode && ((FunctionRootNode)rootNode).getFunctionData() == thisFunctionData) {
   ...
   static final class BoundConstructNewTargetRootNode extends JSFunction.BoundRootNode {
   ...
   static final class BoundConstructRootNode extends JSFunction.BoundRootNode {
            ...
            public Object visitFrame(FrameInstance frameInstance) {
               ...
               if (rootNode instanceof FunctionRootNode) {
                  if (this.seenThisFunction) {
                     ...
                     if (!":program".equals(rootNode.getName())) {
                        return callerFunction;
                     }
                  } else if (((FunctionRootNode)rootNode).getFunctionData() == thisFunctionData) {
                     Frame framex = frameInstance.getFrame(FrameInstance.FrameAccess.READ_WRITE);
                     Object functionx = JSArguments.getFunctionObject(framex.getArguments());
                     if (functionx == thisFunction) {
   ...
   public static final class FunctionLengthPropertyProxy extends PropertyProxy {
      ...
      public Object get(JSDynamicObject store) {
         ...
         return JSFunction.isBoundFunction(store) ? ((JSFunctionObject.Bound)store).getBoundLength() : JSFunction.getLength(store);
      }
   ...
   public static final class FunctionNamePropertyProxy extends PropertyProxy {
      public TruffleString get(JSDynamicObject store) {
         ...
         return JSFunction.isBoundFunction(store) ? ((JSFunctionObject.Bound)store).getBoundName() : JSFunction.getName(store);
      }
