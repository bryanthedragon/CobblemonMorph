...
import com.cobblemon.mod.relocations.ibm.icu.impl.coll.CollationDataBuilder;
...
import com.cobblemon.mod.relocations.ibm.icu.impl.coll.CollationRootElements;
import com.cobblemon.mod.relocations.ibm.icu.impl.coll.CollationRuleParser;
...
import com.cobblemon.mod.relocations.ibm.icu.impl.coll.CollationWeights;
...
extends CollationRuleParser.Sink {
    ...
    private static final int binarySearchForRootPrimaryNode(int[] rootPrimaryIndexes, int length, long[] nodes, long p) {
        ...
        long node;
        long nodePrimary;
        while (p != (nodePrimary = (node = nodes[rootPrimaryIndexes[i = (int)(((long)start2 + (long)limit) / 2L)]]) >>> 32)) {
            if (p < nodePrimary) {
                if (i == start2) {
                    return ~start2;
                }
    ...
    void suppressContractions(UnicodeSet set2) {
        this.dataBuilder.suppressContractions(set2);
    }

    @Override
    void optimize(UnicodeSet set2) {
        this.optimizeSet.addAll(set2);
    }
