...
import com.cobblemon.mod.relocations.ibm.icu.text.CanonicalIterator;
...
public final class CollationBuilder extends CollationRuleParser.Sink {
   ...
   public CollationTailoring parseAndBuild(String ruleString) throws ParseException {
      if (this.baseData.rootElements == null) {
         ...
         parser.setSink(this);
         parser.setImporter(new CollationBuilder.BundleImporter());
            ...
            if (strengthFromNode(node) != strength) {
               ...
               if (weight16 == 0) {
                  throw new UnsupportedOperationException(
                     strength == 1 ? "reset secondary-before secondary ignorable not possible" : "reset tertiary-before completely ignorable not possible"
                  );
               }
      ...
      if (weight16 == 1280) {
         ...
         if (weight16 != 0 && weight16 < 1280) {
            ...
            if ((node & hasThisLevelBefore) == 0L) {
               ...
               this.nodes.setElementAt(node | hasThisLevelBefore, index);
               int nextIndex = nextIndexFromNode(node);
   ...
   private int findCommonNode(int index, int strength) {
      ...
      if (strengthFromNode(node) >= strength) {
         return index;
      } else if (strength == 1 ? nodeHasBefore2(node) : nodeHasBefore3(node)) {
   ...
   private void setCaseBits(CharSequence nfdString) {
      ...
      if (numTailoredPrimaries > 0) {
         ...
         for (int ix = 0; ix < baseCEsLength; ix++) {
            long ce = baseCEs.getCE(ix);
            if (ce >>> 32 != 0L) {
               numBasePrimaries++;
               int c = (int)ce >> 14 & 3;
      ...
      for (int ixx = 0; ixx < this.cesLength; ixx++) {
         long ce = this.ces[ixx] & -49153L;
      ...
      if (nfdPrefix.length() == 0) {
         ...
         while (true) {
            ...
            if (!this.ignorePrefix(prefix)) {
               ...
               while (true) {
                  String strx = stringIter.next();
                  if (strx == null) {
                     stringIter.reset();
                     break;
                  }

                  if (!this.ignoreString(strx) && (!samePrefix || !strx.contentEquals(nfdString))) {
                     ce32 = this.addIfDifferent(prefix, strx, newCEs, newCEsLength, ce32);
                  }
               }
            }
         }
   ...
   private boolean mergeCompositeIntoString(
      CharSequence nfdString, int indexAfterLastStarter, int composite, CharSequence decomp, StringBuilder newNFDString, StringBuilder newString
   ) {
   ...
   private void finalizeCEs() {
      ...
      CollationBuilder.CEFinalizer finalizer = new CollationBuilder.CEFinalizer(this.nodes.getBuffer());
   ...
   private static long tempCEFromIndexAndStrength(int index, int strength) {
      return 4629700417037541376L + ((long)(index & 1040384) << 43) + ((long)(index & 8128) << 42) + ((index & 63) << 24) + (strength << 8);
   }
   ...
   private static int ceStrength(long ce) {
      return isTempCE(ce) ? strengthFromTempCE(ce) : ((ce & -72057594037927936L) != 0L ? 0 : (((int)ce & 0xFF000000) != 0 ? 1 : (ce != 0L ? 2 : 15)));
   }
   ...
   private static final class CEFinalizer implements CollationDataBuilder.CEModifier {
      ...
      public long modifyCE32(int ce32) {
         ...
         return CollationBuilder.isTempCE32(ce32) ? this.finalCEs[CollationBuilder.indexFromTempCE32(ce32)] | (ce32 & 192) << 8 : 4311744768L;
      }
