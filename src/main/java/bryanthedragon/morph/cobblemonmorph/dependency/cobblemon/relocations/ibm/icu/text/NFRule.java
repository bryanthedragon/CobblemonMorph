...
import com.cobblemon.mod.relocations.ibm.icu.text.RbnfLenientScanner;
...
final class NFRule {
    ...
    public Number doParse(String text, ParsePosition parsePosition, boolean isFractionRule, double upperBound, int nonNumericalExecutedRuleMask) {
        ...
        do {
            pp.setIndex(0);
            double partialResult = this.matchToDelimiter(workText, start2, tempBaseValue, this.ruleText.substring(sub1Pos, sub2Pos), this.rulePatternFormat, pp, this.sub1, upperBound, nonNumericalExecutedRuleMask).doubleValue();
        ...
        } while (sub1Pos != sub2Pos && pp.getIndex() > 0 && pp.getIndex() < workText.length() && pp.getIndex() != start2);
    ...
    private int[] findText(String str, String key, PluralFormat pluralFormatKey, int startingAt) {
        ...
        if (pluralFormatKey != null) {
            ...
            int start2 = position.getBeginIndex();
            if (start2 >= 0) {
                int pluralRuleStart = this.ruleText.indexOf("$(");
                int pluralRuleSuffix = this.ruleText.indexOf(")$", pluralRuleStart) + 2;
                int matchLen = position.getEndIndex() - start2;
                String prefix = this.ruleText.substring(0, pluralRuleStart);
                String suffix = this.ruleText.substring(pluralRuleSuffix);
                if (str.regionMatches(start2 - prefix.length(), prefix, 0, prefix.length()) && str.regionMatches(start2 + matchLen, suffix, 0, suffix.length())) {
                    return new int[]{start2 - prefix.length(), matchLen + prefix.length() + suffix.length()};
                }
            }
            return new int[]{-1, 0};
        }
