...
final class NFRule {
   ...
   private NFSubstitution extractSubstitution(NFRuleSet owner, NFRule predecessor) {
      ...
      if (subStart == -1) {
         ...
         if (subEnd == -1) {
            ...
            NFSubstitution result = NFSubstitution.makeSubstitution(
               subStart, this, predecessor, owner, this.formatter, this.ruleText.substring(subStart, subEnd + 1)
            );
   ...
   private short expectedExponent() {
      if (this.radix != 0 && this.baseValue >= 1L) {
         ...
         return power(this.radix, (short)(tempResult + 1)) <= this.baseValue ? (short)(tempResult + 1) : tempResult;
      } else {
         return 0;
      }
   }
   ...
   public boolean equals(Object that) {
      if (!(that instanceof NFRule)) {
         ...
         return this.baseValue == that2.baseValue
            && this.radix == that2.radix
            && this.exponent == that2.exponent
            && this.ruleText.equals(that2.ruleText)
            && Objects.equals(this.sub1, that2.sub1)
            && Objects.equals(this.sub2, that2.sub2);
      }
   }
   ...
   public void doFormat(double number, StringBuilder toInsertInto, int pos, int recursionCount) {
      ...
      if (this.rulePatternFormat == null) {
         ...
         if (0.0 <= number && number < 1.0) {
            pluralVal = Math.round(number * power(this.radix, this.exponent));
         } else {
            pluralVal = number / power(this.radix, this.exponent);
         }
   ...
   public Number doParse(String text, ParsePosition parsePosition, boolean isFractionRule, double upperBound, int nonNumericalExecutedRuleMask) {
      ...
      if (pp.getIndex() == 0 && sub1Pos != 0) {
         ...
         do {
            pp.setIndex(0);
            double partialResult = this.matchToDelimiter(
                  workText,
                  start,
                  tempBaseValue,
                  this.ruleText.substring(sub1Pos, sub2Pos),
                  this.rulePatternFormat,
                  pp,
                  this.sub1,
                  upperBound,
            ...
            if (pp.getIndex() != 0 || this.sub1 == null) {
               ...
               partialResult = this.matchToDelimiter(
                     workText2,
                     0,
                     partialResult,
                     this.ruleText.substring(sub2Pos),
                     this.rulePatternFormat,
                     pp2,
                     this.sub2,
                     upperBound,
