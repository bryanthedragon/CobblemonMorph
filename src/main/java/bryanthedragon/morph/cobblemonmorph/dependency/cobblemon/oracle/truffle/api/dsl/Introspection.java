...
public final class Introspection {
   ...
   public static boolean isIntrospectable(Node node) {
      return node instanceof Introspection.Provider;
   }
   ...
   private static Introspection getIntrospectionData(Node node) {
      if (!(node instanceof Introspection.Provider)) {
         throw new IllegalArgumentException(
            String.format("Provided node is not introspectable. Annotate with @%s to make a node introspectable.", Introspectable.class.getSimpleName())
         );
      } else {
         return ((Introspection.Provider)node).getIntrospectionData();
      }
   }

   Introspection.SpecializationInfo getSpecialization(String methodName) {
      this.checkVersion();

      for (int i = 1; i < this.data.length; i++) {
         Object[] fieldData = getIntrospectionData(this.data[i]);
   ...
   List<Introspection.SpecializationInfo> getSpecializations() {
      this.checkVersion();
      List<Introspection.SpecializationInfo> specializations = new ArrayList<>();

      for (int i = 1; i < this.data.length; i++) {
         specializations.add(createSpecialization(getIntrospectionData(this.data[i])));
      }
   ...
   private static Introspection.SpecializationInfo createSpecialization(Object[] fieldData) {
      ...
      return new Introspection.SpecializationInfo(id, state, cachedData);
   }
   ...
   public static final class SpecializationInfo {
      ...
      public String toString() {
         return "SpecializationInfo[name="
            + this.methodName
            + ", active="
            + this.isActive()
            + ", excluded"
