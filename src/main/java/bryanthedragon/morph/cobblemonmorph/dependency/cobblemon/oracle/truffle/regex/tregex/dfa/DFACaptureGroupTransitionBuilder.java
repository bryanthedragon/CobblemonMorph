...
public class DFACaptureGroupTransitionBuilder extends DFAStateTransitionBuilder {
   ...
   private DFACaptureGroupPartialTransition createPartialTransition(
      StateSet<NFA, NFAState> targetStates, int[] targetStatesIndexMap, CompilationBuffer compilationBuffer
   ) {
      int numberOfNFAStates = Math.max(this.getRequiredStates().size(), targetStates.size());
      DFACaptureGroupTransitionBuilder.PartialTransitionDebugInfo partialTransitionDebugInfo = null;
      if (this.dfaGen.getOptions().isDumpAutomata()) {
         partialTransitionDebugInfo = new DFACaptureGroupTransitionBuilder.PartialTransitionDebugInfo(numberOfNFAStates);
      }
      ...
      for (NFAStateTransition nfaTransition : (NFAStateTransition[])this.getTransitionSet().getTransitions()) {
         if (targetStates.contains(nfaTransition.getTarget())) {
            int sourceIndex = getStateIndex(this.getRequiredStatesIndexMap(), nfaTransition.getSource());
      ...
      byte[] reorderSwaps = this.skipReorder() ? DFACaptureGroupPartialTransition.EMPTY : newOrderToSequenceOfSwaps(newOrder, compilationBuffer);
   ...
   public DFACaptureGroupLazyTransitionBuilder toLazyTransition(CompilationBuffer compilationBuffer) {
      if (this.lazyTransition == null) {
         ...
         for (int i = 0; i < ((DFAStateTransitionBuilder[])successor.getSuccessors()).length; i++) {
            ...
            partialTransitions[i] = this.createPartialTransition(
               successorTransition.getRequiredStates(), successorTransition.getRequiredStatesIndexMap(), compilationBuffer
            );
         }
         ...
         this.lazyTransition = new DFACaptureGroupLazyTransitionBuilder(
            (short)this.getId(), partialTransitions, transitionToFinalState, transitionToAnchoredFinalState
         );
      }
